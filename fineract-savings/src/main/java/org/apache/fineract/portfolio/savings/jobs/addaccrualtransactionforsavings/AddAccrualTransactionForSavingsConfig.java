...
public class AddAccrualTransactionForSavingsConfig {
    ...
    @Autowired
    private SavingsAccrualDomainServiceImpl savingsAccrualDomainService;

    @Bean
    protected Step addAccrualTransactionForSavingsStep() {
        return new StepBuilder(JobName.ADD_PERIODIC_ACCRUAL_ENTRIES_FOR_SAVINGS.name(), jobRepository)
                .tasklet(addAccrualTransactionForSavingsTasklet(), transactionManager).build();
    }

    @Bean
    public Job addAccrualTransactionForSavingsJob() {
        return new JobBuilder(JobName.ADD_PERIODIC_ACCRUAL_ENTRIES_FOR_SAVINGS.name(), jobRepository)
    ...
    public AddAccrualTransactionForSavingsTasklet addAccrualTransactionForSavingsTasklet() {
        return new AddAccrualTransactionForSavingsTasklet(savingsAccrualDomainService);
    }
}
