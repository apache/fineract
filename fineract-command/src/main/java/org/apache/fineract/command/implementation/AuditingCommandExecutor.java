...
@Component
@ConditionalOnProperty(value = "fineract.command.executor", havingValue = "audit")
public class AuditingCommandExecutor implements CommandExecutor {
    ...
    public <REQ, RES> Supplier<RES> execute(Command<REQ> command) {
        ...
        return () -> {
            ...
            try {
                RES result = handler.handle(command);
