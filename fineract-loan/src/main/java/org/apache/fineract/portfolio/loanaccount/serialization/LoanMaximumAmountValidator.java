...
import org.apache.fineract.portfolio.loanproduct.domain.LoanOverAppliedCalculationType;
...
public final class LoanMaximumAmountValidator {
    ...
    public BigDecimal getOverAppliedMax(Loan loan) {
        ...
        if (LoanOverAppliedCalculationType.valueOf(loanProduct.getOverAppliedCalculationType()).isPercentage()) {
    ...
    public BigDecimal getOverAppliedAppovedMax(Loan loan) {
        final LoanProduct loanProduct = loan.getLoanProduct();
        if (LoanOverAppliedCalculationType.valueOf(loanProduct.getOverAppliedCalculationType()).isPercentage()) {
            BigDecimal overAppliedNumber = BigDecimal.valueOf(loanProduct.getOverAppliedNumber());
            BigDecimal totalPercentage = BigDecimal.valueOf(1).add(overAppliedNumber.divide(BigDecimal.valueOf(100)));
            return loan.getApprovedPrincipal().multiply(totalPercentage);
        } else {
            return loan.getApprovedPrincipal().add(BigDecimal.valueOf(loanProduct.getOverAppliedNumber()));
        }
    }
