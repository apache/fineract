...
Feature: Working Capital Loan Charge Accrual
  ...
  @TestRailId:C102442
  Scenario: Verify Working Capital - Real-time accrual posting for charges - UC9: switching wcl-charge-accrual-time from eod to real-time before COB still accrues pending due-date charge on COB
    Given Admin sets the business date to "01 July 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct         | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ACC_DEF_REV_AM | 01 July 2026    | 01 July 2026             | 9000            | 100000       | 18                | 0        |
    # --- Configure EOD + due-date ---
    When Global config "charge-accrual-date" value set to "due-date"
    When Global config "wcl-charge-accrual-time" value set to "eod"
    And Admin sets the business date to "05 July 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    # --- Add fee charge under EOD (due date in the future, so not accrued yet) ---
    And Admin adds "WORKING_CAPITAL_SPECIFIED_DUE_DATE_FEE" specified due date charge to working capital loan with "15 July 2026" due date and 75.0 transaction amount
    Then Working Capital Loan has transactions:
      | transactionDate | type         | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 July 2026    | Disbursement | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
    # --- Flip to real-time before the charge is accrued ---
    When Global config "wcl-charge-accrual-time" value set to "real-time"
    And Admin sets the business date to "10 July 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital Loan has transactions:
      | transactionDate | type         | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 July 2026    | Disbursement | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
    # --- COB after due date must still catch up the pending charge (dated on due date, not closing date) ---
    When Admin sets the business date to "16 July 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital Loan has transactions:
      | transactionDate | type         | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 July 2026    | Disbursement | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 15 July 2026    | Accrual      | 75.0              | 0.0              | 75.0              | 0.0                   | false    |
    And a Working Capital Loan Accrual transaction business event is raised with "75.0" EUR amount
    And Working Capital Loan Transactions tab has a "ACCRUAL" transaction with date "15 July 2026" which has the following Journal entries:
      | Type   | Account code | Account name            | Debit | Credit |
      | ASSET  | 112603       | Interest/Fee Receivable | 75.0  |        |
      | INCOME | 404007       | Fee Income              |       | 75.0   |
    # --- Close loan ---
    And Admin sets the business date to "25 July 2026"
    Then Admin closes the Working Capital loan with a full repayment on "25 July 2026"

  @TestRailId:C102443
  Scenario: Verify Working Capital - Real-time accrual posting for charges - UC10: switching wcl-charge-accrual-time from eod to real-time before next COB still accrues pending submitted-date charge on COB
    Given Admin sets the business date to "01 August 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct         | submittedOnDate  | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ACC_DEF_REV_AM | 01 August 2026   | 01 August 2026           | 9000            | 100000       | 18                | 0        |
    # --- Configure EOD + submitted-date ---
    When Global config "charge-accrual-date" value set to "submitted-date"
    When Global config "wcl-charge-accrual-time" value set to "eod"
    And Admin sets the business date to "05 August 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    # --- Add fee charge under EOD (accrual waits for next COB) ---
    And Admin adds "WORKING_CAPITAL_SPECIFIED_DUE_DATE_FEE" specified due date charge to working capital loan with "20 August 2026" due date and 60.0 transaction amount
    Then Working Capital Loan has transactions:
      | transactionDate  | type         | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 August 2026   | Disbursement | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
    # --- Flip to real-time before COB catch-up ---
    When Global config "wcl-charge-accrual-time" value set to "real-time"
    And Admin sets the business date to "06 August 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital Loan has transactions:
      | transactionDate  | type         | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 August 2026   | Disbursement | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 05 August 2026   | Accrual      | 60.0              | 0.0              | 60.0              | 0.0                   | false    |
    # --- Close loan ---
    And Admin sets the business date to "25 August 2026"
    Then Admin closes the Working Capital loan with a full repayment on "25 August 2026"

  @TestRailId:C102444
  Scenario: Verify Working Capital - Real-time accrual posting for charges - UC11: charge added to overpaid loan under eod and submitted-date still accrues immediately via closure path
    Given Admin sets the business date to "01 September 2028"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct         | submittedOnDate    | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ACC_DEF_REV_AM | 01 September 2028  | 01 September 2028        | 9000            | 100000       | 18                | 0        |
    # --- Configure EOD + submitted-date (on-add accrual is disabled) ---
    When Global config "charge-accrual-date" value set to "submitted-date"
    When Global config "wcl-charge-accrual-time" value set to "eod"
    ...
    # --- Add fee smaller than overpayment: fee is settled from overpayment, loan stays OVERPAID ---
    ...
    And Working Capital Loan has transactions:
      ...
      | 05 September 2028   | Accrual      | 40.0              | 0.0              | 40.0              | 0.0                   | false    |
    And a Working Capital Loan Accrual transaction business event is raised with "40.0" EUR amount
    And Working Capital Loan Transactions tab has a "ACCRUAL" transaction with date "05 September 2028" which has the following Journal entries:
      | Type   | Account code | Account name            | Debit | Credit |
      | ASSET  | 112603       | Interest/Fee Receivable | 40.0  |        |
      | INCOME | 404007       | Fee Income              |       | 40.0   |
    And Working Capital Loan charge balances has the following data:
