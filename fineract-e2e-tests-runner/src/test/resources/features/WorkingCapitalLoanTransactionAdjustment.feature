...
Feature: Working Capital Loan Transaction Adjustment
  ...
  Scenario: Verify working capital loan goodwill credit adjustment with new amount creates replacement - UC4
    When Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data
    And Admin creates a working capital loan with the following data:
      | LoanProduct         | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
    ...
    Then Working Capital Loan Transactions tab has a "GOODWILL_CREDIT" transaction with date "10 January 2026" which has the following Journal entries:
      ...
      | ASSET   | 112601       | Loans Receivable         |       | 270.0  |
    When Customer adjust "1"th "GOODWILL_CREDIT" transaction made on "10 January 2026" on Working Capital loan with amount "150"
  ...
  Scenario: Verify working capital loan payout refund adjustment with new amount creates replacement - UC5
    When Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data
    And Admin creates a working capital loan with the following data:
      | LoanProduct         | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
    ...
    Then Working Capital Loan Transactions tab has a "PAYOUT_REFUND" transaction with date "10 January 2026" which has the following Journal entries:
      ...
      | LIABILITY | 145023       | Suspense/Clearing account | 270.0 |        |
      | ASSET     | 112601       | Loans Receivable          |       | 270.0  |
    When Customer adjust "1"th "PAYOUT_REFUND" transaction made on "10 January 2026" on Working Capital loan with amount "150"
  ...
  Scenario: Verify working capital loan charge adjustment transaction adjustment with new amount creates replacement - UC6
    When Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data
    And Admin creates a working capital loan with the following data:
      | LoanProduct         | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
    ...
    When Customer adjust "1"th "CHARGE_ADJUSTMENT" transaction made on "10 January 2026" on Working Capital loan with amount "40"
