...
# --- undo write-off the working capital loan account--- #
  ...
  @TestRailId:C102447
  Scenario: Verify Working Capital Write-off: write-off with mapped write-off reason routes the final discount fee amortization to the reason expense account - UC26
    Given Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct              | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ADVANCED_ACCOUNTING | 01 January 2026 | 01 January 2026          | 9000            | 100000       | 18                | 0        |
    ...
    And Admin sets the business date to "03 January 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    # --- write-off with reason Bad Debt: loss and final amortization both land on the mapped expense account --- #
    And Admin does write-off the Working Capital loan on "03 January 2026" with write off reason: "BAD_DEBT"
    Then Working Capital loan status will be "CLOSED_WRITTEN_OFF"
    Then Working Capital Loan Transactions tab has a "WRITE_OFF" transaction with date "03 January 2026" which has the following Journal entries:
      | Type    | Account code | Account name         | Debit  | Credit |
      | ASSET   | 112601       | Loans Receivable     |        | 9950.0 |
  ...
  @TestRailId:C102448
  Scenario: Verify Working Capital Write-off: write-off after charge-off does not amortize the discount fee twice and both undos restore income - UC27
    Given Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct              | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ADVANCED_ACCOUNTING | 01 January 2026 | 01 January 2026          | 9000            | 100000       | 18                | 0        |
    ...
    And Admin sets the business date to "03 January 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin charges off the Working Capital loan on "03 January 2026"
    Then Working Capital Loan Transactions tab has a "DISCOUNT_FEE_AMORTIZATION" transaction with date "03 January 2026" which has the following Journal entries:
      | Type      | Account code | Account name              | Debit  | Credit |
      | LIABILITY | 240005       | Deferred Interest Revenue | 990.39 |        |
      | EXPENSE   | 744007       | Credit Loss/Bad Debt      |        | 990.39 |
    ...
    And Working Capital Loan has transactions:
      ...
      | 02 January 2026 | Repayment                 | 50.0              | 50.0             | 0.0               | 0.0                   | false    |
      | 02 January 2026 | Discount Fee Amortization | 9.61              |                  |                   |                       | false    |
      | 03 January 2026 | Charge-off                | 9950.0            | 9950.0           | 0.0               | 0.0                   | false    |
      | 03 January 2026 | Discount Fee Amortization | 990.39            |                  |                   |                       | false    |
      | 04 January 2026 | Close (as written-off)    | 9950.0            | 9950.0           | 0.0               | 0.0                   | false    |
    Then Working Capital Loan Transactions tab has a "WRITE_OFF" transaction with date "04 January 2026" which has the following Journal entries:
      | Type    | Account code | Account name         | Debit  | Credit |
      | EXPENSE | e4           | Written off          | 9950.0 |        |
      | EXPENSE | 744007       | Credit Loss/Bad Debt |        | 9950.0 |
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
      | 10000.0   | 50.0               | 1000.0         | 0.0              | 0.0               |
    # --- undo write-off keeps the charge-off amortization in place --- #
    When Admin undoes the write-off on the Working Capital loan
    Then Working Capital loan status will be "ACTIVE"
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount | chargedOff |
    ...
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount | chargedOff |
      | 10000.0   | 50.0               | 9.61           | 990.39           | 0.0               | false      |
    Then Admin closes the Working Capital loan with a full repayment on "04 January 2026"

  @TestRailId:C102449
  Scenario: Verify Working Capital Write-off: backdated write-off releases the discount fee still deferred as of the write-off date - UC28
    Given Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct              | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ADVANCED_ACCOUNTING | 01 January 2026 | 01 January 2026          | 9000            | 100000       | 18                | 0        |
    ...
    When Admin sets the business date to "02 January 2026"
    And Customer makes repayment on "02 January 2026" with 50 transaction amount on Working Capital loan
    # --- first COB only on 06 January: the periodic amortization is dated 05 January --- #
    ...
    Then Working Capital Loan Transactions tab has a "DISCOUNT_FEE_AMORTIZATION" transaction with date "05 January 2026" which has the following Journal entries:
      ...
      | LIABILITY | 240005       | Deferred Interest Revenue | 9.61  |        |
      | INCOME    | 404000       | Interest Income           |       | 9.61   |
    # --- write-off backdated to 03 January (after the last user transaction) --- #
    When Admin writes off the Working Capital loan on "03 January 2026"
    Then Working Capital loan status will be "CLOSED_WRITTEN_OFF"
    And Working Capital Loan has transactions:
      | transactionDate | type                      | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 January 2026 | Disbursement              | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
  ...
  @TestRailId:C102450
  Scenario: Verify Working Capital Write-off: write-off with unrealized discount fee raises the discount fee amortization business event and undo raises the adjust event - UC29
    Given Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct              | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP_ADVANCED_ACCOUNTING | 01 January 2026 | 01 January 2026          | 9000            | 100000       | 18                | 0        |
    ...
    And Admin writes off the Working Capital loan on "05 January 2026"
    Then Working Capital loan status will be "CLOSED_WRITTEN_OFF"
    And a Working Capital Loan Write Off transaction business event is raised with "10000.0" EUR amount
    And a Working Capital Loan Discount Fee Amortization transaction business event is raised on "05 January 2026"
    ...
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
      | 10000.0   | 0.0                | 1000.0         | 0.0              | 0.0               |
    When Admin undoes the write-off on the Working Capital loan
    Then Working Capital loan status will be "ACTIVE"
    And a Working Capital Loan Undo Write Off transaction business event is raised with "10000.0" EUR amount
  ...
  Scenario: Verify Working Capital Write-off: same-day discount fee adjustment below realized income posts amortization adjustment - UC30
    ...
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
      | 10000.0   | 50.0               | 9.61           | 990.39           | 0.0               |
# --- Discount fee adjustment drops net discount below already realized income; no COB before write-off --- #
    When Admin sets the business date to "04 January 2026"
    ...
    And Working Capital Loan has transactions:
      ...
      | 04 January 2026 | Close (as written-off)               | 8955.0            | 8955.0           | 0.0               | 0.0                   | false    |
    ...
    And Working Capital Loan has transactions:
      ...
      | 02 January 2026 | Discount Fee Amortization            | 9.61              |                  |                   |                       | false    |
      | 04 January 2026 | Discount Fee Adjustment              | 995.0             | 995.0            | 0.0               | 0.0                   | false    |
      | 04 January 2026 | Close (as written-off)               | 8955.0            | 8955.0           | 0.0               | 0.0                   | true     |
