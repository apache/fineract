...
#  ###############################################
  ...
  @TestRailId:C106783
  Scenario: Verify Working Capital Charge on loan account level - user without CREATE_WORKINGCAPITALLOANCHARGE permission is rejected
    Given Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data and creates-approves-disburses a working capital loan with the following data:
      | LoanProduct | submittedOnDate | expectedDisbursementDate | principalAmount | totalPayment | periodPaymentRate | discount |
      | WCLP        | 01 January 2026 | 01 January 2026          | 9000            | 100000       | 18                | 0        |
    And Admin creates new user with "WC_NO_ADD_CHARGE_USER" username, "WC_NO_ADD_CHARGE_ROLE" role name and given permissions:
      | REPAYMENT_WORKINGCAPITALLOAN |
    Then Created user without CREATE_WORKINGCAPITALLOANCHARGE permission fails to add "WORKING_CAPITAL_SPECIFIED_DUE_DATE_FEE" specified due date charge to working capital loan with "01 January 2026" due date and 35.0 transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Admin closes the Working Capital loan with all obligations met with a full repayment on "01 January 2026"
