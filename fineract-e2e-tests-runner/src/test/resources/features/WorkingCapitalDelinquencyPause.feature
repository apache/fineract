...
Feature: Working Capital Delinquency Pause
  ...
  Scenario: Verify working capital loan delinquency pause - UC4: backdated delinquency pause, pause period is overlapping actual business date
    ...
    And Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-24 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    Then Admin closes the Working Capital loan with all obligations met with a full repayment on "15 January 2026"
  ...
  Scenario: Verify working capital loan delinquency pause - UC6: delinquency pause in the middle of first period with pause overlapping to second period
    ...
    Then Admin closes the Working Capital loan with all obligations met with a full repayment on "25 February 2026"
  ...
  Scenario: Verify working capital loan delinquency pause - UC6.4: backdated delinquency pause overlaps to an existing pause results an error (Negative)
    ...
    And Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-03-13 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    And Admin initiate a Working Capital loan delinquency pause with startDate "05 January 2026" and endDate "10 January 2026"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-15 | 2026-02-25 |
      | PAUSE  | 2026-01-05 | 2026-01-10 |
    Then Initiating a Working Capital loan delinquency pause with startDate "05 January 2026" and endDate "14 January 2026" results an error with the following data:
      | httpCode | errorMessage                                                      |
      | 400      | Delinquency pause period cannot overlap with another pause period |
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-15 | 2026-02-25 |
      | PAUSE  | 2026-01-05 | 2026-01-10 |
    Then Initiating a Working Capital loan delinquency pause with startDate "02 January 2026" and endDate "14 January 2026" results an error with the following data:
      | httpCode | errorMessage                                                      |
      | 400      | Delinquency pause period cannot overlap with another pause period |
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan delinquency pause - UC7: backdated delinquency pause into an already evaluated period shifts and re-evaluates the schedule
    ...
    And Admin initiate a Working Capital loan delinquency pause with startDate "30 January 2026" and endDate "25 February 2026"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-30 | 2026-02-25 |
    And Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-26 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
      | 2            | 2026-02-27 | 2026-03-28 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    When Admin sets the business date to "27 February 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-26 | 270.0          | 0.0        | 270.0             | false                 | 270.0            | 1              |
      | 2            | 2026-02-27 | 2026-03-28 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    Then Admin closes the Working Capital loan with all obligations met with a full repayment on "27 February 2026"
  ...
  Scenario: Verify working capital loan delinquency pause - UC11: backdated pause into an evaluated period shifts subsequent generated periods and reopens them until re-evaluated
    ...
    Then Working Capital loan delinquency range schedule has the following data:
      ...
      | 1            | 2026-01-01 | 2026-01-30 | 270.0          | 0.0        | 270.0             | false                 | 270.0            | 16             |
      | 2            | 2026-01-31 | 2026-03-01 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    And Admin initiate a Working Capital loan delinquency pause with startDate "05 January 2026" and endDate "01 February 2026"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-05 | 2026-02-01 |
    And Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-27 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
      | 2            | 2026-02-28 | 2026-03-29 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    When Admin sets the business date to "28 February 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-27 | 270.0          | 0.0        | 270.0             | false                 | 270.0            | 1              |
      | 2            | 2026-02-28 | 2026-03-29 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
