...
# create it with a future due date and let the business date overtake it.
  ...
  Scenario: Waive a partially paid charge waives only the remainder - UC3
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin adds "WORKING_CAPITAL_SPECIFIED_DUE_DATE_FEE" specified due date charge to working capital loan with "15 January 2026" due date and 100.0 transaction amount
# The fee is not due yet, so the allocation order leaves it alone and the repayment settles principal and discount
# fee in full. The outstanding fee is then the only thing keeping the loan open until the waiver clears it.
    When Admin sets the business date to "12 January 2026"
    And Customer makes repayment on "12 January 2026" with 10000.0 transaction amount on Working Capital loan
...
# The waiver itself is dated back to the due date, but the loan's own lifecycle keeps the day the relief was
# granted: the closing dates and the two postings the closure triggers all take the business date.
    And Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
    And Working capital loan details has the following field values:
      | timeline.closedOnDate       | 2026-01-20 |
      | timeline.actualMaturityDate | 2026-01-20 |
    And Working Capital Loan has transactions:
      | transactionDate | type                      | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 01 January 2026 | Disbursement              | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 01 January 2026 | Discount Fee              | 1000.0            | 1000.0           | 0.0               | 0.0                   | false    |
      | 12 January 2026 | Repayment                 | 10000.0           | 10000.0          | 0.0               | 0.0                   | false    |
      | 15 January 2026 | Waive loan charges        | 60.0              | 0.0              | 60.0              | 0.0                   | false    |
      | 20 January 2026 | Charge Adjustment         | 40.0              | 0.0              | 40.0              | 0.0                   | false    |
      | 20 January 2026 | Discount Fee Amortization | 1000.0            |                  |                   |                       | false    |
      | 20 January 2026 | Accrual                   | 40.0              | 0.0              | 40.0              | 0.0                   | false    |
    And Working Capital Loan Transactions tab has a "ACCRUAL" transaction with date "20 January 2026" which has the following Journal entries:
    ...
    And Working Capital Loan has charges with the following data:
      ...
      | Working Capital Loan Fee | 15 January 2026 | 100.0  | 40.0        | 60.0          | 0.0                |
    And Working Capital Loan charge balances has the following data:
      | Fee Amount | Fee Outstanding | Fee Paid | Penalty Amount | Penalty Outstanding | Penalty Paid |
  ...
  Scenario: Undo a charge waiver restores the charge to outstanding - UC5
    ...
    And Working Capital loan balance payload contains the following fields:
      ...
      | feeWaived | 100.0 |
    And Working capital loan details has the following field values:
      | summary.feeWaived     | 100.0 |
      | summary.penaltyWaived | 0.0   |
