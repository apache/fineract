...
#--- make backdated discount fee adjustment to make loan closed as per finalize test scenario ---#
  ...
  @TestRailId:C102441
  Scenario: Verify overpaidOnDate when a backdated discount fee adjustment tips an ACTIVE loan into overpayment - UC14
    When Admin sets the business date to "01 January 2026"
    And Admin creates a client with random data
    And Admin creates a working capital loan with the following data:
      | LoanProduct              | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | WCLP_ADVANCED_ACCOUNTING | 01 January 2026 | 01 January 2026          | 1000            | 100000             | 18                |          |
    Then Working capital loan creation was successful
    ...
    And Admin successfully disburse the Working Capital loan on "01 January 2026" with "1000" EUR transaction amount
    And Admin successfully add discount with "100" amount on Working Capital loan account
    And Admin loads discount fee transaction from Working Capital loan for adjustment
    When Admin sets the business date to "06 January 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    # 1100 due against 1050 paid: the loan is still active with 50 outstanding
    And Customer makes repayment on "06 January 2026" with 1050.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "ACTIVE"
    And Working capital loan details has the following field values:
      | overpaidOnDate | null |
    When Admin sets the business date to "07 January 2026"
    And Admin runs inline COB job for Working Capital Loan by loanId
    # removing the 100 discount with a backdated date leaves 1000 due against 1050 paid, so the loan becomes
    # overpaid - but the money only arrived on 06 January, so the overpayment cannot start on 03 January
    ...
    And Working capital loan details has the following field values:
      | overpaidOnDate            | 2026-01-06 |
      | balance.overpaymentAmount | 50.0       |
    # the repayment must not allocate more than the 1000 principal the loan is left with, the rest is overpayment
    And Working capital loan account has the correct data:
      | principal | totalPaidPrincipal | overpaymentAmount |
      | 1000.0    | 1000.0             | 50.0              |
