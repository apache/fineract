...
public class UpdateChargeDefinitionCommandHandler implements CommandHandler<UpdateChargeRequest, UpdateChargeResponse> {

    private final ChargeWritePlatformService clientWritePlatformService;

    @Transactional
    @Override
    public UpdateChargeResponse handle(Command<UpdateChargeRequest> command) {
        return clientWritePlatformService.updateCharge(command.getPayload());
    }
}
