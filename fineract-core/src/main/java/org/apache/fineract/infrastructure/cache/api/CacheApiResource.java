...
import org.apache.fineract.infrastructure.cache.command.UpdateCacheCommand;
...
public class CacheApiResource {
    ...
    public CacheResponse switchCache(@HeaderParam("Idempotency-Key") @Valid CacheRequest cacheRequest) {
        var cacheCommand = new UpdateCacheCommand();
        cacheCommand.setId(UUID.randomUUID());
        cacheCommand.setCreatedAt(DateUtils.getAuditOffsetDateTime());
        cacheCommand.setPayload(cacheRequest);

        Supplier<CacheResponse> response = commandPipeline.send(cacheCommand);
