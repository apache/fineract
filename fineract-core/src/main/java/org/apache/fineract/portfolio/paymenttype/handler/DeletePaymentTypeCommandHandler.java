...
public class DeletePaymentTypeCommandHandler implements CommandHandler<DeletePaymentTypeRequest, UpdatablePaymentTypeResponse> {

    private final PaymentTypeWriteService paymentTypeWriteService;

    @Override
    @Transactional
    public UpdatablePaymentTypeResponse handle(Command<DeletePaymentTypeRequest> command) {
        return paymentTypeWriteService.deletePaymentType(command.getPayload());
    }
}
