...
public class UpdatePaymentTypeCommandHandler implements CommandHandler<UpdatePaymentTypeRequest, UpdatablePaymentTypeResponse> {

    private final PaymentTypeWriteService paymentTypeWriteService;

    @Override
    @Transactional
    public UpdatablePaymentTypeResponse handle(Command<UpdatePaymentTypeRequest> command) {
        return this.paymentTypeWriteService.updatePaymentType(command.getPayload());
    }
}
