...
public class FeignDepositProductArchivingTest extends FeignIntegrationTest {
    ...
    private static final String INACTIVE_STATUS = "savingsProduct.inActive";
    ...
    public void testFixedDepositProductCanBeArchived() throws JsonProcessingException {
        ...
        final Long existingAccountId = ok(() -> fineractClient().fixedDepositAccount()
                .createFixedDepositAccount(fixedDepositAccountRequest(clientId, productId, businessDate))).getResourceId();
        ...
        assertApplicationDateError(
                fail(() -> fineractClient().fixedDepositAccount()
                        .createFixedDepositAccount(fixedDepositAccountRequest(clientId, productId, startDate.minusDays(1)))),
                BEFORE_START_DATE_ERROR);
        assertApplicationDateError(
                fail(() -> fineractClient().fixedDepositAccount()
                        .createFixedDepositAccount(fixedDepositAccountRequest(clientId, productId, businessDate))),
    ...
    public void testArchivedDepositProductsRejectReinvestment() throws JsonProcessingException {
        ...
        final Long fixedDepositAccountId = ok(() -> fineractClient().fixedDepositAccount().createFixedDepositAccount(
        ...
        ok(() -> fineractClient().fixedDepositAccount().handleCommandsFixedDepositAccount(fixedDepositAccountId,
                fixedDepositDatedCommandRequest("approvedOnDate", applicationDate), "approve"));
        ok(() -> fineractClient().fixedDepositAccount().handleCommandsFixedDepositAccount(fixedDepositAccountId,
                fixedDepositDatedCommandRequest("activatedOnDate", applicationDate), "activate"));
        ...
        ok(() -> fineractClient().recurringDepositAccount().handleCommandsRecurringDepositAccount(recurringDepositAccountId,
                recurringDepositDatedCommandRequest("approvedOnDate", applicationDate), "approve"));
        ok(() -> fineractClient().recurringDepositAccount().handleCommandsRecurringDepositAccount(recurringDepositAccountId,
                recurringDepositDatedCommandRequest("activatedOnDate", applicationDate), "activate"));
        ...
        assertApplicationDateError(fail(() -> fineractClient().fixedDepositAccount()
                .handleCommandsFixedDepositAccount(fixedDepositAccountId, fixedDepositReinvestmentClosureRequest(businessDate), "close")),
                AFTER_CLOSE_DATE_ERROR);
        assertApplicationDateError(fail(() -> fineractClient().recurringDepositAccount().handleCommandsRecurringDepositAccount(
                recurringDepositAccountId, recurringDepositReinvestmentClosureRequest(businessDate), "close")),
                AFTER_CLOSE_DATE_ERROR);
    }
    ...
    private Long createFixedDepositProduct(final LocalDate chartStartDate) {
        ...
        final PostFixedDepositProductsRequest request = new PostFixedDepositProductsRequest().accountingRule(1).charts(Set.of(chart))
                .currencyCode("USD").depositAmount(BigDecimal.valueOf(100000)).description("Fixed deposit product for archiving test")
                .digitsAfterDecimal(4).inMultiplesOf(100).interestCalculationDaysInYearType(365).interestCalculationType(1)
                .interestCompoundingPeriodType(4).interestPostingPeriodType(4).locale(Utils.LOCALE).maxDepositTerm(10)
                .maxDepositTermTypeId(3).minDepositTerm(6).minDepositTermTypeId(2)
                .name(Utils.uniqueRandomStringGenerator("FIXED_DEPOSIT_PRODUCT_", 6)).preClosurePenalApplicable(true)
    ...
    private Long createRecurringDepositProduct(final LocalDate chartStartDate) {
        ...
        final PostRecurringDepositProductsRequest request = new PostRecurringDepositProductsRequest().accountingRule(1)
                .charts(Set.of(chart)).currencyCode("USD").depositAmount(BigDecimal.valueOf(100000))
                ...
                .interestPostingPeriodType(4).locale(Utils.LOCALE).maxDepositAmount(BigDecimal.valueOf(1000000)).maxDepositTerm(10)
                .maxDepositTermTypeId(3).minDepositAmount(BigDecimal.valueOf(100)).minDepositTerm(6).minDepositTermTypeId(2)
    ...
    private PostFixedDepositAccountsRequest fixedDepositAccountRequest(final Long clientId, final Long productId,
        ...
        return new PostFixedDepositAccountsRequest().clientId(clientId).productId(productId).locale(Utils.LOCALE)
                .dateFormat(Utils.DATE_FORMAT).submittedOnDate(format(submittedOnDate)).depositAmount(BigDecimal.valueOf(100000))
                .depositPeriod(14).depositPeriodFrequencyId(2L);
    }
    ...
    private PostRecurringDepositAccountsRequest recurringDepositAccountRequest(final Long clientId, final Long productId,
        ...
        return new PostRecurringDepositAccountsRequest().clientId(clientId).productId(productId).locale(Utils.LOCALE)
                .dateFormat(Utils.DATE_FORMAT).submittedOnDate(format(submittedOnDate)).depositAmount(BigDecimal.valueOf(2000))
                .depositPeriod(14).depositPeriodFrequencyId(2).isCalendarInherited(false).recurringFrequency(1).recurringFrequencyType(2)
                .mandatoryRecommendedDepositAmount(BigDecimal.valueOf(2000));
    }

    private PostFixedDepositAccountsAccountIdRequest fixedDepositDatedCommandRequest(final String dateParameter, final LocalDate date) {
        final PostFixedDepositAccountsAccountIdRequest request = new PostFixedDepositAccountsAccountIdRequest().locale(Utils.LOCALE)
                .dateFormat(Utils.DATE_FORMAT);
        if ("approvedOnDate".equals(dateParameter)) {
            return request.approvedOnDate(format(date));
        } else if ("activatedOnDate".equals(dateParameter)) {
            return request.activatedOnDate(format(date));
        }
        throw new IllegalArgumentException("Unsupported date parameter: " + dateParameter);
    }

    private PostRecurringDepositAccountsAccountIdRequest recurringDepositDatedCommandRequest(final String dateParameter,
            final LocalDate date) {
        final PostRecurringDepositAccountsAccountIdRequest request = new PostRecurringDepositAccountsAccountIdRequest()
                .locale(Utils.LOCALE).dateFormat(Utils.DATE_FORMAT);
        if ("approvedOnDate".equals(dateParameter)) {
            return request.approvedOnDate(format(date));
        } else if ("activatedOnDate".equals(dateParameter)) {
            return request.activatedOnDate(format(date));
    ...
    private PostFixedDepositAccountsAccountIdRequest fixedDepositReinvestmentClosureRequest(final LocalDate closedOnDate) {
        return new PostFixedDepositAccountsAccountIdRequest().locale(Utils.LOCALE).dateFormat(Utils.DATE_FORMAT)
                .closedOnDate(format(closedOnDate)).onAccountClosureId(300);
    }

    private PostRecurringDepositAccountsAccountIdRequest recurringDepositReinvestmentClosureRequest(final LocalDate closedOnDate) {
        return new PostRecurringDepositAccountsAccountIdRequest().locale(Utils.LOCALE).dateFormat(Utils.DATE_FORMAT)
                .closedOnDate(format(closedOnDate)).onAccountClosureId(300);
    }
