...
public class FeignWorkingCapitalLoanBreachResetUndoTest extends FeignWorkingCapitalTestBase {
    ...
    @Test
    @DisplayName("Undo on a grace-days loan after a pause restores period 1 to its pause and grace extended 68 days")
    void undoAfterAPauseOnAGraceDaysLoan_restoresThePauseAndGraceExtendedFirstPeriod() {
        runAt("2026-01-01", () -> {
            final Long loanId = setupGraceDaysBreachLoanWithPause();

            advanceBusinessDateWithCob(loanId, "2026-02-10", "2026-02-20");
            createBreachResetWithRestartPeriod(loanId);
            validateBreachSchedule(getBreachSchedule(loanId), //
                    period(1, "2026-01-01", "2026-02-19", 50, "400.00", "200.00", true, false), //
                    period(2, "2026-02-20", "2026-04-20", 60, "400.00", "400.00", null, true));
            validateBreachPastDueAmount(getBreachPastDueAmount(loanId), "0");

            createBreachUndoReset(loanId);

            validateBreachSchedule(getBreachSchedule(loanId), //
                    period(1, "2026-01-01", "2026-03-09", 68, "400.00", "200.00", null, false));
            validateBreachPastDueAmount(getBreachPastDueAmount(loanId), "0");
            validateBreachActions(getBreachActions(loanId), //
                    action("PAUSE", "2026-02-10"), //
                    action("RESET", "2026-02-20"), //
                    action("UNDO_RESET", "2026-02-20"));
        });
    }

    @Test
    @DisplayName("Undo of a no-split reset on a grace-days loan after a pause is flag-only: schedule structure unchanged")
    void undoOfANoSplitResetAfterAPauseOnAGraceDaysLoan_leavesTheScheduleUntouched() {
        runAt("2026-01-01", () -> {
            final Long loanId = setupGraceDaysBreachLoanWithPause();

            advanceBusinessDateWithCob(loanId, "2026-02-10", "2026-03-10");
            validateBreachSchedule(getBreachSchedule(loanId), //
                    period(1, "2026-01-01", "2026-03-09", 68, "400.00", "200.00", true, false), //
                    period(2, "2026-03-10", "2026-05-08", 60, "400.00", "400.00", null, false));
            validateBreachPastDueAmount(getBreachPastDueAmount(loanId), "200");

            createBreachResetWithRestartPeriod(loanId);
            validateBreachSchedule(getBreachSchedule(loanId), //
                    period(1, "2026-01-01", "2026-03-09", 68, "400.00", "200.00", true, false), //
                    period(2, "2026-03-10", "2026-05-08", 60, "400.00", "400.00", null, true));
            validateBreachPastDueAmount(getBreachPastDueAmount(loanId), "0");
            ...
            validateBreachActions(getBreachActions(loanId), //
                    action("PAUSE", "2026-02-10"), //
                    action("RESET", "2026-03-10"), //
                    action("UNDO_RESET", "2026-03-10"));
        });
    }
    ...
    /**
     * Grace 3 on the same breach config, one repayment of 200 on 15 Jan, pause 10..14 Feb: period 1
     * [2026-01-01..2026-03-09] (60 days + 3 grace + 5 paused), business date left at 10 Feb.
     */
    private Long setupGraceDaysBreachLoanWithPause() {
        final Long clientId = createClient("01 January 2026");
        final Long productId = createWcProductWithBreachConfig(BREACH_FREQUENCY, BREACH_FREQUENCY_TYPE, BREACH_AMOUNT_CALCULATION_TYPE,
                BREACH_AMOUNT_PERCENT, 3);
        final Long loanId = createApproveAndDisburseWcLoan(clientId, productId, PRINCIPAL, "01 January 2026");
        runInlineWcCob(loanId);
        ...
        advanceBusinessDateWithCob(loanId, "2026-01-15", "2026-02-10");
        createBreachPause(loanId, "10 February 2026", "14 February 2026");
        validateBreachSchedule(getBreachSchedule(loanId), //
                period(1, "2026-01-01", "2026-03-09", 68, "400.00", "200.00", null, false));
        return loanId;
    }
