...
public class SystemCodeTest {
	...
	public void testCreateCode() {
		...
		final Integer createResponseId = (Integer) CodeHelper.createCode(
				this.requestSpec, this.responseSpec, codeName,
				CodeHelper.RESPONSE_ID_ATTRIBUTE_NAME);

		// verify code created
		final HashMap newCodeAttributes = (HashMap) CodeHelper.getCodeById(
				this.requestSpec, this.responseSpec, createResponseId, "");
		...
		assertEquals("Verify system defined is false", false,
				newCodeAttributes
						.get(CodeHelper.CODE_SYSTEM_DEFINED_ATTRIBUTE_NAME));

		// update code
		final HashMap updateChangeResponse = (HashMap) CodeHelper.updateCode(
				this.requestSpec, this.responseSpec, createResponseId, codeName
						+ "(CHANGE)", "changes");
		...
		// delete code
		final Integer deleteResponseId = (Integer) CodeHelper.deleteCodeById(
				this.requestSpec, this.responseSpec, createResponseId,
				CodeHelper.RESPONSE_ID_ATTRIBUTE_NAME);
		assertEquals("Verify code deleted", createResponseId, deleteResponseId);

		// verify code deleted
		final HashMap deletedCodeValues = (HashMap) CodeHelper.getCodeById(
	...
	public void testPreventCreateDuplicateCode() {
		...
		final List<HashMap> error = (List) CodeHelper.createCode(
				this.requestSpec, this.generalResponseSpec, codeName,
		...
		// verify code deleted
		final HashMap deletedCodeAttributes = (HashMap) CodeHelper.getCodeById(
				this.requestSpec, this.generalResponseSpec, deleteResponseId,
				"");
	...
	public void testUpdateDeleteSystemDefinedCode() {
		...
		final HashMap systemDefinedCode = (HashMap) CodeHelper
				.getSystemDefinedCodes(this.requestSpec, this.responseSpec);

		// delete system-defined code should fail
		final List<HashMap> error = (List) CodeHelper.deleteCodeById(
				this.requestSpec, this.generalResponseSpec,
				(Integer) systemDefinedCode
						.get(CodeHelper.CODE_ID_ATTRIBUTE_NAME),
				CommonConstants.RESPONSE_ERROR);
		...
		final List<HashMap> updateError = (List) CodeHelper.updateCode(
				this.requestSpec, this.generalResponseSpec,
				(Integer) systemDefinedCode
						.get(CodeHelper.CODE_ID_ATTRIBUTE_NAME),
	...
	public void testCodeValuesNotAssignedToTable() {
		...
		// create code
		final Integer createCodeResponseId = (Integer) CodeHelper.createCode(
				this.requestSpec, this.responseSpec, codeName,
				CodeHelper.RESPONSE_ID_ATTRIBUTE_NAME);

		// create first code value
		final Integer createCodeValueResponseId1 = (Integer) CodeHelper
				.createCodeValue(this.requestSpec, this.responseSpec,
						createCodeResponseId, codeValue1, codeValue1Position,
						CodeHelper.SUBRESPONSE_ID_ATTRIBUTE_NAME);

		// create second code value
		final Integer createCodeValueResponseId2 = (Integer) CodeHelper
				.createCodeValue(this.requestSpec, this.responseSpec,
						createCodeResponseId, codeValue2, codeValue2Position,
						CodeHelper.SUBRESPONSE_ID_ATTRIBUTE_NAME);

		// verify two code values created

		final List<HashMap> codeValuesList = (List) CodeHelper
				.getCodeValuesForCode(this.requestSpec, this.responseSpec,
		...
		final HashMap codeValuesAttributes1 = (HashMap) CodeHelper
				.getCodeValueById(this.requestSpec, this.responseSpec,
		...
		assertEquals("Verify value of codeValueId", createCodeValueResponseId1,
				codeValuesAttributes1
						.get(CodeHelper.CODE_VALUE_ID_ATTRIBUTE_NAME));

		assertEquals("Verify value of code name", codeValue1,
				codeValuesAttributes1
						.get(CodeHelper.CODE_VALUE_NAME_ATTRIBUTE_NAME));

		assertEquals("Verify position of code value", codeValue1Position,
				codeValuesAttributes1
						.get(CodeHelper.CODE_VALUE_POSITION_ATTRIBUTE_NAME));

		// verify values of second code value
		final HashMap codeValuesAttributes2 = (HashMap) CodeHelper
				.getCodeValueById(this.requestSpec, this.responseSpec,
		...
		final HashMap codeValueChanges = (HashMap) CodeHelper
				.updateCodeValue(this.requestSpec, this.responseSpec,
						createCodeResponseId, createCodeValueResponseId1,
						codeValue1 + "CHANGE", 4, "changes");

		assertEquals("Verify changed code value name",
				codeValueChanges.get("name"), codeValue1 + "CHANGE");

		// delete code value
		final Integer deletedCodeValueResponseId1 = (Integer) CodeHelper
				.deleteCodeValueById(this.requestSpec,
		...
		final HashMap deletedCodeValueAttributes1 = (HashMap) CodeHelper
				.getCodeValueById(this.requestSpec, this.generalResponseSpec,
						createCodeResponseId, deletedCodeValueResponseId1, "");

		// Assert.assertNotNull(deletedCodeValueAttributes1);
		assertNull("Verify value of codeValueId",
				deletedCodeValueAttributes1
						.get(CodeHelper.CODE_VALUE_ID_ATTRIBUTE_NAME));

		assertNull("Verify value of code name",
				deletedCodeValueAttributes1
						.get(CodeHelper.CODE_VALUE_NAME_ATTRIBUTE_NAME));

		assertNull("Verify position of code value",
				deletedCodeValueAttributes1
						.get(CodeHelper.CODE_VALUE_POSITION_ATTRIBUTE_NAME));

		final List<HashMap> deletedCodeValuesList = (List) CodeHelper
				.getCodeValuesForCode(this.requestSpec, this.responseSpec,
						createCodeResponseId, "");
		...
		final Integer deleteCodeResponseId = (Integer) CodeHelper
				.deleteCodeById(this.requestSpec, this.generalResponseSpec,
						createCodeResponseId,
						CodeHelper.RESPONSE_ID_ATTRIBUTE_NAME);
		assertEquals("Verify code deleted", createCodeResponseId,
				deleteCodeResponseId);
		...
		Assert.assertNotNull(deletedCodeAttributes);
		assertNull("Verify value of codeId is null",
				deletedCodeAttributes.get(CodeHelper.CODE_ID_ATTRIBUTE_NAME));

		assertNull("Verify code name is null",
				deletedCodeAttributes.get(CodeHelper.CODE_NAME_ATTRIBUTE_NAME));
		assertNull("Verify system defined is null",
				deletedCodeAttributes
						.get(CodeHelper.CODE_SYSTEM_DEFINED_ATTRIBUTE_NAME));

		// Verify remaining code value deleted
		final HashMap deletedCodeValues2 = (HashMap) CodeHelper
				.getCodeValueById(this.requestSpec, this.generalResponseSpec,
						createCodeResponseId, createCodeValueResponseId2, "");

		assertNull("Verify value of codeValueId is null",
				deletedCodeValues2.get(CodeHelper.CODE_VALUE_ID_ATTRIBUTE_NAME));

		assertNull("Verify value of code name is null",
				deletedCodeValues2
						.get(CodeHelper.CODE_VALUE_NAME_ATTRIBUTE_NAME));

		assertNull("Verify position of code value null",
				deletedCodeValues2
						.get(CodeHelper.CODE_VALUE_POSITION_ATTRIBUTE_NAME));
