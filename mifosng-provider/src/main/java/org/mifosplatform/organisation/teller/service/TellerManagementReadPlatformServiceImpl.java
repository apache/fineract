...
public class TellerManagementReadPlatformServiceImpl implements TellerManagementReadPlatformService {
    ...
    private static final class TellerMapper implements RowMapper<TellerData> {
        ...
        public TellerData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return TellerData.instance(id, officeId, debitAccountId, creditAccountId,
            		tellerName, description, startDate, endDate, tellerStatus, officeName, null, null);
        }
    }
	...
	@Override
	public Collection<CashierData> getCashiersForTeller(Long tellerId, 
	...
	@Override
	public Collection<CashierData> getCashierData(Long officeId, Long tellerId,
			Long staffId, Date date) {
	...
	@Override
	public Collection<TellerJournalData> getJournals(Long officeId,
			Long tellerId, Long cashierId, Date dateFrom, Date dateTo) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Collection<TellerJournalData> fetchTellerJournals(Long tellerId,
			Long cashierId, Date fromDate, Date toDate) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CashierData retrieveCashierTemplate(Long officeId, Long tellerId,
        ...
        return CashierData.template(officeId, officeName, 
        		tellerId, tellerName, staffOptions);
	}
	...
	public CashierTransactionData retrieveCashierTxnTemplate(Long cashierId) {
        ...
        return CashierTransactionData.template(cashierId, tellerId, tellerName, officeId, 
        		officeName, cashierName, cashierData, startDate, endDate);
	}
	
	@Override
    public CashierTransactionsWithSummaryData retrieveCashierTransactionsWithSummary (
    		final Long cashierId, final boolean includeAllTellers,
        ...
        final CashierTransactionSummaryMapper ctsm = 
        		new CashierTransactionSummaryMapper();
        ...
        Collection<CashierTransactionTypeTotalsData> cashierTxnTypeTotals = 
        		this.jdbcTemplate.query(sql, ctsm, new Object[] { 
    ...
    public Collection<CashierTransactionData> retrieveCashierTransactions (
        ...
        final String sql = "select * from (select " + 
        		...
        		" where txn.cashier_id = ? and o.hierarchy like ? ) cashier_txns " +
        		" union (select " +
        		...
        		" union (select " +
        		ctm.loansTxnSchema() +
        		" where loan_txn.is_reversed = 0 and c.id = ? and o.hierarchy like ? and " +
        		" created_date between c.start_date and date_add(c.end_date, interval 1 day) " +
        		" and renum.enum_value in ('Repayment At Disbursement','Repayment', 'Recovery Payment','Disbursement') ) " +
        		" order by created_date ";

        return this.jdbcTemplate.query(sql, ctm, 
        		new Object[] { cashierId, hierarchySearchString,
        		 	cashierId, hierarchySearchString,
        			cashierId, hierarchySearchString});
    }
	...
	private static final class CashierMapper implements RowMapper<CashierData> {
        ...
        public CashierData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return CashierData.instance(id, null, null, 
            		staffId, staffName, tellerId, tellerName,
            		description, startDate.toDate(), endDate.toDate(), fullDay, startTime, endTime);            
        }
    }
	...
	private static final class CashierTransactionMapper implements RowMapper<CashierTransactionData> {
        ...
        public CashierTransactionData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return CashierTransactionData.instance(
            		id, cashierId, txnType, txnAmount, txnDate, txnNote,
            		entityType, entityId, createdDate, officeId, officeName,
            		tellerId, tellerName, cashierName, null, null, null);
        }
    }
	...
	private static final class CashierTransactionSummaryMapper implements RowMapper<CashierTransactionTypeTotalsData> {
        ...
        @Override
        public CashierTransactionTypeTotalsData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {

        	final Integer cashierTxnType = rs.getInt("cash_txn_type");
            final BigDecimal txnTotal = rs.getBigDecimal("txn_total");
            
            return CashierTransactionTypeTotalsData.instance(
            		cashierTxnType, txnTotal);
        }
    }
