...
public class ReccuringDepositInterestCalculator {
    ...
    public Money calculateInterestOnMaturityFor(final Money savingsDepositPerPeriod, final Integer tenure,
            final BigDecimal reccuringInterestRate, final SavingFrequencyType savingFrequencyType,
            final SavingInterestCalculationMethod savingInterestCalculationMethod, Integer depositEvery) {
        ...
        if (savingInterestCalculationMethod.isAverageBalance()) {
            ...
            while (noOfTimestobeCalculated > 0) {
                ...
                BigDecimal interestRatePerTerm = BigDecimal.valueOf(interestRateAsFraction.doubleValue() / monthsInYear.doubleValue()
                        * depositEveryOriginalValue.doubleValue());
             ...
             * A=P(1+r/n)^nt
             * 
             * A = final amount P = principal amount (initial investment) r =
             * annual nominal interest rate (as a decimal, not in percentage) n
             * = number of times the interest is compounded per year t = number
             * of years
             */
            ...
            while (noOfTimestobeCalculated > 0) {
                ...
                amountPerPeriod = savingsDepositPerPeriod.getAmount()
                        .multiply(
                                BigDecimal.valueOf(Math.pow(onePlusInterestRatePerPeriod.doubleValue(),
                                        timeforOneCalculationPeriod.doubleValue())));
    ...
    private Integer determineIntValue(final SavingFrequencyType frequencyType) {
