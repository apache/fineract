...
public class LoanApplicationWritePlatformServiceJpaRepositoryImpl implements LoanApplicationWritePlatformService {
    ...
    public EntityIdentifier submitLoanApplication(final JsonCommand command) {
        ...
        final CalculateLoanScheduleQuery calculateLoanScheduleQuery = this.calculateLoanScheduleQueryFromApiJsonDeserializer.commandFromApiJson(command.json());
        ...
        if (!combinedErrors.isEmpty()) { 
            throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.", combinedErrors); 
        }
    ...
    @Transactional
    @Override
    public EntityIdentifier deleteLoanApplication(final Long loanId) {
