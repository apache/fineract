...
public class LoanRescheduleRequestDataValidator {
	...
	public void validateForCreateAction(final JsonCommand jsonCommand, final Loan loan) {
        ...
        if(!loan.status().isActive()) {
        	dataValidatorBuilder
    			.reset()
    			.failWithCodeNoParameterAddedToErrorCode("loan.is.not.active", 
    					"Loan is not active");
        }
        
        final Long loanId = this.fromJsonHelper.extractLongNamed(
        		RescheduleLoansApiConstants.loanIdParamName, jsonElement);
        dataValidatorBuilder.reset().parameter(RescheduleLoansApiConstants.loanIdParamName)
        		.value(loanId).notNull().integerGreaterThanZero();
        
        final LocalDate submittedOnDate = this.fromJsonHelper.extractLocalDateNamed(
        ...
        final Integer extraTerms = this.fromJsonHelper.extractIntegerWithLocaleNamed(
        ...
        dataValidatorBuilder.reset().parameter(RescheduleLoansApiConstants.extraTermsParamName)
    			.value(extraTerms).ignoreIfNull().integerGreaterThanZero();
        
        final Long rescheduleReasonId = this.fromJsonHelper.extractLongNamed(
        ...
        final LocalDate adjustedDueDate = this.fromJsonHelper.extractLocalDateNamed(
        ...
        // at least one of the following must be provided => graceOnPrincipal, graceOnInterest, extraTerms, newInterestRate
        ...
        if(loanId != null) {
        	...
        	if(loanRescheduleRequestData.size() > 0) {
            	dataValidatorBuilder
    				.reset()
    				.failWithCodeNoParameterAddedToErrorCode("loan.already.rescheduled", 
	...
	public void validateForApproveAction(final JsonCommand jsonCommand, LoanRescheduleRequest loanRescheduleRequest) {
        ...
        final LocalDate approvedOnDate = this.fromJsonHelper.extractLocalDateNamed(
        ...
        if(approvedOnDate != null && loanRescheduleRequest.getSubmittedOnDate().isAfter(approvedOnDate)) {
        	dataValidatorBuilder
				...
				.failWithCode("before.submission.date", 
        ...
        if(loan != null) {
        	...
        	if(rescheduleFromDate != null) {
                ...
                if(installment == null) {
                	dataValidatorBuilder
                		.reset()
                		.failWithCodeNoParameterAddedToErrorCode("loan.repayment.schedule.installment.does.not.exist", 
                				"Repayment schedule installment does not exist");
                }
                ...
                if(installment != null && installment.isObligationsMet()) {
                	dataValidatorBuilder
            			.reset()
            			.failWithCodeNoParameterAddedToErrorCode("loan.repayment.schedule.installment."
	...
	public void validateForRejectAction(final JsonCommand jsonCommand, LoanRescheduleRequest loanRescheduleRequest) {
        ...
        final LocalDate rejectedOnDate = this.fromJsonHelper.extractLocalDateNamed(
