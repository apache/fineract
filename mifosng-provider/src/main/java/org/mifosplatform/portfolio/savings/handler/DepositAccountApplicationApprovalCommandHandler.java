...
@Service
public class DepositAccountApplicationApprovalCommandHandler implements NewCommandSourceHandler {

    private final DepositApplicationProcessWritePlatformService depositAccountWritePlatformService;

    @Autowired
    public DepositAccountApplicationApprovalCommandHandler(
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return this.depositAccountWritePlatformService.approveApplication(command.entityId(), command, command.depositAccounttype());
    }
}