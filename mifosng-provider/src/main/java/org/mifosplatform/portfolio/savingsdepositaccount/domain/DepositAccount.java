...
public class DepositAccount extends AbstractAuditableCustom<AppUser, Long> {
    ...
    public DepositAccount openNew(final Client client, final DepositProduct product, final String externalId, final Money deposit,
        ...
        if (interestCompoundingAllowed) {
            ...
            futureValueOnMaturity = fixedTermDepositInterestCalculator.calculateInterestOnMaturityForSimpleInterest(deposit,
                    tenureInMonths, maturityInterestRate, interestCompoundedEvery);
        }
    ...
    public Map<String, Object> approve(final LocalDate actualCommencementDate,
            final DepositLifecycleStateMachine depositLifecycleStateMachine, final JsonCommand command,
        ...
        if (this.interestCompoundingAllowed) {
            ...
            futureValueOnMaturity = calculator.calculateInterestOnMaturityForSimpleInterest(getDeposit(), this.tenureInMonths,
                    this.interestRate, this.interestCompoundedEvery);
        }
    ...
    public void withdrawDepositAccountMoney(final DepositLifecycleStateMachine depositLifecycleStateMachine,
        ...
        DepositAccountTransaction depositaccountTransaction = DepositAccountTransaction.withdraw(getDeposit(), eventDate,
                Money.of(currency, this.availableInterest), getDeposit().getAmount().add(this.availableInterest));
        // DepositAccountTransaction depositaccountTransaction =
        // DepositAccountTransaction.withdraw(getDeposit(), eventDate,
        // getAccuredInterest());
        depositaccountTransaction.updateAccount(this);
        this.depositaccountTransactions.add(depositaccountTransaction);
    ...
    public void withdrawInterest(final Money interest) {
        ...
        if (isActive()) {
            // DepositAccountTransaction depositAccountTransaction =
            // DepositAccountTransaction.withdraw(null, new LocalDate(),
            // interest);
            DepositAccountTransaction depositAccountTransaction = DepositAccountTransaction.withdraw(null, new LocalDate(), interest,
                    getDeposit().getAmount().add(this.availableInterest).subtract(interest.getAmount()));
            depositAccountTransaction.updateAccount(this);
            this.depositaccountTransactions.add(depositAccountTransaction);
    ...
    public void closeDepositAccount(final DepositLifecycleStateMachine depositLifecycleStateMachine) {
        ...
        // DepositAccountTransaction depositaccountTransaction =
        // DepositAccountTransaction.withdraw(getDeposit(), new
        // LocalDate(),Money.of(getDeposit().getCurrency(),
        // getTotal()).minus(getDeposit()));
        ...
        while (postInterestItereations > 0) {
            ...
            if (lastInterestPostedDate.isBefore(getMaturityDate())) {
                if (nextInterestPostingDate.isAfter(getMaturityDate())) {
                    ...
                    Integer noOfMonthsBeforeMaturity = Months.monthsBetween(lastInterestPostedDate, getMaturityDate().plusDays(1))
                            .getMonths();
                    Money futureValueOnMaturity = Money.zero(this.currency);
                    ...
                    DepositAccountTransaction depositaccountTransaction = DepositAccountTransaction.postInterest(null,
                            nextInterestPostingDate, futureValueOnMaturity.minus(deposit), deposit.getAmount().add(this.availableInterest));
                ...
                DepositAccountTransaction depositaccountTransaction = DepositAccountTransaction.postInterest(null, nextInterestPostingDate,
                        futureValueOnMaturity.minus(deposit), deposit.getAmount().add(this.availableInterest));
                depositaccountTransaction.updateAccount(this);
                this.depositaccountTransactions.add(depositaccountTransaction);
            }
