...
public class ReadWriteNonCoreDataServiceImpl implements
	...
	@Override
	public void newDatatableEntry(final String datatable,
		...
		Map<String, String> dataParams = this.fromJsonHelper.extractDataMap(
				typeOfMap, command.json());

		String sql = getAddSql(columnHeaders, datatable, getFKField(appTable),
				appTableId, dataParams);
	...
	@Override
	public void deleteDatatableEntries(final String datatable,
	...
	@Override
	public void deleteDatatableEntry(final String datatable,
	...
	public GenericResultsetData retrieveDataTableGenericResultSet(
			final String datatable, final Long appTableId, final String order,
			final Long id) {
	...
	private void checkMainResourceExistsWithinScope(final String appTable,
			final Long appTableId) {
	...
	private String getWithinScopeApplicationTableName(final String datatable,
			final Long appTableId) {
	...
	private void addCodesValueIfNecessary(final ResultsetColumnHeader rsch,
			final String code_suffix) {
	...
	private String getAddSql(final List<ResultsetColumnHeader> columnHeaders,
	...
	private Map<String, String> getAffectedColumns(
			final List<ResultsetColumnHeader> columnHeaders,
	...
	private String validateColumn(final ResultsetColumnHeader columnHeader,
		...
		if ((columnHeader.getColumnDisplayType().equals("DATE"))
				|| (columnHeader.getColumnDisplayType().equals("INTEGER"))
				|| (columnHeader.getColumnDisplayType().equals("DECIMAL")))
		...
		if (!StringUtils.isEmpty(paramValue)) {
			...
			if (columnHeader.getColumnDisplayType().equals("DATE")) {
				LocalDate tmpDate = helper.convertFrom(paramValue,
			...
			if (columnHeader.getColumnDisplayType().equals("INTEGER")) {
				Integer tmpInt = helper.convertToInteger(paramValue,
			...
			if (columnHeader.getColumnDisplayType().equals("DECIMAL")) {
				BigDecimal tmpDecimal = helper.convertFrom(paramValue,
						columnHeader.getColumnName(), clientApplicationLocale);
				if (tmpDecimal == null)
					return null;
