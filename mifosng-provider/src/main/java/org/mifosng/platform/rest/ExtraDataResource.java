...
@Path("/protected/extradata")
@Component
@Scope("singleton")
public class ExtraDataResource {

	private final static Logger logger = LoggerFactory
			.getLogger(ExtraDataResource.class);
...
//	}
	...
	@GET
	@Path("datasets/{datasetType}")
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public Response datasets(@PathParam("datasetType") final String datasetType, @Context UriInfo uriInfo) {
		
		//MultivaluedMap<String, String> queryParams = uriInfo.getQueryParameters();
		//String tableName = queryParams.getFirst("tableName"); 

		ExtraDatasets result = this.readPlatformService.retrieveExtraDatasetNames(datasetType);
		return Response.ok().entity(result).build();
	}
