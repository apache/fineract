...
public class ReccuringDepositInterestCalculator {
	...
	public Money calculateInterestOnMaturityFor(Money savingsDepositPerPeriod,
		...
		/*
		 // Assuming tenure in months
		 // see http://www.onemint.com/2012/04/03/how-to-calculate-interest-on-recurring-deposits/
	...
	*/
		...
		while (tenure > 0) {
			...
			amountPerPeriod = savingsDepositPerPeriod.getAmount().multiply(BigDecimal.valueOf(Math.pow(onePlusInterestRatePerPeriod.doubleValue(), timeforOneCalculationPeriod.doubleValue())));
