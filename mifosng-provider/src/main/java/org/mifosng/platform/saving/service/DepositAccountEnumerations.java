...
public class DepositAccountEnumerations {
	...
	public static EnumOptionData status(final DepositAccountStatus status) {
		...
		case ACTIVE:
			...
			break;
		case REJECTED:
			optionData = new EnumOptionData(DepositAccountStatus.REJECTED.getValue().longValue(), DepositAccountStatus.REJECTED.getCode(), "Rejected");
			break;
		case WITHDRAWN_BY_CLIENT:
			optionData = new EnumOptionData(DepositAccountStatus.WITHDRAWN_BY_CLIENT.getValue().longValue(), DepositAccountStatus.WITHDRAWN_BY_CLIENT.getCode(), "Withdrawn by applicant");
			break;
		case MATURED:
			optionData = new EnumOptionData(DepositAccountStatus.MATURED.getValue().longValue(), DepositAccountStatus.MATURED.getCode(), "Matured");
			break;
		case CLOSED:
