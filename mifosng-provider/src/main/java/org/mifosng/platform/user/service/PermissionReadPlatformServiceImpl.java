...
public class PermissionReadPlatformServiceImpl implements
	...
	private final static Logger logger = LoggerFactory
			.getLogger(PermissionReadPlatformService.class);
	...
	@Override
	public Collection<PermissionUsageData> retrieveAllRolePermissions(
			final Long roleId) {
	...
	private static final class PermissionUsageDataMapper implements
			RowMapper<PermissionUsageData> {

		@Override
		public PermissionUsageData mapRow(final ResultSet rs,
			...
			return new PermissionUsageData(grouping, code, entityName,
					actionName, selected);
		}
		...
		public String makerCheckerablePermissionSchema() {
			...
			return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName, p.can_maker_checker as selected"
					+ " from m_permission p "
					+ " where grouping != 'special' and code not like 'READ_%' and code not like '%\\_CHECKER'"
					+ " order by p.grouping, ifnull(entity_name, ''), p.code";
		}
