...
public class DecliningBalanceLoanScheduleGenerator implements
	...
	public LoanSchedule generate(
			...
			LocalDate interestCalculatedFrom, CurrencyData currencyData) {

		if (firstRepaymentDate != null
				&& disbursementDate.isAfter(firstRepaymentDate)) {
			throw new NoAuthorizationException(
					"cannot have disbursement date after first repayment date.");
		}
		...
		for (LocalDate scheduledDueDate : scheduledDates) {
			...
			Money interestPerInstallment = outstandingBalance
