...
public class UpdateNoteHandler implements CommandHandler<NoteUpdateRequest, NoteUpdateResponse> {

    private final NoteWritePlatformService writePlatformService;

    @Override
    public Class<NoteUpdateRequest> getRequestType() {
        return NoteUpdateRequest.class;
    }

    @Transactional
    @Override
    public NoteUpdateResponse handle(NoteUpdateRequest request) {
        return writePlatformService.updateNote(request);
    }
}
