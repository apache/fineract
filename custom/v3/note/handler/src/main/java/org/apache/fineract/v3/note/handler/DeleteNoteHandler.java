...
public class DeleteNoteHandler implements CommandHandler<NoteDeleteRequest, NoteDeleteResponse> {

    private final NoteWritePlatformService writePlatformService;

    @Override
    public Class<NoteDeleteRequest> getRequestType() {
        return NoteDeleteRequest.class;
    }

    @Transactional
    @Override
    public NoteDeleteResponse handle(NoteDeleteRequest request) {
        return writePlatformService.deleteNote(request);
    }
}
