...
# Check if we should send actual notifications
SEND_MODE=false
if [[ "$1" == "--send" ]]; then
  SEND_MODE=true
  log "⚠️ Running in SEND mode - notifications will actually be sent!"
  sleep 2
else
  log "ℹ️ Running in simulation mode - notifications will NOT be sent"
  log "ℹ️ Use --send flag to send actual notifications"
fi
