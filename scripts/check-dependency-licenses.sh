...
# Usage: ./scripts/check-dependency-licenses.sh [--base-ref <ref>] [--check-jar] [--help]
set -e
...
Options:
  --base-ref <ref>   Base ref to compare against: a branch, tag or commit SHA
                     (e.g. origin/develop). If specified, the script only gates
                     on new/changed violations. If not specified, the script
                     gates on all violations.
  --check-jar        Also build the Fineract bootJar and fail if any dependency
                     actually bundled in it (BOOT-INF/lib) is not on the
                     allowlist. Unlike --base-ref, this check is absolute: it
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --base-ref)
            if [ $# -lt 2 ]; then echo "--base-ref requires a <ref> argument" >&2; show_help; exit 1; fi
            BASE_REF="$2"; shift 2 ;;
...
# Fail loudly on a base ref that does not resolve to a commit, rather than
# silently degrading to a full (non-diffed) check.
if [ -n "$BASE_REF" ] && ! git rev-parse --verify --quiet "$BASE_REF^{commit}" > /dev/null; then
    echo "--base-ref: '$BASE_REF' does not resolve to a commit in this repository." >&2
    echo "Pass a branch, tag or commit SHA that exists locally (fetch it first if needed)." >&2
    exit 1
fi
...
cat > "$ALLOWED_LICENSES_FILE" <<'EOF'
{
  "allowedLicenses": [
    ...
    { "moduleLicense": "GNU Lesser General Public License (LGPL), Version 2.1" },
    { "moduleLicense": "The GNU Lesser General Public License, Version 2.1" },
    { "moduleLicense": "GNU Library General Public License v2.1 or later" },
    { "moduleLicense": "Lesser General Public License, version 3 or greater" },
    { "moduleLicense": "GNU General Public License, version 2 with the GNU Classpath Exception" },
    { "moduleLicense": "GNU General Public License (GPL), version 2, with the Classpath exception" },
    ...
    { "moduleLicense": "Mozilla Public License, Version 2.0" },
    { "moduleName": "org\\.apache\\.fineract:.*" },
    { "moduleName": "com\\.acme\\.fineract.*" }
  ]
}
EOF
...
# 3. Handle base branch if BASE_REF is specified
if [ -n "$BASE_REF" ]; then
...
if [ "$COUNT" -gt 0 ]; then
    if [ -n "$BASE_REF" ]; then
        echo "New/changed dependency licenses not on the allowlist, relative to the base branch ($BASE_REF):"
    else
