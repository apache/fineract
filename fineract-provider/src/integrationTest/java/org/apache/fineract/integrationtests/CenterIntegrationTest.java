...
public class CenterIntegrationTest {
  ...
  public void testFullCenterCreation() {
    ...
    int resourceId =
        CenterHelper.createCenter(
            name, officeId, externalId, staffId, groupMembers, requestSpec, responseSpec);
    CenterDomain center = CenterHelper.retrieveByID(resourceId, requestSpec, responseSpec);
  ...
  public void testListCenters() {
    ArrayList<CenterDomain> paginatedList =
        CenterHelper.paginatedListCenters(requestSpec, responseSpec);
    ...
    Assert.assertTrue(
        Arrays.equals(
            paginatedList.toArray(new CenterDomain[paginatedList.size()]),
  ...
  public void testCenterUpdate() {
    ...
    int[] associateResponse =
        CenterHelper.associateGroups(resourceId, associateGroupMembers, requestSpec, responseSpec);
    ...
    Assert.assertEquals(
        Integer.valueOf(newStaffId), Integer.valueOf(response.get("staffId").toString()));
  ...
  public void testStaffAssignmentDuringCenterCreation() {
    ...
    final int centerWithStaffId =
        CenterHelper.createCenterWithStaffId(this.requestSpec, this.responseSpec, staffId);
  ...
  public void testAssignStaffToCenter() {
    ...
    final HashMap assignStaffToCenterResponseMap =
        (HashMap)
            CenterHelper.assignStaff(
                this.requestSpec, this.responseSpec, groupID.toString(), staffId.longValue());
    assertEquals(
        "Verify assigned staff id is the same as id sent",
        assignStaffToCenterResponseMap.get("staffId"),
        staffId);
  ...
  public void testUnassignStaffToCenter() {
    ...
    final HashMap unassignStaffToCenterResponseMap =
        (HashMap)
            CenterHelper.unassignStaff(
                this.requestSpec, this.responseSpec, groupID.toString(), staffId.longValue());
    assertEquals(
        "Verify staffId is null after unassigning ",
        unassignStaffToCenterResponseMap.get("staffId"),
        null);
