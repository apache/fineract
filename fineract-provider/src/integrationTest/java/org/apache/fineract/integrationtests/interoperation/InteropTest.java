...
public class InteropTest {
  ...
  public void setup() {
    ...
    interopHelper =
        new InteropHelper(requestSpec, responseSpec, savingsExternalId, transactionCode);
  }
  ...
  private void createSavingsProduct() {
    log.debug(
        "------------------------------ Create Interoperable Saving Product"
            + " ---------------------------------------");
    ...
    final String savingsProductJSON =
        savingsProductHelper
            ...
            .build();
    savingsProductId =
        SavingsProductHelper.createSavingsProduct(savingsProductJSON, requestSpec, responseSpec);
    ...
    chargeId =
        ChargesHelper.createCharges(
            ...
            ChargesHelper.getSavingsJSON(
                interopHelper.getFee().toString(),
                interopHelper.getCurrency(),
  ...
  private void openSavingsAccount() {
    log.debug(
        "------------------------------ Create Interoperable Saving Account"
            + " ---------------------------------------");
    savingsId =
        savingsAccountHelper.applyForSavingsApplicationWithExternalId(
            clientId,
            savingsProductId,
            ACCOUNT_TYPE_INDIVIDUAL,
            interopHelper.getAccountExternalId(),
            true);
  ...
  private void testRequests() {
    ...
    String response =
        interopHelper.postTransactionRequest(requestCode, InteropTransactionRole.PAYER);
    JsonPath json = JsonPath.from(response);
    Assert.assertEquals(requestCode, json.getString(InteropUtil.PARAM_REQUEST_CODE));
    Assert.assertEquals(
        InteropActionState.ACCEPTED.toString(), json.getString(InteropHelper.PARAM_ACTION_STATE));
  ...
  private void testQuotes() {
    ...
    BigDecimal feeAmount =
        ObjectConverter.convertObjectTo(fee.get(InteropUtil.PARAM_AMOUNT), BigDecimal.class);
    Assert.assertTrue(
    ...
    if (fee != null) {
      feeAmount =
          ObjectConverter.convertObjectTo(fee.get(InteropUtil.PARAM_AMOUNT), BigDecimal.class);
      Assert.assertTrue(
          "PAYEE Quote fee expected: " + BigDecimal.ZERO + ", actual: " + feeAmount,
  ...
  private void testTransfers() {
    ...
    BigDecimal onHold =
        ObjectConverter.convertObjectTo(
            savingsJson.get(SavingsApiConstants.savingsAmountOnHold), BigDecimal.class);
    BigDecimal balance =
        ObjectConverter.convertObjectTo(savingsJson.get(PARAM_ACCOUNT_BALANCE), BigDecimal.class);
    ...
    BigDecimal balance3 =
        ObjectConverter.convertObjectTo(savingsJson.get(PARAM_ACCOUNT_BALANCE), BigDecimal.class);
    Assert.assertTrue(
        "On hold amount expected: " + onHold + ", actual: " + onHold3,
    ...
    Assert.assertTrue(
        "On hold amount expected: " + onHold + ", actual: " + onHold4,
