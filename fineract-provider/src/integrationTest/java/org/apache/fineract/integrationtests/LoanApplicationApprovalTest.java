...
public class LoanApplicationApprovalTest {
  ...
  public void loanApplicationApprovedAmountLessThanProposedAmount() {
    ...
    final Integer clientID =
        ClientHelper.createClient(this.requestSpec, this.responseSpec, "01 January 2012");
    final Integer loanProductID =
        this.loanTransactionHelper.getLoanProductId(new LoanProductTestBuilder().build(null));
  ...
  public void loanApplicationApprovedAmountGreaterThanProposedAmount() {
    ...
    List<HashMap> error =
        (List<HashMap>)
            this.loanTransactionHelper.approveLoan(
                approveDate, approvalAmount, loanID, CommonConstants.RESPONSE_ERROR);
  ...
  public void loanApplicationApprovalAndValidationForMultiDisburseLoans() {
    ...
    this.trancheLoansApprovedAmountLesserThanProposedAmount(
        clientID, loanProductID, createTranches);
  ...
  private void trancheLoansApprovedAmountLesserThanProposedAmount(
      Integer clientID, Integer loanProductID, List<HashMap> createTranches) {
    final String proposedAmount = "5000";
    ...
    LoanStatusChecker.verifyLoanIsWaitingForDisbursal(loanStatusHashMap);
    LOG.info(
        "-----------------------------------MULTI DISBURSAL LOAN APPROVED"
            + " SUCCESSFULLY---------------------------------------");
  }

  private void trancheLoansApprovalValidation(
      Integer clientID, Integer loanProductID, List<HashMap> createTranches) {
    final String proposedAmount = "5000";
    ...
    LOG.info(
        "-----------------------------------APPROVE"
            + " LOAN-----------------------------------------------------------");
    this.loanTransactionHelper =
        new LoanTransactionHelper(this.requestSpec, this.responseSpecForStatusCode400);

    /* Tranches with same expected disbursement date */
    List<HashMap<String, Object>> error =
        this.loanTransactionHelper.approveLoanForTranches(
            approveDate,
    ...
    error =
        this.loanTransactionHelper.approveLoanForTranches(
            ...
            CommonConstants.RESPONSE_ERROR);
    assertEquals(
        "error.msg.disbursementData.exceeding.max.tranche.count",
        error.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
  ...
  private Integer applyForLoanApplication(
      final Integer clientID, final Integer loanProductID, final String proposedAmount) {
