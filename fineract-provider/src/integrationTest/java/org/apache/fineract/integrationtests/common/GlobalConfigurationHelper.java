...
public class GlobalConfigurationHelper {
  ...
  public static ArrayList<HashMap> getAllGlobalConfigurations(
      final RequestSpecification requestSpec, final ResponseSpecification responseSpec) {
  ...
  public static HashMap getGlobalConfigurationById(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
      final String configId) {
    final String GET_GLOBAL_CONFIG_BY_ID_URL =
        "/fineract-provider/api/v1/configurations/" + configId + "?" + Utils.TENANT_IDENTIFIER;
    LOG.info(
        "------------------------ RETRIEVING GLOBAL CONFIGURATION BY ID -------------------------");
    return Utils.performServerGet(requestSpec, responseSpec, GET_GLOBAL_CONFIG_BY_ID_URL, "");
  }

  public static void resetAllDefaultGlobalConfigurations(
      final RequestSpecification requestSpec, final ResponseSpecification responseSpec) {
    ...
    for (HashMap configDefault : defaults) {
      ...
      // If any other column is modified by the integration test suite in the future, it needs to be
      // reset here.
      ...
      updateValueForGlobalConfiguration(
          requestSpec, responseSpec, configDefaultId.toString(), configDefaultValue.toString());
  ...
  public static void verifyAllDefaultGlobalConfigurations(
      final RequestSpecification requestSpec, final ResponseSpecification responseSpec) {
    ...
    for (int i = 0; i < expectedGlobalConfigurations.size(); i++) {
      ...
      Assert.assertEquals(
          expectedGlobalConfiguration.get("id"), actualGlobalConfiguration.get("id"));
      final String assertionFailedMessage =
          "Assertion failed for configID:<" + expectedGlobalConfiguration.get("id") + ">";
      Assert.assertEquals(
          assertionFailedMessage,
          expectedGlobalConfiguration.get("name"),
          actualGlobalConfiguration.get("name"));
      Assert.assertEquals(
          assertionFailedMessage,
          expectedGlobalConfiguration.get("value"),
          actualGlobalConfiguration.get("value"));
      Assert.assertEquals(
          assertionFailedMessage,
          expectedGlobalConfiguration.get("enabled"),
          actualGlobalConfiguration.get("enabled"));
      Assert.assertEquals(
          assertionFailedMessage,
          expectedGlobalConfiguration.get("trapDoor"),
  ...
  private static ArrayList<HashMap> getAllDefaultGlobalConfigurations() {
    ...
    savingsInterestPostingCurrentPeriodEndDefault.put(
    ...
    paymentTypeApplicableForDisbursementChargesDefault.put(
    ...
    interestChargedFromDateSameAsDisbursalDateDefault.put(
    ...
    changeEmiIfRepaymentDateSameAsDisbursementDateDefault.put(
  ...
  public static Integer updateValueForGlobalConfiguration(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
      final String configId,
      final String value) {
    ...
    return Utils.performServerPut(
        ...
        updateGlobalConfigUpdateValueAsJSON(value),
        "resourceId");
  }
  ...
  public static Integer updateEnabledFlagForGlobalConfiguration(
    ...
    final String GLOBAL_CONFIG_UPDATE_URL =
        "/fineract-provider/api/v1/configurations/" + configId + "?" + Utils.TENANT_IDENTIFIER;
    LOG.info(
        "---------------------------------UPDATE GLOBAL CONFIG FOR ENABLED"
            + " FLAG---------------------------------------------");
    return Utils.performServerPut(
        requestSpec,
        responseSpec,
  ...
  public static ArrayList getGlobalConfigurationIsCacheEnabled(
      final RequestSpecification requestSpec, final ResponseSpecification responseSpec) {
    final String GET_IS_CACHE_GLOBAL_CONFIG_URL =
        "/fineract-provider/api/v1/caches?" + Utils.TENANT_IDENTIFIER;
    LOG.info(
        "------------------------ RETRIEVING IS CACHE ENABLED GLOBAL CONFIGURATION"
            + " -------------------------");
    final ArrayList<HashMap> response =
        Utils.performServerGet(requestSpec, responseSpec, GET_IS_CACHE_GLOBAL_CONFIG_URL, "");
    return response;
  }

  public static HashMap updateIsCacheEnabledForGlobalConfiguration(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
  ...
  public static Object updatePasswordResetDaysForGlobalConfiguration(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
    ...
    final String UPDATE_URL =
        "/fineract-provider/api/v1/configurations/" + configId + "?" + Utils.TENANT_IDENTIFIER;
    LOG.info(
        "------------------UPDATE GLOBAL CONFIG FOR FORCE PASSWORD RESET"
            + " DAYS----------------------");
    return Utils.performServerPut(
        requestSpec,
        responseSpec,
  ...
  public static String updatePasswordResetDaysGlobalConfigAsJSON(
