...
public class SchedulerJobsTestResults {
  ...
  public void testApplyAnnualFeeForSavingsJobOutcome() throws InterruptedException {
    ...
    final Integer savingsId =
        this.savingsAccountHelper.applyForSavingsApplication(
            clientID, savingsProductID, ClientSavingsIntegrationTest.ACCOUNT_TYPE_INDIVIDUAL);
    Assert.assertNotNull(savingsProductID);
    ...
    LocalDate nextDueDateForAnnualFee =
        LocalDate.of(
            annualFeeDueDateAsArrayList.get(0),
  ...
  public void testInterestPostingForSavingsJobOutcome() throws InterruptedException {
    ...
    Assert.assertNotSame(
        ...
        summaryBefore.get("accountBalance"),
  ...
  public void testTransferFeeForLoansFromSavingsJobOutcome() throws InterruptedException {
    ...
    final Integer loanID =
        applyForLoanApplication(
            clientID.toString(), loanProductID.toString(), savingsId.toString());
    ...
    loanStatusHashMap =
        this.loanTransactionHelper.approveLoan(AccountTransferTest.LOAN_APPROVAL_DATE, loanID);
    ...
    loanStatusHashMap =
        this.loanTransactionHelper.disburseLoan(AccountTransferTest.LOAN_DISBURSAL_DATE, loanID);
  ...
  public void testApplyHolidaysToLoansJobOutcome() throws InterruptedException {
    ...
    HashMap configData =
        this.globalConfigurationHelper.getGlobalConfigurationById(
            this.requestSpec, this.responseSpec, configId.toString());
    ...
    holidayId =
        this.holidayHelper.activateHolidays(
            this.requestSpec, this.responseSpec, holidayId.toString());
    Assert.assertNotNull(holidayId);
    ...
    HashMap holidayData =
        this.holidayHelper.getHolidayById(
            this.requestSpec, this.responseSpec, holidayId.toString());
    ArrayList<Integer> repaymentsRescheduledDate =
        (ArrayList<Integer>) holidayData.get("repaymentsRescheduledTo");

    ArrayList<Integer> rescheduleDateAfter =
        (ArrayList<Integer>) repaymentScheduleDataAfterJob.get(2).get("fromDate");
  ...
  public void testUpdateAccountingRunningBalancesJobOutcome() throws InterruptedException {
    ...
    this.schedulerJobHelper.executeJob(JobName);
    final HashMap runningBalanceAfter =
        this.accountHelper.getAccountingWithRunningBalanceById(accountID.toString());

    final Integer INT_BALANCE = Integer.valueOf(MINIMUM_OPENING_BALANCE);

    Assert.assertEquals(
        "Verifying Account Running Balance after running Update Accounting Running Balances"
            + " Scheduler Job",
        INT_BALANCE,
  ...
  public void testUpdateLoanPaidInAdvanceJobOutcome() throws InterruptedException {
    ...
    ArrayList<HashMap> loanScheduleBefore =
        this.loanTransactionHelper.getLoanRepaymentSchedule(
            this.requestSpec, this.responseSpec, loanID);
    ...
    Assert.assertEquals(
        "Verifying Loan Repayment in Advance after Running Update Loan Paid in Advance Scheduler"
            + " Job",
        totalDueForCurrentPeriod,
        totalPaidInAdvance);
  }
  ...
  public void testExecuteStandingInstructionsJobOutcome() throws InterruptedException {
    this.schedulerJobHelper = new SchedulerJobHelper(this.requestSpec, this.responseSpec);
    this.savingsAccountHelper = new SavingsAccountHelper(this.requestSpec, this.responseSpec);
    this.standingInstructionsHelper =
    ...
    Integer standingInstructionId =
        this.standingInstructionsHelper.createStandingInstruction(
            clientID.toString(),
            fromSavingsId.toString(),
            toSavingsId.toString(),
            FROM_ACCOUNT_TYPE_SAVINGS,
    ...
    final HashMap standingInstructionData =
        this.standingInstructionsHelper.getStandingInstructionById(
            standingInstructionId.toString());
    Float expectedFromSavingsBalance =
        fromSavingsBalanceBefore - (Float) standingInstructionData.get("amount");
    Float expectedToSavingsBalance =
        toSavingsBalanceBefore + (Float) standingInstructionData.get("amount");

    Assert.assertEquals(
        "Verifying From Savings Balance after Successful completion of Scheduler Job",
        expectedFromSavingsBalance,
        fromSavingsBalanceAfter);
    Assert.assertEquals(
        "Verifying To Savings Balance after Successful completion of Scheduler Job",
        expectedToSavingsBalance,
    ...
    List<HashMap> standinInstructionHistoryData =
        this.standingInstructionsHelper.getStandingInstructionHistory(
            fromSavingsId, fromAccountType, clientID, transferType);
    Assert.assertEquals(
  ...
  public void testApplyPenaltyForOverdueLoansJobOutcome() throws InterruptedException {
    ...
    loanStatusHashMap =
        this.loanTransactionHelper.disburseLoan(
            AccountTransferTest.LOAN_APPROVAL_DATE_PLUS_ONE, loanID);
    LoanStatusChecker.verifyLoanIsActive(loanStatusHashMap);
    ...
    Assert.assertEquals(
        "Verifying From Penalty Charges due fot first Repayment after Successful completion of"
            + " Scheduler Job",
        chargeAmount,
        (Float) repaymentScheduleDataAfter.get(1).get("penaltyChargesDue"));
  ...
  @Test
  @Ignore // TODO FINERACT-857
  public void testAvoidUnncessaryPenaltyWhenAmountZeroForOverdueLoansJobOutcome()
    ...
    final ArrayList loanCharges =
        this.loanTransactionHelper.getLoanCharges(this.requestSpec, this.responseSpec, loanID);
  ...
  public void testUpdateOverdueDaysForNPA() throws InterruptedException {
    ...
    final Boolean isNPABefore =
        (Boolean)
            this.loanTransactionHelper.getLoanDetail(requestSpec, responseSpec, loanID, "isNPA");
  ...
  public void testInterestTransferForSavings() throws InterruptedException {
    ...
    // this.responseSpec);
    FixedDepositAccountHelper fixedDepositAccountHelper =
    ...
    final Integer savingsProductID =
        createSavingsProduct(this.requestSpec, this.responseSpec, String.valueOf(balance));
    Assert.assertNotNull(savingsProductID);
    ...
    fixedDepositAccountStatusHashMap =
        fixedDepositAccountHelper.approveFixedDeposit(fixedDepositAccountId, APPROVED_ON_DATE);
    FixedDepositAccountStatusChecker.verifyFixedDepositIsApproved(fixedDepositAccountStatusHashMap);

    fixedDepositAccountStatusHashMap =
        fixedDepositAccountHelper.activateFixedDeposit(fixedDepositAccountId, ACTIVATION_DATE);
    ...
    Float interestPosted =
        (Float) fixedDepositSummary.get("accountBalance")
            - Float.valueOf(FixedDepositAccountHelper.depositAmount);
    ...
    validateNumberForEqualExcludePrecission(
        String.valueOf(balance), String.valueOf(summary.get("accountBalance")));
  }
  ...
  private Integer applyForLoanApplication(
      final String clientID, final String loanProductID, final String savingsID) {
  ...
  private Integer applyForFixedDepositApplication(
      ...
      String savingsId,
      final boolean transferInterest,
    ...
    return fixedDepositAccountHelper.applyFixedDepositApplication(
