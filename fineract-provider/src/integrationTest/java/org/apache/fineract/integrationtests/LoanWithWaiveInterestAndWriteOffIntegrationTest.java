...
public class LoanWithWaiveInterestAndWriteOffIntegrationTest {
  private static final Logger LOG =
      LoggerFactory.getLogger(LoanWithWaiveInterestAndWriteOffIntegrationTest.class);
  ...
  public void checkClientLoanCreateAndDisburseFlow() {
    ...
    this.loanTransactionHelper.waiveInterest(
        "1 September 2011", this.INTEREST_VALUE_AMOUNT, loanID);
    this.loanTransactionHelper.makeRepayment(
        "1 September 2011", repayment_without_interest, loanID);
    ...
    LoanStatusChecker.verifyLoanAccountIsClosed(
        this.loanTransactionHelper.writeOffLoan("1 March 2012", loanID));
  }
  ...
  public void checkClientLoan_WRITTEN_OFF() {
    ...
    Assert.assertTrue(
        ...
        Float.valueOf("500.0")
                .compareTo(Float.valueOf(String.valueOf(toLoanSummaryAfter.get("principalPaid"))))
            == 0);
    Assert.assertTrue(
        "Checking for interestPaid paid ",
        Float.valueOf("180.0")
                .compareTo(Float.valueOf(String.valueOf(toLoanSummaryAfter.get("interestPaid"))))
            == 0);
    Assert.assertTrue(
        "Checking for total paid ",
        Float.valueOf("680.0")
                .compareTo(Float.valueOf(String.valueOf(toLoanSummaryAfter.get("totalRepayment"))))
            == 0);

    // WRITE OFF LOAN AND CHECK ACCOUNT IS CLOSED
    LoanStatusChecker.verifyLoanAccountIsClosed(
        this.loanTransactionHelper.writeOffLoan("1 January 2011", loanID));
    toLoanSummaryAfter =
        this.loanTransactionHelper.getLoanSummary(requestSpec, responseSpec, loanID);
    Assert.assertTrue(
        "Checking for Principal written off ",
        Float.valueOf("4000.0")
                .compareTo(
                    Float.valueOf(String.valueOf(toLoanSummaryAfter.get("principalWrittenOff"))))
            == 0);
    Assert.assertTrue(
        "Checking for interestPaid written off ",
        Float.valueOf("1440.0")
                .compareTo(
                    Float.valueOf(String.valueOf(toLoanSummaryAfter.get("interestWrittenOff"))))
            == 0);
    Assert.assertTrue(
        "Checking for total written off ",
        Float.valueOf("5440.0")
                .compareTo(Float.valueOf(String.valueOf(toLoanSummaryAfter.get("totalWrittenOff"))))
            == 0);
  }
  ...
  private Integer createLoanProduct() {
    ...
    final String loanProductJSON =
        new LoanProductTestBuilder()
            .withPrincipal(this.LP_PRINCIPAL)
  ...
  private Integer applyForLoanApplication(final Integer clientID, final Integer loanProductID) {
    ...
    final String loanApplicationJSON =
        new LoanApplicationTestBuilder()
            ...
            .withNumberOfRepayments(this.NUMBER_OF_REPAYMENTS)
            .withRepaymentEveryAfter(this.REPAYMENT_PERIOD)
            .withRepaymentFrequencyTypeAsMonths()
            .withInterestRatePerPeriod(this.RATE_OF_INTEREST_PER_PERIOD)
