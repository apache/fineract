...
public class DividendsIntegrationTests {
    ...
    public void testCreateDividends() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Integer savingsAccountId = SavingsAccountHelper.openSavingsAccount(requestSpec, responseSpec, clientId,
                    "1000");
            savingAccounts.add(savingsAccountId);
            Assert.assertNotNull(savingsAccountId);
            final Integer shareAccountId = createShareAccount(clientId, productId, savingsAccountId, dates[i],
                    shares[i]);
            ...
            ShareAccountTransactionHelper.postCommand("activate", shareAccountId, activateJson, requestSpec,
                    responseSpec);
        }
        ...
        final Integer dividendId = ShareDividendsTransactionHelper.createShareProductDividends(productId,
                createDividendsJson, requestSpec, responseSpec);

        Map<String, Object> productdividends = ShareDividendsTransactionHelper.retrieveAllDividends(productId,
                requestSpec, responseSpec);
        ...
        Map<String, Object> dividenddetails = ShareDividendsTransactionHelper.retrieveDividendDetails(productId,
                dividendId, requestSpec, responseSpec);
        ...
        String jsonString = "";
        ShareDividendsTransactionHelper.postCommand("approve", productId, dividendId, jsonString, requestSpec,
                responseSpec);
        ...
        dividenddetails = ShareDividendsTransactionHelper.retrieveDividendDetails(productId, dividendId, requestSpec,
                responseSpec);
        Assert.assertEquals("5", String.valueOf(dividenddetails.get("totalFilteredRecords")));
