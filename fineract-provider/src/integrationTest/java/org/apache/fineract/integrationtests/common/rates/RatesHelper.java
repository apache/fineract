...
public class RatesHelper {
  ...
  public static ArrayList<HashMap> getRates(
      final RequestSpecification requestSpec, final ResponseSpecification responseSpec) {
    return (ArrayList)
        Utils.performServerGet(
            requestSpec, responseSpec, RATES_URL + "?" + Utils.TENANT_IDENTIFIER, "");
  }

  public static Integer createRates(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
      final String request) {
    return Utils.performServerPost(
        requestSpec, responseSpec, CREATE_RATES_URL, request, "resourceId");
  }

  public static HashMap getRateById(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
      final Integer rateId) {
    return Utils.performServerGet(
        requestSpec, responseSpec, RATES_URL + "/" + rateId + "?" + Utils.TENANT_IDENTIFIER, "");
  }

  public static HashMap updateRates(
      final RequestSpecification requestSpec,
      final ResponseSpecification responseSpec,
      final Integer rateId,
      final String request) {
