...
public class DisbursalAndRepaymentScheduleTest {
  ...
  public void testRescheduleJLGLoanSynk() {
    ...
    final String groupMeetingNewStartDate =
        this.dateFormatterStandard.format(groupMeetingChangeCalendar.getTime());
    // The date
    // from
    // which we
    // start the
    ...
    this.loanRescheduleRequestHelper =
        new LoanRescheduleRequestHelper(this.requestSpec, this.responseSpec);
    ...
    final String loanApplicationJSON =
        new LoanApplicationTestBuilder()
            ...
            .withSubmittedOnDate(disbursalDate)
            .withExpectedDisbursementDate(disbursalDate)
            .withLoanType(loanType)
            .withSyncDisbursementWithMeetin()
            .withCalendarID(this.groupCalendarId.toString())
            .build(
                this.clientId.toString(),
                this.groupId.toString(),
                this.loanProductId.toString(),
                null);
    ...
    HashMap loanStatusHashMap =
        LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, this.loanId);
    ...
    // DATE ------------------------------------------");
    CalendarHelper.updateMeetingCalendarForGroup(
        this.requestSpec,
        this.responseSpec,
        this.groupId,
        this.groupCalendarId.toString(),
        groupMeetingNewStartDate,
        "2",
        "2",
        "2"); // New
    // meeting
    ...
    ArrayList loanRepaymnetSchedule =
        this.loanTransactionHelper.getLoanRepaymentSchedule(
            requestSpec, generalResponseSpec, this.loanId);

    ArrayList dueDateLoanSchedule =
        (ArrayList) ((HashMap) loanRepaymnetSchedule.get(2)).get("dueDate");
    Calendar dueDateCalendar = Calendar.getInstance();
    dueDateCalendar.setFirstDayOfWeek(Calendar.MONDAY);
    dueDateCalendar.set(
        (Integer) dueDateLoanSchedule.get(0),
        (Integer) dueDateLoanSchedule.get(1) - 1,
    ...
    String requestJSON =
        new LoanRescheduleRequestTestBuilder()
            .updateGraceOnInterest("2")
            .updateGraceOnPrincipal("2")
            .updateNewInterestRate(rescheduleInterestRate)
            .updateRescheduleFromDate(rescheduleDate)
            .updateSubmittedOnDate(rescheduleSubmittedDate)
    ...
    this.loanRescheduleRequestHelper.verifyCreationOfLoanRescheduleRequest(
        this.loanRescheduleRequestId);

    loanRepaymnetSchedule =
        this.loanTransactionHelper.getLoanRepaymentSchedule(
            requestSpec, generalResponseSpec, this.loanId);
    ...
    dueDateCalendar.set(
        ...
        (Integer) dueDateLoanSchedule.get(2));
    assertEquals(
        "AFTER MEETING CHANGE DATE THE NEXT REPAYMENT SHOULD BE ON TUESDAY, EVEN AFTER LOAN"
            + " RESCHEDULE REQUEST WAS SENT",
        3,
        dueDateCalendar.get(Calendar.DAY_OF_WEEK));
    ...
    this.loanRescheduleRequestHelper.approveLoanRescheduleRequest(
        this.loanRescheduleRequestId, requestJSON);

    final HashMap response =
        (HashMap)
            this.loanRescheduleRequestHelper.getLoanRescheduleRequest(
                loanRescheduleRequestId, "statusEnum");
    ...
    this.loanTransactionHelper =
        new LoanTransactionHelper(this.requestSpec, this.responseSpecForStatusCode403);
  }
  ...
  public void testChangeGroupMeetingMaturedOnDate() {
    ...
    ArrayList expectedMaturityDate =
        ((ArrayList)
            ((HashMap)
                    this.loanTransactionHelper.getLoanDetail(
                        requestSpec, generalResponseSpec, this.loanId, "timeline"))
                .get("expectedMaturityDate"));

    expectedMaturityCalendar.set(
        (Integer) expectedMaturityDate.get(0),
        (Integer) expectedMaturityDate.get(1) - 1,
  ...
  private void associateClientToGroup(final Integer groupId, final Integer clientId) {
    GroupHelper.associateClient(
        this.requestSpec, this.responseSpec, groupId.toString(), clientId.toString());
  ...
  private void createGroupEntityWithCalendar(
    ...
    this.groupId =
        GroupHelper.createGroup(this.requestSpec, this.responseSpec, groupActivationDate);
    ...
    this.setGroupCalendarId(
        CalendarHelper.createMeetingCalendarForGroup(
            this.requestSpec,
            this.responseSpec,
            this.groupId,
            startDate,
