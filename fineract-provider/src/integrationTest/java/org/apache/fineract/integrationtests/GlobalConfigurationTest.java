...
public class GlobalConfigurationTest {
  ...
  public void testGlobalConfigurations() {
    ...
    for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
      if (globalConfig.get(configIndex).get("name").equals(configName)) {
        ...
        configId =
            this.globalConfigurationHelper.updateValueForGlobalConfiguration(
                this.requestSpec, this.responseSpec, configId.toString(), value.toString());
        Assert.assertNotNull(configId);
        ...
        Assert.assertEquals(
            "Verifying Global Config Value after Updation", value, configDataAfter.get("value"));
        ...
        Assert.assertEquals(
            "Verifying Enabled Flag Global Config after Updation",
            enabled,
  ...
  public void testGlobalConfigurationIsCacheEnabled() {
    ...
    ArrayList<HashMap> isCacheGlobalConfig =
        this.globalConfigurationHelper.getGlobalConfigurationIsCacheEnabled(
            this.requestSpec, this.responseSpec);
    ...
    for (Integer cacheType = 0; cacheType <= ((isCacheGlobalConfig.size()) - 1); cacheType++) {
      ...
      HashMap changes =
          this.globalConfigurationHelper.updateIsCacheEnabledForGlobalConfiguration(
              this.requestSpec, this.responseSpec, cacheTypeId.toString());
      Assert.assertEquals(
          "Verifying Is Cache Enabled Global Config after Updation",
          cacheTypeId,
  ...
  public void testGlobalConfigForcePasswordResetDays() {
    ...
    for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
      if (globalConfig.get(configIndex).get("name").equals(configName)) {
        ...
        ArrayList error =
            (ArrayList)
                this.globalConfigurationHelper.updatePasswordResetDaysForGlobalConfiguration(
                    this.requestSpec,
                    this.httpStatusForidden,
                    configId,
                    newValue,
                    newBooleanValue,
    ...
    for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
      if (globalConfig.get(configIndex).get("name").equals(otherConfigName)) {
        ...
        Integer updateConfigId =
            this.globalConfigurationHelper.updateValueForGlobalConfiguration(
                this.requestSpec, this.responseSpec, configId, newValue);
