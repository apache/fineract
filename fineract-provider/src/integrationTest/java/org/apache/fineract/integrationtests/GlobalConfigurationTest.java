...
public class GlobalConfigurationTest {
        ...
        public void testGlobalConfigurations() {
                ...
                for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
                        if (globalConfig.get(configIndex).get("name").equals(configName)) {
                                ...
                                HashMap configDataBefore = this.globalConfigurationHelper.getGlobalConfigurationById(
                                ...
                                configId = this.globalConfigurationHelper.updateValueForGlobalConfiguration(
                                                this.requestSpec, this.responseSpec, configId.toString(),
                                                value.toString());
                                Assert.assertNotNull(configId);

                                HashMap configDataAfter = this.globalConfigurationHelper.getGlobalConfigurationById(
                                                this.requestSpec, this.responseSpec, configId.toString());

                                // Verifying Value for penalty-wait-period after Updation
                                Assert.assertEquals("Verifying Global Config Value after Updation", value,
                                                configDataAfter.get("value"));
                                ...
                                configDataAfter = this.globalConfigurationHelper.getGlobalConfigurationById(
                                                this.requestSpec, this.responseSpec, configId.toString());

                                // Verifying Enabled Flag for penalty-wait-period after Updation
                                Assert.assertEquals("Verifying Enabled Flag Global Config after Updation", enabled,
                                                configDataAfter.get("enabled"));
                                break;
                        }
                }
        }
        ...
        public void testGlobalConfigurationIsCacheEnabled() {
                ...
                ArrayList<HashMap> isCacheGlobalConfig = this.globalConfigurationHelper
                                .getGlobalConfigurationIsCacheEnabled(this.requestSpec, this.responseSpec);
                Assert.assertNotNull(isCacheGlobalConfig);

                for (Integer cacheType = 0; cacheType <= ((isCacheGlobalConfig.size()) - 1); cacheType++) {

                        // Retrieving Is Cache Enabled Global Configuration details
                        isCacheGlobalConfig = this.globalConfigurationHelper
                                        .getGlobalConfigurationIsCacheEnabled(this.requestSpec, this.responseSpec);
                        Assert.assertNotNull(isCacheGlobalConfig);
                        ...
                        HashMap changes = this.globalConfigurationHelper.updateIsCacheEnabledForGlobalConfiguration(
                                        this.requestSpec, this.responseSpec, cacheTypeId.toString());
                        Assert.assertEquals("Verifying Is Cache Enabled Global Config after Updation", cacheTypeId,
                                        changes.get("cacheType"));
                }
        }
        ...
        public void testGlobalConfigForcePasswordResetDays() {
                ...
                for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
                        if (globalConfig.get(configIndex).get("name").equals(configName)) {
                                ...
                                /*
                                 * Update force-password-reset-days with value as 0 and Enable as true - failure
                                 * case
                                 */
                                ArrayList error = (ArrayList) this.globalConfigurationHelper
                                                .updatePasswordResetDaysForGlobalConfiguration(this.requestSpec,
                                                                this.httpStatusForidden, configId, newValue,
                                                                newBooleanValue, CommonConstants.RESPONSE_ERROR);
                                HashMap hash = (HashMap) error.get(0);

                                Assert.assertEquals("Force Password Reset days value must be greater than zero.",
                                                "error.msg.password.reset.days.value.must.be.greater.than.zero",
                                                hash.get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));

                                /*
                                 * Update force-password-reset-days with value as 50 and Enable as true -
                                 * success case
                                 */
                                final HashMap updateSuccess = (HashMap) this.globalConfigurationHelper
                                                .updatePasswordResetDaysForGlobalConfiguration(this.requestSpec,
                                                                this.responseSpec, configId, "50", newBooleanValue,
                                                                "changes");
                                ...
                                final HashMap updateSuccess1 = (HashMap) this.globalConfigurationHelper
                                                .updatePasswordResetDaysForGlobalConfiguration(this.requestSpec,
                                                                this.responseSpec, configId, newValue, "false",
                                                                "changes");
                                ...
                                ArrayList failure = (ArrayList) this.globalConfigurationHelper
                                                .updatePasswordResetDaysForGlobalConfiguration(this.requestSpec,
                                                                this.httpStatusForidden, configId, null,
                                                                newBooleanValue, CommonConstants.RESPONSE_ERROR);
                ...
                for (Integer configIndex = 0; configIndex < (globalConfig.size() - 1); configIndex++) {
                        if (globalConfig.get(configIndex).get("name").equals(otherConfigName)) {
                                ...
                                Integer updateConfigId = this.globalConfigurationHelper
                                                .updateValueForGlobalConfiguration(this.requestSpec, this.responseSpec,
                                                                configId, newValue);
