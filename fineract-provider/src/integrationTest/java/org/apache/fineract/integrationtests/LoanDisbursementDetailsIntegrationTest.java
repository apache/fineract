...
public class LoanDisbursementDetailsIntegrationTest {
    ...
    public void createAndValidateMultiDisburseLoansBasedOnEmi() {
        ...
        final Integer loanProductID = this.loanTransactionHelper.getLoanProductId(new LoanProductTestBuilder()
                ...
                .withTranches(true).withInterestCalculationPeriodTypeAsRepaymentPeriod(true).build(null));
        System.out.println("------------LOAN PRODUCT CREATED WITH ID----------" + loanProductID);

        final Integer loanIDWithEmi = applyForLoanApplicationWithEmiAmount(clientID, loanProductID, proposedAmount,
                createTranches, installmentAmount);

        System.out.println("-------------LOAN CREATED WITH EMI LOANID----------------" + loanIDWithEmi);

        HashMap repaymentScheduleWithEmi = (HashMap) this.loanTransactionHelper.getLoanDetail(this.requestSpec,
                this.responseSpec, loanIDWithEmi, "repaymentSchedule");
        ...
        loanStatusHashMap = this.loanTransactionHelper.approveLoanWithApproveAmount("01 June 2015", "01 June 2015",
                "10000", loanIDWithEmi, createTranches);
        LoanStatusChecker.verifyLoanIsApproved(loanStatusHashMap);
        LoanStatusChecker.verifyLoanIsWaitingForDisbursal(loanStatusHashMap);
        System.out.println("-------------MULTI DISBURSAL LOAN WITH EMI APPROVED SUCCESSFULLY-----------------");

        final Integer loanIDWithoutEmi = applyForLoanApplicationWithEmiAmount(clientID, loanProductID, proposedAmount,
                createTranches, withoutInstallmentAmount);

        HashMap repaymentScheduleWithoutEmi = (HashMap) this.loanTransactionHelper.getLoanDetail(this.requestSpec,
                this.responseSpec, loanIDWithoutEmi, "repaymentSchedule");

        ArrayList<HashMap> periods1 = (ArrayList<HashMap>) repaymentScheduleWithEmi.get("periods");
        assertEquals(periods1.size(), 15);

        System.out.println("-------------LOAN CREATED WITHOUT EMI LOANID----------------" + loanIDWithoutEmi);

        /* To be uncommented once issue MIFOSX-2006 is closed. */
        // this.validateRepaymentScheduleWithoutEMI(periods1);

        HashMap loanStatusMap = LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec,
                loanIDWithoutEmi);
        ...
        LoanStatusChecker.verifyLoanIsApproved(loanStatusHashMap);
        LoanStatusChecker.verifyLoanIsWaitingForDisbursal(loanStatusHashMap);
        System.out.println("-------------MULTI DISBURSAL LOAN WITHOUT EMI APPROVED SUCCESSFULLY-----------------");

    }

    private void validateRepaymentScheduleWithEMI(ArrayList<HashMap> periods) {
        LoanDisbursementTestBuilder expectedRepaymentSchedule0 = new LoanDisbursementTestBuilder("[2015, 6, 1]", 0.0f,
                0.0f, null, null, 5000.0f, null, null, null);

        LoanDisbursementTestBuilder expectedRepaymentSchedule1 = new LoanDisbursementTestBuilder("[2015, 7, 1]", 800f,
    ...
    private void assertRepaymentScheduleValuesWithEMI(HashMap period,
        ...
        if (position != 0 && position != 3) {
            ...
            assertEquals(new Float(period.get("principalDue").toString()).floatValue(),
                    expectedRepaymentSchedule.getPrincipalDue(), 0.0f);
     ...
     * LoanDisbursementTestBuilder expectedRepaymentSchedule0 = new
     * LoanDisbursementTestBuilder( "[2015, 6, 1]", 0.0f, 0.0f, null, null, 5000.0f,
     * null, null, null);
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule1 = new
     * LoanDisbursementTestBuilder( "[2015, 7, 1]", 800.0f, 800.0f, 50.0f, 750.0f,
     * 4250.0f, 750.0f, 750.0f, "[2015, 6, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule2 = new
     * LoanDisbursementTestBuilder( "[2015, 8, 1]", 800.0f, 800.0f, 42.5f, 757.5f,
     * 3492.5f, 757.5f, 757.5f, "[2015, 7, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule3 = new
     * LoanDisbursementTestBuilder( "[2015, 9, 1]", 0.0f, 0.0f, null, null, 5000.0f,
     * null, null, null);
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule4 = new
     * LoanDisbursementTestBuilder( "[2015, 9, 1]", 800.0f, 800.0f, 34.92f, 765.08f,
     * 7727.42f, 765.08f, 765.08f, "[2015, 8, 1]");
     *
     ...
     * LoanDisbursementTestBuilder( "[2016, 1, 1]", 800.0f, 800.0f, 55.37f, 744.63f,
     * 4792.86f, 744.63f, 744.63f, "[2015, 12, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule9 = new
     * LoanDisbursementTestBuilder( "[2016, 2, 1]", 800.0f, 800.0f, 47.93f, 752.07f,
     * 4040.79f, 752.07f, 752.07f, "[2016, 1, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule10 = new
     * LoanDisbursementTestBuilder( "[2016, 3, 1]", 800.0f, 800.0f, 40.41f, 759.59f,
     * 3281.2f, 759.59f, 759.59f, "[2016, 2, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule11 = new
     * LoanDisbursementTestBuilder( "[2016, 4, 1]", 800.0f, 800.0f, 32.81f, 767.19f,
     * 2514.01f, 767.19f, 767.19f, "[2016, 3, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule12 = new
     * LoanDisbursementTestBuilder( "[2016, 5, 1]", 800.0f, 800.0f, 25.14f, 774.86f,
     * 1739.15f, 774.86f, 774.86f, "[2016, 4, 1]");
     *
     * LoanDisbursementTestBuilder expectedRepaymentSchedule13 = new
     * LoanDisbursementTestBuilder( "[2016, 6, 1]", 800.0f, 800.0f, 17.39f, 782.61f,
     * 956.54f, 782.61f, 782.61f, "[2016, 5, 1]");
     *
     ...
     * list.add(expectedRepaymentSchedule0); list.add(expectedRepaymentSchedule1);
     * list.add(expectedRepaymentSchedule2); list.add(expectedRepaymentSchedule3);
     * list.add(expectedRepaymentSchedule4); list.add(expectedRepaymentSchedule5);
     * list.add(expectedRepaymentSchedule6); list.add(expectedRepaymentSchedule7);
     * list.add(expectedRepaymentSchedule8); list.add(expectedRepaymentSchedule9);
     * list.add(expectedRepaymentSchedule10); list.add(expectedRepaymentSchedule11);
     ...
     */
    private Integer applyForLoanApplicationWithEmiAmount(final Integer clientID, final Integer loanProductID,
            final String proposedAmount, List<HashMap> tranches, final String installmentAmount) {
    ...
    public void createApproveAndValidateMultiDisburseLoan() {
        ...
        loanStatusHashMap = this.loanTransactionHelper.approveLoanWithApproveAmount(approveDate,
                expectedDisbursementDate, approvalAmount, loanID, createTranches);
        LoanStatusChecker.verifyLoanIsApproved(loanStatusHashMap);
        LoanStatusChecker.verifyLoanIsWaitingForDisbursal(loanStatusHashMap);
        ...
        ArrayList<HashMap> disbursementDetails = (ArrayList<HashMap>) this.loanTransactionHelper
                .getLoanDetail(this.requestSpec, this.responseSpec, this.loanID, "disbursementDetails");
    ...
    private void addNewDisbursementDetails() {
        ...
        String id = null;
        addTranches.add(this.loanTransactionHelper.createTrancheDetail(disbursementDetails.get(0).get("id").toString(),
                date, disbursementDetails.get(0).get("principal").toString()));
        ...
        this.loanTransactionHelper.addAndDeleteDisbursementDetail(this.loanID, this.approvalAmount,
                this.expectedDisbursementDate, addTranches, "");
    }
    ...
    private void deleteDisbursmentDetails() {
        ...
        for (int i = 0; i < disbursementDetails.size() - 1; i++) {
            ...
            deleteTranches
                    .add(this.loanTransactionHelper.createTrancheDetail(disbursementDetails.get(i).get("id").toString(),
                            disbursementDate, disbursementDetails.get(i).get("principal").toString()));
        }

        /* Add disbursement detail */
        this.loanTransactionHelper.addAndDeleteDisbursementDetail(this.loanID, this.approvalAmount,
                this.expectedDisbursementDate, deleteTranches, "");
    }
