...
public class ConcurrencyIntegrationTest {
  ...
  public void verifyConcurrentLoanRepayments() {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Runnable worker =
          new LoanRepaymentExecutor(loanTransactionHelper, loanID, repaymentAmount, date);
  ...
  private Integer applyForLoanApplication(
      final Integer clientID, final Integer loanProductID, String principal) {
    LOG.info(
        "--------------------------------APPLYING FOR LOAN"
            + " APPLICATION--------------------------------");
  ...
  public static class LoanRepaymentExecutor implements Runnable {
    ...
    public void run() {
      try {
        ...
        LOG.info(
            "Details of failed concurrent transaction (date, amount, loanId) are {},{},{}",
            repaymentDate,
            repaymentAmount,
            loanId);
        throw (e);
      ...
      LOG.info(
          "Details of passed concurrent transaction, details (date, amount, loanId) are {},{},{}",
          repaymentDate,
          repaymentAmount,
          loanId);
    }
