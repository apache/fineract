...
public class ShareAccountIntegrationTests {
  ...
  public void testCreateShareProduct() {
    ...
    LOG.info(
        "------------------------------CREATING SHARE PRODUCT"
            + " COMPLETE---------------------------------------");

    LOG.info(
        "------------------------------RETRIEVING SHARE"
            + " PRODUCT---------------------------------------");
    Map<String, Object> shareProductData =
        ShareProductTransactionHelper.retrieveShareProduct(
            shareProductId, requestSpec, responseSpec);
    Assert.assertNotNull(shareProductData);
    shareProductHelper.verifyShareProduct(shareProductData);

    LOG.info(
        "------------------------------RETRIEVING SHARE PRODUCT"
            + " COMPLETE---------------------------------------");
    ...
    Integer updatedProductId =
        ShareProductTransactionHelper.updateShareProduct(
            shareProductId, updateShareProductJsonString, requestSpec, responseSpec);
    Assert.assertNotNull(updatedProductId);
    Map<String, Object> updatedShareProductData =
        ShareProductTransactionHelper.retrieveShareProduct(
            updatedProductId, requestSpec, responseSpec);
  ...
  public void testCreateShareAccount() {
    ...
    Integer savingsAccountId =
        SavingsAccountHelper.openSavingsAccount(requestSpec, responseSpec, clientId, "1000");
    ...
    Map<String, Object> shareProductData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    Assert.assertNotNull(shareProductData);
    ...
    ShareAccountTransactionHelper.updateShareAccount(
        shareAccountId, updateShareAccountJsonString, requestSpec, responseSpec);
    shareProductData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    List<Map<String, Object>> transactions =
        (List<Map<String, Object>>) shareProductData.get("purchasedShares");
  ...
  public void testShareAccountApproval() {
    ...
    Integer activationChargeId =
        ChargesHelper.createCharges(requestSpec, responseSpec, activationCharge);
    String purchaseCharge = ChargesHelper.getShareAccountPurchaseChargeJson();
    Integer purchaseChargeId =
        ChargesHelper.createCharges(requestSpec, responseSpec, purchaseCharge);
    ...
    final Integer shareAccountId =
        createShareAccount(clientId, productId, savingsAccountId, charges);
    Assert.assertNotNull(shareAccountId);
    ...
    ShareAccountTransactionHelper.postCommand(
        "approve", shareAccountId, approve, requestSpec, responseSpec);
    shareAccountData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    ...
    List<Map<String, Object>> transactions =
        (List<Map<String, Object>>) shareAccountData.get("purchasedShares");
    Assert.assertNotNull(transactions);
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")) {
        ...
        Assert.assertEquals(
            simple.format(Utils.getLocalDateOfTenant().toDate()), simple.format(date));
      }
    }
  ...
  public void rejectShareAccount() {
    ...
    ShareAccountTransactionHelper.postCommand(
        "reject", shareAccountId, rejectJson, requestSpec, responseSpec);
    shareAccountData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    DateFormat simple = new SimpleDateFormat("dd MMM yyyy");
    ...
    Assert.assertEquals(
        simple.format(Utils.getLocalDateOfTenant().toDate()), simple.format(rejectedDate));
  ...
  public void testShareAccountUndoApproval() {
    ...
    ShareAccountTransactionHelper.postCommand(
        "undoapproval", shareAccountId, undoApprovalJson, requestSpec, responseSpec);
  ...
  public void testCreateShareAccountWithCharges() {
    ...
    List<Map<String, Object>> chargesList =
        (List<Map<String, Object>>) shareAccountData.get("charges");
    ...
    ShareAccountTransactionHelper.postCommand(
        "activate", shareAccountId, activateJson, requestSpec, responseSpec);
    shareAccountData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    statusMap = (Map<String, Object>) shareAccountData.get("status");
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")
          && transactionDate.equals("02 Mar 2016")) {
        Assert.assertEquals("30", String.valueOf(transaction.get("numberOfShares")));
        Assert.assertEquals("62.0", String.valueOf(transaction.get("amount")));
        Assert.assertEquals("62.0", String.valueOf(transaction.get("amountPaid")));
        Assert.assertEquals("2.0", String.valueOf(transaction.get("chargeAmount")));
        Assert.assertEquals("2.0", String.valueOf(transaction.get("purchasedPrice")));
      } else if (transactionType.equals("purchasedSharesType.purchased")
          && transactionDate.equals("01 Apr 2016")) {
        ...
        Assert.assertEquals(
            "purchasedSharesStatusType.applied", String.valueOf(transactionstatusMap.get("code")));
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")
        ...
        Map<String, Object> transactionstatusMap = (Map<String, Object>) transaction.get("status");
        Assert.assertEquals(
            "purchasedSharesStatusType.approved", String.valueOf(transactionstatusMap.get("code")));
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      String transactionDate = simple.format(date);
      if (transactionType.equals("purchasedSharesType.purchased")
          && transactionDate.equals("01 May 2016")) {
        addtionalSharesRequestId = String.valueOf(transaction.get("id"));
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")
        ...
        Map<String, Object> transactionstatusMap = (Map<String, Object>) transaction.get("status");
        Assert.assertEquals(
            "purchasedSharesStatusType.rejected", String.valueOf(transactionstatusMap.get("code")));
      }
    }
    ...
    ShareAccountTransactionHelper.postCommand(
        "redeemshares", shareAccountId, redeemRequestJson, requestSpec, responseSpec);
    shareAccountData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    transactions = (List<Map<String, Object>>) shareAccountData.get("purchasedShares");
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")
        ...
        Assert.assertEquals(
            "purchasedSharesStatusType.approved", String.valueOf(transactionstatusMap.get("code")));
      } else if (transactionType.equals("purchasedSharesType.redeemed")
          && transactionDate.equals("05 May 2016")) {
        Assert.assertEquals("15", String.valueOf(transaction.get("numberOfShares")));
    ...
    ShareAccountTransactionHelper.postCommand(
        "close", shareAccountId, closeJson, requestSpec, responseSpec);
    shareAccountData =
        ShareAccountTransactionHelper.retrieveShareAccount(
            shareAccountId, requestSpec, responseSpec);
    statusMap = (Map<String, Object>) shareAccountData.get("status");
    ...
    for (int i = 0; i < transactions.size(); i++) {
      ...
      if (transactionType.equals("purchasedSharesType.purchased")
        ...
        Assert.assertEquals(
          ...
          && transactionDate.equals("10 May 2016")) {
        Assert.assertEquals("30", String.valueOf(transaction.get("numberOfShares")));
  ...
  private Integer createShareProduct() {
    ...
    return ShareProductTransactionHelper.createShareProduct(
        shareProductJson, requestSpec, responseSpec);
  }

  private Integer createShareAccount(
      final Integer clientId, final Integer productId, final Integer savingsAccountId) {
    String josn =
        new ShareAccountHelper()
            .withClientId(String.valueOf(clientId))
            .withProductId(String.valueOf(productId))
            .withExternalId("External1")
            .withSavingsAccountId(String.valueOf(savingsAccountId))
            .withSubmittedDate("01 Jan 2016")
            .withApplicationDate("01 Jan 2016")
            .withRequestedShares("25")
            .build();
