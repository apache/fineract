...
public class AccountTransferTest {
  ...
  public void setup() {
    ...
    if (financialActivities.isEmpty()) {
      ...
      for (HashMap financialActivity : financialActivities) {
        ...
        if (financialActivityData
            .get("id")
            .equals(FinancialActivityAccountsTest.liabilityTransferFinancialActivityId)) {
  ...
  public void testFromSavingsToSavingsAccountTransfer() {
    ...
    final Integer toClientID =
        ClientHelper.createClient(
            this.requestSpec, this.responseSpec, "01 January 2011", String.valueOf(toOfficeId));
    ...
    HashMap toSavingsStatusHashMap =
        SavingsStatusChecker.getStatusOfSavings(this.requestSpec, this.responseSpec, toSavingsID);
    ...
    final Integer fromClientID =
        ClientHelper.createClient(
            this.requestSpec, this.responseSpec, "01 January 2011", String.valueOf(fromOfficeId));
    ...
    final Integer fromSavingsID =
        this.savingsAccountHelper.applyForSavingsApplication(
            fromClientID, fromSavingsProductID, ACCOUNT_TYPE_INDIVIDUAL);
    Assert.assertNotNull(fromSavingsID);

    HashMap fromSavingsStatusHashMap =
        SavingsStatusChecker.getStatusOfSavings(this.requestSpec, this.responseSpec, fromSavingsID);
    ...
    this.accountTransferHelper.accountTransfer(
        ...
        fromClientID,
        toSavingsID,
        FROM_SAVINGS_ACCOUNT_TYPE,
        TO_SAVINGS_ACCOUNT_TYPE,
    ...
    this.journalEntryHelper.checkJournalEntryForLiabilityAccount(
        fromOfficeId,
        liabilityTransferAccount,
        AccountTransferHelper.ACCOUNT_TRANSFER_DATE,
  ...
  public void testFromSavingsToLoanAccountTransfer() {
    ...
    final Integer toLoanProductID =
        createLoanProduct(
            loanAssetAccount, loanIncomeAccount, loanExpenseAccount, overpaymentAccount);
    ...
    HashMap toLoanStatusHashMap =
        LoanStatusChecker.getStatusOfLoan(this.requestSpec, this.responseSpec, toLoanID);
    ...
    this.accountTransferHelper.accountTransfer(
        ...
        toLoanID,
        FROM_SAVINGS_ACCOUNT_TYPE,
        TO_LOAN_ACCOUNT_TYPE,
        ACCOUNT_TRANSFER_AMOUNT_ADJUST);
    ...
    HashMap toLoanSummaryAfter =
        this.loanTransactionHelper.getLoanSummary(requestSpec, responseSpec, toLoanID);
    assertEquals(
        "Verifying To Loan Repayment Amount after Account Transfer",
        TRANSFER_AMOUNT_ADJUST,
        toLoanSummaryAfter.get("totalRepayment"));

    final JournalEntry[] office1LiabilityEntries = {
      new JournalEntry(
          Float.valueOf(ACCOUNT_TRANSFER_AMOUNT_ADJUST), JournalEntry.TransactionType.CREDIT)
    };
    final JournalEntry[] office2LiabilityEntries = {
      new JournalEntry(
          Float.valueOf(ACCOUNT_TRANSFER_AMOUNT_ADJUST), JournalEntry.TransactionType.DEBIT)
    };
