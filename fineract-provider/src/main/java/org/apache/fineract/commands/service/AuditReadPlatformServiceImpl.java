...
public class AuditReadPlatformServiceImpl implements AuditReadPlatformService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AuditReadPlatformServiceImpl.class);
  ...
  @Autowired
  public AuditReadPlatformServiceImpl(
  ...
  private static final class AuditMapper implements RowMapper<AuditData> {
    ...
    public String schema(final boolean includeJson, final String hierarchy) {
      ...
      String partSql =
          " aud.id as id, aud.action_name as actionName, aud.entity_name as entityName,"
              + " aud.resource_id as resourceId, aud.subresource_id as subresourceId,aud.client_id"
              + " as clientId, aud.loan_id as loanId, mk.username as maker, aud.made_on_date as"
              + " madeOnDate,  aud.api_get_url as resourceGetUrl, ck.username as checker,"
              + " aud.checked_on_date as checkedOnDate, ev.enum_message_property as"
              + " processingResult "
              + commandAsJsonString
              + ",  o.name as officeName, gl.level_name as groupLevelName, g.display_name as"
              + " groupName, c.display_name as clientName,  l.account_no as loanAccountNo,"
              + " s.account_no as savingsAccountNo  from m_portfolio_command_source aud  left join"
              + " m_appuser mk on mk.id = aud.maker_id left join m_appuser ck on ck.id ="
              + " aud.checker_id left join m_office o on o.id = aud.office_id left join m_group g"
              + " on g.id = aud.group_id left join m_group_level gl on gl.id = g.level_id left"
              + " join m_client c on c.id = aud.client_id left join m_loan l on l.id = aud.loan_id"
              ...
              + " aud.processing_result_enum";

      // data scoping: head office (hierarchy = ".") can see all audit
      // entries
      if (!(hierarchy.equals("."))) {
        partSql +=
            " join m_office o2 on o2.id = aud.office_id and o2.hierarchy like '"
                + hierarchy
                + "%' ";
      }
  ...
  public Collection<AuditData> retrieveAuditEntries(
      final SQLBuilder extraCriteria, final boolean includeJson) {
  ...
  public Page<AuditData> retrievePaginatedAuditEntries(
    ...
    return this.paginationHelper.fetchPage(
        this.jdbcTemplate, sqlCountRows, sqlBuilder.toString(), extraCriteria.getArguments(), rm);
  }
  ...
  public Collection<AuditData> retrieveAllEntriesToBeChecked(
    ...
    return retrieveEntries(
        "makerchecker", extraCriteria, " group by aud.id order by aud.id", includeJson);
  }

  private Collection<AuditData> retrieveEntries(
      final String useType,
      final SQLBuilder extraCriteria,
      final String groupAndOrderBySQL,
      final boolean includeJson) {
    ...
    if (isLimitedChecker) {
      sql +=
          " join m_permission p on REPLACE(p.action_name, '_CHECKER', '')  = aud.action_name and"
              + " p.entity_name = aud.entity_name and p.code like '%\\_CHECKER' join"
              + " m_role_permission rp on rp.permission_id = p.id join m_role r on r.id ="
              + " rp.role_id  join m_appuser_role ur on ur.role_id = r.id and ur.appuser_id = "
              + currentUser.getId();
    }
  ...
  private AuditData replaceIdsOnAuditData(final AuditData auditResult) {
    ...
    final Map<String, Object> commandAsJsonMap =
        this.fromApiJsonHelper.extractObjectMap(typeOfMap, auditAsJson);
    ...
    if (commandAsJsonMap.containsKey("productId")) {
      ...
      if (StringUtils.isNotBlank(productIdStr)) {
        ...
        if (auditResult.getEntityName().equalsIgnoreCase("LOAN")) {
          ...
          final SavingsProductData savingProduct =
    ...
    if (commandAsJsonMap.containsKey("loanOfficerId")
        || commandAsJsonMap.containsKey("fieldOfficerId")
  ...
  private void replaceStaffIdWithStaffName(
      final String staffIdStr,
      final String staffNameParamName,
  ...
  public AuditSearchData retrieveSearchTemplate(final String useType) {
    ...
    sql +=
        " order by if(action_name in ('CREATE', 'DELETE', 'UPDATE'), action_name, 'ZZZ'),"
            + " action_name";
  ...
  private static final class ProcessingResultsMapper implements RowMapper<ProcessingResultLookup> {
    ...
    public String schema() {
      return " select enum_id as id, enum_message_property as processingResult from r_enum_value"
          + " where enum_name = 'processing_result_enum'  order by enum_id";
    }
  }
}
