...
public class MakercheckersApiResource {
  ...
  @ApiOperation(
      value = "List Maker Checker Entries",
      notes =
          "Get a list of entries that can be checked by the requestor that match the criteria"
              + " supplied.\n"
              + "\n"
              + "Example Requests:\n"
              + "\n"
  ...
  public String retrieveCommands(
    ...
    final Collection<AuditData> entries =
        this.readPlatformService.retrieveAllEntriesToBeChecked(
            extraCriteria, settings.isIncludeJson());

    return this.toApiJsonSerializerAudit.serialize(
        settings, entries, this.RESPONSE_DATA_PARAMETERS);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Maker Checker Search Template",
      notes =
          "This is a convenience resource. It can be useful when building a Checker Inbox UI."
              + " \"appUsers\" are data scoped to the office/branch the requestor is associated"
              + " with. \"actionNames\" and \"entityNames\" returned are those that the requestor"
              + " has Checker approval permissions for.\n"
              + "\n"
              + "Example Requests:\n"
              + "\n"
  ...
  public String retrieveAuditSearchTemplate(@Context final UriInfo uriInfo) {
    ...
    final AuditSearchData auditSearchData =
        this.readPlatformService.retrieveSearchTemplate("makerchecker");

    final Set<String> RESPONSE_DATA_PARAMETERS_SEARCH_TEMPLATE =
        new HashSet<>(Arrays.asList("appUsers", "actionNames", "entityNames"));
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = MakercheckersApiResourceSwagger.PostMakerCheckersResponse.class)
  })
