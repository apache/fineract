...
@Service
public class PortfolioCommandSourceWritePlatformServiceImpl
    implements PortfolioCommandSourceWritePlatformService {
  ...
  public CommandProcessingResult logCommandSource(final CommandWrapper wrapper) {
    ...
    while (numberOfRetries <= maxNumberOfRetries) {
      try {
        result =
            this.processAndLogCommandService.processAndLogCommand(
                wrapper, command, isApprovedByChecker);
        numberOfRetries = maxNumberOfRetries + 1;
      } catch (CannotAcquireLockException | ObjectOptimisticLockingFailureException exception) {
        logger.info(
            "The following command {} has been retried  {} time(s)",
            command.json(),
            numberOfRetries);
        /***
        ...
        if (numberOfRetries >= maxNumberOfRetries) {
          logger.warn(
              "The following command {} has been retried for the max allowed attempts of {} and"
                  + " will be rolled back",
              command.json(),
              numberOfRetries);
          throw (exception);
  ...
  public CommandProcessingResult approveEntry(final Long makerCheckerId) {
    ...
    final CommandWrapper wrapper =
        CommandWrapper.fromExistingCommand(
            makerCheckerId,
    ...
    return this.processAndLogCommandService.processAndLogCommand(
        wrapper, command, makerCheckerApproval);
  }
  ...
  private CommandSource validateMakerCheckerTransaction(final Long makerCheckerId) {
    ...
    this.context
        .authenticatedUser()
        .validateHasCheckerPermissionTo(commandSourceInput.getPermissionCode());
