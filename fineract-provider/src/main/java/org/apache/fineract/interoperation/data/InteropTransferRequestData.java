...
public class InteropTransferRequestData extends InteropRequestData {
  ...
  private InteropTransferRequestData(
      InteropRequestData other,
      @NotNull String transferCode,
      MoneyData fspFee,
      MoneyData fspCommission) {
    this(
  ...
  public void normalizeAmounts(@NotNull MonetaryCurrency currency) {
    ...
    if (fspFee != null) fspFee.normalizeAmount(currency);
  }

  public static InteropTransferRequestData validateAndParse(
    ...
    DataValidatorBuilder dataValidatorCopy =
        dataValidator.reset().parameter(PARAM_TRANSFER_CODE).value(transferCode).notBlank();
    ...
    JsonObject fspCommissionElement =
        jsonHelper.extractJsonObjectNamed(PARAM_FSP_COMMISSION, element);
    dataValidator.merge(dataValidatorCopy);
    MoneyData fspCommission =
        MoneyData.validateAndParse(dataValidator, fspCommissionElement, jsonHelper);
