...
public class InteropDataValidator {
  ...
  public InteropIdentifierRequestData validateAndParseCreateIdentifier(
      @NotNull InteropIdentifierType idType,
      @NotNull String idValue,
      String subIdOrType,
      JsonCommand command) {
    final DataValidatorBuilder dataValidator =
        new DataValidatorBuilder(new ArrayList<>()).resource("interoperation.identifier");
    JsonObject element = extractJsonObject(command);

    InteropIdentifierRequestData result =
        InteropIdentifierRequestData.validateAndParse(
            dataValidator, idType, idValue, subIdOrType, element, jsonHelper);
  ...
  public InteropTransactionRequestData validateAndParseCreateRequest(JsonCommand command) {
    final DataValidatorBuilder dataValidator =
        new DataValidatorBuilder(new ArrayList<>()).resource("interoperation.request");
    JsonObject element = extractJsonObject(command);

    InteropTransactionRequestData result =
        InteropTransactionRequestData.validateAndParse(dataValidator, element, jsonHelper);
    throwExceptionIfValidationWarningsExist(dataValidator);
  ...
  public InteropQuoteRequestData validateAndParseCreateQuote(JsonCommand command) {
    final DataValidatorBuilder dataValidator =
        new DataValidatorBuilder(new ArrayList<>()).resource("interoperation.quote");
    JsonObject element = extractJsonObject(command);

    InteropQuoteRequestData result =
        InteropQuoteRequestData.validateAndParse(dataValidator, element, jsonHelper);
    throwExceptionIfValidationWarningsExist(dataValidator);
  ...
  public InteropTransferRequestData validateAndParseCreateTransfer(JsonCommand command) {
    final DataValidatorBuilder dataValidator =
        new DataValidatorBuilder(new ArrayList<>()).resource("interoperation.transfer");
    JsonObject element = extractJsonObject(command);

    InteropTransferRequestData result =
        InteropTransferRequestData.validateAndParse(dataValidator, element, jsonHelper);
    throwExceptionIfValidationWarningsExist(dataValidator);
