...
public class InteropServiceImpl implements InteropService {
    ...
    public InteropTransferResponseData commitTransfer(@NotNull JsonCommand command) {
        ...
        ExternalId finalExternalId = ExternalId.empty();

        // If auto-generation is enabled, generate an external ID
        if (isAutoGeneratedExternalIdEnabled) {
            finalExternalId = ExternalId.generate();
        }
