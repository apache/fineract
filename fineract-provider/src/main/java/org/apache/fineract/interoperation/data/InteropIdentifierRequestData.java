...
public class InteropIdentifierRequestData {
  ...
  @NotEmpty private final InteropIdentifierType idType;
  @NotEmpty private final String idValue;
  ...
  public static InteropIdentifierRequestData validateAndParse(
    ...
    DataValidatorBuilder dataValidatorCopy =
        dataValidator.reset().parameter(PARAM_ACCOUNT_ID).value(accountId).notBlank();

    dataValidator.merge(dataValidatorCopy);
    return dataValidator.hasError()
        ? null
        : new InteropIdentifierRequestData(idType, idValue, subIdOrType, accountId);
  }
}
