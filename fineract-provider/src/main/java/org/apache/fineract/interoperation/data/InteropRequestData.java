...
public class InteropRequestData {
  ...
  @NotNull private final MoneyData amount;
  @NotNull private final InteropTransactionRole transactionRole;
  ...
  public static InteropRequestData validateAndParse(
      final DataValidatorBuilder dataValidator, JsonObject element, FromJsonHelper jsonHelper) {
    ...
    DataValidatorBuilder dataValidatorCopy =
        dataValidator.reset().parameter(PARAM_TRANSACTION_CODE).value(transactionCode).notBlank();
    ...
    dataValidatorCopy =
        dataValidatorCopy.reset().parameter(PARAM_ACCOUNT_ID).value(accountId).notBlank();

    JsonObject moneyElement = jsonHelper.extractJsonObjectNamed(PARAM_AMOUNT, element);
    dataValidatorCopy =
        dataValidatorCopy.reset().parameter(PARAM_AMOUNT).value(moneyElement).notNull();
    dataValidator.merge(dataValidatorCopy);
    MoneyData amount = MoneyData.validateAndParse(dataValidator, moneyElement, jsonHelper);

    JsonObject transactionTypeElement =
        jsonHelper.extractJsonObjectNamed(PARAM_TRANSACTION_TYPE, element);
    InteropTransactionTypeData transactionType =
        InteropTransactionTypeData.validateAndParse(
            dataValidator, transactionTypeElement, jsonHelper);
    ...
    InteropTransactionRole transactionRole =
        transactionRoleString == null
            ? InteropTransactionRole.PAYER
    ...
    LocalDateTime expiration =
        locale == null
            ? jsonHelper.extractLocalTimeNamed(
                PARAM_EXPIRATION, element, ISO8601_DATE_TIME_FORMAT, DEFAULT_LOCALE)
            : jsonHelper.extractLocalTimeNamed(
                PARAM_EXPIRATION, element); // PARAM_DATE_FORMAT also must be set
    ...
    if (extensionArray != null) {
      ...
      for (JsonElement jsonElement : extensionArray) {
        if (jsonElement.isJsonObject())
          extensionList.add(
              ExtensionData.validateAndParse(
                  dataValidator, jsonElement.getAsJsonObject(), jsonHelper));
      }
    }

    return dataValidator.hasError()
        ? null
        : new InteropRequestData(
            transactionCode,
            requestCode,
            accountId,
            amount,
