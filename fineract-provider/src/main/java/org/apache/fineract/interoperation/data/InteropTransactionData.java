...
public class InteropTransactionData extends CommandProcessingResult {
  ...
  @NotNull private final String savingTransactionId;
  @NotNull private final SavingsAccountTransactionType transactionType;
  ...
  @NotNull private final LocalDate bookingDateTime;
  @NotNull private final LocalDate valueDateTime;
  ...
  public static InteropTransactionData build(SavingsAccountTransaction transaction) {
    ...
    LocalDate valueDateTime =
        endOfBalanceLocalDate == null ? bookingDateTime : endOfBalanceLocalDate;
    ...
    for (Note note : transaction.getNotes()) {
      ...
      if (availableLength <= 1) break;

      if (currLength > 0) {
        sb.append(' ');
        availableLength--;
      }
      if (s.length() > availableLength) s = s.substring(availableLength);
      sb.append(s);
