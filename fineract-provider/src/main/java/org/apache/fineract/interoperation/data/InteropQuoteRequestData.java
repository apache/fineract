...
public class InteropQuoteRequestData extends InteropRequestData {
  ...
  public InteropQuoteRequestData(
      ...
      @NotNull InteropAmountType amountType,
      MoneyData fees) {
    super(
        transactionCode,
        requestCode,
  ...
  private InteropQuoteRequestData(
      @NotNull InteropRequestData other,
      @NotNull String quoteCode,
      @NotNull InteropAmountType amountType,
      MoneyData fees) {
    this(
  ...
  public void normalizeAmounts(@NotNull MonetaryCurrency currency) {
    ...
    if (fees != null) fees.normalizeAmount(currency);
  }

  public static InteropQuoteRequestData validateAndParse(
    ...
    DataValidatorBuilder dataValidatorCopy =
        dataValidator.reset().parameter(PARAM_QUOTE_CODE).value(quoteCode).notBlank();

    String amountTypeString = jsonHelper.extractStringNamed(PARAM_AMOUNT_TYPE, element);
    dataValidatorCopy =
        dataValidatorCopy.reset().parameter(PARAM_AMOUNT_TYPE).value(amountTypeString).notBlank();
