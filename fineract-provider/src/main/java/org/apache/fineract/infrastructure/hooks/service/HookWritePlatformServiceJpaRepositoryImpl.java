  ...
  public CommandProcessingResult createHook(final JsonCommand command) {
    ...
    try {
      ...
      if (command.hasParameter(templateIdParamName)) {
        ...
        ugdTemplate =
            this.ugdTemplateRepository
                .findById(ugdTemplateId)
      ...
      if (!changes.isEmpty()) {
        ...
        if (changes.containsKey(templateIdParamName)) {
          ...
          final Template ugdTemplate =
              this.ugdTemplateRepository.findById(ugdTemplateId).orElse(null);
          if (ugdTemplate == null) {
  ...
  private Set<HookResource> assembleSetOfEvents(final JsonArray eventsArray) {
    ...
    for (int i = 0; i < eventsArray.size(); i++) {
      ...
      final String entityName =
          this.fromApiJsonHelper.extractStringNamed(entityNameParamName, eventElement);
