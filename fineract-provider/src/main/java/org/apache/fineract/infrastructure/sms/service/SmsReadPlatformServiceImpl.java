...
public class SmsReadPlatformServiceImpl implements SmsReadPlatformService {
  ...
  @Autowired
  public SmsReadPlatformServiceImpl(
  ...
  private static final class SmsMapper implements RowMapper<SmsData> {
    ...
    @Override
    public SmsData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
      ...
      return SmsData.instance(
          id, groupId, clientId, staffId, status, mobileNo, message, providerId, campaignName);
    }
  }
  ...
  public Collection<SmsData> retrieveAllPending(final Long campaignId, final Integer limit) {
    ...
    String sql =
        "select "
            + this.smsRowMapper.schema()
            + " where smo.status_enum = "
  ...
  public Collection<SmsData> retrieveAllDelivered(final Integer limit) {
    ...
    final String sql =
        "select "
            + this.smsRowMapper.schema()
            + " where smo.status_enum = "
            + SmsMessageStatusType.DELIVERED.getValue()
            + sqlPlusLimit;
  ...
  public Collection<SmsData> retrieveAllFailed(final Integer limit) {
    ...
    final String sql =
        "select "
            + this.smsRowMapper.schema()
            + " where smo.status_enum = "
            + SmsMessageStatusType.FAILED.getValue()
            + sqlPlusLimit;
  ...
  @Override
  public Page<SmsData> retrieveSmsByStatus(
