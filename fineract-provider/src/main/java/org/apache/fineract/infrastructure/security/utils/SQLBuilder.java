  ...
  public void addCriteria(String criteria, Object argument) {
    ...
    if (trimmedCriteria.contains("?")) {
      throw new IllegalArgumentException(
          "criteria cannot contain a '?' (that is automatically added at the end): "
              + trimmedCriteria);
    }
    ...
    String columnName =
        trimmedCriteria.substring(0, columnOperatorIndex).trim().toLowerCase(Locale.ROOT);
    if (!ATOZ.matcher(columnName).matches()) {
      throw new IllegalArgumentException(
          "criteria column name must match [a-z]: " + trimmedCriteria);
    }
    ...
    if (!operator.equals("=")
        ...
        && !operator.equals("like")) {
      // add support for SQL's BETWEEN and IN, if/when ever needed.. (it's a little more than just
      // adding above, as it can have multiple arguments)
      throw new IllegalArgumentException(
          "criteria must end with valid SQL operator for WHERE: " + trimmedCriteria);
    }
