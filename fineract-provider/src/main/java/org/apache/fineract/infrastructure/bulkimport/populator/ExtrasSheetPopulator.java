...
public class ExtrasSheetPopulator extends AbstractWorkbookPopulator {
  ...
  public ExtrasSheetPopulator(
      List<FundData> funds, List<PaymentTypeData> paymentTypes, List<CurrencyData> currencies) {
    this.funds = funds;
  ...
  public void populate(Workbook workbook, String dateFormat) {
    ...
    for (PaymentTypeData paymentType : paymentTypes) {
      Row row;
      if (paymentTypeRowIndex < fundRowIndex) row = extrasSheet.getRow(paymentTypeRowIndex++);
      else row = extrasSheet.createRow(paymentTypeRowIndex++);
      writeLong(PAYMENT_TYPE_ID_COL, row, paymentType.getId());
      writeString(
          PAYMENT_TYPE_NAME_COL, row, paymentType.getName().trim().replaceAll("[ )(]", "_"));
    }
    ...
    for (CurrencyData currencies : currencies) {
      ...
      else row = extrasSheet.createRow(currencyCodeRowIndex++);
  ...
  private void setLayout(Sheet worksheet) {
    ...
    worksheet.setColumnWidth(
        PAYMENT_TYPE_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
