...
public class DocumentReadPlatformServiceImpl implements DocumentReadPlatformService {
  ...
  @Override
  public Collection<DocumentData> retrieveAllDocuments(
  ...
  public FileData retrieveFileData(
      final String entityType, final Long entityId, final Long documentId) {
    try {
      final DocumentMapper mapper = new DocumentMapper(false, false);
      final DocumentData documentData =
          fetchDocumentDetails(entityType, entityId, documentId, mapper);
      final ContentRepository contentRepository =
          this.contentRepositoryFactory.getRepository(documentData.storageType());
  ...
  @Override
  public DocumentData retrieveDocument(
      final String entityType, final Long entityId, final Long documentId) {
    try {
      final DocumentMapper mapper = new DocumentMapper(true, true);
   ...
   * @return
   */
  private DocumentData fetchDocumentDetails(
    ...
    return this.jdbcTemplate.queryForObject(
        sql, mapper, new Object[] {entityType, entityId, documentId});
  }
  ...
  private static final class DocumentMapper implements RowMapper<DocumentData> {
    ...
    public String schema() {
      return "d.id as id, d.parent_entity_type as parentEntityType, d.parent_entity_id as"
          + " parentEntityId, d.name as name,  d.file_name as fileName, d.size as fileSize,"
          + " d.type as fileType,  d.description as description, d.location as location,"
          + " d.storage_type_enum as storageType from m_document d where"
          + " d.parent_entity_type=? and d.parent_entity_id=?";
    }
