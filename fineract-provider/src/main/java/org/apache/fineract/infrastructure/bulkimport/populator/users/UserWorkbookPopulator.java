...
public class UserWorkbookPopulator extends AbstractWorkbookPopulator {
  ...
  private void setRules(Sheet usersheet) {
    CellRangeAddressList officeNameRange =
        new CellRangeAddressList(
            ...
            UserConstants.OFFICE_NAME_COL,
            UserConstants.OFFICE_NAME_COL);
    CellRangeAddressList staffNameRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
            UserConstants.STAFF_NAME_COL,
            UserConstants.STAFF_NAME_COL);
    CellRangeAddressList autoGenPwRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
            UserConstants.AUTO_GEN_PW_COL,
            UserConstants.AUTO_GEN_PW_COL);
    CellRangeAddressList overridePwExpiryPolicyRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
    ...
    DataValidation autoGenPwValidation =
        validationHelper.createValidation(booleanConstraint, autoGenPwRange);
    DataValidation overridePwExpiryPolicyValidation =
        validationHelper.createValidation(booleanConstraint, overridePwExpiryPolicyRange);
  ...
  private void setNames(Sheet usersheet, List<OfficeData> offices) {
    ...
    for (Integer i = 0; i < offices.size(); i++) {
      ...
      if (officeNameToBeginEndIndexesOfStaff != null) {
        userOfficeName.setNameName(
            "Staff_" + offices.get(i).name().trim().replaceAll("[ )(]", "_"));
        userOfficeName.setRefersToFormula(
  ...
  private void setLayout(Sheet usersheet) {
    ...
    usersheet.setColumnWidth(
        UserConstants.OFFICE_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.STAFF_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.USER_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.FIRST_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.LAST_NAME_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.EMAIL_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.AUTO_GEN_PW_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.OVERRIDE_PW_EXPIRY_POLICY_COL,
        TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.STATUS_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    usersheet.setColumnWidth(
        UserConstants.ROLE_NAME_START_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    ...
    writeString(
        UserConstants.OVERRIDE_PW_EXPIRY_POLICY_COL, rowHeader, "Override pw expiry policy");
    writeString(
        UserConstants.ROLE_NAME_START_COL,
        rowHeader,
