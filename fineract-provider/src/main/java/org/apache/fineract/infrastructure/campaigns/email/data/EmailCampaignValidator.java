...
public class EmailCampaignValidator {
  ...
  public static final Set<String> supportedParams =
      new HashSet<String>(
          Arrays.asList(
              campaignName,
              campaignType,
  ...
  public static final Set<String> supportedParamsForUpdate =
      new HashSet<>(
          Arrays.asList(
              campaignName,
              campaignType,
  ...
  public static final Set<String> ACTIVATION_REQUEST_DATA_PARAMETERS =
      new HashSet<String>(
          Arrays.asList(localeParamName, dateFormatParamName, activationDateParamName));

  public static final Set<String> CLOSE_REQUEST_DATA_PARAMETERS =
      new HashSet<String>(
          Arrays.asList(localeParamName, dateFormatParamName, closureDateParamName));

  public static final Set<String> PREVIEW_REQUEST_DATA_PARAMETERS =
      new HashSet<String>(Arrays.asList(paramValue, emailMessage));
  ...
  public void validateCreate(String json) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, EmailCampaignValidator.supportedParams);
    ...
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.campaignName)
        .value(campaignName)
        .notBlank()
    ...
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.campaignType)
        .value(campaignType)
        .notNull()
    ...
    if (campaignType.intValue() == EmailCampaignType.SCHEDULE.getValue()) {
      final String recurrenceParamName =
          this.fromApiJsonHelper.extractStringNamed(
              EmailCampaignValidator.recurrenceParamName, element);
      baseDataValidator
          .reset()
          .parameter(EmailCampaignValidator.recurrenceParamName)
          .value(recurrenceParamName)
          .notBlank();

      final String recurrenceStartDate =
          this.fromApiJsonHelper.extractStringNamed(
              EmailCampaignValidator.recurrenceStartDate, element);
      baseDataValidator
          .reset()
          .parameter(EmailCampaignValidator.recurrenceStartDate)
    ...
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.businessRuleId)
        .value(businessRuleId)
        .notNull()
        .integerGreaterThanZero();

    final String emailSubject =
        this.fromApiJsonHelper.extractStringNamed(EmailCampaignValidator.emailSubject, element);
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.emailSubject)
        .value(emailSubject)
        .notBlank()
        .notExceedingLengthOf(50);

    final String emailMessage =
        this.fromApiJsonHelper.extractStringNamed(EmailCampaignValidator.emailMessage, element);
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.emailMessage)
        .value(emailMessage)
        .notBlank()
        .notExceedingLengthOf(480);

    final String paramValue =
        this.fromApiJsonHelper.extractStringNamed(EmailCampaignValidator.paramValue, element);
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.paramValue)
        .value(paramValue)
        .notBlank();

    if (this.fromApiJsonHelper.parameterExists(
        EmailCampaignValidator.submittedOnDateParamName, element)) {
      final LocalDate submittedOnDate =
          this.fromApiJsonHelper.extractLocalDateNamed(
              EmailCampaignValidator.submittedOnDateParamName, element);
      baseDataValidator
          .reset()
          .parameter(EmailCampaignValidator.submittedOnDateParamName)
  ...
  public void validateForUpdate(String json) {
    ...
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, EmailCampaignValidator.supportedParamsForUpdate);
    final JsonElement element = this.fromApiJsonHelper.parse(json);
  ...
  public void validatePreviewMessage(String json) {
    ...
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, EmailCampaignValidator.PREVIEW_REQUEST_DATA_PARAMETERS);
  ...
  public void validateClosedDate(String json) {
    ...
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, EmailCampaignValidator.CLOSE_REQUEST_DATA_PARAMETERS);
    ...
    final LocalDate closeDate =
        this.fromApiJsonHelper.extractLocalDateNamed(
            EmailCampaignValidator.closureDateParamName, element);
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.closureDateParamName)
        .value(closeDate)
        .notNull();
  ...
  public void validateActivation(String json) {
    ...
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, EmailCampaignValidator.ACTIVATION_REQUEST_DATA_PARAMETERS);
    ...
    final LocalDate activationDate =
        this.fromApiJsonHelper.extractLocalDateNamed(
            EmailCampaignValidator.activationDateParamName, element);
    baseDataValidator
        .reset()
        .parameter(EmailCampaignValidator.activationDateParamName)
