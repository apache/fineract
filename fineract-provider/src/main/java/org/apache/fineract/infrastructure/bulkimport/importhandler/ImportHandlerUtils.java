...
public class ImportHandlerUtils {
  ...
  public static boolean isNotImported(Row row, int statusColumn) {
    if (readAsString(statusColumn, row) != null) {
      return !readAsString(statusColumn, row)
          .equals(TemplatePopulateImportConstants.STATUS_CELL_IMPORTED);
    } else {
      return true;
    }
  }
  ...
  public static String trimEmptyDecimalPortion(String result) {
    if (result != null && result.endsWith(".0")) return result.split("\\.")[0];
    else return result;
  }
  ...
  public static Boolean readAsBoolean(int colIndex, Row row) {
    ...
    if (c.getCellType() == CellType.FORMULA) {
      ...
      return false;
    } else if (c.getCellType() == CellType.BOOLEAN) return c.getBooleanCellValue();
    else {
  ...
  public static void writeString(int colIndex, Row row, String value) {
    if (value != null) row.createCell(colIndex).setCellValue(value);
  }
  ...
  public static void writeErrorMessage(
      Sheet sheet, Integer rowIndex, String errorMessage, int statusColumn) {
  ...
  public static String getErrorMessage(RuntimeException re) {
    if (re instanceof AbstractPlatformDomainRuleException) {
      AbstractPlatformDomainRuleException abstractPlatformDomainRuleException =
          (AbstractPlatformDomainRuleException) re;
      ...
      AbstractPlatformResourceNotFoundException abstractPlatformResourceNotFoundException =
      ...
      AbstractPlatformServiceUnavailableException abstractPlatformServiceUnavailableException =
      ...
      PlatformApiDataValidationException platformApiDataValidationException =
          (PlatformApiDataValidationException) re;
  ...
  public static String getRepeatsOnDayId(String repeatsOnDay) {
    if (repeatsOnDay != null) {
      if (repeatsOnDay.equalsIgnoreCase(TemplatePopulateImportConstants.MONDAY)) repeatsOnDay = "1";
