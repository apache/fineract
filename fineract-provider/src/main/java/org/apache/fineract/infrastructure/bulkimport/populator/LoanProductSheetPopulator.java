...
public class LoanProductSheetPopulator extends AbstractWorkbookPopulator {
  ...
  public void populate(Workbook workbook, String dateFormat) {
    ...
    for (LoanProductData product : products) {
      ...
      else writeInt(MIN_INTEREST_RATE_COL, row, 1);
      if (product.getMaxInterestRatePerPeriod() != null)
        writeBigDecimal(MAX_INTEREST_RATE_COL, row, product.getMaxInterestRatePerPeriod());
      else writeInt(MAX_INTEREST_RATE_COL, row, 999999999);
      writeString(
          INTEREST_RATE_FREQUENCY_COL, row, product.getInterestRateFrequencyType().getValue());
      ...
      writeString(
          INTEREST_CALCULATION_PERIOD_TYPE_COL,
          row,
      ...
      if (product.getStartDate() != null)
        writeDate(
            START_DATE_COL, row, product.getStartDate().toString(), dateCellStyle, dateFormat);
      else writeDate(START_DATE_COL, row, "1/1/1970", dateCellStyle, dateFormat);
      if (product.getCloseDate() != null)
        writeDate(
            CLOSE_DATE_COL, row, product.getCloseDate().toString(), dateCellStyle, dateFormat);
      else writeDate(CLOSE_DATE_COL, row, "1/1/2040", dateCellStyle, dateFormat);
  ...
  private void setLayout(Sheet worksheet) {
    ...
    worksheet.setColumnWidth(
        REPAYMENT_FREQUENCY_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    ...
    worksheet.setColumnWidth(
        INTEREST_RATE_FREQUENCY_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        AMORTIZATION_TYPE_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    worksheet.setColumnWidth(INTEREST_TYPE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        INTEREST_CALCULATION_PERIOD_TYPE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        IN_ARREARS_TOLERANCE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        TRANSACTION_PROCESSING_STRATEGY_NAME_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        GRACE_ON_PRINCIPAL_PAYMENT_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    worksheet.setColumnWidth(
        GRACE_ON_INTEREST_PAYMENT_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    worksheet.setColumnWidth(
        GRACE_ON_INTEREST_CHARGED_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    ...
    writeString(
        TRANSACTION_PROCESSING_STRATEGY_NAME_COL, rowHeader, "Transaction Processing Strategy");
