...
public class EmailCampaignReadPlatformServiceImpl implements EmailCampaignReadPlatformService {
  ...
  private static final class EmailCampaignMapper implements RowMapper<EmailCampaignData> {
    ...
    private EmailCampaignMapper() {
      ...
      sql.append(
          "sr.report_name as reportName, sr.report_type as reportType, sr.report_subtype as"
              + " reportSubType, ");
      sql.append(
          "sr.report_category as reportCategory, sr.report_sql as reportSql, sr.description as"
              + " reportDescription, ");
    ...
    public EmailCampaignData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      if (emailAttachmentFileFormatString != null) {
        emailAttachmentFileFormat =
            ScheduledEmailEnumerations.emailAttachementFileFormat(emailAttachmentFileFormatString);
      }
      ...
      return EmailCampaignData.instance(
          id,
          campaignName,
          campaignType,
          businessRuleId,
          ...
          emailSubject,
          emailMessage,
          emailAttachmentFileFormatString,
          reportId,
  ...
  private static final class BusinessRuleMapper
      implements ResultSetExtractor<List<EmailBusinessRulesData>> {
    ...
    @Override
    public List<EmailBusinessRulesData> extractData(ResultSet rs)
        throws SQLException, DataAccessException {
      List<EmailBusinessRulesData> emailBusinessRulesDataList =
          new ArrayList<EmailBusinessRulesData>();

      EmailBusinessRulesData emailBusinessRulesData = null;

      Map<Long, EmailBusinessRulesData> mapOfSameObjects =
          new HashMap<Long, EmailBusinessRulesData>();
      ...
      while (rs.next()) {
        ...
        if (emailBusinessRulesData == null) {
          ...
          emailBusinessRulesData =
              EmailBusinessRulesData.instance(
                  id, reportName, reportType, hashMap, reportSubType, description);
  ...
  public EmailBusinessRulesData retrieveOneTemplate(Long resourceId) {
    ...
    final String sql =
        "select " + this.businessRuleMapper.schema() + " where sr.report_type = ? and sr.id = ?";

    List<EmailBusinessRulesData> retrieveOne =
        this.jdbcTemplate.query(sql, this.businessRuleMapper, searchType, resourceId);
    try {
  ...
  public EmailCampaignData retrieveOne(Long resourceId) {
    ...
    try {
      final String sql =
          "select " + this.emailCampaignMapper.schema + " where ec.id = ? and ec.is_visible = ?";
  ...
  public Collection<EmailCampaignData> retrieveAllScheduleActiveCampaign() {
    ...
    final String sql =
        "select "
            + this.emailCampaignMapper.schema()
            + " where ec.status_enum = ? and ec.campaign_type = ? and ec.is_visible = ?";
    return this.jdbcTemplate.query(
        sql, this.emailCampaignMapper, statusEnum, scheduleCampaignType, visible);
  }
}
