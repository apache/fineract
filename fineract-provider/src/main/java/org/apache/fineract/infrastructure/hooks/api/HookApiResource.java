...
public class HookApiResource {
  ...
  @GET
  @ApiOperation(
      value = "Retrieve Hooks",
      notes = "Returns the list of hooks.\n" + "\n" + "Example Requests:\n" + "\n" + "hooks",
      responseContainer = "List",
      response = HookApiResourceSwagger.GetHookResponse.class)
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
  ...
  @Path("{hookId}")
  @ApiOperation(
      value = "Retrieve a Hook",
      notes = "Returns the details of a Hook.\n" + "\n" + "Example Requests:\n" + "\n" + "hooks/1")
  @ApiResponses({
    @ApiResponse(code = 200, message = "", response = HookApiResourceSwagger.GetHookResponse.class)
  })
  public String retrieveHook(
      @PathParam("hookId") @ApiParam(value = "hookId") final Long hookId,
    ...
    if (settings.isTemplate()) {
      final HookData hookData =
          this.readPlatformService.retrieveNewHookDetails(hook.getTemplateName());
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = HookApiResourceSwagger.GetHookTemplateResponse.class)
  })
  ...
  @ApiOperation(
      ...
      notes =
          "The following parameters can be passed for the creation of a hook :-\n"
              + "\n"
              + "name - string - Required. The name of the template that is being called. (See"
              + " /hooks/template for the list of valid hook names.)\n"
              + "\n"
              ...
              + "\n"
              + "config - hash - Required. Key/value pairs to provide settings for this hook."
              + " These settings vary between the templates.\n"
              + "\n"
              + "templateId - Optional. The UGD template ID associated with the same entity"
              + " (client or loan).")
  @ApiImplicitParams({
    @ApiImplicitParam(
        paramType = "body",
        ...
        required = true,
        type = "body",
        dataTypeClass = HookApiResourceSwagger.PostHookRequest.class)
  })
  @ApiResponses({
    @ApiResponse(code = 200, message = "", response = HookApiResourceSwagger.PostHookResponse.class)
  })
  public String createHook(@ApiParam(hidden = true) final String apiRequestBodyAsJson) {

    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().createHook().withJson(apiRequestBodyAsJson).build();
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        required = true,
        type = "body",
        dataTypeClass = HookApiResourceSwagger.PutHookRequest.class)
  })
  @ApiResponses({
    @ApiResponse(code = 200, message = "", response = HookApiResourceSwagger.PutHookResponse.class)
  })
  public String updateHook(
      @PathParam("hookId") @ApiParam(value = "hookId") final Long hookId,
      @ApiParam(hidden = true) final String apiRequestBodyAsJson) {

    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().updateHook(hookId).withJson(apiRequestBodyAsJson).build();
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = HookApiResourceSwagger.DeleteHookResponse.class)
  })
