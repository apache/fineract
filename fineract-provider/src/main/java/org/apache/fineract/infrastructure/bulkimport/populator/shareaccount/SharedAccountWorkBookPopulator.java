...
public class SharedAccountWorkBookPopulator extends AbstractWorkbookPopulator {
  ...
  public SharedAccountWorkBookPopulator(
      SharedProductsSheetPopulator sharedProductsSheetPopulator,
  ...
  private void setDefaults(Sheet sharedAccountSheet) {
    for (Integer rowNo = 1; rowNo < 3000; rowNo++) {
      ...
      writeFormula(
          SharedAccountsConstants.CURRENCY_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"CURRENCY_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"CURRENCY_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.DECIMAL_PLACES_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"DECIMAL_PLACES_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"DECIMAL_PLACES_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.TODAYS_PRICE_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"TODAYS_PRICE_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"TODAYS_PRICE_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.CURRENCY_IN_MULTIPLES_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"CURRENCY_IN_MULTIPLES_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"CURRENCY_IN_MULTIPLES_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.CHARGES_NAME_1_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"CHARGES_NAME_1_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"CHARGES_NAME_1_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.CHARGES_NAME_2_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"CHARGES_NAME_2_\",$B"
              + (rowNo + 1)
              + "))),\"\",INDIRECT(CONCATENATE(\"CHARGES_NAME_2_\",$B"
              + (rowNo + 1)
              + ")))");
      writeFormula(
          SharedAccountsConstants.CHARGES_NAME_3_COL,
          row,
          "IF(ISERROR(INDIRECT(CONCATENATE(\"CHARGES_NAME_3_\",$B"
              + (rowNo + 1)
  ...
  private void setRules(Sheet sharedAccountSheet, String dateFormat) {
    CellRangeAddressList clientNameRange =
        new CellRangeAddressList(
            ...
            SharedAccountsConstants.CLIENT_NAME_COL,
            SharedAccountsConstants.CLIENT_NAME_COL);
    ...
    CellRangeAddressList submittedDateRange =
        new CellRangeAddressList(
            ...
            SharedAccountsConstants.SUBMITTED_ON_COL);
    CellRangeAddressList lockingFrequencyTypeRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
            SharedAccountsConstants.LOCK_IN_PERIOD_FREQUENCY_TYPE,
            SharedAccountsConstants.LOCK_IN_PERIOD_FREQUENCY_TYPE);
    CellRangeAddressList applicationDateRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
            SharedAccountsConstants.APPLICATION_DATE_COL,
            SharedAccountsConstants.APPLICATION_DATE_COL);
    CellRangeAddressList allowDividendCalcRange =
        new CellRangeAddressList(
            1,
            SpreadsheetVersion.EXCEL97.getLastRowIndex(),
    ...
    DataValidationHelper validationHelper =
        new HSSFDataValidationHelper((HSSFSheet) sharedAccountSheet);
    ...
    DataValidation productValidation =
        validationHelper.createValidation(productNameConstraint, productRange);
    DataValidation submittedOnValidation =
        validationHelper.createValidation(dateConstraint, submittedDateRange);
    DataValidation frequencyValidation =
        validationHelper.createValidation(frequencyConstraint, lockingFrequencyTypeRange);
    DataValidation applicationDateValidation =
        validationHelper.createValidation(dateConstraint, applicationDateRange);
    DataValidation allowDividendValidation =
        validationHelper.createValidation(booleanConstraint, allowDividendCalcRange);
  ...
  private void setNames(Sheet sharedAccountSheet) {
    ...
    clientsGroup.setRefersToFormula(
        TemplatePopulateImportConstants.CLIENT_SHEET_NAME + "!$B$2:$B$" + clients.size() + 1);

    Name productGroup = sharedAccountWorkbook.createName();
    productGroup.setNameName("Products");
    productGroup.setRefersToFormula(
        TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME
            + "!$B$2:$B$"
            + products.size()
            + 1);
    ...
    for (Integer i = 0; i < products.size(); i++) {
      ...
      currecyName.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$C$" + (i + 2));

      decimalPlacesName.setNameName("DECIMAL_PLACES_" + productName);
      decimalPlacesName.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$D$" + (i + 2));

      todaysPriceName.setNameName("TODAYS_PRICE_" + productName);
      todaysPriceName.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$E$" + (i + 2));

      currencyInMultiplesName.setNameName("CURRENCY_IN_MULTIPLES_" + productName);
      currencyInMultiplesName.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$F$" + (i + 2));

      chargesName1.setNameName("CHARGES_NAME_1_" + productName);
      chargesName1.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$I$" + (i + 2));

      chargesName2.setNameName("CHARGES_NAME_2_" + productName);
      chargesName2.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$K$" + (i + 2));

      chargesName3.setNameName("CHARGES_NAME_3_" + productName);
      chargesName3.setRefersToFormula(
          TemplatePopulateImportConstants.SHARED_PRODUCTS_SHEET_NAME + "!$M$" + (i + 2));
    }
  }
  ...
  private void setLayout(Sheet worksheet) {
    ...
    worksheet.setColumnWidth(
        SharedAccountsConstants.CLIENT_NAME_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CLIENT_NAME_COL, rowHeader, "Client Name *");

    worksheet.setColumnWidth(
        SharedAccountsConstants.PRODUCT_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.PRODUCT_COL, rowHeader, "Shared Product *");

    worksheet.setColumnWidth(
        SharedAccountsConstants.SUBMITTED_ON_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.SUBMITTED_ON_COL, rowHeader, "Submitted On Date *");

    worksheet.setColumnWidth(
        SharedAccountsConstants.EXTERNAL_ID_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.EXTERNAL_ID_COL, rowHeader, "External Id ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CURRENCY_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    writeString(SharedAccountsConstants.CURRENCY_COL, rowHeader, "Currency ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.DECIMAL_PLACES_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.DECIMAL_PLACES_COL, rowHeader, "Decimal places ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.TOTAL_NO_SHARES_COL,
    ...
    worksheet.setColumnWidth(
        SharedAccountsConstants.TODAYS_PRICE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    ...
    worksheet.setColumnWidth(
        ...
        TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    writeString(
        SharedAccountsConstants.CURRENCY_IN_MULTIPLES_COL, rowHeader, "Currency in multiples ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.DEFAULT_SAVINGS_AC_COL,
    ...
    worksheet.setColumnWidth(
        SharedAccountsConstants.MINIMUM_ACTIVE_PERIOD_IN_DAYS_COL,
        TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    writeString(
        SharedAccountsConstants.MINIMUM_ACTIVE_PERIOD_IN_DAYS_COL,
        rowHeader,
        "Minimum active period(in days) ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.LOCK_IN_PERIOD_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    ...
    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_NAME_1_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CHARGES_NAME_1_COL, rowHeader, "Charges 1 ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_AMOUNT_1_COL,
        TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CHARGES_AMOUNT_1_COL, rowHeader, "Amount 1 ");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_NAME_2_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CHARGES_NAME_2_COL, rowHeader, "Charge 2");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_AMOUNT_2_COL,
        TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CHARGES_AMOUNT_2_COL, rowHeader, "Amount 2");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_NAME_3_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    writeString(SharedAccountsConstants.CHARGES_NAME_3_COL, rowHeader, "Charge 3");

    worksheet.setColumnWidth(
        SharedAccountsConstants.CHARGES_AMOUNT_3_COL,
        TemplatePopulateImportConstants.SMALL_COL_SIZE);
