...
public class BulkImportApiResource {
  ...
  public String retrieveImportDocuments(
      @Context final UriInfo uriInfo, @QueryParam("entityType") final String entityType) {
    ...
    if (entityType.equals(GlobalEntityType.CLIENT.getCode())) {
      ...
      if (importForClientEntity != null) importData.addAll(importForClientEntity);
      if (importForClientPerson != null) importData.addAll(importForClientPerson);
    } else {
  ...
  public String retriveOutputTemplateLocation(
      @QueryParam("importDocumentId") final String importDocumentId) {
    this.context.authenticatedUser().validateHasReadPermission(this.resourceNameForPermissions);
    final DocumentData documentData =
        this.bulkImportWorkbookService.getOutputTemplateLocation(importDocumentId);
