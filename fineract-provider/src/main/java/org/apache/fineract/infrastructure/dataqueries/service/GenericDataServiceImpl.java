...
public class GenericDataServiceImpl implements GenericDataService {
    ...
    try {
      ...
      for (int i = 0; i < rsmd.getColumnCount(); i++) {
        ...
        final ResultsetColumnHeaderData columnHeader =
            ResultsetColumnHeaderData.basic(columnName, columnType);
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.report.unknown.data.integrity.issue", e.getClass().getName());
    }
  }
  ...
  public List<ResultsetColumnHeaderData> fillResultsetColumnHeaders(final String datatable) {
    ...
    while (columnDefinitions.next()) {
      ...
      final ResultsetColumnHeaderData rsch =
          ResultsetColumnHeaderData.detailed(
              columnName,
              columnType,
              columnLength,
              columnNullable,
  ...
  private List<ResultsetColumnValueData> retreiveColumnValues(final String codeName) {
    ...
    final String sql =
        "select v.id, v.code_score, v.code_value from m_code m "
  ...
  private List<ResultsetColumnValueData> retreiveColumnValues(final Integer codeId) {
    ...
    if (codeId != null) {
      final String sql =
          "select v.id, v.code_value from m_code_value v where v.code_id ="
              + codeId
              + " order by v.order_position, v.id";
      final SqlRowSet rsValues = this.jdbcTemplate.queryForRowSet(sql);
      rsValues.beforeFirst();
  ...
  private SqlRowSet getDatatableMetaData(final String datatable) {
    ...
    final String sql =
        "select COLUMN_NAME, IS_NULLABLE, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, COLUMN_KEY"
  ...
  private SqlRowSet getDatatableCodeData(final String datatable, final String columnName) {
    ...
    final String sql =
        "select mc.id,mc.code_name from m_code mc join x_table_column_code_mappings xcc on"
            + " xcc.code_id = mc.id where xcc.column_alias_name='"
