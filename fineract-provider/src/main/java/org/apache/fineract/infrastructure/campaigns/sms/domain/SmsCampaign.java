...
public class SmsCampaign extends AbstractPersistableCustom {
  ...
  public static SmsCampaign instance(
      final AppUser submittedBy, final Report report, final JsonCommand command) {
    ...
    if (command.parameterExists(SmsCampaignValidator.isNotificationParamName)) {
      isNotification =
          command.booleanPrimitiveValueOfParameterNamed(
              SmsCampaignValidator.isNotificationParamName);
    }
    ...
    if (SmsCampaignTriggerType.fromInt(triggerType.intValue()).isSchedule()) {
      ...
      if (command.hasParameter(SmsCampaignValidator.dateTimeFormat)) {
        ...
        if (command.hasParameter(SmsCampaignValidator.recurrenceStartDate)) {
          recurrenceStartDate =
              LocalDateTime.parse(
                  command.stringValueOfParameterNamed(SmsCampaignValidator.recurrenceStartDate),
                  fmt);
        }
  ...
  public Map<String, Object> update(JsonCommand command) {
    ...
    if (command.isChangeInStringParameterNamed(
        SmsCampaignValidator.campaignName, this.campaignName)) {
    ...
    if (command.isChangeInIntegerParameterNamed(
        SmsCampaignValidator.campaignType, this.campaignType)) {
      final Integer newValue =
          command.integerValueOfParameterNamed(SmsCampaignValidator.campaignType);
    ...
    if (command.isChangeInIntegerParameterNamed(
        SmsCampaignValidator.triggerType, this.triggerType)) {
      final Integer newValue =
          command.integerValueOfParameterNamed(SmsCampaignValidator.triggerType);
    ...
    if (command.isChangeInLongParameterNamed(
        SmsCampaignValidator.runReportId,
    ...
    if (command.isChangeInStringParameterNamed(
        SmsCampaignValidator.recurrenceParamName, this.recurrence)) {
      final String newValue =
    ...
    if (command.isChangeInBooleanParameterNamed(
      ...
      final Boolean newValue =
          command.booleanObjectValueOfParameterNamed(SmsCampaignValidator.isNotificationParamName);
    ...
    if (SmsCampaignTriggerType.fromInt(this.triggerType).isSchedule()) {
      ...
      final String dateTimeFormatAsInput =
          command.stringValueOfParameterNamed(SmsCampaignValidator.dateTimeFormat);
      final String localeAsInput = command.locale();
      final Locale locale = command.extractLocale();
      final DateTimeFormatter fmt =
          DateTimeFormat.forPattern(dateTimeFormatAsInput).withLocale(locale);
    ...
    if (isActive()) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              ...
              defaultUserMessage,
              SmsCampaignValidator.activationDateParamName,
    ...
    if (isClosed()) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              ...
              defaultUserMessage,
              SmsCampaignValidator.statusParamName,
              SmsCampaignStatus.fromInt(this.status).getCode());
    ...
    if (getSubmittedOnDate() != null && isDateInTheFuture(getSubmittedOnDate())) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              ...
              defaultUserMessage,
              SmsCampaignValidator.submittedOnDateParamName,
    ...
    if (getClosureDate() != null && isDateInTheFuture(getClosureDate())) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              ...
              defaultUserMessage,
              SmsCampaignValidator.closureDateParamName,
              this.closureDate);
  ...
  private static String constructRecurrence(final JsonCommand command) {
    final Integer frequency =
        command.integerValueOfParameterNamed(SmsCampaignValidator.frequencyParamName);
    final CalendarFrequencyType frequencyType = CalendarFrequencyType.fromInt(frequency);
    final Integer interval =
        command.integerValueOfParameterNamed(SmsCampaignValidator.intervalParamName);
    Integer repeatsOnDay = null;
    if (frequencyType.isWeekly()) {
      repeatsOnDay =
          command.integerValueOfParameterNamed(SmsCampaignValidator.repeatsOnDayParamName);
    }
