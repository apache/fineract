...
public class ReportWritePlatformServiceImpl implements ReportWritePlatformService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(ReportWritePlatformServiceImpl.class);
  ...
  public ReportWritePlatformServiceImpl(
      ...
      final ReportParameterUsageRepository reportParameterUsageRepository,
  ...
  public CommandProcessingResult createReport(final JsonCommand command) {
    ...
    try {
      ...
      final Report report =
          Report.fromJson(command, this.readReportingService.getAllowedReportTypes());
  ...
  public CommandProcessingResult updateReport(final Long reportId, final JsonCommand command) {
    ...
    try {
      ...
      final Map<String, Object> changes =
          report.update(command, this.readReportingService.getAllowedReportTypes());
  ...
  public CommandProcessingResult deleteReport(final Long reportId) {
    ...
    if (report.isCoreReport()) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.cant.delete.core.report", "Core Reports Can't be Deleted", "");
    }

    final Permission permission =
        this.permissionRepository.findOneByCode("READ" + "_" + report.getReportName());
    if (permission == null) {
   ...
   * is.
   */
  private void handleReportDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("unq_report_name")
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.report.duplicate.name",
          "A report with name '" + name + "' already exists",
          "name",
          name);
    }
      ...
      if (reportParametersArray != null) {
        ...
        for (int i = 0; i < reportParametersArray.size(); i++) {
          ...
          if (id != null) {
            ...
            reportParameterUsageItem =
                this.reportParameterUsageRepository.findById(id).orElse(null);
            ...
            if (jsonObject.has("parameterId")) {
              ...
              reportParameter =
                  this.reportParameterRepository
            ...
            reportParameterUsageItem =
                new ReportParameterUsage(report, reportParameter, reportParameterName);
          }
