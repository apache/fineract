...
public class EmailCampaign extends AbstractPersistableCustom {
  ...
  public static EmailCampaign instance(
      final AppUser submittedBy,
      final Report businessRuleId,
      final Report stretchyReport,
    ...
    final String campaignName =
        command.stringValueOfParameterNamed(EmailCampaignValidator.campaignName);
    final Long campaignType =
        command.longValueOfParameterNamed(EmailCampaignValidator.campaignType);

    final String paramValue =
        command.stringValueOfParameterNamed(EmailCampaignValidator.paramValue);
    final String emailSubject =
        command.stringValueOfParameterNamed(EmailCampaignValidator.emailSubject);
    final String emailMessage =
        command.stringValueOfParameterNamed(EmailCampaignValidator.emailMessage);
    final String stretchyReportParamMap =
        command.stringValueOfParameterNamed(
            ScheduledEmailConstants.STRETCHY_REPORT_PARAM_MAP_PARAM_NAME);
    ...
    if (emailAttachmentFileFormatId != null) {
      emailAttachmentFileFormat =
          ScheduledEmailAttachmentFileFormat.instance(emailAttachmentFileFormatId);
    } else {
    ...
    if (command.hasParameter(EmailCampaignValidator.submittedOnDateParamName)) {
      submittedOnDate =
          command.localDateValueOfParameterNamed(EmailCampaignValidator.submittedOnDateParamName);
    }

    final String recurrence =
        command.stringValueOfParameterNamed(EmailCampaignValidator.recurrenceParamName);
  ...
  public Map<String, Object> update(JsonCommand command) {
    ...
    if (command.isChangeInStringParameterNamed(
        EmailCampaignValidator.campaignName, this.campaignName)) {
    ...
    if (command.isChangeInStringParameterNamed(
        EmailCampaignValidator.emailMessage, this.emailMessage)) {
      final String newValue =
          command.stringValueOfParameterNamed(EmailCampaignValidator.emailMessage);
    ...
    if (command.isChangeInStringParameterNamed(
        EmailCampaignValidator.paramValue, this.paramValue)) {
      final String newValue =
          command.stringValueOfParameterNamed(EmailCampaignValidator.paramValue);
    ...
    if (command.isChangeInIntegerParameterNamed(
        EmailCampaignValidator.campaignType, this.campaignType)) {
      final Integer newValue =
          command.integerValueOfParameterNamed(EmailCampaignValidator.campaignType);
    ...
    if (command.isChangeInLongParameterNamed(
        EmailCampaignValidator.businessRuleId,
        (this.businessRuleId != null) ? this.businessRuleId.getId() : null)) {
      final String newValue =
          command.stringValueOfParameterNamed(EmailCampaignValidator.businessRuleId);
      actualChanges.put(EmailCampaignValidator.businessRuleId, newValue);
    }
    if (command.isChangeInStringParameterNamed(
        EmailCampaignValidator.recurrenceParamName, this.recurrence)) {
      final String newValue =
          command.stringValueOfParameterNamed(EmailCampaignValidator.recurrenceParamName);
    ...
    if (command.isChangeInLocalDateParameterNamed(
        EmailCampaignValidator.recurrenceStartDate, getRecurrenceStartDate())) {
    ...
    if (isActive()) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              ...
              defaultUserMessage,
              EmailCampaignValidator.activationDateParamName,
  ...
  public void close(
      ...
      final LocalDate closureLocalDate) {
    if (isClosed()) {
      // handle errors if already activated
      final String defaultUserMessage = "Cannot close campaign. Campaign already in closed state.";
      final ApiParameterError error =
          ApiParameterError.parameterError(
              "error.msg.campaign.already.closed",
              defaultUserMessage,
              EmailCampaignValidator.statusParamName,
              EmailCampaignStatus.fromInt(this.status).getCode());
  ...
  public void reactivate(
      ...
      final LocalDate reactivateLocalDate) {

    if (!isClosed()) {
      // handle errors if already activated
      final String defaultUserMessage =
          "Cannot reactivate campaign. Campaign must be in closed state.";
      final ApiParameterError error =
          ApiParameterError.parameterError(
              "error.msg.campaign.must.be.closed",
              defaultUserMessage,
              EmailCampaignValidator.statusParamName,
              EmailCampaignStatus.fromInt(this.status).getCode());
  ...
  private void validateActivationDate(final List<ApiParameterError> dataValidationErrors) {
    ...
    if (getSubmittedOnDate() != null && isDateInTheFuture(getSubmittedOnDate())) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              "error.msg.campaign.submittedOnDate.in.the.future",
              defaultUserMessage,
    ...
    if (getActivationLocalDate() != null
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              "error.msg.campaign.submittedOnDate.after.activation.date",
              defaultUserMessage,
              EmailCampaignValidator.submittedOnDateParamName,
              this.submittedOnDate);
  ...
  private void validateClosureDate(final List<ApiParameterError> dataValidationErrors) {
    if (getClosureDate() != null && isDateInTheFuture(getClosureDate())) {
      ...
      final ApiParameterError error =
          ApiParameterError.parameterError(
              "error.msg.campaign.closureDate.in.the.future",
              defaultUserMessage,
              EmailCampaignValidator.closureDateParamName,
              this.closureDate);
