...
public class EntityDatatableChecksReadPlatformServiceImpl
  ...
  @Autowired
  public EntityDatatableChecksReadPlatformServiceImpl(
  ...
  @Override
  public Page<EntityDataTableChecksData> retrieveAll(
  ...
  @Override
  public List<DatatableData> retrieveTemplates(
      final Long status, final String entity, final Long productId) {

    List<EntityDatatableChecks> tableRequiredBeforeAction = null;
    if (productId != null) {
      tableRequiredBeforeAction =
          this.entityDatatableChecksRepository.findByEntityStatusAndProduct(
              entity, status, productId);
    }

    if (tableRequiredBeforeAction == null || tableRequiredBeforeAction.size() < 1) {
      tableRequiredBeforeAction =
          this.entityDatatableChecksRepository.findByEntityStatusAndNoProduct(entity, status);
    }
  ...
  public EntityDataTableChecksTemplateData retrieveTemplate() {
    ...
    List<DatatableCheckStatusData> statusSavings =
        getStatusList(EntityTables.getStatus("m_savings_account"));

    Collection<LoanProductData> loanProductDatas =
        this.loanProductReadPlatformService.retrieveAllLoanProductsForLookup(true);
    Collection<SavingsProductData> savingsProductDatas =
        this.savingsProductReadPlatformService.retrieveAllForLookup();

    return new EntityDataTableChecksTemplateData(
        entities,
        statusClient,
        statusGroup,
        statusSavings,
  ...
  protected static final class RegisterDataTableMapper implements RowMapper<DatatableChecksData> {
    ...
    @Override
    public DatatableChecksData mapRow(
  ...
  protected static final class EntityDataTableChecksMapper
      implements RowMapper<EntityDataTableChecksData> {

    @Override
    public EntityDataTableChecksData mapRow(
        final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
      ...
      return new EntityDataTableChecksData(
          id, entity, statusEnum, datatableName, systemDefined, productId, productName);
    }

    public String schema() {
      return " t.id as id, t.application_table_name as entity, t.status_enum as status, "
          + " t.system_defined as systemDefined,  t.x_registered_table_name as"
          + " datatableName,  t.product_id as productId,  (CASE t.application_table_name"
          + " WHEN 'm_loan' THEN lp.name WHEN 'm_savings_account' THEN sp.name ELSE NULL "
          + " END) as productName from m_entity_datatable_check as t  left join"
          + " m_product_loan lp on lp.id = t.product_id and t.application_table_name ="
          + " 'm_loan' left join m_savings_product sp on sp.id = t.product_id and"
          + " t.application_table_name = 'm_savings_account' ";
    }
  }
}
