...
public class BasicAuthTenantDetailsServiceJdbc implements BasicAuthTenantDetailsService {
  ...
  @Autowired
  public BasicAuthTenantDetailsServiceJdbc(
  ...
  private static final class TenantMapper implements RowMapper<FineractPlatformTenant> {
    ...
    private final StringBuilder sqlBuilder =
        new StringBuilder(" t.id, ts.id as connectionId , ") //
            .append(
            ...
            .append(
                " ts.pool_validation_interval as validationInterval, ts.pool_remove_abandoned as"
                    + " removeAbandoned, ts.pool_remove_abandoned_timeout as"
                    + " removeAbandonedTimeout,") //
            .append(
                " ts.pool_log_abandoned as logAbandoned, ts.pool_abandon_when_percentage_full as"
                    + " abandonedWhenPercentageFull, ts.pool_test_on_borrow as testOnBorrow,") //
            .append(
                " ts.pool_max_active as poolMaxActive, ts.pool_min_idle as poolMinIdle,"
                    + " ts.pool_max_idle as poolMaxIdle,") //
            .append(
                " ts.pool_suspect_timeout as poolSuspectTimeout,"
                    + " ts.pool_time_between_eviction_runs_millis as"
    ...
    @Override
    public FineractPlatformTenant mapRow(
    ...
    private FineractPlatformTenantConnection getDBConnection(ResultSet rs) throws SQLException {
      ...
      return new FineractPlatformTenantConnection(
          connectionId,
          schemaName,
          schemaServer,
          schemaServerPort,
  ...
  public FineractPlatformTenant loadTenantById(
      final String tenantIdentifier, final boolean isReport) {
    ...
    try {
      ...
      throw new InvalidTenantIdentiferException(
          "The tenant identifier: " + tenantIdentifier + " is not valid.");
    }
  }
}
