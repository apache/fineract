...
public class SmsCampaignReadPlatformServiceImpl implements SmsCampaignReadPlatformService {
  ...
  public SmsCampaignData retrieveOne(Long campaignId) {
    ...
    try {
      final String sql =
          "select " + this.smsCampaignMapper.schema + " where sc.id = ? and sc.is_visible = ?";
      return this.jdbcTemplate.queryForObject(
          sql, this.smsCampaignMapper, new Object[] {campaignId, isVisible});
  ...
  public SmsCampaignData retrieveTemplate(final String reportType) {
    ...
    final Collection<SmsBusinessRulesData> businessRulesOptions =
        this.jdbcTemplate.query(sql, this.businessRuleMapper, new Object[] {reportType});
    ...
    final Collection<EnumOptionData> campaignTriggerTypeOptions =
        this.smsCampaignDropdownReadPlatformService.retrieveCampaignTriggerTypes();
    final Collection<EnumOptionData> months =
        this.smsCampaignDropdownReadPlatformService.retrieveMonths();
    final Collection<EnumOptionData> weekDays =
        this.smsCampaignDropdownReadPlatformService.retrieveWeeks();
    final Collection<EnumOptionData> frequencyTypeOptions =
        this.calendarDropdownReadPlatformService.retrieveCalendarFrequencyTypeOptions();
    final Collection<EnumOptionData> periodFrequencyOptions =
        this.smsCampaignDropdownReadPlatformService.retrivePeriodFrequencyTypes();
    ...
    return SmsCampaignData.template(
        smsProviderOptions,
        campaignTypeOptions,
        businessRulesOptions,
        campaignTriggerTypeOptions,
        months,
        weekDays,
  ...
  private static final class BusinessRuleMapper
    ...
    @Override
    public List<SmsBusinessRulesData> extractData(ResultSet rs)
      ...
      while (rs.next()) {
        ...
        if (smsBusinessRulesData == null) {
          ...
          smsBusinessRulesData =
              SmsBusinessRulesData.instance(
                  id, reportName, reportType, reportSubType, hashMap, description);
  ...
  private static final class SmsCampaignMapper implements RowMapper<SmsCampaignData> {
    ...
    public SmsCampaignData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      final SmsCampaignTimeLine smsCampaignTimeLine =
          new SmsCampaignTimeLine(
              submittedOnDate,
              submittedByUsername,
              activatedOnDate,
              activatedByUsername,
      ...
      return SmsCampaignData.instance(
          id,
          campaignName,
          campaignTypeEnum,
          triggerTypeEnum,
