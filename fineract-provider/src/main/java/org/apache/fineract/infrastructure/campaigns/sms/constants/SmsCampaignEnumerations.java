...
public class SmsCampaignEnumerations {
  ...
  public static EnumOptionData smscampaignType(final CampaignType type) {
    EnumOptionData optionData =
        new EnumOptionData(
            CampaignType.INVALID.getValue().longValue(), CampaignType.INVALID.getCode(), "Invalid");
    switch (type) {
      case INVALID:
        break;
      case SMS:
        optionData =
            new EnumOptionData(
                CampaignType.SMS.getValue().longValue(), CampaignType.SMS.getCode(), "SMS");
        break;
      case NOTIFICATION:
        optionData =
            new EnumOptionData(
                CampaignType.NOTIFICATION.getValue().longValue(),
                CampaignType.NOTIFICATION.getCode(),
                "NOTIFICATION");
        break;
    }
    return optionData;
  ...
  public static EnumOptionData calendarMonthType(final Month entityType) {
    final EnumOptionData optionData =
        new EnumOptionData(
            Long.valueOf(entityType.getValue()), entityType.name(), entityType.name());
    return optionData;
  }
  ...
  public static List<EnumOptionData> calendarMonthType() {
    ...
    for (final Month monthType : Month.values()) {
      if (Month.DECEMBER.compareTo(monthType)
          != 0) { // We are removing December because we are adding yearly frequency
  ...
  public static List<EnumOptionData> calendarPeriodFrequencyTypes(
      final PeriodFrequencyType[] periodFrequencyTypes) {
    final List<EnumOptionData> optionDatas = new ArrayList<>();
    for (final PeriodFrequencyType periodFrequencyType : periodFrequencyTypes) {
      if (!periodFrequencyType.getValue().equals(PeriodFrequencyType.INVALID.getValue())) {
        final EnumOptionData optionData =
            new EnumOptionData(
                periodFrequencyType.getValue().longValue(),
