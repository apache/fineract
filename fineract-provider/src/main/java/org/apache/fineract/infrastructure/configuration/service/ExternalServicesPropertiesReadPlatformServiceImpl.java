...
public class ExternalServicesPropertiesReadPlatformServiceImpl
  ...
  private static final class S3CredentialsDataExtractor
      implements ResultSetExtractor<S3CredentialsData> {

    @Override
    public S3CredentialsData extractData(final ResultSet rs)
      ...
      while (rs.next()) {
        if (rs.getString("name").equalsIgnoreCase(ExternalServicesConstants.S3_ACCESS_KEY)) {
          accessKey = rs.getString("value");
            ...
            .equalsIgnoreCase(ExternalServicesConstants.S3_BUCKET_NAME)) {
  ...
  private static final class SMTPCredentialsDataExtractor
      implements ResultSetExtractor<SMTPCredentialsData> {

    @Override
    public SMTPCredentialsData extractData(final ResultSet rs)
        throws SQLException, DataAccessException {
  ...
  private static final class MessageGatewayDataExtractor
      implements ResultSetExtractor<MessageGatewayConfigurationData> {

    @Override
    public MessageGatewayConfigurationData extractData(final ResultSet rs)
        throws SQLException, DataAccessException {
      String host = null;
      int port = 9191;
      ...
      while (rs.next()) {
        if (rs.getString("name").equalsIgnoreCase(ExternalServicesConstants.SMS_HOST)) {
          ...
          endPoint = rs.getString("value");
            ...
            .equalsIgnoreCase(ExternalServicesConstants.SMS_TENANT_APP_KEY)) {
          tenantAppKey = rs.getString("value");
        }
      }
      return new MessageGatewayConfigurationData(
          null, null, host, port, endPoint, null, null, false, tenantAppKey);
    }
  }

  @Override
  public S3CredentialsData getS3Credentials() {
    final ResultSetExtractor<S3CredentialsData> resultSetExtractor =
        new S3CredentialsDataExtractor();
    final String sql =
  ...
  public SMTPCredentialsData getSMTPCredentials() {
    // TODO Auto-generated method stub
    final ResultSetExtractor<SMTPCredentialsData> resultSetExtractor =
        new SMTPCredentialsDataExtractor();
    final String sql =
        "SELECT esp.name, esp.value FROM c_external_service_properties esp inner join"
            + " c_external_service es on esp.external_service_id = es.id where es.name = '"
  ...
  public MessageGatewayConfigurationData getSMSGateway() {
    final ResultSetExtractor<MessageGatewayConfigurationData> resultSetExtractor =
        new MessageGatewayDataExtractor();
    final String sql =
        "SELECT esp.name, esp.value FROM c_external_service_properties esp inner join"
            + " c_external_service es on esp.external_service_id = es.id where es.name = '"
            + ExternalServicesConstants.SMS_SERVICE_NAME
            + "'";
    final MessageGatewayConfigurationData messageGatewayConfigurationData =
        this.jdbcTemplate.query(sql, resultSetExtractor, new Object[] {});
  ...
  private static final class NotificationDataExtractor
      implements ResultSetExtractor<NotificationConfigurationData> {

    @Override
    public NotificationConfigurationData extractData(final ResultSet rs)
        throws SQLException, DataAccessException {
      ...
      while (rs.next()) {
        if (rs.getString("name")
            .equalsIgnoreCase(ExternalServicesConstants.NOTIFICATION_SERVER_KEY)) {
          serverKey = rs.getString("value");
        } else if (rs.getString("name")
            .equalsIgnoreCase(ExternalServicesConstants.NOTIFICATION_GCM_END_POINT)) {
          gcmEndPoint = rs.getString("value");
        } else if (rs.getString("name")
            .equalsIgnoreCase(ExternalServicesConstants.NOTIFICATION_FCM_END_POINT)) {
  ...
  public NotificationConfigurationData getNotificationConfiguration() {
    final ResultSetExtractor<NotificationConfigurationData> resultSetExtractor =
        new NotificationDataExtractor();
    final String sql =
        "SELECT esp.name, esp.value FROM c_external_service_properties esp inner join"
            + " c_external_service es on esp.external_service_id = es.id where es.name = '"
            + ExternalServicesConstants.NOTIFICATION_SERVICE_NAME
            + "'";
    final NotificationConfigurationData notificationConfigurationData =
        this.jdbcTemplate.query(sql, resultSetExtractor, new Object[] {});
