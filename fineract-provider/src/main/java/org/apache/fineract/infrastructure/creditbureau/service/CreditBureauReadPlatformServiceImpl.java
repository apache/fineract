...
public class CreditBureauReadPlatformServiceImpl implements CreditBureauReadPlatformService {
  ...
  @Autowired
  public CreditBureauReadPlatformServiceImpl(
  ...
  private static final class CBMapper implements RowMapper<CreditBureauData> {
    public String schema() {
      return "cb.id as creditBureauID,cb.name as creditBureauName,cb.product as"
          + " creditBureauProduct,cb.country as country,concat(cb.product,' - ',cb.name,' -"
          + " ',cb.country) as cbSummary,cb.implementationKey as implementationKey from"
          + " m_creditbureau cb";
    }
