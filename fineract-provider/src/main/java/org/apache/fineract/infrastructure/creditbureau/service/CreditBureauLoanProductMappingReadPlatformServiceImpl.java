...
public class CreditBureauLoanProductMappingReadPlatformServiceImpl
  ...
  @Autowired
  public CreditBureauLoanProductMappingReadPlatformServiceImpl(
      final PlatformSecurityContext context, final RoutingDataSource dataSource) {
    this.context = context;
    this.jdbcTemplate = new JdbcTemplate(dataSource);
  }

  private static final class CreditBureauLoanProductMapper
      implements RowMapper<CreditBureauLoanProductMappingData> {

    public String schema() {
      return "cblp.id as mappingId, cblp.organisation_creditbureau_id as orgcbId,orgcb.alias as"
          + " alias,concat( cb.product,' - ' ,cb.name,' - ',cb.country) as"
          + " creditbureau,cblp.loan_product_id as lpId,lp.name as"
          + " loan_product_name,cblp.is_creditcheck_mandatory as"
          + " crCheck,cblp.skip_creditcheck_in_failure as skipcheck,cblp.stale_period as"
          + " staleperiod,cblp.is_active as is_active from"
          + " m_creditbureau_loanproduct_mapping cblp, m_organisation_creditbureau"
          + " orgcb,m_product_loan lp,m_creditbureau cb where"
          + " cblp.organisation_creditbureau_id=orgcb.id and cblp.loan_product_id=lp.id and"
          + " orgcb.creditbureau_id=cb.id";
    }

    @Override
    public CreditBureauLoanProductMappingData mapRow(
      ...
      return CreditBureauLoanProductMappingData.instance(
          mapping_id,
          orgcbID,
          alias,
          credit_bureau_name,
          ...
          skip_credit_check_in_failure,
          stale_period,
          is_active);
    }
  }

  private static final class LoanProductMapper
      implements RowMapper<CreditBureauLoanProductMappingData> {
