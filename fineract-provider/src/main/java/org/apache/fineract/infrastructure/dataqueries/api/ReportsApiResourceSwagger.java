    ...
    @ApiModelProperty(
        example =
            "Individual Client Report Lists the small number of defined fields on the client"
                + " table.  Would expect to copy this report and add any one to one additional"
                + " data for specific tenant needs. Can be run for any size MFI but you expect it"
                + " only to be run within a branch for larger ones.  Depending on how many columns"
                + " are displayed, there is probably is a limit of about 20/50k clients returned"
                + " for html display (export to excel doesnt have that client browser/memory"
                + " impact).")
