...
public class BulkImportWorkbookServiceImpl implements BulkImportWorkbookService {
    ...
    try {
      if (entity != null
        ...
        if (!fileType.contains("msoffice") && !fileType.contains("application/vnd.ms-excel")) {
          // We had a problem where we tried to upload the downloaded file from the import options,
          // it was somehow changed the
        ...
        if (entity.trim().equalsIgnoreCase(GlobalEntityType.CLIENTS_PERSON.toString())) {
          ...
          primaryColumn = 0;
            ...
            .trim()
            .equalsIgnoreCase(GlobalEntityType.SAVINGS_TRANSACTIONS.toString())) {
          entityType = GlobalEntityType.SAVINGS_TRANSACTIONS;
          primaryColumn = 0;
        } else if (entity
            .trim()
            .equalsIgnoreCase(GlobalEntityType.RECURRING_DEPOSIT_ACCOUNTS.toString())) {
          ...
          primaryColumn = 0;
            ...
            .trim()
            .equalsIgnoreCase(GlobalEntityType.FIXED_DEPOSIT_ACCOUNTS.toString())) {
          entityType = GlobalEntityType.FIXED_DEPOSIT_ACCOUNTS;
          primaryColumn = 0;
        } else if (entity
            .trim()
            .equalsIgnoreCase(GlobalEntityType.FIXED_DEPOSIT_TRANSACTIONS.toString())) {
        ...
        throw new GeneralPlatformDomainRuleException(
            "error.msg.null", "One or more of the given parameters not found");
      }
    } catch (IOException e) {
      LOG.error("Problem occurred in importWorkbook function", e);
      throw new GeneralPlatformDomainRuleException(
          "error.msg.io.exception",
          "IO exception occured with " + fileDetail.getFileName() + " " + e.getMessage());
    }
  }

  private Long publishEvent(
      final Integer primaryColumn,
      final FormDataContentDisposition fileDetail,
      final InputStream clonedInputStreamWorkbook,
    ...
    final Long documentId =
        this.documentWritePlatformService.createInternalDocument(
            ...
            this.securityContext.authenticatedUser().getId(),
            null,
            clonedInputStreamWorkbook,
  ...
  private static final class ImportMapper implements RowMapper<ImportData> {
    ...
    public String schema() {
      ...
      sql.append(
              "i.id as id, i.document_id as documentId, d.name as name, i.import_time as"
                  + " importTime, i.end_time as endTime, ")
          .append(
              "i.completed as completed, i.total_records as totalRecords, i.success_count as"
                  + " successCount, ")
    ...
    @Override
    public ImportData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  public DocumentData getOutputTemplateLocation(String importDocumentId) {
    this.securityContext.authenticatedUser();
    final ImportTemplateLocationMapper importTemplateLocationMapper =
        new ImportTemplateLocationMapper();
    final String sql = "select " + importTemplateLocationMapper.schema();

    return this.jdbcTemplate.queryForObject(
        sql, importTemplateLocationMapper, new Object[] {importDocumentId});
  }
