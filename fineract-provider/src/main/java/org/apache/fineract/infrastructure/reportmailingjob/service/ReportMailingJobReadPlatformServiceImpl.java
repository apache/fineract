...
public class ReportMailingJobReadPlatformServiceImpl
  ...
  @Autowired
  public ReportMailingJobReadPlatformServiceImpl(
  ...
  @Override
  public Page<ReportMailingJobData> retrieveAllReportMailingJobs(
    ...
    if (searchParameters.isOrderByRequested()) {
      ...
      this.columnValidator.validateSqlInjection(
          sqlStringBuilder.toString(), searchParameters.getOrderBy());
      if (searchParameters.isSortOrderProvided()) {
        sqlStringBuilder.append(" ").append(searchParameters.getSortOrder());
        this.columnValidator.validateSqlInjection(
            sqlStringBuilder.toString(), searchParameters.getSortOrder());
      }
    } else {
  ...
  public ReportMailingJobData retrieveReportMailingJob(final Long reportMailingJobId) {
    try {
      final ReportMailingJobMapper mapper = new ReportMailingJobMapper();
      final String sql =
          "select " + mapper.ReportMailingJobSchema() + " where rmj.id = ? and rmj.is_deleted = 0";
  ...
  public ReportMailingJobData retrieveReportMailingJobEnumOptions() {
    ...
    return ReportMailingJobData.newInstance(
        emailAttachmentFileFormatOptions, stretchyReportParamDateOptions);
  }
  ...
  private static final class ReportMailingJobMapper implements RowMapper<ReportMailingJobData> {
    public String ReportMailingJobSchema() {
      return "rmj.id, rmj.name, rmj.description, rmj.start_datetime as startDateTime,"
          + " rmj.recurrence, rmj.created_date as createdOnDate, cbu.username as"
          + " createdByUsername, cbu.firstname as createdByFirstname, cbu.lastname as"
          + " createdByLastname, rmj.lastmodified_date as updatedOnDate, mbu.username as"
          + " updatedByUsername, mbu.firstname as updatedByFirstname, mbu.lastname as"
          + " updatedByLastname, rmj.email_recipients as emailRecipients, rmj.email_subject"
          + " as emailSubject, rmj.email_message as emailMessage,"
          + " rmj.email_attachment_file_format as emailAttachmentFileFormat,"
          + " rmj.stretchy_report_param_map as stretchyReportParamMap,"
          + " rmj.previous_run_datetime as previousRunDateTime, rmj.next_run_datetime as"
          + " nextRunDateTime, rmj.previous_run_status as previousRunStatus,"
          + " rmj.previous_run_error_log as previousRunErrorLog,"
          + " rmj.previous_run_error_message as previousRunErrorMessage, rmj.number_of_runs"
          + " as numberOfRuns, rmj.is_active as isActive, rmj.run_as_userid as runAsUserId,"
          + " sr.id as reportId, sr.report_name as reportName, sr.report_type as"
          + " reportType, sr.report_subtype as reportSubType, sr.report_category as"
          + " reportCategory, sr.report_sql as reportSql, sr.description as"
          + " reportDescription, sr.core_report as coreReport, sr.use_report as useReport"
          + " from m_report_mailing_job rmj inner join m_appuser cbu on cbu.id ="
          + " rmj.createdby_id left join m_appuser mbu on mbu.id = rmj.lastmodifiedby_id"
          + " left join stretchy_report sr on rmj.stretchy_report_id = sr.id";
    }
    ...
    public ReportMailingJobData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      final ReportMailingJobTimelineData timeline =
          new ReportMailingJobTimelineData(
              createdOnDate,
              createdByUsername,
              createdByFirstname,
              createdByLastname,
      ...
      final ReportData stretchyReport =
          new ReportData(
              reportId,
              reportName,
              reportType,
