...
public class EmailReadPlatformServiceImpl implements EmailReadPlatformService {
  ...
  private static final class EmailMapper implements RowMapper<EmailData> {
    ...
    @Override
    public EmailData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  public Collection<EmailData> retrieveAllPending(final SearchParameters searchParameters) {
    final String sqlPlusLimit =
        (searchParameters.getLimit() > 0) ? " limit 0, " + searchParameters.getLimit() : "";
    final String sql =
        "select "
            + this.emailRowMapper.schema()
            + " where emo.status_enum = "
  ...
  public Collection<EmailData> retrieveAllDelivered(final Integer limit) {
    ...
    final String sql =
        "select "
            + this.emailRowMapper.schema()
            + " where emo.status_enum = "
            + EmailMessageStatusType.DELIVERED.getValue()
            + sqlPlusLimit;
  ...
  public Collection<EmailData> retrieveAllFailed(final SearchParameters searchParameters) {
    ...
    final String sql =
        "select "
            + this.emailRowMapper.schema()
            + " where emo.status_enum = "
            + EmailMessageStatusType.FAILED.getValue()
            + sqlPlusLimit;
  ...
  @Override
  public Page<EmailData> retrieveEmailByStatus(
      final Integer limit, final Integer status, final Date dateFrom, final Date dateTo) {
