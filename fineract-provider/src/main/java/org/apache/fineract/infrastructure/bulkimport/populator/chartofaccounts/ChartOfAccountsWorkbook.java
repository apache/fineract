...
public class ChartOfAccountsWorkbook extends AbstractWorkbookPopulator {
  ...
  public void populate(Workbook workbook, String dateFormat) {
    Sheet chartOfAccountsSheet =
        workbook.createSheet(TemplatePopulateImportConstants.CHART_OF_ACCOUNTS_SHEET_NAME);
  ...
  private void setAccountTypeToAccountNameAndTag() {
    ...
    for (GLAccountData glAccount : glAccounts) {
      addToaccountTypeToAccountNameMap(
          glAccount.getType().getValue(),
          glAccount.getName()
              + "-"
              + glAccount.getId()
              + "-"
              + glAccount.getTagId().getName()
              + "-"
              + glAccount.getTagId().getId());
    }
  }
  ...
  private void setRules(Sheet chartOfAccountsSheet) {
    ...
    DataValidationHelper validationHelper =
        new HSSFDataValidationHelper((HSSFSheet) chartOfAccountsSheet);
    setNames(chartOfAccountsSheet, accountTypesNoDuplicatesList);

    DataValidationConstraint accountTypeConstraint =
    ...
    DataValidationConstraint accountUsageConstraint =
        validationHelper.createExplicitListConstraint(
            new String[] {GLAccountUsage.DETAIL.toString(), GLAccountUsage.HEADER.toString()});
    DataValidationConstraint booleanConstraint =
        validationHelper.createExplicitListConstraint(new String[] {"True", "False"});
    DataValidationConstraint parentConstraint =
        validationHelper.createFormulaListConstraint("INDIRECT(CONCATENATE(\"AccountName_\",$A1))");
    ...
    for (Integer i = 0; i < accountTypesNoDuplicatesList.size(); i++) {
      ...
      if (accountTypeToBeginEndIndexesofAccountNames != null) {
        ...
        tags.setRefersToFormula(
            TemplatePopulateImportConstants.CHART_OF_ACCOUNTS_SHEET_NAME
                + "!$S$"
                + tagValueBeginEndIndexes[0]
                + ":$S$"
      ...
      if (accountNamesBeginEndIndexes != null) {
        ...
        accountNames.setRefersToFormula(
            TemplatePopulateImportConstants.CHART_OF_ACCOUNTS_SHEET_NAME
                + "!$Q$"
                + accountNamesBeginEndIndexes[0]
                + ":$Q$"
  ...
  private void setDefaults(Sheet worksheet) {
    try {
      for (Integer rowNo = 1; rowNo < 3000; rowNo++) {
        ...
        writeFormula(
            ChartOfAcountsConstants.PARENT_ID_COL,
            row,
            "IF(ISERROR(VLOOKUP($E"
                + (rowNo + 1)
                + ",$Q$2:$R$"
                + (glAccounts.size() + 1)
                + ",2,FALSE)),"
                ...
                + ",2,FALSE)))");
        writeFormula(
            ChartOfAcountsConstants.TAG_ID_COL,
            row,
            "IF(ISERROR(VLOOKUP($H"
                + (rowNo + 1)
                + ",$S$2:$T$"
                + (glAccounts.size() + 1)
                + ",2,FALSE)),"
      ...
      if (!accountNamesandTags.isEmpty()) {
        for (String accountNameandTag : accountNamesandTags) {
          if (chartOfAccountsSheet.getRow(rowIndex) != null) {
            ...
            writeString(
                ChartOfAcountsConstants.LOOKUP_ACCOUNT_NAME_COL, row, accountNameAndTagAr[0]);
        ...
        accountTypeToBeginEndIndexesofAccountNames.put(
            accountTypeIndex++, new Integer[] {startIndex, rowIndex});
      } else {
        accountTypeIndex++;
      }
    }
  ...
  private void setLayout(Sheet chartOfAccountsSheet) {
    ...
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.ACCOUNT_TYPE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.ACCOUNT_NAME_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.ACCOUNT_USAGE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.MANUAL_ENTRIES_ALLOWED_COL,
        TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.PARENT_COL, TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.PARENT_ID_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.GL_CODE_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.TAG_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.TAG_ID_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.DESCRIPTION_COL,
        TemplatePopulateImportConstants.EXTRALARGE_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.LOOKUP_ACCOUNT_TYPE_COL,
        TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.LOOKUP_ACCOUNT_NAME_COL,
        TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.LOOKUP_ACCOUNT_ID_COL,
        TemplatePopulateImportConstants.MEDIUM_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.LOOKUP_TAG_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    chartOfAccountsSheet.setColumnWidth(
        ChartOfAcountsConstants.LOOKUP_TAG_ID_COL, TemplatePopulateImportConstants.SMALL_COL_SIZE);
    ...
    writeString(
        ChartOfAcountsConstants.MANUAL_ENTRIES_ALLOWED_COL, rowHeader, "Manual entries allowed *");
    ...
    writeString(
        ChartOfAcountsConstants.LOOKUP_ACCOUNT_NAME_COL, rowHeader, "Lookup Account name *");
