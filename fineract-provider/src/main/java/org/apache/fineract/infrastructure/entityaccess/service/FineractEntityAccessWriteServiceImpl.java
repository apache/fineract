...
public class FineractEntityAccessWriteServiceImpl implements FineractEntityAccessWriteService {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(FineractEntityAccessWriteServiceImpl.class);
  ...
  private final FineractEntityToEntityMappingRepositoryWrapper
      fineractEntityToEntityMappingRepositoryWrapper;
  private final FineractEntityDataValidator fromApiJsonDeserializer;

  @Autowired
  public FineractEntityAccessWriteServiceImpl(
      final FineractEntityAccessRepository entityAccessRepository,
      ...
      final FineractEntityToEntityMappingRepositoryWrapper
          fineractEntityToEntityMappingRepositoryWrapper,
    ...
    this.fineractEntityToEntityMappingRepositoryWrapper =
        fineractEntityToEntityMappingRepositoryWrapper;
  }

  @Override
  public CommandProcessingResult createEntityAccess(
  ...
  @Override
  @Transactional
  public void addNewEntityAccess(
      final String entityType,
      final Long entityId,
      final CodeValue accessType,
      final String secondEntityType,
      final Long secondEntityId) {
    FineractEntityAccess entityAccess =
        FineractEntityAccess.createNew(
  ...
  public CommandProcessingResult createEntityToEntityMapping(Long relId, JsonCommand command) {
    ...
    try {
      ...
      final FineractEntityRelation mapId =
          this.fineractEntityRelationRepositoryWrapper.findOneWithNotFoundDetection(relId);
      ...
      final FineractEntityToEntityMapping newMap =
          FineractEntityToEntityMapping.newMap(mapId, fromId, toId, startDate, endDate);
  ...
  public CommandProcessingResult updateEntityToEntityMapping(Long mapId, JsonCommand command) {
    ...
    try {
      ...
      final FineractEntityToEntityMapping mapForUpdate =
          this.fineractEntityToEntityMappingRepositoryWrapper.findOneWithNotFoundDetection(mapId);
  ...
  public CommandProcessingResult deleteEntityToEntityMapping(Long mapId) {
    ...
    final FineractEntityToEntityMapping deleteMap =
        this.fineractEntityToEntityMappingRepositoryWrapper.findOneWithNotFoundDetection(mapId);
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("rel_id_from_id_to_id")) {
      final String fromId =
          command.stringValueOfParameterNamed(FineractEntityApiResourceConstants.fromEnityType);
      final String toId =
          command.stringValueOfParameterNamed(FineractEntityApiResourceConstants.toEntityType);
      throw new PlatformDataIntegrityException(
          "error.msg.duplicate.entity.mapping",
          "EntityMapping from " + fromId + " to " + toId + " already exist");
    }

    logAsErrorUnexpectedDataIntegrityException(dve);
    throw new PlatformDataIntegrityException(
        "error.msg.entity.mapping", "Unknown data integrity issue with resource.");
  }
