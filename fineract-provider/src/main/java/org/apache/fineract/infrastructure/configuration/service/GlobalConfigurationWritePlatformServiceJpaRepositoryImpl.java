...
public class GlobalConfigurationWritePlatformServiceJpaRepositoryImpl
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(GlobalConfigurationWritePlatformServiceJpaRepositoryImpl.class);
  ...
  public GlobalConfigurationWritePlatformServiceJpaRepositoryImpl(
      ...
      final GlobalConfigurationRepositoryWrapper codeRepository,
  ...
  public CommandProcessingResult update(final Long configId, final JsonCommand command) {
    ...
    try {
      ...
      final GlobalConfigurationProperty configItemForUpdate =
          this.repository.findOneWithNotFoundDetection(configId);

      final Map<String, Object> changes = configItemForUpdate.update(command);

      if (!changes.isEmpty()) {
        this.configurationDomainService.removeGlobalConfigurationPropertyDataFromCache(
            configItemForUpdate.getName());
  ...
  public void addSurveyConfig(final String name) {
    try {
      final GlobalConfigurationProperty ppi =
          GlobalConfigurationProperty.newSurveyConfiguration(name);
  ...
  private void handleDataIntegrityIssues(final DataIntegrityViolationException dve) {
    ...
    throw new PlatformDataIntegrityException(
        "error.msg.globalConfiguration.unknown.data.integrity.issue",
