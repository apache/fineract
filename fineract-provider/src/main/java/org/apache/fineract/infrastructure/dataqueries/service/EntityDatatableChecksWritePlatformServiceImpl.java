...
public class EntityDatatableChecksWritePlatformServiceImpl
  ...
  public CommandProcessingResult createCheck(final JsonCommand command) {
    ...
    try {
      ...
      logger.info(
          "{} has column {} ? {}",
          new Object[] {datatableData.getRegisteredTableName(), foreignKeyColumnName, columnExist});
      ...
      if (productId == null) {
        entityDatatableCheck =
            this.entityDatatableChecksRepository.findByEntityStatusAndDatatableIdAndNoProduct(
        ...
        entityDatatableCheck =
            this.entityDatatableChecksRepository.findByEntityStatusAndDatatableIdAndProductId(
                entity, status, datatableName, productId);
        if (!entityDatatableCheck.isEmpty()) {
          throw new EntityDatatableCheckAlreadyExistsException(
              entity, status, datatableName, productId);
        }
      }
    ...
    if (tableRequiredBeforeClientActivation != null) {
      ...
      for (EntityDatatableChecks t : tableRequiredBeforeClientActivation) {
        ...
        final Long countEntries =
            readWriteNonCoreDataService.countDatatableEntries(
  ...
  @Override
  public void runTheCheckForProduct(
      final Long entityId,
      final String entityName,
      final Long statusCode,
      String foreignKeyColumn,
    ...
    List<EntityDatatableChecks> tableRequiredBeforAction =
        entityDatatableChecksRepository.findByEntityStatusAndProduct(
            entityName, statusCode, productId);

    if (tableRequiredBeforAction == null || tableRequiredBeforAction.size() < 1) {
      tableRequiredBeforAction =
          entityDatatableChecksRepository.findByEntityStatusAndNoProduct(entityName, statusCode);
    }
      ...
      for (JsonElement element : datatableDatas) {
        ...
        if (datatableName == null || datatableData == null) {
          final ApiParameterError error =
              ApiParameterError.generalError(
                  ...
                  "registeredTableName and data parameters must be present in each list items in"
                      + " datatables");
        ...
        try {
          this.readWriteNonCoreDataService.createNewDatatableEntry(
              datatableName, entityId, datatableData.toString());
  ...
  public CommandProcessingResult deleteCheck(final Long entityDatatableCheckId) {
    ...
    final EntityDatatableChecks check =
        this.entityDatatableChecksRepository
            .findById(entityDatatableCheckId)
  ...
  private void handleReportDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("FOREIGN KEY (`x_registered_table_name`)")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.entityDatatableCheck.foreign.key.constraint",
          "datatable with name '" + datatableName + "' do not exist",
          "datatableName",
          datatableName);
    }
