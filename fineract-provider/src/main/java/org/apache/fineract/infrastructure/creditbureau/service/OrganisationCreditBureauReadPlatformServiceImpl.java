...
public class OrganisationCreditBureauReadPlatformServiceImpl
  ...
  @Autowired
  public OrganisationCreditBureauReadPlatformServiceImpl(
      final PlatformSecurityContext context, final RoutingDataSource dataSource) {
    this.context = context;
    this.jdbcTemplate = new JdbcTemplate(dataSource);
  }

  private static final class OrganisationCreditBureauMapper
      implements RowMapper<OrganisationCreditBureauData> {
    public String schema() {
      return "ocb.id as orgCbId,ocb.alias as orgCbAlias,cb.name as creditbureauName,cb.product as"
          + " creditbureauProduct,cb.country as creditbureauCountry,concat(cb.product,' -"
          + " ',cb.name,' - ',cb.country) as CreditBureauSummary,ocb.creditbureau_id as"
          + " cbid,ocb.is_active as is_active from m_organisation_creditbureau"
          + " ocb,m_creditbureau cb where ocb.creditbureau_id=cb.id";
    }

    @Override
    public OrganisationCreditBureauData mapRow(
      ...
      return OrganisationCreditBureauData.instance(
          orgCbId,
          orgCbAlias,
          cbid,
          creditbureauName,
