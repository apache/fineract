...
public class SmsCampaignWritePlatformServiceJpaImpl implements SmsCampaignWritePlatformService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(SmsCampaignWritePlatformServiceJpaImpl.class);
  ...
  public CommandProcessingResult create(JsonCommand command) {
    ...
    Report report =
        this.reportRepository
            .findById(runReportId)
            .orElseThrow(() -> new ReportNotFoundException(runReportId));
    ...
    if (smsCampaign.getRecurrenceStartDate() != null
      ...
      throw new GeneralPlatformDomainRuleException(
          "error.msg.campaign.recurrenceStartDate.in.the.past",
    ...
    try {
      ...
      if (runReportObject != null) {
        for (HashMap<String, Object> entry : runReportObject) {
          String textMessage =
              this.compileSmsTemplate(
                  smsCampaign.getMessage(), smsCampaign.getCampaignName(), entry);
  ...
  public void insertDirectCampaignIntoSmsOutboundTable(
      final Loan loan, final SmsCampaign smsCampaign) {
    try {
      ...
      if (loan.isGroupLoan()) {
        ...
        Client client =
            this.clientRepositoryWrapper.findOneWithNotFoundDetection(loan.getClientId());
      ...
      for (Client client : clientSet) {
        ...
        List<HashMap<String, Object>> runReportObject =
            this.getRunReportByServiceImpl(
  ...
  private void updateTriggerDates(Long campaignId) {
    ...
    LocalDate nextRuntime =
        CalendarUtils.getNextRecurringDate(
            smsCampaign.getRecurrence(),
            smsCampaign.getNextTriggerDate().toLocalDate(),
  ...
  @Override
  public String compileSmsTemplate(
      final String textMessageTemplate,
      final String campaignName,
      final Map<String, Object> smsParams) {
  ...
  public CampaignPreviewData previewMessage(final JsonQuery query) {
    ...
    final JsonElement smsParamsElement =
        this.fromJsonHelper.extractJsonObjectNamed(
            SmsCampaignValidator.paramValue, query.parsedJson());
    String smsParams = smsParamsElement.toString();
    final String textMessageTemplate =
        this.fromJsonHelper.extractStringNamed("message", query.parsedJson());

    try {
      HashMap<String, String> campaignParams =
          new ObjectMapper().readValue(smsParams, new TypeReference<HashMap<String, String>>() {});
  ...
  public void storeTemplateMessageIntoSmsOutBoundTable() throws JobExecutionException {
    final Collection<SmsCampaign> smsCampaignDataCollection =
        this.smsCampaignRepository.findByTriggerTypeAndStatus(
  ...
  //
  //        catch (Exception e) {
  //            logger.error("Invalid phone number or country calling code, must contain only
  // numbers", e);
  //        }
  //
