...
public class CodeValueReadPlatformServiceImpl implements CodeValueReadPlatformService {
  ...
  @Autowired
  public CodeValueReadPlatformServiceImpl(
  ...
  private static final class CodeValueDataMapper implements RowMapper<CodeValueData> {
    ...
    public String schema() {
      return " cv.id as id, cv.code_value as value, cv.code_id as codeId, cv.code_description as"
          + " description, cv.order_position as position, cv.is_active isActive,"
          + " cv.is_mandatory as mandatory from m_code_value as cv join m_code c on"
          + " cv.code_id = c.id ";
    }
  ...
  public Collection<CodeValueData> retrieveCodeValuesByCode(final String code) {
    ...
    final String sql =
        "select " + rm.schema() + "where c.code_name like ? and cv.is_active = 1 order by position";
