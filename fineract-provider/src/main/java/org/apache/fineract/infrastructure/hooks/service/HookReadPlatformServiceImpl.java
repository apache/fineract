...
public class HookReadPlatformServiceImpl implements HookReadPlatformService {
  ...
  @Cacheable(
      value = "hooks",
      key =
          "T(org.apache.fineract.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('HK')")
  public List<Hook> retrieveHooksByEvent(final String actionName, final String entityName) {

    return this.hookRepository.findAllHooksListeningToEvent(actionName, entityName);
  }
  ...
  private List<Grouping> getTemplateForEvents() {
    final String sql =
        "select p.grouping, p.entity_name, p.action_name from m_permission p "
  ...
  private static final class HookMapper implements RowMapper<HookData> {
    ...
    @Override
    public HookData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  private static final class HookEventMapper implements RowMapper<Event> {
    ...
    public String schema() {
      return " re.action_name, re.entity_name from m_hook h inner join m_hook_registered_events re"
          + " on h.id = re.hook_id ";
    }

    @Override
    public Event mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  private static final class HookConfigMapper implements RowMapper<Field> {
    ...
    public String schema() {
      return " hc.field_name, hc.field_value from m_hook h inner join m_hook_configuration hc on"
          + " h.id = hc.hook_id ";
    }

    @Override
    public Field mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  private static final class TemplateMapper implements RowMapper<HookTemplateData> {
    ...
    @Override
    public HookTemplateData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
