    ...
    private static final class JobDetailMapper implements RowMapper<JobDetailData> {
        ...
        JobDetailMapper(DatabaseSpecificSQLGenerator sqlGenerator) {
            sqlBuilder = new StringBuilder("select").append(
                    ...
                    .append(" (SELECT max(jrh.`version`) FROM job_run_history jrh WHERE jrh.job_id = job.id) AS version,runHistory.start_time as lastRunStartTime,runHistory.end_time as lastRunEndTime,runHistory."
                    ...
                    .append(" from job job  left join job_run_history runHistory ON job.id=runHistory.job_id and job.previous_run_start_time=runHistory.start_time AND runHistory.trigger_type = 'cron' ");
        }
