...
public class SchedulerJobRunnerReadServiceImpl implements SchedulerJobRunnerReadService {
  ...
  @Autowired
  public SchedulerJobRunnerReadServiceImpl(
  ...
  public List<JobDetailData> findAllJobDeatils() {
    ...
    final List<JobDetailData> JobDeatils =
        this.jdbcTemplate.query(sql, detailMapper, new Object[] {});
    return JobDeatils;
  }
  ...
  @Override
  public Page<JobDetailHistoryData> retrieveJobHistory(
      final Long jobId, final SearchParameters searchParameters) {
    if (!isJobExist(jobId)) {
      throw new JobNotFoundException(String.valueOf(jobId));
    }
  ...
  public boolean isUpdatesAllowed() {
    final String sql =
        "select job.display_name from job job where job.currently_running=true and"
            + " job.updates_allowed=false";
  ...
  private static final class JobDetailMapper implements RowMapper<JobDetailData> {
    ...
    private final StringBuilder sqlBuilder =
        new StringBuilder("select")
            .append(
                " job.id,job.display_name as displayName,job.next_run_time as"
                    + " nextRunTime,job.initializing_errorlog as"
                    + " initializingError,job.cron_expression as cronExpression,job.is_active as"
                    + " active,job.currently_running as currentlyRunning,")
            .append(
                " runHistory.version,runHistory.start_time as lastRunStartTime,runHistory.end_time"
                    + " as lastRunEndTime,runHistory.`status`,runHistory.error_message as"
                    + " jobRunErrorMessage,runHistory.trigger_type as"
                    + " triggerType,runHistory.error_log as jobRunErrorLog ")
            .append(
                " from job job  left join job_run_history runHistory ON job.id=runHistory.job_id"
                    + " and job.previous_run_start_time=runHistory.start_time ");
  ...
  private static final class JobHistoryMapper implements RowMapper<JobDetailHistoryData> {
    ...
    private final StringBuilder sqlBuilder =
        new StringBuilder(200)
            .append(
                " runHistory.version,runHistory.start_time as runStartTime,runHistory.end_time as"
                    + " runEndTime,runHistory.`status`,runHistory.error_message as"
                    + " jobRunErrorMessage,runHistory.trigger_type as"
                    + " triggerType,runHistory.error_log as jobRunErrorLog ")
    ...
    @Override
    public JobDetailHistoryData mapRow(
      ...
      final String triggerType = rs.getString("triggerType");
      final String jobRunErrorLog = rs.getString("jobRunErrorLog");
      final JobDetailHistoryData jobDetailHistory =
          new JobDetailHistoryData(
              version,
              jobRunStartTime,
              jobRunEndTime,
