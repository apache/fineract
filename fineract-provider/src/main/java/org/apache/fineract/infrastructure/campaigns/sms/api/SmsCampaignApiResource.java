...
public class SmsCampaignApiResource {
  ...
  public SmsCampaignApiResource(
      ...
      final SmsCampaignReadPlatformService smsCampaignReadPlatformService,
  ...
  public String template(@Context final UriInfo uriInfo) {
    this.platformSecurityContext
        .authenticatedUser()
        .validateHasReadPermission(SmsCampaignConstants.RESOURCE_NAME);
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = CommandWrapper.class)
  })
  ...
  public String createCampaign(@ApiParam(hidden = true) final String apiRequestBodyAsJson) {
    ...
    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().createSmsCampaign().withJson(apiRequestBodyAsJson).build();
  ...
  @ApiResponse(code = 200, message = "", response = SmsCampaignData.class)
  public String retrieveCampaign(
  ...
  @ApiResponse(code = 200, message = "", response = SmsCampaignData.class)
  public String retrieveAllEmails(
    ...
    Page<SmsCampaignData> smsCampaignDataCollection =
        this.smsCampaignReadPlatformService.retrieveAll(searchParameters);
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
  ...
  public String preview(final String apiRequestBodyAsJson, @Context final UriInfo uriInfo) {
    ...
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.previewCampaignMessageDefaultToApiJsonSerializer.serialize(
        settings, campaignMessage, new HashSet<String>());
  }

  @DELETE
  @Path("{campaignId}")
  @ApiOperation(
      value = "Delete a SMS Campaign",
      notes = "Note: Only closed SMS Campaigns can be deleted")
  @ApiResponse(code = 200, message = "", response = CommandProcessingResult.class)
  public String delete(@PathParam("campaignId") final Long campaignId) {
    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().deleteSmsCampaign(campaignId).build();
    final CommandProcessingResult result =
        this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    return this.toApiJsonSerializer.serialize(result);
  }
