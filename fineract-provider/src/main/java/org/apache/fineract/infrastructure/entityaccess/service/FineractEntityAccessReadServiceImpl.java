    ...
    public String getSQLQueryInClause_WithListOfIDsForEntityAccess( FineractEntityType firstEntityType,
        ...
        if ((accesslist != null) && (accesslist.size() > 0)) {
          ...
          /*for (int i = 0; i < accesslist.size(); i++) {
            	FineractEntityToEntityMappingData accessData = (FineractEntityToEntityMappingData) accesslist.toArray()[i];
                if (accessData == null) {  
                	throw new FineractEntityMappingConfigurationException();
                }
                if (accessData.getToId() == 0) { // If there is
                                                                 // any ID that
                                                                 // zero, then
                                                                 // allow access
                                                                 // to all
