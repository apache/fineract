...
public class S3ContentRepository implements ContentRepository {
        ...
        return new FileData(new ByteSource() {
            ...
            public InputStream openStream() throws IOException {
                return s3Client.getObject(
                        GetObjectRequest.builder().bucket(fineractProperties.getContent().getS3().getBucketName()).key(documentData.fileLocation())
                                .build(), ResponseTransformer.toBytes()).asInputStream();
            }
    ...
    public FileData fetchImage(final ImageData imageData) {
        return new FileData(new ByteSource() {
            ...
            public InputStream openStream() throws IOException {
                return s3Client.getObject(
                        GetObjectRequest.builder().bucket(fineractProperties.getContent().getS3().getBucketName()).key(imageData.location())
                                .build(), ResponseTransformer.toBytes()).asInputStream();
            }
    ...
    private void deleteObject(final String location) {
        try {
            ...
            throw new ContentManagementException(location, "message=" + ase.getMessage() + ", Error Code=" + ase.awsErrorDetails().errorCode(), ase);
    ...
    public void putObject(final String filename, final InputStream inputStream, final String s3UploadLocation)
        ...
        try {
            ...
            this.s3Client.putObject(builder -> builder.bucket(fineractProperties.getContent().getS3().getBucketName()).key(s3UploadLocation),
    ...
    public ResponseBytes<GetObjectResponse> getObject(String key) {
        try {
            ...
            return this.s3Client.getObject(builder -> builder.bucket(fineractProperties.getContent().getS3().getBucketName()).key(key), ResponseTransformer.toBytes());
