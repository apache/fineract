...
public class AccountNumberFormatWritePlatformServiceJpaRepositoryImpl
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(AccountNumberFormatWritePlatformServiceJpaRepositoryImpl.class);
  ...
  public CommandProcessingResult createAccountNumberFormat(JsonCommand command) {
    try {
      ...
      AccountNumberFormat accountNumberFormat =
          new AccountNumberFormat(entityAccountType, accountNumberPrefixType);
  ...
  @Override
  @Transactional
  public CommandProcessingResult updateAccountNumberFormat(
      Long accountNumberFormatId, JsonCommand command) {
    try {

      final AccountNumberFormat accountNumberFormatForUpdate =
          this.accountNumberFormatRepository.findOneWithNotFoundDetection(accountNumberFormatId);
      ...
      if (command.isChangeInIntegerSansLocaleParameterNamed(
        ...
        final Integer newValue =
            command.integerValueSansLocaleOfParameterNamed(
                AccountNumberFormatConstants.prefixTypeParamName);
        final AccountNumberPrefixType accountNumberPrefixType =
            AccountNumberPrefixType.fromInt(newValue);
        actualChanges.put(
            AccountNumberFormatConstants.prefixTypeParamName, accountNumberPrefixType);
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause
        .getMessage()
        .contains(AccountNumberFormatConstants.ACCOUNT_TYPE_UNIQUE_CONSTRAINT_NAME)) {
      ...
      throw new PlatformDataIntegrityException(
          AccountNumberFormatConstants.EXCEPTION_DUPLICATE_ACCOUNT_TYPE,
          "Account Format preferences for Account type `"
              + entityAccountType.getCode()
              + "` already exists",
          "externalId",
          entityAccountType.getValue(),
          entityAccountType.getCode());
    }
    logger.error("Error occured.", dve);
    throw new PlatformDataIntegrityException(
        "error.msg.account.number.format.unknown.data.integrity.issue",
