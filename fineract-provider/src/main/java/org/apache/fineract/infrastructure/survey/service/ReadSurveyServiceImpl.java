...
public class ReadSurveyServiceImpl implements ReadSurveyService {
  ...
  private String retrieveAllSurveySQL(String andClause) {
    ...
    return "select application_table_name, cf.enabled, registered_table_name"
        ...
        + " and (p.code in ('ALL_FUNCTIONS', 'ALL_FUNCTIONS_READ') or p.code = concat('READ_',"
        + " registered_table_name)))  and x_registered_table.category = "
  ...
  public SurveyDataTableData retrieveSurvey(String surveyName) {
    ...
    final String sql =
        "select cf.enabled, application_table_name, registered_table_name"
            + " from x_registered_table "
            + " left join c_configuration cf on x_registered_table.registered_table_name = cf.name "
            + " where exists"
            + " (select 'f'"
            ...
            + surveyName
            + "' and (p.code in ('ALL_FUNCTIONS', 'ALL_FUNCTIONS_READ') or p.code ="
            + " concat('READ_', registered_table_name)))  order by application_table_name,"
  ...
  @Override
  public List<ClientScoresOverview> retrieveClientSurveyScoreOverview(
      String surveyName, Long clientId) {

    final String sql =
        "SELECT  tz.id, lkh.name, lkh.code, poverty_line, tz.date, tz.score FROM ? tz JOIN"
            + " ppi_likelihoods_ppi lkp on lkp.ppi_name = ? AND enabled = ?  JOIN ppi_scores sc on"
            + " score_from  <= tz.score AND score_to >=tz.score JOIN ppi_poverty_line pvl on"
            + " pvl.likelihood_ppi_id = lkp.id AND pvl.score_id = sc.id JOIN ppi_likelihoods lkh"
            + " on lkh.id = lkp.likelihood_id  WHERE  client_id = ? ";

    final SqlRowSet rs =
        this.jdbcTemplate.queryForRowSet(
            sql, new Object[] {surveyName, surveyName, LikelihoodStatus.ENABLED, clientId});

    List<ClientScoresOverview> scoresOverviews = new ArrayList<>();

    while (rs.next()) {
      scoresOverviews.add(
          new ClientScoresOverview(
              rs.getString("code"),
              rs.getString("name"),
              rs.getLong("score"),
  ...
  public List<ClientScoresOverview> retrieveClientSurveyScoreOverview(Long clientId) {
    ...
    while (surveyNames.next()) {
      sqls.add(
          "SELECT '"
              + surveyNames.getString("name")
              + "' as surveyName, tz.id, lkh.name, lkh.code, poverty_line, tz.date, tz.score FROM "
              + surveyNames.getString("name")
              + " tz"
              + " JOIN ppi_likelihoods_ppi lkp on lkp.ppi_name = '"
              + surveyNames.getString("name")
              + "' AND enabled = '"
              + LikelihoodStatus.ENABLED
              + "' JOIN ppi_scores sc on score_from  <= tz.score AND score_to >=tz.score JOIN"
              + " ppi_poverty_line pvl on pvl.likelihood_ppi_id = lkp.id AND pvl.score_id = sc.id"
  ...
  public GenericResultsetData retrieveSurveyEntry(String surveyName, Long clientId, Long entryId) {
    ...
    return readWriteNonCoreDataService.retrieveDataTableGenericResultSet(
        surveyName, clientId, null, entryId);
  }
}
