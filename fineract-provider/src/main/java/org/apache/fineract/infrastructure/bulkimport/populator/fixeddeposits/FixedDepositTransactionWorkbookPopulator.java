...
public class FixedDepositTransactionWorkbookPopulator extends AbstractWorkbookPopulator {
  ...
  public FixedDepositTransactionWorkbookPopulator(
  ...
  private void setDefaults(Sheet worksheet) {
    for (Integer rowNo = 1; rowNo < 3000; rowNo++) {
      ...
      writeFormula(
          TransactionConstants.PRODUCT_COL,
          row,
  ...
  private void setNames(Sheet worksheet) {
    ...
    for (int j = 0; j < clientsWithActiveSavings.size(); j++) {
      ...
      name.setNameName(
          "Account_"
              + clientsWithActiveSavings.get(j).replace(" ", "_")
              + "_"
              + clientIdsWithActiveSavings.get(j)
              + "_");
      name.setRefersToFormula(
          TemplatePopulateImportConstants.FIXED_DEPOSIT_TRANSACTION_SHEET_NAME
              + "!$Q$"
  ...
  private void populateSavingsTable(Sheet savingsTransactionSheet, String dateFormat) {
    ...
    for (SavingsAccountData savingsAccount : savingsAccounts) {
      ...
      writeString(
          TransactionConstants.LOOKUP_CLIENT_NAME_COL,
          row,
      ...
      writeString(
          TransactionConstants.LOOKUP_PRODUCT_COL, row, savingsAccount.getSavingsProductName());
      if (savingsAccount.getMinRequiredOpeningBalance() != null)
        writeBigDecimal(
            TransactionConstants.LOOKUP_OPENING_BALANCE_COL,
            row,
            savingsAccount.getMinRequiredOpeningBalance());
      writeDate(
          TransactionConstants.LOOKUP_SAVINGS_ACTIVATION_DATE_COL,
          row,
          ""
  ...
  private void setLayout(Sheet worksheet) {
    ...
    writeString(
        TransactionConstants.LOOKUP_OPENING_BALANCE_COL, rowHeader, "Lookup Opening Balance");
    writeString(
        TransactionConstants.LOOKUP_SAVINGS_ACTIVATION_DATE_COL,
        rowHeader,
        "Lookup Savings Activation Date");
  }
}
