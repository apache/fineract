...
public class ConfigurationReadPlatformServiceImpl implements ConfigurationReadPlatformService {
  ...
  @Autowired
  public ConfigurationReadPlatformServiceImpl(
  ...
  public GlobalConfigurationData retrieveGlobalConfiguration(final boolean survey) {
    ...
    String sql =
        "SELECT c.id, c.name, c.enabled, c.value, c.date_value, c.description, c.is_trap_door FROM"
            + " c_configuration c ";
    ...
    sql += "  order by c.id";
    final List<GlobalConfigurationPropertyData> globalConfiguration =
        this.jdbcTemplate.query(sql, this.rm, new Object[] {});
  ...
  public GlobalConfigurationPropertyData retrieveGlobalConfiguration(Long configId) {
    ...
    final GlobalConfigurationPropertyData globalConfiguration =
        this.jdbcTemplate.queryForObject(sql, this.rm, new Object[] {configId});

    return globalConfiguration;
  }

  private static final class GlobalConfigurationRowMapper
      implements RowMapper<GlobalConfigurationPropertyData> {

    @Override
    public GlobalConfigurationPropertyData mapRow(
      ...
      return new GlobalConfigurationPropertyData(
          name, enabled, value, dateValue, id, description, isTrapDoor);
    }
  }
}
