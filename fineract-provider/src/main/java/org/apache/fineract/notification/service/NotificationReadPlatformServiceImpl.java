...
public class NotificationReadPlatformServiceImpl implements NotificationReadPlatformService {
  ...
  private boolean initializeTenantNotificationResponseHeaderCache(
  ...
  private boolean createUpdateCacheValue(
      Long appUserId,
      Long now,
      HashMap<Long, CacheNotificationResponseHeader> notificationResponseHeaderCache) {
  ...
  private boolean checkForUnreadNotifications(Long appUserId) {
    String sql =
        "SELECT id, notification_id as notificationId, user_id as userId, is_read as isRead,"
            + " created_at as createdAt FROM notification_mapper WHERE user_id = ? AND is_read ="
            + " false";
  ...
  public void updateNotificationReadStatus() {
    ...
    String sql =
        "UPDATE notification_mapper SET is_read = true WHERE is_read = false and user_id = ?";
  ...
  public Page<NotificationData> getAllUnreadNotifications(final SearchParameters searchParameters) {
    final Long appUserId = context.authenticatedUser().getId();
    String sql =
        "SELECT SQL_CALC_FOUND_ROWS ng.id as id, nm.user_id as userId, ng.object_type as"
            + " objectType, ng.object_identifier as objectId, ng.actor as actor, ng.action action,"
            + " ng.notification_content as content, ng.is_system_generated as isSystemGenerated,"
            + " nm.created_at as createdAt FROM notification_mapper nm INNER JOIN"
            + " notification_generator ng ON nm.notification_id = ng.id WHERE nm.user_id = ? AND"
            + " nm.is_read = false order by nm.created_at desc";
  ...
  public Page<NotificationData> getAllNotifications(SearchParameters searchParameters) {
    ...
    String sql =
        "SELECT SQL_CALC_FOUND_ROWS ng.id as id, nm.user_id as userId, ng.object_type as"
            ...
            + " ng.notification_content as content, ng.is_system_generated as isSystemGenerated,"
            + " nm.created_at as createdAt FROM notification_mapper nm INNER JOIN"
            + " notification_generator ng ON nm.notification_id = ng.id WHERE nm.user_id = ? order"
            + " by nm.created_at desc";

    return getNotificationDataPage(searchParameters, appUserId, sql);
  }

  private Page<NotificationData> getNotificationDataPage(
      SearchParameters searchParameters, Long appUserId, String sql) {
    ...
    return this.paginationHelper.fetchPage(
        this.jdbcTemplate, sqlCountRows, sqlBuilder.toString(), params, this.notificationDataRow);
  }
