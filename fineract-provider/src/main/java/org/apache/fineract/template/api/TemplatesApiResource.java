      ...
      @Tag(
          ...
          description =
              "User Generated Documents(alternatively, Templates) are used for end-user features"
                  ...
                  + " {{ moustache }} templates. Think of them as a sort of built-in \"mail"
                  + " merge\" functionality.\n"
                  + "\n"
                  + "User Generated Documents (and other types of templates) can aggregate data"
                  + " from several Apache Fineract back-end API calls via mappers. Mappers can"
                  + " even access non-Apache Fineract REST services from other servers. UGDs can"
                  + " render such data in tables, show images, etc. TBD: Please have a look at"
                  + " some of the Example UGDs included in Apache Fineract (or the Wiki page, for"
                  + " now.).\n"
                  + "\n"
                  + "UGDs can be assigned to an entity like client or loan and be of a type like"
                  + " Document or SMS. The entity and type of a UGD is only there for the"
                  + " convenience of user agents (UIs), in order to know where to show UGDs for"
                  + " the user (i.e. which tab). The Template Engine back-end runner does not"
                  + " actually need this metadata.")
    })
public class TemplatesApiResource {
  ...
  private final Set<String> RESPONSE_TEMPLATE_DATA_PARAMETERS =
      new HashSet<>(Arrays.asList("id", "entities", "types", "template"));
  ...
  public TemplatesApiResource(
      ...
      final DefaultToApiJsonSerializer<Template> toApiJsonSerializer,
      final DefaultToApiJsonSerializer<TemplateData> templateDataApiJsonSerializer,
      final ApiRequestParameterHelper apiRequestParameterHelper,
      final TemplateDomainService templateService,
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.GetTemplatesResponse.class)
  })
  public String retrieveAll(
      @DefaultValue("-1") @QueryParam("typeId") @ApiParam(value = "typeId") final int typeId,
      @DefaultValue("-1") @QueryParam("entityId") @ApiParam(value = "entityId") final int entityId,
  ...
  @Path("template")
  @ApiOperation(
      value = "Retrieve UGD Details Template",
      notes =
          "This is a convenience resource. It can be useful when building maintenance user"
              + " interface screens for UGDs. The UGD data returned consists of any or all of:\n"
              + "\n"
              + "ARGUMENTS\n"
              + "name String entity String type String text String optional mappers Mapper"
              + " optional\n"
              + "Example Request:\n"
              + "\n"
              + "templates/template")
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.GetTemplatesTemplateResponse.class)
  })
  public String template(@Context final UriInfo uriInfo) {
    ...
    return this.templateDataApiJsonSerializer.serialize(
        settings, templateData, this.RESPONSE_TEMPLATES_DATA_PARAMETERS);
  }
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = TemplatesApiResourcesSwagger.PostTemplatesRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.PostTemplatesResponse.class)
  })
  public String createTemplate(@ApiParam(hidden = true) final String apiRequestBodyAsJson) {
    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().createTemplate().withJson(apiRequestBodyAsJson).build();
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.GetTemplatesTemplateIdResponse.class)
  })
  public String retrieveOne(
      @PathParam("templateId") @ApiParam(value = "templateId") final Long templateId,
      @Context final UriInfo uriInfo) {
    ...
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, template, this.RESPONSE_TEMPLATES_DATA_PARAMETERS);
  }

  @GET
  @Path("{templateId}/template")
  public String getTemplateByTemplate(
      @PathParam("templateId") final Long templateId, @Context final UriInfo uriInfo) {

    this.context.authenticatedUser().validateHasReadPermission(this.RESOURCE_NAME_FOR_PERMISSION);

    final TemplateData template =
        TemplateData.template(this.templateService.findOneById(templateId));

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.templateDataApiJsonSerializer.serialize(
        settings, template, this.RESPONSE_TEMPLATE_DATA_PARAMETERS);
  }
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = TemplatesApiResourcesSwagger.PutTemplatesTemplateIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.PutTemplatesTemplateIdResponse.class)
  })
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = TemplatesApiResourcesSwagger.DeleteTemplatesTemplateIdResponse.class)
  })
