...
@Slf4j
public class SavingsLockingServiceImpl extends AbstractLockingService {

    private static final String TABLE_NAME = "m_savings_account_locks";
    private static final String ID_COLUMN = "savings_id";
    ...
    private static final String BATCH_SAVINGS_LOCK_UPGRADE = """
                UPDATE m_savings_account_locks SET version = version + 1, lock_owner = ?, lock_placed_on = ? WHERE savings_id = ?
            """;

    public SavingsLockingServiceImpl(JdbcTemplate jdbcTemplate, DatabaseSpecificSQLGenerator sqlGenerator,
    ...
    protected String getBatchLoanLockInsert() {
        return BATCH_SAVINGS_LOCK_INSERT;
    }

    @Override
    protected String getBatchLoanLockUpgrade() {
        return BATCH_SAVINGS_LOCK_UPGRADE;
    }
}
