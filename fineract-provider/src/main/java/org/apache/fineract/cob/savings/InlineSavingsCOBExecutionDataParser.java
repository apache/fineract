...
public class InlineSavingsCOBExecutionDataParser {

    private final FromJsonHelper jsonHelper;

    public List<Long> parseExecution(JsonCommand command) {
        JsonObject element = extractJsonObject(command);
        String[] savingsArray = jsonHelper.extractArrayNamed("savingsIds", element);
        return Arrays.stream(savingsArray).map(Long::parseLong).toList();
    }
