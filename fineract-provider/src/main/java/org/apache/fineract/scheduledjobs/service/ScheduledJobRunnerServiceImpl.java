...
public class ScheduledJobRunnerServiceImpl implements ScheduledJobRunnerService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ScheduledJobRunnerServiceImpl.class);
  ...
  public void updateLoanSummaryDetails() {
    ...
    updateSqlBuilder.append(
        "SUM(IFNULL(mr.fee_charges_amount,0)) + IFNULL((select SUM(lc.amount) from  m_loan_charge"
            + " lc where lc.loan_id=ml.id and lc.is_active=1 and lc.charge_time_enum=1),0) as"
            + " fee_charges_charged_derived,");
    updateSqlBuilder.append(
        "SUM(IFNULL(mr.fee_charges_completed_derived,0)) + IFNULL((select"
            + " SUM(lc.amount_paid_derived) from  m_loan_charge lc where lc.loan_id=ml.id and"
            + " lc.is_active=1 and lc.charge_time_enum=1),0) as fee_charges_repaid_derived,");
    updateSqlBuilder.append(
    ...
    updateSqlBuilder.append(
        "SUM(IFNULL(mr.penalty_charges_writtenoff_derived,0)) as"
    ...
    updateSqlBuilder.append(
        "m_loan.penalty_charges_outstanding_derived = (x.penalty_charges_charged_derived -"
            + " (x.penalty_charges_repaid_derived + x.penalty_charges_waived_derived +"
            + " x.penalty_charges_writtenoff_derived)),");
    updateSqlBuilder.append(
        "m_loan.total_expected_repayment_derived = (x.principal_disbursed_derived +"
    ...
    updateSqlBuilder.append(
        "m_loan.total_costofloan_derived = (x.interest_repaid_derived +"
            + " x.fee_charges_repaid_derived + x.penalty_charges_repaid_derived),");
    updateSqlBuilder.append(
        "m_loan.total_waived_derived = (x.interest_waived_derived + x.fee_charges_waived_derived +"
    ...
    updateSqlBuilder.append(
        " (x.interest_charged_derived - (x.interest_repaid_derived + x.interest_waived_derived +"
            + " x.interest_writtenoff_derived)) +");
    updateSqlBuilder.append(
  ...
  public void updateLoanPaidInAdvance() {
    ...
    updateSqlBuilder.append(
        "INSERT INTO m_loan_paid_in_advance(loan_id, principal_in_advance_derived,"
            + " interest_in_advance_derived, fee_charges_in_advance_derived,"
            + " penalty_charges_in_advance_derived, total_in_advance_derived)");
    updateSqlBuilder.append(" select ml.id as loanId,");
    updateSqlBuilder.append(
        " SUM(ifnull(mr.principal_completed_derived, 0)) as principal_in_advance_derived,");
    updateSqlBuilder.append(
  ...
  public void applyAnnualFeeForSavings() {
    ...
    final Collection<SavingsAccountAnnualFeeData> annualFeeData =
        this.savingsAccountChargeReadPlatformService.retrieveChargesWithAnnualFeeDue();
    ...
    for (final SavingsAccountAnnualFeeData savingsAccountReference : annualFeeData) {
      try {
        this.savingsAccountWritePlatformService.applyAnnualFee(
            savingsAccountReference.getId(), savingsAccountReference.getAccountId());
      } catch (final PlatformApiDataValidationException e) {
        final List<ApiParameterError> errors = e.getErrors();
        for (final ApiParameterError error : errors) {
          logger.error(
              "Apply annual fee failed for account: {} with message {}",
  ...
  public void applyDueChargesForSavings() throws JobExecutionException {
    ...
    for (final SavingsAccountAnnualFeeData savingsAccountReference : chargesDueData) {
      try {
        ...
        for (final ApiParameterError error : errors) {
          logger.error(
              "Apply Charges due for savings failed for account {} with message: {}",
              savingsAccountReference.getAccountNo(),
  ...
  public void updateNPA() {
    ...
    resetNPASqlBuilder.append(
        "inner join m_product_loan mpl on mpl.id = loan.product_id and mpl.overdue_days_for_npa is"
            + " not null ");
    ...
    resetNPASqlBuilder.append(
        "or (mpl.account_moves_out_of_npa_only_on_arrears_completion = 1 and"
    ...
    updateSqlBuilder.append(
        " INNER JOIN m_product_loan mpl on mpl.id = loan.product_id AND mpl.overdue_days_for_npa"
            + " is not null ");
    updateSqlBuilder.append("WHERE loan.loan_status_id = 300  and ");
    updateSqlBuilder.append(
        "laa.overdue_since_date_derived < SUBDATE(CURDATE(),INTERVAL "
            + " ifnull(mpl.overdue_days_for_npa,0) day) ");
  ...
  public void updateMaturityDetailsOfDepositAccounts() {
    ...
    final Collection<DepositAccountData> depositAccounts =
        this.depositAccountReadPlatformService.retrieveForMaturityUpdate();

    for (final DepositAccountData depositAccount : depositAccounts) {
      try {
        final DepositAccountType depositAccountType =
            DepositAccountType.fromInt(depositAccount.depositType().getId().intValue());
        ...
        for (final ApiParameterError error : errors) {
          logger.error(
              "Update maturity details failed for account: {} with message {}",
  ...
  public void generateRDSchedule() {
    ...
    String insertSql =
        "INSERT INTO `m_mandatory_savings_schedule` (`savings_account_id`, `duedate`,"
            + " `installment`, `deposit_amount`, `completed_derived`, `created_date`,"
            + " `lastmodified_date`) VALUES ";
  ...
  public void postDividends() throws JobExecutionException {
    ...
    for (Map<String, Object> dividendMap : dividendDetails) {
      ...
      try {
        ...
        for (final ApiParameterError error : errors) {
          logger.error(
              "Post Dividends to savings failed due to ApiParameterError for Divident detail Id:"
                  + " {} and savings Id: {} with message: {}",
              id,
              savingsId,
              error.getDeveloperMessage(),
              e);
        ...
        logger.error(
            "Post Dividends to savings failed for Divident detail Id: {} and savings Id: {}",
            id,
            savingsId,
            e);
        exceptions.add(e);
  ...
  public void updateTrialBalanceDetails() throws JobExecutionException {
    ...
    tbGapSqlBuilder
        .append("select distinct(je.transaction_date) ")
        .append("from acc_gl_journal_entry je ")
        .append(
            "where je.transaction_date > (select IFNULL(MAX(created_date),'2010-01-01') from"
                + " m_trial_balance)");
    ...
    for (Date tbGap : tbGaps) {
      ...
      sqlBuilder
          .append(
              "Insert Into m_trial_balance(office_id, account_id, Amount, entry_date,"
                  + " created_date,closing_balance) ")
          .append(
              "Select je.office_id, je.account_id, sum(if(je.type_enum=1, (-1) * je.amount,"
                  + " je.amount)) ")
          .append(
              "as Amount, Date(je.entry_date) as 'Entry_Date', je.transaction_date as"
                  + " 'Created_Date',sum(je.amount) as closing_balance ")
    ...
    String distinctOfficeQuery =
        "select distinct(office_id) from m_trial_balance where closing_balance is null group by"
            + " office_id";
    final List<Long> officeIds =
        jdbcTemplate.queryForList(distinctOfficeQuery, new Object[] {}, Long.class);

    for (Long officeId : officeIds) {
      String distinctAccountQuery =
          "select distinct(account_id) from m_trial_balance where office_id=? and closing_balance"
              + " is null group by account_id";
      final List<Long> accountIds =
          jdbcTemplate.queryForList(distinctAccountQuery, new Object[] {officeId}, Long.class);
      for (Long accountId : accountIds) {
        final String closingBalanceQuery =
            "select closing_balance from m_trial_balance where office_id=? and account_id=? and"
                + " closing_balance is not null order by created_date desc, entry_date desc limit"
                + " 1";
        List<BigDecimal> closingBalanceData =
            jdbcTemplate.queryForList(
                closingBalanceQuery, new Object[] {officeId, accountId}, BigDecimal.class);
        List<TrialBalance> tbRows =
            this.trialBalanceRepositoryWrapper.findNewByOfficeAndAccount(officeId, accountId);
