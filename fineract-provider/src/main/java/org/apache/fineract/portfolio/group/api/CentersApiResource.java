      ...
      @Tag(
          ...
          description =
              "Centers along with Groups are used to provided a distinctive banking distribution"
                  + " channel used in microfinance. Its common in areas such as Southern Asia to"
                  + " use Centers and Group as administrative units in grameen style lending."
                  + " Typically groups will contain one to five people and centers themselves will"
                  + " be made of anywhere between 2-10 groups.")
    })
public class CentersApiResource {
  ...
  public CentersApiResource(
      ...
      final ToApiJsonSerializer<CenterData> centerApiJsonSerializer,
  ...
  @ApiOperation(
      value = "Retrieve a Center Template",
      httpMethod = "GET",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.GetCentersTemplateResponse.class)
  })
  public String retrieveTemplate(
    ...
    this.context
        .authenticatedUser()
        .validateHasReadPermission(GroupingTypesApiConstants.CENTER_RESOURCE_NAME);

    if (is(commandParam, "close")) {
      final CenterData centerClosureTemplate =
          this.centerReadPlatformService.retrieveCenterWithClosureReasons();
      final ApiRequestJsonSerializationSettings settings =
          this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
      return this.centerApiJsonSerializer.serialize(
          settings,
          centerClosureTemplate,
          GroupingTypesApiConstants.CENTER_RESPONSE_DATA_PARAMETERS);
    }

    final CenterData template =
        this.centerReadPlatformService.retrieveTemplate(officeId, staffInSelectedOfficeOnly);
    ...
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.centerApiJsonSerializer.serialize(
        settings, template, GroupingTypesApiConstants.CENTER_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiOperation(
      ...
      httpMethod = "GET",
      notes =
          "The default implementation supports pagination and sorting with the default pagination"
              + " size set to 200 records. The parameter limit with value -1 will return all"
              + " entries.\n\n"
              + "Example Requests:\n\n"
              + "\n\n"
              + "centers\n\n"
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.GetCentersResponse.class)
  })
  public String retrieveAll(
    ...
    if (meetingDateParam != null && officeId != null) {
      ...
      Collection<StaffCenterData> staffCenterDataArray =
          this.centerReadPlatformService.retriveAllCentersByMeetingDate(
              officeId, meetingDate, staffId);
    ...
    if (parameters.isPaged()) {
      final Page<CenterData> centers =
          this.centerReadPlatformService.retrievePagedAll(searchParameters, parameters);
      return this.toApiJsonSerializer.serialize(
          settings, centers, GroupingTypesApiConstants.CENTER_RESPONSE_DATA_PARAMETERS);
    }

    final Collection<CenterData> centers =
        this.centerReadPlatformService.retrieveAll(searchParameters, parameters);
    return this.toApiJsonSerializer.serialize(
        settings, centers, GroupingTypesApiConstants.CENTER_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.GetCentersCenterIdResponse.class)
  })
  public String retrieveOne(
      @Context final UriInfo uriInfo,
      @PathParam("centerId") @ApiParam(value = "centerId") final Long centerId,
      @DefaultValue("false")
          @QueryParam("staffInSelectedOfficeOnly")
          @ApiParam(value = "staffInSelectedOfficeOnly")
          final boolean staffInSelectedOfficeOnly) {
    ...
    if (template) {
      final CenterData templateCenter =
          this.centerReadPlatformService.retrieveTemplate(
              center.officeId(), staffInSelectedOfficeOnly);
    ...
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.centerApiJsonSerializer.serialize(
        settings, center, GroupingTypesApiConstants.CENTER_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = CentersApiResourceSwagger.PostCentersRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.PostCentersResponse.class)
  })
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = CentersApiResourceSwagger.PutCentersCenterIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.PutCentersCenterIdResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Delete a Center",
      httpMethod = "DELETE",
      notes =
          "A Center can be deleted if it is in pending state and has no association - groups,"
              + " loans or savings")
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.DeleteCentersCenterIdResponse.class)
  })
  ...
  @ApiOperation(
      value =
          "Activate a Center | Generate Collection Sheet | Save Collection Sheet | Close a Center"
              + " | Associate Groups | Disassociate Groups",
      httpMethod = "POST",
      notes =
          "Activate a Center:\n\n"
              + "Centers can be created in a Pending state. This API exists to enable center"
              + " activation. If the center happens to be already active, this API will result in"
              + " an error.\n\n"
              + "Close a Center:\n\n"
              + "Centers can be closed if they don't have any non-closed groups or saving"
              + " accounts. If the Center has any active groups or savings accounts, this API will"
              + " result in an error.\n\n"
              + "Associate Groups:\n\n"
              + "This API allows associating existing groups to a center. The groups are listed"
              + " from the office to which the center is associated. If group(s) is already"
              + " associated with a center, this API will result in an error.\n\n"
              + "Disassociate Groups:\n\n"
              + "This API allows to disassociate groups from a center.\n\n"
              + "Generate Collection Sheet:\n\n"
              + "This Api retrieves repayment details of all jlg loans under a center as on a"
              + " specified meeting date.\n\n"
              + "Save Collection Sheet:\n\n"
              + "This Api allows the loan officer to perform bulk repayments of JLG loans for a"
              + " center on a given meeting date.\n\n"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = CentersApiResourceSwagger.PostCentersCenterIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.PostCentersCenterIdResponse.class)
  })
  public String activate(
    ...
    if (is(commandParam, "activate")) {
      ...
      final JLGCollectionSheetData collectionSheet =
          this.collectionSheetReadPlatformService.generateCenterCollectionSheet(centerId, query);
      final ApiRequestJsonSerializationSettings settings =
          this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
      return this.toApiJsonSerializer.serialize(
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve Center accounts overview",
      httpMethod = "GET",
      notes =
          "An example of how a savings summary for a Center can be provided. This is requested in"
              + " a specific use case of the reference application.\n\n"
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CentersApiResourceSwagger.GetCentersCenterIdAccountsResponse.class)
  })
  public String retrieveGroupAccount(
      @PathParam("centerId") @ApiParam(value = "centerId") final Long centerId,
      @Context final UriInfo uriInfo) {
    ...
    final AccountSummaryCollectionData groupAccount =
        this.accountDetailsReadPlatformService.retrieveGroupAccountDetails(centerId);

    final Set<String> GROUP_ACCOUNTS_DATA_PARAMETERS =
        new HashSet<>(
            Arrays.asList(
                "loanAccounts", "savingsAccounts", "memberLoanAccounts", "memberSavingsAccounts"));
  ...
  public Response getCentersTemplate(
      @QueryParam("officeId") final Long officeId,
      @QueryParam("staffId") final Long staffId,
      @QueryParam("dateFormat") final String dateFormat) {
    return bulkImportWorkbookPopulatorService.getTemplate(
        GlobalEntityType.CENTERS.toString(), officeId, staffId, dateFormat);
  }

  @POST
  @Path("uploadtemplate")
  @Consumes(MediaType.MULTIPART_FORM_DATA)
  public String postCentersTemplate(
    ...
    final Long importDocumentId =
        this.bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.CENTERS.toString(),
            uploadedInputStream,
            fileDetail,
            locale,
            dateFormat);
