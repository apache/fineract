...
public class ProductMixReadPlatformServiceImpl implements ProductMixReadPlatformService {
  ...
  @Autowired
  public ProductMixReadPlatformServiceImpl(
  ...
  public ProductMixData retrieveLoanProductMixDetails(final Long productId) {
    try {
      ...
      final ProductMixDataExtractor extractor =
          new ProductMixDataExtractor(this.loanProductReadPlatformService, productId);

      final String sql =
          "Select " + extractor.schema() + " where pm.product_id=? group by pm.product_id";

      final Map<Long, ProductMixData> productMixData =
          this.jdbcTemplate.query(sql, extractor, new Object[] {productId});
  ...
  public Collection<ProductMixData> retrieveAllProductMixes() {
    ...
    final ProductMixDataExtractor extractor =
        new ProductMixDataExtractor(this.loanProductReadPlatformService, null);

    final String sql = "Select " + extractor.schema() + " group by pm.product_id";

    final Map<Long, ProductMixData> productMixData =
        this.jdbcTemplate.query(sql, extractor, new Object[] {});

    return productMixData.values();
  }

  private static final class ProductMixDataExtractor
      implements ResultSetExtractor<Map<Long, ProductMixData>> {

    private final LoanProductReadPlatformService loanProductReadPlatformService;
    private final Long productId;

    public String schema() {
      return "pm.product_id as productId, lp.name as name from m_product_mix pm join"
          + " m_product_loan lp on lp.id=pm.product_id";
    }

    public ProductMixDataExtractor(
        final LoanProductReadPlatformService loanProductReadPlatformService, final Long productId) {
      this.loanProductReadPlatformService = loanProductReadPlatformService;
      this.productId = productId;
    }

    @Override
    public Map<Long, ProductMixData> extractData(final ResultSet rs)
