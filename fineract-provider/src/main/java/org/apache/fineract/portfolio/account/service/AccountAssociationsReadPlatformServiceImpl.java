...
public class AccountAssociationsReadPlatformServiceImpl
  ...
  public PortfolioAccountData retriveLoanLinkedAssociation(final Long loanId) {
    ...
    final String sql =
        "select "
            + mapper.schema()
            + " where aa.loan_account_id = ? and aa.association_type_enum = ?";
    try {
      final AccountAssociationsData accountAssociationsData =
          this.jdbcTemplate.queryForObject(
              sql, mapper, loanId, AccountAssociationType.LINKED_ACCOUNT_ASSOCIATION.getValue());
  ...
  @Override
  public Collection<AccountAssociationsData> retriveLoanAssociations(
      final Long loanId, final Integer associationType) {
    final AccountAssociationsMapper mapper = new AccountAssociationsMapper();
    final String sql =
        "select "
            + mapper.schema()
  ...
  public PortfolioAccountData retriveSavingsLinkedAssociation(final Long savingsId) {
    ...
    final String sql =
        "select "
            + mapper.schema()
            + " where aa.savings_account_id = ? and aa.association_type_enum = ?";
    try {
      final AccountAssociationsData accountAssociationsData =
          this.jdbcTemplate.queryForObject(
              sql, mapper, savingsId, AccountAssociationType.LINKED_ACCOUNT_ASSOCIATION.getValue());
      if (accountAssociationsData != null) {
        linkedAccount = accountAssociationsData.linkedAccount();
      }
    } catch (final EmptyResultDataAccessException e) {
  ...
  public boolean isLinkedWithAnyActiveAccount(final Long savingsId) {
    ...
    final String sql1 =
        "select aa.is_active as active,aa.association_type_enum as type,"
            + " loanAccount.loan_status_id as loanStatus,savingAccount.status_enum as"
            + " savingsStatus from m_portfolio_account_associations aa left join m_loan"
            + " loanAccount on loanAccount.id = aa.loan_account_id left join m_savings_account"
            + " savingAccount on savingAccount.id = aa.savings_account_id where"
    ...
    for (final Map<String, Object> statusMap : statusList) {
      ...
      if (statusMap.get("savingsStatus") != null) {
        final SavingsAccountStatusType saveStatus =
            SavingsAccountStatusType.fromInt((Integer) statusMap.get("savingsStatus"));
  ...
  private static final class AccountAssociationsMapper
      implements RowMapper<AccountAssociationsData> {
    ...
    public AccountAssociationsMapper() {
      ...
      // sqlBuilder.append("savingsAccount.id as savingsAccountId, savingsAccount.account_no as
      // savingsAccountNo,");
      sqlBuilder.append(
          "loanAccount.id as loanAccountId, loanAccount.account_no as loanAccountNo,");
      // sqlBuilder.append("linkLoanAccount.id as linkLoanAccountId, linkLoanAccount.account_no as
      // linkLoanAccountNo, ");
      sqlBuilder.append(
          "linkSavingsAccount.id as linkSavingsAccountId, linkSavingsAccount.account_no as"
              + " linkSavingsAccountNo ");
      sqlBuilder.append("from m_portfolio_account_associations aa ");
      // sqlBuilder.append("left join m_savings_account savingsAccount on savingsAccount.id =
      // aa.savings_account_id ");
      sqlBuilder.append("left join m_loan loanAccount on loanAccount.id = aa.loan_account_id ");
      sqlBuilder.append(
          "left join m_savings_account linkSavingsAccount on linkSavingsAccount.id ="
              + " aa.linked_savings_account_id ");
      // sqlBuilder.append("left join m_loan linkLoanAccount on linkLoanAccount.id =
      // aa.linked_loan_account_id ");
    ...
    @Override
    public AccountAssociationsData mapRow(
      ...
      final PortfolioAccountData account =
      ...
      final PortfolioAccountData linkedAccount =
          PortfolioAccountData.lookup(linkSavingsAccountId, linkSavingsAccountNo);
      /*
