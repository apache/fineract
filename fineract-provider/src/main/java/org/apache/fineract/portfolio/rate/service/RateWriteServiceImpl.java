...
public class RateWriteServiceImpl implements RateWriteService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(RateWriteServiceImpl.class);
  ...
  public CommandProcessingResult updateRate(final Long rateId, final JsonCommand command) {
    try {
      ...
      final Rate rateToUpdate =
          this.rateRepository.findById(rateId).orElseThrow(() -> new RateNotFoundException(rateId));
   ...
   * is.
   */
  private void handleRateDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("rate_name_org")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.fund.duplicate.externalId",
          "A rate with name '" + name + "' already exists",
          "name",
          name);
    }
    ...
    throw new PlatformDataIntegrityException(
        "error.msg.fund.unknown.data.integrity.issue",
