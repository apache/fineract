...
public class ClientRepositoryWrapper {
  ...
  public ClientRepositoryWrapper(
      final ClientRepository repository, final PlatformSecurityContext context) {
  ...
  public Client findOneWithNotFoundDetection(
      final Long clientId, final boolean loadLazyCollections) {
    final Client client =
        this.repository.findById(clientId).orElseThrow(() -> new ClientNotFoundException(clientId));
