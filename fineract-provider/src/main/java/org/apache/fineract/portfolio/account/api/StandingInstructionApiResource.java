...
@SwaggerDefinition(
    tags = {
      @Tag(
          ...
          description =
              "Standing instructions (or standing orders) refer to instructions a bank account"
                  + " holder (\"the payer\") gives to his or her bank to pay a set amount at"
                  + " regular intervals to another's (\"the payee's\") account.\n"
                  + "\n"
                  + "Note: At present only savings account to savings account and savings account"
                  + " to Loan account transfers are permitted.")
    })
public class StandingInstructionApiResource {
  ...
  @ApiOperation(
      value = "Retrieve Standing Instruction Template",
      httpMethod = "GET",
      notes =
  ...
  public String template(
      @QueryParam("fromOfficeId") @ApiParam(value = "fromOfficeId") final Long fromOfficeId,
    ...
    final StandingInstructionData standingInstructionData =
        this.standingInstructionReadPlatformService.retrieveTemplate(
            fromOfficeId,
            fromClientId,
            fromAccountId,
            fromAccountType,
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Create new Standing Instruction",
      httpMethod = "POST",
      notes =
          "Ability to create new instruction for transfer of monetary funds from one account to"
              + " another")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = StandingInstructionApiResourceSwagger.PostStandingInstructionsRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = StandingInstructionApiResourceSwagger.PostStandingInstructionsResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Update Standing Instruction | Delete Standing Instruction",
      httpMethod = "PUT",
      notes =
          "Ability to modify existing instruction for transfer of monetary funds from one account"
              + " to another.\n"
              + "\n"
              + "PUT https://DomainName/api/v1/standinginstructions/1?command=update\n"
              + "\n\n"
              + "Ability to modify existing instruction for transfer of monetary funds from one"
              + " account to another.\n"
              + "\n"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        format = "body",
        dataTypeClass =
            StandingInstructionApiResourceSwagger
                .PutStandingInstructionsStandingInstructionIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        ...
        response =
            StandingInstructionApiResourceSwagger
                .PutStandingInstructionsStandingInstructionIdResponse.class)
  })
  public String update(
      @PathParam("standingInstructionId") @ApiParam(value = "standingInstructionId")
    ...
    if (is(commandParam, "update")) {
      commandRequest =
          new CommandWrapperBuilder()
              .updateStandingInstruction(standingInstructionId)
              .withJson(apiRequestBodyAsJson)
              .build();
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "List Standing Instructions",
      httpMethod = "GET",
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = StandingInstructionApiResourceSwagger.GetStandingInstructionsResponse.class)
  })
  public String retrieveAll(
    ...
    final Page<StandingInstructionData> transfers =
        this.standingInstructionReadPlatformService.retrieveAll(standingInstructionDTO);

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, transfers, StandingInstructionApiConstants.RESPONSE_DATA_PARAMETERS);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve Standing Instruction",
      httpMethod = "GET",
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        response =
            StandingInstructionApiResourceSwagger
                .GetStandingInstructionsStandingInstructionIdResponse.class)
  })
  public String retrieveOne(
    ...
    if (!associationParameters.isEmpty()) {
      ...
      if (associationParameters.contains("transactions")) {
        transfers =
            this.accountTransfersReadPlatformService.retrieveByStandingInstruction(
                standingInstructionId, searchParameters);
        standingInstructionData =
            StandingInstructionData.withTransferData(standingInstructionData, transfers);
      }
      if (associationParameters.contains("template")) {
        final StandingInstructionData templateData =
            this.standingInstructionReadPlatformService.retrieveTemplate(
                standingInstructionData.fromClient().officeId(),
                standingInstructionData.fromClient().id(),
                standingInstructionData.fromAccount().accountId(),
                standingInstructionData.fromAccountType().getValue(),
                standingInstructionData.toClient().officeId(),
                standingInstructionData.toClient().id(),
                standingInstructionData.toAccount().accountId(),
                standingInstructionData.toAccountType().getValue(),
                standingInstructionData.transferType().getValue());
        standingInstructionData =
            StandingInstructionData.withTemplateData(standingInstructionData, templateData);
      }
    }
