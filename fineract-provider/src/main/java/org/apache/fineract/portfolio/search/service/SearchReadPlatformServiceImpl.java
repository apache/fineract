...
public class SearchReadPlatformServiceImpl implements SearchReadPlatformService {
  ...
  @Autowired
  public SearchReadPlatformServiceImpl(
  ...
  private static final class SearchMapper implements RowMapper<SearchData> {
    ...
    public String searchSchema(final SearchConditions searchConditions) {
      ...
      final String clientMatchSql =
          " (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName,"
              ...
              + " c.office_id as parentId, o.name as parentName, c.mobile_no as"
              + " entityMobileNo,c.status_enum as entityStatusEnum, null as parentType  from"
              + " m_client c join m_office o on o.id = c.office_id where o.hierarchy like"
              + " :hierarchy and (c.account_no like :search or c.display_name like :search or"
              + " c.external_id like :search or c.mobile_no like :search)) ";

      final String loanMatchSql =
          " (select 'LOAN' as entityType, l.id as entityId, pl.name as entityName, l.external_id"
              + " as entityExternalId, l.account_no as entityAccountNo  , IFNULL(c.id,g.id) as"
              + " parentId, IFNULL(c.display_name,g.display_name) as parentName, null as"
              + " entityMobileNo, l.loan_status_id as entityStatusEnum, IF(g.id is null, 'client',"
              + " 'group') as parentType  from m_loan l left join m_client c on l.client_id = c.id"
              + " left join m_group g ON l.group_id = g.id left join m_office o on o.id ="
              + " c.office_id left join m_product_loan pl on pl.id=l.product_id where (o.hierarchy"
              + " IS NULL OR o.hierarchy like :hierarchy) and (l.account_no like :search or"
              + " l.external_id like :search)) ";

      final String savingMatchSql =
          " (select 'SAVING' as entityType, s.id as entityId, sp.name as entityName, s.external_id"
              + " as entityExternalId, s.account_no as entityAccountNo  , IFNULL(c.id,g.id) as"
              + " parentId, IFNULL(c.display_name,g.display_name) as parentName, null as"
              + " entityMobileNo, s.status_enum as entityStatusEnum, IF(g.id is null, 'client',"
              + " 'group') as parentType  from m_savings_account s left join m_client c on"
              + " s.client_id = c.id left join m_group g ON s.group_id = g.id left join m_office o"
              + " on o.id = c.office_id left join m_savings_product sp on sp.id=s.product_id "
              + " where (o.hierarchy IS NULL OR o.hierarchy like :hierarchy) and (s.account_no"
              + " like :search or s.external_id like :search)) ";

      final String shareMatchSql =
          " (select 'SHARE' as entityType, s.id as entityId, sp.name as entityName, s.external_id"
              + " as entityExternalId, s.account_no as entityAccountNo  , c.id as parentId,"
              + " c.display_name as parentName, null as entityMobileNo, s.status_enum as"
              + " entityStatusEnum, 'client' as parentType  from m_share_account s left join"
              + " m_client c on s.client_id = c.id left join m_office o on o.id = c.office_id left"
              + " join m_share_product sp on sp.id=s.product_id  where (o.hierarchy IS NULL OR"
              + " o.hierarchy like :hierarchy) and (s.account_no like :search or s.external_id"
              + " like :search)) ";

      final String clientIdentifierMatchSql =
          " (select 'CLIENTIDENTIFIER' as entityType, ci.id as entityId, ci.document_key as"
              + " entityName,  null as entityExternalId, null as entityAccountNo, c.id as"
              + " parentId, c.display_name as parentName,null as entityMobileNo, c.status_enum as"
              + " entityStatusEnum, null as parentType  from m_client_identifier ci join m_client"
              + " c on ci.client_id=c.id join m_office o on o.id = c.office_id  where o.hierarchy"
              + " like :hierarchy and ci.document_key like :search ) ";
      final String groupMatchSql =
          " (select IF(g.level_id=1,'CENTER','GROUP') as entityType, g.id as entityId,"
              + " g.display_name as entityName, g.external_id as entityExternalId, g.account_no as"
              + " entityAccountNo  , g.office_id as parentId, o.name as parentName, null as"
              + " entityMobileNo, g.status_enum as entityStatusEnum, null as parentType  from"
              + " m_group g join m_office o on o.id = g.office_id where o.hierarchy like"
              + " :hierarchy and (g.account_no like :search or g.display_name like :search or"
              + " g.external_id like :search or g.id like :search )) ";
    ...
    @Override
    public SearchData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  private static final class AdHocQuerySearchMapper implements RowMapper<AdHocSearchQueryData> {
    ...
    public String schema(
        final AdHocQuerySearchConditions searchConditions, final MapSqlParameterSource params) {
      final StringBuilder sql = new StringBuilder();
      sql.append(
              "Select a.name as officeName, a.Product as productName, a.cnt as 'count',"
                  + " a.outstandingAmt as outstanding, a.percentOut as percentOut  ")
          .append(
              "from (select mo.name, mp.name Product,"
                  + " sum(ifnull(ml.total_expected_repayment_derived,0.0)) TotalAmt, count(*)"
                  + " cnt, ")
          .append("sum(ifnull(ml.total_outstanding_derived,0.0)) outstandingAmt,  ")
          .append(
              "(sum(ifnull(ml.total_outstanding_derived,0.0)) * 100 /"
                  + " sum(ifnull(ml.total_expected_repayment_derived,0.0))) percentOut ")
      ...
      if (searchConditions.getLoanProducts() != null
          && searchConditions.getLoanProducts().size() > 0) {
        checkAndUpdateWhereClause(sql);
      ...
      if (StringUtils.isNotBlank(searchConditions.getLoanDateOption())) {
        if (searchConditions
            .getLoanDateOption()
            .equals(SearchConstants.SearchLoanDate.APPROVAL_DATE.getValue())) {
          checkAndUpdateWhereClause(sql);
          ...
          sql.append(" ( ml.approvedon_date between :loanFromDate and :loanToDate ) ");
            ...
            .getLoanDateOption()
            .equals(SearchConstants.SearchLoanDate.CREATED_DATE.getValue())) {
          checkAndUpdateWhereClause(sql);
          params.addValue("loanFromDate", searchConditions.getLoanFromDate().toDate());
          params.addValue("loanToDate", searchConditions.getLoanToDate().toDate());
          sql.append(" ( ml.submittedon_date between :loanFromDate and :loanToDate ) ");
            ...
            .getLoanDateOption()
            .equals(SearchConstants.SearchLoanDate.DISBURSAL_DATE.getValue())) {
          checkAndUpdateWhereClause(sql);
          params.addValue("loanFromDate", searchConditions.getLoanFromDate().toDate());
          params.addValue("loanToDate", searchConditions.getLoanToDate().toDate());
      ...
      if (searchConditions.getIncludeOutStandingAmountPercentage()) {
        if (searchConditions.getOutStandingAmountPercentageCondition().equals("between")) {
          ...
          sql.append(
              " ( a.percentOut between :minOutStandingAmountPercentage and"
                  + " :maxOutStandingAmountPercentage ) ");
        } else {
          checkAndUpdateWhereClause(sql);
          // params.addValue("outStandingAmountPercentageCondition",
          // searchConditions.getOutStandingAmountPercentageCondition());
          params.addValue(
              "outStandingAmountPercentage", searchConditions.getOutStandingAmountPercentage());
      ...
      if (searchConditions.getIncludeOutstandingAmount()) {
        if (searchConditions.getOutstandingAmountCondition().equals("between")) {
          ...
          sql.append(
              " ( a.outstandingAmt between :minOutstandingAmount and :maxOutstandingAmount ) ");
        } else {
          checkAndUpdateWhereClause(sql);
          // params.addValue("outstandingAmountCondition",
          // searchConditions.getOutstandingAmountCondition());
    ...
    @Override
    public AdHocSearchQueryData mapRow(ResultSet rs, @SuppressWarnings("unused") int rowNum)
        throws SQLException {
      ...
      final BigDecimal loanOutStanding =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "outstanding")
              .setScale(2, RoundingMode.HALF_UP);
      final Double percentage =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "percentOut")
              .setScale(2, RoundingMode.HALF_UP)
              .doubleValue();
      return AdHocSearchQueryData.matchedResult(
          officeName, loanProductName, count, loanOutStanding, percentage);
    }
  }
}
