...
public class InternalLoanInformationApiResource implements InitializingBean {
    ...
    public String getLoansByStatus(@Context final UriInfo uriInfo, @PathParam("statusId") Integer statusId) {
        ...
        final List<Long> loanIds = loanRepositoryWrapper.findLoanIdByStatusId(statusId);
        final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        return this.toApiJsonSerializerForList.serialize(settings, loanIds);
    }

    @POST
    @Path("{loanId}/status/{statusId}")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public Response changeLoanStatus(@Context final UriInfo uriInfo, @PathParam("loanId") Long loanId,
            @PathParam("statusId") Integer statusId) {
        log.warn("------------------------------------------------------------");
        log.warn("                                                            ");
        log.warn("Set loan status with loanId {}, statusId {}", loanId, statusId);
        log.warn("                                                            ");
        log.warn("------------------------------------------------------------");

        loanRepositoryWrapper.updateStatus(loanId, statusId);
