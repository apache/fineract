...
public class RecurringDepositScheduleInstallment extends AbstractAuditableCustom {
   ...
   */
  private RecurringDepositScheduleInstallment(
      final RecurringDepositAccount account,
      final Integer installmentNumber,
      final Date fromDate,
      final Date dueDate,
      final BigDecimal depositAmount,
      final BigDecimal depositAmountCompleted,
      final BigDecimal totalPaidInAdvance,
      final BigDecimal totalPaidLate,
      final boolean obligationsMet,
  ...
  public static RecurringDepositScheduleInstallment from(
      final RecurringDepositAccount account,
      final Integer installmentNumber,
      final Date fromDate,
      final Date dueDate,
  ...
  public static RecurringDepositScheduleInstallment installment(
      final RecurringDepositAccount account,
      final Integer installmentNumber,
      final Date dueDate,
      final BigDecimal depositAmount) {
  ...
  public Money payInstallment(
    ...
    if (transactionAmountRemaining.isGreaterThanOrEqualTo(depositAmount)) {
      this.depositAmountCompleted =
          getDepositAmountCompleted(currency).plus(depositAmount).getAmount();
      depositAmountPortionOfTransaction = depositAmountPortionOfTransaction.plus(depositAmount);
    } else {
      this.depositAmountCompleted =
          getDepositAmountCompleted(currency).plus(transactionAmountRemaining).getAmount();
      depositAmountPortionOfTransaction =
          depositAmountPortionOfTransaction.plus(transactionAmountRemaining);
    }
    ...
    trackAdvanceAndLateTotalsForInstallment(
        transactionDate, currency, depositAmountPortionOfTransaction);

    return depositAmountPortionOfTransaction;
  }

  private void checkIfInstallmentObligationsAreMet(
  ...
  private void trackAdvanceAndLateTotalsForInstallment(
      final LocalDate transactionDate,
      final MonetaryCurrency currency,
      final Money amountPaidInInstallment) {
    if (isInAdvance(transactionDate)) {
      this.totalPaidInAdvance =
          asMoney(this.totalPaidInAdvance, currency).plus(amountPaidInInstallment).getAmount();
    } else if (isLatePayment(transactionDate)) {
      this.totalPaidLate =
          asMoney(this.totalPaidLate, currency).plus(amountPaidInInstallment).getAmount();
    }
  }
