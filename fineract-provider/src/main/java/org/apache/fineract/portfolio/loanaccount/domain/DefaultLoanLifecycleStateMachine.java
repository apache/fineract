...
public class DefaultLoanLifecycleStateMachine implements LoanLifecycleStateMachine {
  ...
  public LoanStatus transition(final LoanEvent loanEvent, final LoanStatus from) {
    ...
    switch (loanEvent) {
      ...
      case LOAN_REPAYMENT_OR_WAIVER:
        if (anyOfAllowedWhenComingFrom(
            from, LoanStatus.ACTIVE, LoanStatus.CLOSED_OBLIGATIONS_MET, LoanStatus.OVERPAID)) {
          newState = stateOf(LoanStatus.ACTIVE, this.allowedLoanStatuses);
        } else {
          newState = from;
        }
      ...
      case LOAN_RESCHEDULE:
        if (anyOfAllowedWhenComingFrom(from, LoanStatus.ACTIVE)) {
          newState =
              stateOf(LoanStatus.CLOSED_RESCHEDULE_OUTSTANDING_AMOUNT, this.allowedLoanStatuses);
        }
        break;
      ...
      case LOAN_OVERPAYMENT:
        if (anyOfAllowedWhenComingFrom(
            from, LoanStatus.CLOSED_OBLIGATIONS_MET, LoanStatus.ACTIVE)) {
  ...
  private boolean anyOfAllowedWhenComingFrom(
      final LoanStatus state, final LoanStatus... allowedStates) {
