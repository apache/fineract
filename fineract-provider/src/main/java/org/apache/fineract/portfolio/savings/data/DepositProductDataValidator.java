...
public class DepositProductDataValidator {
  ...
  public DepositProductDataValidator(
      FromJsonHelper fromApiJsonHelper, InterestRateChartDataValidator chartDataValidator) {
  ...
  public void validateForFixedDepositCreate(final String json) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, FIXED_DEPOSIT_PRODUCT_REQUEST_DATA_PARAMETERS);
  ...
  public void validateForRecurringDepositCreate(final String json) {
    ...
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, RECURRING_DEPOSIT_PRODUCT_REQUEST_DATA_PARAMETERS);
  ...
  private void validateDepositDetailForCreate(
    ...
    if (fromApiJsonHelper.parameterExists(inMultiplesOfParamName, element)) {
      final Integer inMultiplesOf =
          fromApiJsonHelper.extractIntegerNamed(
              inMultiplesOfParamName, element, Locale.getDefault());
      baseDataValidator
          .reset()
    ...
    if (this.fromApiJsonHelper.parameterExists(
      ...
      baseDataValidator
          .reset()
          .parameter(minBalanceForInterestCalculationParamName)
  ...
  public void validatePreClosureDetailForCreate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
    if (fromApiJsonHelper.parameterExists(preClosurePenalApplicableParamName, element)) {
      final boolean preClosurePenalApplicable =
          fromApiJsonHelper.extractBooleanNamed(preClosurePenalApplicableParamName, element);

      if (preClosurePenalApplicable) {
        final BigDecimal penalInterestRate =
            fromApiJsonHelper.extractBigDecimalWithLocaleNamed(
                preClosurePenalInterestParamName, element);
        baseDataValidator
            .reset()
            .parameter(preClosurePenalInterestParamName)
            .value(penalInterestRate)
            .cantBeBlankWhenParameterProvidedIs(
                preClosurePenalApplicableParamName, preClosurePenalApplicable)
            .zeroOrPositiveAmount();

        final Integer preClosurePenalInterestType =
            this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
                preClosurePenalInterestOnTypeIdParamName, element);
        baseDataValidator
            .reset()
            .parameter(preClosurePenalInterestOnTypeIdParamName)
            .value(preClosurePenalInterestType)
            .cantBeBlankWhenParameterProvidedIs(
                preClosurePenalApplicableParamName, preClosurePenalApplicable)
  ...
  public void validateDepositTermDeatilForCreate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {

    final Integer minTerm =
        fromApiJsonHelper.extractIntegerSansLocaleNamed(minDepositTermParamName, element);
    baseDataValidator
        .reset()
    ...
    if (fromApiJsonHelper.parameterExists(maxDepositTermParamName, element)) {
      final Integer maxTerm =
          fromApiJsonHelper.extractIntegerSansLocaleNamed(maxDepositTermParamName, element);
      baseDataValidator
          .reset()
    ...
    final Integer minDepositTermType =
        this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
            minDepositTermTypeIdParamName, element);
    baseDataValidator
        .reset()
        .parameter(minDepositTermTypeIdParamName)
        .value(minDepositTermType)
        .ignoreIfNull()
    ...
    if (fromApiJsonHelper.parameterExists(maxDepositTermTypeIdParamName, element)) {
      final Integer maxDepositTermType =
          this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
              maxDepositTermTypeIdParamName, element);
      baseDataValidator
          .reset()
          .parameter(maxDepositTermTypeIdParamName)
          .value(maxDepositTermType)
          .isOneOfTheseValues(SavingsPeriodFrequencyType.integerValues());
    }
    ...
    if (fromApiJsonHelper.parameterExists(inMultiplesOfDepositTermParamName, element)) {
      final Integer inMultiplesOfDepositTerm =
          fromApiJsonHelper.extractIntegerSansLocaleNamed(
              inMultiplesOfDepositTermParamName, element);
      baseDataValidator
          .reset()
          .parameter(inMultiplesOfDepositTermParamName)
          .value(inMultiplesOfDepositTerm)
          .integerGreaterThanZero();
      final Integer inMultiplesOfDepositTermType =
          this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
              inMultiplesOfDepositTermTypeIdParamName, element);
      baseDataValidator
          .reset()
          .parameter(inMultiplesOfDepositTermTypeIdParamName)
          .value(inMultiplesOfDepositTermType)
          .cantBeBlankWhenParameterProvidedIs(
              inMultiplesOfDepositTermParamName, inMultiplesOfDepositTerm)
          .isOneOfTheseValues(SavingsPeriodFrequencyType.integerValues());
    }
  }
  ...
  public void validateDepositDetailForUpdate(
      final JsonElement element,
      final FromJsonHelper fromApiJsonHelper,
      final DataValidatorBuilder baseDataValidator) {
    ...
    if (fromApiJsonHelper.parameterExists(descriptionParamName, element)) {
      final String description =
          fromApiJsonHelper.extractStringNamed(descriptionParamName, element);
      baseDataValidator
          .reset()
          .parameter(descriptionParamName)
          .value(description)
    ...
    if (fromApiJsonHelper.parameterExists(withdrawalFeeForTransfersParamName, element)) {
      final Boolean isWithdrawalFeeApplicableForTransfers =
          fromApiJsonHelper.extractBooleanNamed(withdrawalFeeForTransfersParamName, element);
      baseDataValidator
          .reset()
    ...
    if (fromApiJsonHelper.parameterExists(feeAmountParamName, element)) {
      final BigDecimal annualFeeAmount =
          fromApiJsonHelper.extractBigDecimalWithLocaleNamed(feeAmountParamName, element);
      baseDataValidator
          .reset()
          .parameter(feeAmountParamName)
    ...
    if (fromApiJsonHelper.parameterExists(feeOnMonthDayParamName, element)) {
      final MonthDay monthDayOfAnnualFee =
          fromApiJsonHelper.extractMonthDayNamed(feeOnMonthDayParamName, element);
      baseDataValidator
          .reset()
          .parameter(feeOnMonthDayParamName)
  ...
  public void validatePreClosureDetailForUpdate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
    if (fromApiJsonHelper.parameterExists(preClosurePenalApplicableParamName, element)) {
      final Boolean preClosurePenalApplicable =
          fromApiJsonHelper.extractBooleanNamed(preClosurePenalApplicableParamName, element);
      baseDataValidator
          .reset()
          .parameter(preClosurePenalApplicableParamName)
          .value(preClosurePenalApplicable)
          .notNull();
    }
  ...
  public void validateDepositTermDetailForUpdate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
  ...
  private void validatePaymentChannelFundSourceMappings(
    ...
    if (fromApiJsonHelper.parameterExists(
        SavingProductAccountingParams.PAYMENT_CHANNEL_FUND_SOURCE_MAPPING.getValue(), element)) {
      ...
      if (paymentChannelMappingArray != null && paymentChannelMappingArray.size() > 0) {
        ...
        do {
          ...
          final Long paymentTypeId =
              jsonObject.get(SavingProductAccountingParams.PAYMENT_TYPE.getValue()).getAsLong();
          final Long paymentSpecificFundAccountId =
              jsonObject.get(SavingProductAccountingParams.FUND_SOURCE.getValue()).getAsLong();
          baseDataValidator
              .reset()
              .parameter(
                  SavingProductAccountingParams.PAYMENT_CHANNEL_FUND_SOURCE_MAPPING.getValue()
  ...
  private void validateChargeToIncomeAccountMappings(
    ...
    if (fromApiJsonHelper.parameterExists(parameterName, element)) {
      final JsonArray chargeToIncomeAccountMappingArray =
          fromApiJsonHelper.extractJsonArrayNamed(parameterName, element);
      if (chargeToIncomeAccountMappingArray != null
        ...
        do {
          ...
          final Long chargeId =
              fromApiJsonHelper.extractLongNamed(
                  SavingProductAccountingParams.CHARGE_ID.getValue(), jsonObject);
          final Long incomeAccountId =
              fromApiJsonHelper.extractLongNamed(
                  SavingProductAccountingParams.INCOME_ACCOUNT_ID.getValue(), jsonObject);
          baseDataValidator
              .reset()
              .parameter(
                  parameterName
                      ...
                      + i
                      + "]."
                      + SavingProductAccountingParams.CHARGE_ID.getValue())
              .value(chargeId)
              .notNull()
              .integerGreaterThanZero();
          baseDataValidator
              ...
              .parameter(
                  parameterName
                      ...
                      + i
                      + "]."
                      + SavingProductAccountingParams.INCOME_ACCOUNT_ID.getValue())
  ...
  public void validateRecurringDetailForCreate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {

    final Boolean isMandatoryDeposit =
        this.fromApiJsonHelper.extractBooleanNamed(isMandatoryDepositParamName, element);
    baseDataValidator
        .reset()
        .parameter(isMandatoryDepositParamName)
        .value(isMandatoryDeposit)
        .ignoreIfNull()
        .validateForBooleanValue();
    final Boolean allowWithdrawal =
        this.fromApiJsonHelper.extractBooleanNamed(allowWithdrawalParamName, element);
    baseDataValidator
        .reset()
        .parameter(allowWithdrawalParamName)
    ...
    baseDataValidator
        .reset()
        .parameter(adjustAdvanceTowardsFuturePaymentsParamName)
        .value(adjustAdvanceTowardsFuturePayments)
        .ignoreIfNull()
        .validateForBooleanValue();
  }

  public void validateRecurringDepositUpdate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
  ...
  private void validateDepositAmountForCreate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
    final BigDecimal depositAmount =
        this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(depositAmountParamName, element);
    baseDataValidator
        .reset()
        .parameter(depositAmountParamName)
        .value(depositAmount)
        .notNull()
        .positiveAmount();
    ...
    if (this.fromApiJsonHelper.parameterExists(depositMinAmountParamName, element)) {
      depositMinAmount =
          this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(
              depositMinAmountParamName, element);
      baseDataValidator
          .reset()
          .parameter(depositMinAmountParamName)
    ...
    if (this.fromApiJsonHelper.parameterExists(depositMaxAmountParamName, element)) {
      depositMaxAmount =
          this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(
              depositMaxAmountParamName, element);
      baseDataValidator
          .reset()
          .parameter(depositMaxAmountParamName)
  ...
  private void validateDepositAmountForUpdate(
      JsonElement element, DataValidatorBuilder baseDataValidator) {
