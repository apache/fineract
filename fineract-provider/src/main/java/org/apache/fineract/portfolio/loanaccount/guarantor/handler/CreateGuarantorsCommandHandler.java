...
public class CreateGuarantorsCommandHandler implements CommandHandler<CreateGuarantorsRequest, CreateGuarantorsResponse> {
    ...
    // The additional Transactional annotation is throwing an exception
    // @Transactional
    @Override
    public CreateGuarantorsResponse handle(Command<CreateGuarantorsRequest> command) {

        final CommandWrapper commandRequest = new CommandWrapperBuilder().createGuarantor(command.getPayload().getLoanId())
                .withJson(apiJsonSerializerService.serialize(command.getPayload())).build();

        final CommandProcessingResult result = commandsSourceWritePlatformService.logCommandSource(commandRequest);

        return CreateGuarantorsResponse.builder().commandId(result.getCommandId()).officeId(result.getOfficeId()).loanId(result.getLoanId())
