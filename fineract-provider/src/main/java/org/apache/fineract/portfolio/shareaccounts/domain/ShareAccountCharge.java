...
public class ShareAccountCharge extends AbstractPersistableCustom {
  ...
  public static ShareAccountCharge createNewWithoutShareAccount(
    ...
    return new ShareAccountCharge(
        null, chargeDefinition, amountPayable, chargeTime, chargeCalculation, status);
  }
  ...
  private ShareAccountCharge(
      final ShareAccount shareAccount,
      final Charge chargeDefinition,
  ...
  public BigDecimal deriveChargeAmount(
      BigDecimal transactionAmount, final MonetaryCurrency currency) {
    ...
    if (ChargeCalculationType.fromInt(this.chargeCalculation)
      ...
      toReturnAmount =
          Money.of(currency, percentageOf(transactionAmount, this.percentage)).getAmount();
      this.amountPercentageAppliedTo = transactionAmount;
      this.amount =
          Money.of(currency, percentageOf(this.amountPercentageAppliedTo, this.percentage))
              .getAmount();
      this.amountPaid = null;
      this.amountOutstanding = calculateOutstanding();
      this.amountWaived = null;
  ...
  public BigDecimal updateChargeDetailsForAdditionalSharesRequest(
      final BigDecimal transactionAmount, final MonetaryCurrency currency) {
    BigDecimal toReturnAmount = amountOrPercentage;
    if (ChargeCalculationType.fromInt(this.chargeCalculation)
        == ChargeCalculationType.PERCENT_OF_AMOUNT) {
      toReturnAmount =
