...
public class CreateClientIdentifierCommandHandler
        implements CommandHandler<ClientIdentifierCreateNewRequest, ClientIdentifierCreateResponse> {

    private final ClientIdentifierWritePlatformService clientIdentifierWritePlatformService;

    @Transactional
    @Override
    public ClientIdentifierCreateResponse handle(Command<ClientIdentifierCreateNewRequest> command) {
        return this.clientIdentifierWritePlatformService.addClientIdentifier(command.getPayload().getClientId(), command.getPayload());
    }
}
