...
public class GuarantorReadPlatformServiceImpl implements GuarantorReadPlatformService {
  ...
  public GuarantorData retrieveGuarantor(final Long loanId, final Long guarantorId) {
    ...
    final GuarantorData guarantorData =
        this.jdbcTemplate.queryForObject(
            ...
            rm,
            new Object[] {
              AccountAssociationType.GUARANTOR_ACCOUNT_ASSOCIATION.getValue(), loanId, guarantorId
            });
  ...
  private static final class GuarantorMapper implements RowMapper<GuarantorData> {
    ...
    private GuarantorTransactionMapper guarantorTransactionMapper =
        new GuarantorTransactionMapper();
    private GuarantorFundingMapper guarantorFundingMapper =
        new GuarantorFundingMapper(guarantorTransactionMapper);

    private final StringBuilder sqlBuilder =
        new StringBuilder(
                " g.id as id, g.loan_id as loanId, g.client_reln_cv_id clientRelationshipTypeId,"
                    + " g.entity_id as entityId, g.type_enum guarantorType ,g.firstname as"
                    + " firstname, g.lastname as lastname, g.dob as dateOfBirth, g.address_line_1"
                    + " as addressLine1, g.address_line_2 as addressLine2, g.city as city, g.state"
                    + " as state, g.country as country, g.zip as zip, g.house_phone_number as"
                    + " housePhoneNumber, g.mobile_number as mobilePhoneNumber, g.comment as"
                    + " comment, ")
            ...
            .append(
                " left JOIN m_portfolio_account_associations aa on gfd.account_associations_id ="
                    + " aa.id and aa.is_active = 1 and aa.association_type_enum = ?") //
            ...
            .append(
                " left join m_deposit_account_on_hold_transaction oht on oht.id ="
  ...
  private static final class GuarantorFundingMapper implements RowMapper<GuarantorFundingData> {
    ...
    public GuarantorFundingMapper(final GuarantorTransactionMapper guarantorTransactionMapper) {
      ...
      sb.append(
          " gfd.amount_transfered_derived as amountTransfered, gfd.status_enum as statusEnum, ");
    ...
    public GuarantorFundingData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
      ...
      if (id != null && id > 0) {
        ...
        final BigDecimal amountTransfered =
            JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "amountTransfered");
        ...
        final PortfolioAccountData portfolioAccountData =
            PortfolioAccountData.lookup(savingsId, savingsAccountNumber);
        List<GuarantorTransactionData> guarantorTransactions = new ArrayList<>();
        if (this.guarantorTransactionMapper != null) {
          GuarantorTransactionData guarantorTransactionData =
  ...
  private static final class GuarantorTransactionMapper
      implements RowMapper<GuarantorTransactionData> {
    ...
    public GuarantorTransactionMapper() {
      ...
      sb.append(
          " oht.transaction_date as transactionDate, oht.is_reversed as transactionReversed ");
      sql = sb.toString();
    }
    ...
    @Override
    public GuarantorTransactionData mapRow(final ResultSet rs, final int rowNum)
        throws SQLException {
      ...
      if (id != null) {
        DepositAccountOnHoldTransactionData onHoldTransactionData =
            DepositAccountOnHoldTransactionData.instance(
                transactionId, amount, transactionType, date, transactionReversed);
  ...
  private GuarantorData mergeDetailsForClientOrStaffGuarantor(final GuarantorData guarantorData) {
    if (guarantorData.isExistingClient()) {
      final ClientData clientData =
          this.clientReadPlatformService.retrieveOne(guarantorData.getEntityId());
      ...
      final StaffData staffData =
          this.staffReadPlatformService.retrieveStaff(guarantorData.getEntityId());
  ...
  private static final class ObligeeMapper implements RowMapper<ObligeeData> {
    ...
    public ObligeeMapper() {
      StringBuilder sb =
          new StringBuilder(
              "SELECT cl.firstname, cl.lastname,cl.display_name as displayName, loan.account_no as"
                  + " loanAccountNumber, loan.principal_amount as loanAmount, gfd.amount as"
                  + " guaranteedAmount, ");
      sb.append(
          "gfd.amount_released_derived as amountReleased, gfd.amount_transfered_derived as"
              + " amountTransferred FROM m_guarantor mg");
      sb.append(" JOIN m_guarantor_funding_details gfd on mg.id = gfd.guarantor_id ");
      sb.append(
          " JOIN m_client mc ON mg.entity_id = mc.id AND mc.id = ? JOIN m_loan loan ON mg.loan_id"
              + " = loan.id ");
