...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Client",
          description =
              "Clients are people and businesses that have applied (or may apply) to an MFI for"
                  + " loans.\n"
                  + "\n"
...
public class ClientsApiResource {
  ...
  @ApiOperation(
      value = "Retrieve Client Details Template",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.GetClientsTemplateResponse.class)
  })
  public String retrieveTemplate(
      @Context final UriInfo uriInfo,
      @ApiParam(value = "officeId") @QueryParam("officeId") final Long officeId,
      @QueryParam("commandParam") @ApiParam(value = "commandParam") final String commandParam,
      @DefaultValue("false")
    ...
    this.context
        .authenticatedUser()
        .validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
    ...
    if (is(commandParam, "close")) {
      clientData =
          this.clientReadPlatformService.retrieveAllNarrations(
              ClientApiConstants.CLIENT_CLOSURE_REASON);
      ...
      clientData =
          this.clientReadPlatformService.retrieveAllNarrations(
              ClientApiConstants.CLIENT_REJECT_REASON);
    } else if (is(commandParam, "withdraw")) {
      clientData =
          this.clientReadPlatformService.retrieveAllNarrations(
              ClientApiConstants.CLIENT_WITHDRAW_REASON);
    } else {
      clientData =
          this.clientReadPlatformService.retrieveTemplate(officeId, staffInSelectedOfficeOnly);
    }

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, clientData, ClientApiConstants.CLIENT_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.GetClientsResponse.class)
  })
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.GetClientsClientIdResponse.class)
  })
  public String retrieveOne(
    ...
    if (settings.isTemplate()) {
      final ClientData templateData =
          this.clientReadPlatformService.retrieveTemplate(
              clientData.officeId(), staffInSelectedOfficeOnly);
  ...
  @ApiOperation(
      ...
      notes =
          "Note:\n\n"
              + "1. You can enter either:firstname/middlename/lastname - for a person (middlename"
              + " is optional) OR fullname - for a business or organisation (or person known by"
              + " one name).\n"
              + "\n"
              + "2.If address is enable(enable-address=true), then additional field called address"
              + " has to be passed.\n\n"
              + "Mandatory Fields: firstname and lastname OR fullname, officeId, active=true and"
              + " activationDate OR active=false, if(address enabled) address\n\n"
              + "Optional Fields: groupId, externalId, accountNo, staffId, mobileNo,"
              + " savingsProductId, genderId, clientTypeId, clientClassificationId")
  @ApiImplicitParams({
    @ApiImplicitParam(
        paramType = "body",
        dataType = "ClientData",
        required = true,
        type = "body",
        dataTypeClass = ClientsApiResourceSwagger.PostClientsRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.PostClientsResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Update a Client",
      notes =
          "Note: You can update any of the basic attributes of a client (but not its associations)"
              + " using this API.\n"
              + "\n"
              + "Changing the relationship between a client and its office is not supported"
              + " through this API. An API specific to handling transfers of clients between"
              + " offices is available for the same.\n"
              + "\n"
              + "The relationship between a client and a group must be removed through the Groups"
              + " API.")
  @ApiImplicitParams({
    @ApiImplicitParam(
        paramType = "body",
        ...
        required = true,
        type = "body",
        dataTypeClass = ClientsApiResourceSwagger.PutClientsClientIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.PutClientsClientIdResponse.class)
  })
  public String update(
      @ApiParam(value = "clientId") @PathParam("clientId") final Long clientId,
  ...
  @ApiOperation(
      ...
      notes =
          "If a client is in Pending state, you are allowed to Delete it. The delete is a 'hard"
              + " delete' and cannot be recovered from. Once clients become active or have loans"
              + " or savings associated with them, you cannot delete the client but you may Close"
              + " the client if they have left the program.")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        required = true,
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = ClientsApiResourceSwagger.DeleteClientsClientIdRequest.class)
  })
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "OK",
            response = ClientsApiResourceSwagger.DeleteClientsClientIdResponse.class)
      })
  ...
  @ApiOperation(
      value =
          "Activate a Client | Close a Client | Reject a Client | Withdraw a Client | Reactivate a"
              + " Client | UndoReject a Client | UndoWithdraw a Client | Assign a Staff | Unassign"
              + " a Staff | Update Default Savings Account | Propose a Client Transfer | Withdraw"
              + " a Client Transfer | Reject a Client Transfer | Accept a Client Transfer |"
              + " Propose and Accept a Client Transfer",
      notes =
          "Activate a Client:\n\n"
              + "Clients can be created in a Pending state. This API exists to enable client"
              + " activation (for when a client becomes an approved member of the financial"
              + " Institution).\n"
              + "\n"
              ...
              + "\n"
              + "If the client have any active loans/savingsAccount this API will result in an"
              + " error.\n\n"
              + "Reject a Client:\n\n"
              ...
              + "Withdraw a Client:\n\n"
              + "Client applications can be withdrawn when client is in a pending for activation"
              + " status.\n"
              + "\n"
              + "If the client is any other status, this API throws an error.\n\n"
              ...
              + "\n"
              + "The selected Staff should belong to the same office (or an officer higher up in"
              + " the hierarchy) as the Client he manages.\n\n"
              + "Unassign a Staff:\n\n"
              + "Allows you to unassign the Staff assigned to a Client.\n\n"
              + "Update Default Savings Account:\n\n"
              + "Allows you to modify or assign a default savings account for an existing"
              + " Client.\n"
              + "\n"
              + "The selected savings account should be one among the existing savings account for"
              ...
              + "\n"
              + "Withdrawal can happen only if the destination Branch (to which the transfer was"
              + " proposed) has not already accepted the transfer proposal\n\n"
              ...
              + "\n"
              + "The destination branch may also choose to link this client to a group (in which"
              + " case, any existing active JLG loan of the client is rescheduled to match the"
              + " meeting frequency of the group) and loan Officer at the time of accepting the"
              + " transfer\n\n"
              + "Propose and Accept a Client Transfer:\n\n"
              + "Abstraction over the Propose and Accept Client Transfer API's which enable a user"
              + " with Data Scope over both the Target and Destination Branches to directly"
              + " transfer a Client to the destination Office.\n\n"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = ClientsApiResourceSwagger.PostClientsClientIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.PostClientsClientIdResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve client accounts overview",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientsApiResourceSwagger.GetClientsClientIdAccountsResponse.class),
  ...
  public String retrieveAssociatedAccounts(
    ...
    final Set<String> CLIENT_ACCOUNTS_DATA_PARAMETERS =
        new HashSet<>(Arrays.asList("loanAccounts", "savingsAccounts", "shareAccounts"));

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.clientAccountSummaryToApiJsonSerializer.serialize(
        settings, clientAccount, CLIENT_ACCOUNTS_DATA_PARAMETERS);
  }
  ...
  public String postClientTemplate(
    ...
    final Long importDocumentId =
        bulkImportWorkbookService.importWorkbook(
            legalFormType, uploadedInputStream, fileDetail, locale, dateFormat);
  ...
  @Consumes({MediaType.APPLICATION_JSON})
  @Produces({MediaType.APPLICATION_JSON})
  public String retrieveTransferTemplate(
      @PathParam("clientId") final Long clientId, @Context final UriInfo uriInfo) {
    ...
    return this.toApiJsonSerializer.serialize(
        (transferDate != null ? new LocalDate(transferDate) : null));
  }
}
