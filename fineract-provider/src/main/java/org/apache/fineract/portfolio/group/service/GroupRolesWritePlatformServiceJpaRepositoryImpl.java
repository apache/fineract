...
public class GroupRolesWritePlatformServiceJpaRepositoryImpl
  ...
  public GroupRolesWritePlatformServiceJpaRepositoryImpl(
      ...
      final GroupRolesDataValidator fromApiJsonDeserializer,
  ...
  private void handleGroupDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("UNIQUE_GROUP_ROLES")) {
      final String clientId =
          command.stringValueOfParameterNamed(GroupingTypesApiConstants.clientIdParamName);
      final String roleId =
          command.stringValueOfParameterNamed(GroupingTypesApiConstants.roleParamName);
      final String errorMessageForUser =
          "Group Role with roleId `"
              + roleId
              + "`, clientId `"
              + clientId
              + "`, groupId `"
  ...
  public CommandProcessingResult updateRole(final JsonCommand command) {
    try {
      ...
      final GroupRole groupRole =
          this.groupRoleRepository.findOneWithNotFoundDetection(command.entityId());
