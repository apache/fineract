...
public class CalendarUtils {
  ...
  public static LocalDate getNextRecurringDate(
      final String recurringRule, final LocalDate seedDate, final LocalDate startDate) {
  ...
  public static LocalDate adjustDate(
      final LocalDate date, final LocalDate seedDate, final PeriodFrequencyType frequencyType) {
  ...
  private static LocalDate getNextRecurringDate(
      final Recur recur, final LocalDate seedDate, final LocalDate startDate) {
  ...
  public static Collection<LocalDate> getRecurringDates(
      final String recurringRule, final LocalDate seedDate, final LocalDate endDate) {

    final LocalDate periodStartDate = DateUtils.getLocalDateOfTenant();
    final LocalDate periodEndDate =
        (endDate == null) ? DateUtils.getLocalDateOfTenant().plusYears(5) : endDate;
    return getRecurringDates(recurringRule, seedDate, periodStartDate, periodEndDate);
  }

  public static Collection<LocalDate> getRecurringDatesFrom(
      final String recurringRule, final LocalDate seedDate, final LocalDate startDate) {
    final LocalDate periodStartDate =
        (startDate == null) ? DateUtils.getLocalDateOfTenant() : startDate;
  ...
  public static Collection<LocalDate> getRecurringDates(
      ...
      final LocalDate periodEndDate,
      final int maxCount,
      boolean isSkippMeetingOnFirstDay,
    ...
    return getRecurringDates(
        ...
        periodEndDate,
        maxCount,
        isSkippMeetingOnFirstDay,
        numberOfDays);
  }

  private static Collection<LocalDate> getRecurringDates(
      final Recur recur,
      final LocalDate seedDate,
      final LocalDate periodStartDate,
      final LocalDate periodEndDate,
  ...
  private static Collection<LocalDate> convertToLocalDateList(
    ...
    for (@SuppressWarnings("rawtypes") final Iterator iterator = dates.iterator();
        iterator.hasNext(); ) {
  ...
  private static Collection<LocalDate> skipMeetingOnFirstdayOfMonth(
      final Collection<LocalDate> recurringDates, final Integer numberOfDays) {
    final Collection<LocalDate> adjustedRecurringDates = new ArrayList<>();

    for (@SuppressWarnings("rawtypes") final Iterator iterator = recurringDates.iterator();
        iterator.hasNext(); ) {
  ...
  public static LocalDate adjustRecurringDate(
      final LocalDate recuringDate, final Integer numberOfDays) {
  ...
  public static String getRRuleReadable(final LocalDate startDate, final String recurringRule) {
    ...
    if (recur.getFrequency().equals(Recur.Frequency.DAILY)) {
      ...
      for (@SuppressWarnings("rawtypes") final Iterator iterator = weekDayList.iterator();
          iterator.hasNext(); ) {
      ...
      Integer nthDay = null;
      if (!nthDays.isEmpty()) nthDay = nthDays.get(0);
      NumberList monthDays = recur.getMonthDayList();
      Integer monthDay = null;
      if (!monthDays.isEmpty()) monthDay = monthDays.get(0);
      WeekDayList weekdays = recur.getDayList();
      WeekDay weekDay = null;
      if (!weekdays.isEmpty()) weekDay = weekdays.get(0);
      if (nthDay != null && weekDay != null) {
        ...
        if (recur.getInterval() == 1 || recur.getInterval() == -1) {
          humanReadable =
              "Monthly on "
                  + nthDayName.getCode().toLowerCase()
                  + " "
        ...
        if (recur.getInterval() == 1 || recur.getInterval() == -1) {
          ...
          humanReadable =
              "Every " + recur.getInterval() + " months on day " + startDate.getDayOfMonth();
        }
      }
    } else if (recur.getFrequency().equals(Recur.Frequency.YEARLY)) {
      if (recur.getInterval() == 1) {
        humanReadable =
            "Annually on " + startDate.toString("MMM") + " " + startDate.getDayOfMonth();
      } else {
        humanReadable =
            "Every "
                + recur.getInterval()
  ...
  public static boolean isValidRedurringDate(
      final String recurringRule, final LocalDate seedDate, final LocalDate date) {
    ...
    return isValidRecurringDate(
        recur, seedDate, date, isSkipRepaymentonFirstDayOfMonth, numberOfDays);
  }
  ...
  public static boolean isValidRecurringDate(
      final Recur recur,
      final LocalDate seedDate,
      final LocalDate date,
      boolean isSkipRepaymentonFirstDayOfMonth,
    ...
    final Collection<LocalDate> recurDate =
        getRecurringDates(
            recur,
            seedDate,
            startDate,
  ...
  public static String getMeetingFrequencyFromPeriodFrequencyType(
      final PeriodFrequencyType periodFrequency) {
  ...
  public static NthDayType getRepeatsOnNthDayOfMonth(final String recurringRule) {
    ...
    if (recur.getDayList().isEmpty()) monthDays = recur.getMonthDayList();
    else monthDays = recur.getSetPosList();
  ...
  public static LocalDate getFirstRepaymentMeetingDate(
  ...
  public static LocalDate getNewRepaymentMeetingDate(
      final String recurringRule,
      final LocalDate seedDate,
      final LocalDate oldRepaymentDate,
      final Integer loanRepaymentInterval,
      final String frequency,
    ...
    if (isValidRecurringDate(
        recur, seedDate, oldRepaymentDate, isSkipRepaymentOnFirstDayOfMonth, numberOfDays)) {
      return oldRepaymentDate;
    }
    LocalDate nextRepaymentDate =
        getNextRepaymentMeetingDate(
            recurringRule,
            seedDate,
            oldRepaymentDate,
  ...
  public static LocalDate getNextRepaymentMeetingDate(
      final String recurringRule,
      final LocalDate seedDate,
    ...
    if (isValidRecurringDate(
        recur, seedDate, repaymentDate, isSkipRepaymentOnFirstDayOfMonth, numberOfDays)) {
    ...
    final LocalDate nextRepaymentDate =
        getNextRecurringDate(modifiedRecur, seedDate, newRepaymentDate);

    newRepaymentDate =
        WorkingDaysUtil.getOffSetDateIfNonWorkingDay(
            newRepaymentDate, nextRepaymentDate, workingDays);
    if (isSkipRepaymentOnFirstDayOfMonth) {
      LocalDate newRepaymentDateTemp = adjustRecurringDate(newRepaymentDate, numberOfDays);
      return WorkingDaysUtil.getOffSetDateIfNonWorkingDay(
          newRepaymentDateTemp, nextRepaymentDate, workingDays);
    }
  ...
  public static void validateNthDayOfMonthFrequency(
      DataValidatorBuilder baseDataValidator,
      final String repeatsOnNthDayOfMonthParamName,
      final String repeatsOnDayParamName,
      final JsonElement element,
      final FromJsonHelper fromApiJsonHelper) {
    final Integer repeatsOnNthDayOfMonth =
        fromApiJsonHelper.extractIntegerSansLocaleNamed(repeatsOnNthDayOfMonthParamName, element);
    baseDataValidator
        .reset()
        ...
        .ignoreIfNull()
        .isOneOfTheseValues(
            NthDayType.ONE.getValue(),
            NthDayType.TWO.getValue(),
            NthDayType.THREE.getValue(),
            NthDayType.FOUR.getValue(),
            NthDayType.LAST.getValue(),
            NthDayType.ONDAY.getValue());
    final Integer repeatsOnDay =
        fromApiJsonHelper.extractIntegerSansLocaleNamed(repeatsOnDayParamName, element);
    baseDataValidator
        .reset()
    ...
    if (nthDayType != null && nthDayType != NthDayType.INVALID) {
      if (nthDayType == NthDayType.ONE
          || nthDayType == NthDayType.TWO
          || nthDayType == NthDayType.THREE
  ...
  public static LocalDate getNextRepaymentMeetingDate(
      ...
      boolean isSkipRepaymentOnFirstDayOfMonth,
      final Integer numberOfDays,
      boolean applyWorkingDays) {
    boolean isCalledFirstTime = true;
    return getNextRepaymentMeetingDate(
        recurringRule,
        seedDate,
        repaymentDate,
        loanRepaymentInterval,
  ...
  public static LocalDate getNextRepaymentMeetingDate(
    ...
    final Integer repaymentInterval =
        getMeetingIntervalFromFrequency(loanRepaymentInterval, frequency, recur);
    /*
     * Recurring dates should follow loanRepaymentInterval.
     *
     * e.g. The weekly meeting will have interval of 1, if the loan product
    ...
    if (applyWorkingDays) {
      if (WorkingDaysUtil.isNonWorkingDay(workingDays, newRepaymentDate)
          && WorkingDaysUtil.getRepaymentRescheduleType(workingDays, newRepaymentDate)
    ...
    if (isSkipRepaymentOnFirstDayOfMonth) {
      ...
      if (applyWorkingDays) {
        if (WorkingDaysUtil.isNonWorkingDay(workingDays, newRepaymentDateTemp)
            && WorkingDaysUtil.getRepaymentRescheduleType(workingDays, newRepaymentDateTemp)
                .isMoveToNextRepaymentDay()) {
          newRepaymentDate =
              getNextRepaymentMeetingDate(
                  recurringRule,
  ...
  public static Integer getMeetingIntervalFromFrequency(
      final Integer loanRepaymentInterval, final String frequency, final Recur recur) {
     ...
     */
    if (frequency.equals(Recur.Frequency.MONTHLY.name())
        && recur.getFrequency().equals(Recur.Frequency.WEEKLY)) {
