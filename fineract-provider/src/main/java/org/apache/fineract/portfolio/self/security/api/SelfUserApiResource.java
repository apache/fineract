...
public class SelfUserApiResource {
  ...
  @ApiOperation(
      ...
      notes =
          "This API can be used by Self Service user to update their own user information."
              + " Currently, \"password\" and \"repeatPassword\" are the only parameters"
              + " accepted.")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = SelfUserApiResourceSwagger.PutSelfUserRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = SelfUserApiResourceSwagger.PutSelfUserResponse.class)
  })
  public String update(@ApiParam(hidden = true) final String apiRequestBodyAsJson) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, apiRequestBodyAsJson, this.supportedParameters);
