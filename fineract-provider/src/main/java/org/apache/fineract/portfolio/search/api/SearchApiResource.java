...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Search API",
          description =
              "Search API allows to search scoped resources clients, loans and groups on specified"
                  + " fields.")
    })
public class SearchApiResource {
  ...
  public SearchApiResource(
      final SearchReadPlatformService searchReadPlatformService,
  ...
  @ApiOperation(
      value = "Retrive Adhoc Search query template",
      httpMethod = "GET",
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = SearchApiResourceSwagger.GetSearchResponse.class)
  })
  public String retrieveAdHocSearchQueryTemplate(@Context final UriInfo uriInfo) {

    final AdHocSearchQueryData templateData =
        this.searchReadPlatformService.retrieveAdHocQueryTemplate();
  ...
  public String searchData(
      ...
      @QueryParam("query") @ApiParam(value = "query") final String query,
      @QueryParam("resource") @ApiParam(value = "resource") final String resource,
      @DefaultValue("false") @QueryParam("exactMatch") @ApiParam(value = "exactMatch")
          Boolean exactMatch) {

    final SearchConditions searchConditions = new SearchConditions(query, resource, exactMatch);

    final Collection<SearchData> searchResults =
        this.searchReadPlatformService.retriveMatchingData(searchConditions);

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, searchResults, this.searchResponseParameters);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Adhoc query search",
      notes =
          "AdHocQuery search has more search options, it is a POST request, it uses request body"
              + " to send search parameters\n"
              + "\n"
              + "\n"
              ...
              + "minOutStandingAmountPercentage and maxOutStandingAmountPercentage OR"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = SearchApiResourceSwagger.PostAdhocQuerySearchRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = SearchApiResourceSwagger.PostAdhocQuerySearchResponse.class)
  })
  public String advancedSearch(@Context final UriInfo uriInfo, final String json) {
    ...
    final Collection<AdHocSearchQueryData> searchResults =
