...
public final class LoanApplicationTerms {
  ...
  public static LoanApplicationTerms assembleFrom(
    ...
    return new LoanApplicationTerms(
        ...
        repaymentEvery,
        repaymentPeriodFrequencyType,
        ((nthDay != null) ? nthDay.getValue() : null),
        dayOfWeek,
        amortizationMethod,
        interestMethod,
        interestRatePerPeriod,
  ...
  private LoanApplicationTerms(
    ...
    this.actualNumberOfRepayments =
        numberOfRepayments + getLoanTermVariations().adjustNumberOfRepayments();
  ...
  public Money adjustPrincipalIfLastRepaymentPeriod(
    ...
    if (totalPrincipalRemaining.isLessThanZero()) {
      ...
      final Money principalThreshold =
          principalForPeriod
              .multipliedBy(this.principalThresholdForLastInstalment)
              .dividedBy(100, MoneyHelper.getRoundingMode());
  ...
  public Money adjustInterestIfLastRepaymentPeriod(
      ...
      final Money totalInterestDueForLoan,
      final int periodNumber) {
  ...
  public Money calculateTotalInterestCharged(
      final PaymentPeriodsInOneYearCalculator calculator, final MathContext mc) {
    ...
    switch (this.interestMethod) {
      case FLAT:
        ...
        final Money totalGraceOnInterestCharged =
            totalInterestPerInstallment.multiplyRetainScale(
                getInterestChargingGrace(), mc.getRoundingMode());
      ...
      case FLAT:
        principalForInstallment =
            calculateTotalPrincipalPerPeriodWithoutGrace(
                mc, periodNumber, interestForThisInstallment);
        break;
      case DECLINING_BALANCE:
  ...
  public PrincipalInterest calculateTotalInterestForPeriod(
    ...
    switch (interestMethod) {
      case FLAT:
        if (this.isEqualAmortization()
            && this.totalInterestDue != null
            ...
            case EQUAL_PRINCIPAL:
              ...
              final PrincipalInterest result =
                  calculateTotalFlatInterestForPeriod(
                      calculator, periodNumber, mc, interestBroughtForwardDueToGrace);
      ...
      case DECLINING_BALANCE:
        ...
        } else if (isInterestFreeGracePeriodFromDate(
            interestCalculationGraceOnRepaymentPeriodFraction)) {
          interestForInstallment = interestForThisInstallmentAfterGrace;
        } else {
  ...
  private final boolean isLastRepaymentPeriod(
  ...
  private Money calculateTotalFlatInterestDueWithoutGrace(
    ...
    switch (this.interestMethod) {
      case FLAT:
        ...
        totalInterestDue =
            this.principal
                .minus(totalPrincipalAccountedForInterestCalcualtion)
  ...
  public BigDecimal calculatePeriodsBetweenDates(
    ...
    switch (this.repaymentPeriodFrequencyType) {
      ...
      case WEEKS:
        ...
        numberOfPeriods =
            numberOfPeriods
                .add(BigDecimal.valueOf(numberOfWeeks))
      ...
      case MONTHS:
        ...
        if (this.loanCalendar == null) {
          ...
          endDateAfterConsideringMonths =
              CalendarUtils.adjustDate(
                  endDateAfterConsideringMonths, getSeedDate(), this.repaymentPeriodFrequencyType);
        } else {
          ...
          if (!CalendarUtils.isValidRedurringDate(
            ...
            expectedStartDate =
                CalendarUtils.getNewRepaymentMeetingDate(
                    loanCalendar.getRecurrence(),
                    startDate.minusMonths(getRepaymentEvery()),
                    startDate.minusMonths(getRepaymentEvery()),
                    getRepaymentEvery(),
                    CalendarUtils.getMeetingFrequencyFromPeriodFrequencyType(
                        getLoanTermPeriodFrequencyType()),
          ...
          if (numberOfMonths == 0) {
            ...
            startDateAfterConsideringMonths =
                CalendarUtils.getNewRepaymentMeetingDate(
                    loanCalendar.getRecurrence(),
                    expectedStartDate,
                    expectedStartDate.plusMonths(numberOfMonths),
                    getRepaymentEvery(),
                    CalendarUtils.getMeetingFrequencyFromPeriodFrequencyType(
                        getLoanTermPeriodFrequencyType()),
                    this.holidayDetailDTO.getWorkingDays(),
          ...
          endDateAfterConsideringMonths =
              CalendarUtils.getNewRepaymentMeetingDate(
                  loanCalendar.getRecurrence(),
                  startDateAfterConsideringMonths,
                  startDateAfterConsideringMonths.plusDays(1),
                  getRepaymentEvery(),
                  CalendarUtils.getMeetingFrequencyFromPeriodFrequencyType(
                      getLoanTermPeriodFrequencyType()),
                  this.holidayDetailDTO.getWorkingDays(),
        ...
        int daysLeftAfterMonths =
            Days.daysBetween(startDateAfterConsideringMonths, endDate).getDays() + diffDays;
        int daysInPeriodAfterMonths =
            Days.daysBetween(startDateAfterConsideringMonths, endDateAfterConsideringMonths)
                .getDays();
        numberOfPeriods =
            numberOfPeriods
      ...
      case YEARS:
        ...
        int daysInPeriodAfterYears =
            Days.daysBetween(startDateAfterConsideringYears, endDateAfterConsideringYears)
                .getDays();
        numberOfPeriods =
            numberOfPeriods
                .add(BigDecimal.valueOf(numberOfYears))
  ...
  private Money flatInterestPerInstallment(
    ...
    Money interestPerInstallment =
        totalInterestForLoanTerm.dividedBy(
  ...
  private Money calculateTotalPrincipalPerPeriodWithoutGrace(
      final MathContext mc, final int periodNumber, Money interestForThisInstallment) {
    ...
    if (getFixedEmiAmount() == null) {
      principalPerPeriod =
          this.principal
              .minus(totalPrincipalAccounted)
              .dividedBy(totalRepaymentsWithCapitalPayment, mc.getRoundingMode())
      ...
      if (!isPrincipalGraceApplicableForThisPeriod(periodNumber)
        ...
        this.adjustPrincipalForFlatLoans =
            this.adjustPrincipalForFlatLoans.plus(
                principalPerPeriod
                    .minus(currentPeriodFixedPrincipalAmount)
      ...
      principalPerPeriod =
          Money.of(this.getCurrency(), getFixedEmiAmount()).minus(interestForThisInstallment);
  ...
  private Money calculateTotalFlatInterestForInstallmentAveragingOutGracePeriods(
    ...
    if (isInterestPaymentGraceApplicableForThisPeriod(periodNumber)) {
      ...
      Integer interestPaymentDuePeriods =
          calculateNumberOfRemainingInterestPaymentPeriods(
              this.actualNumberOfRepayments, this.excludePeriodsForCalculation);
      interestForInstallment =
          realTotalInterestForLoan.dividedBy(
              BigDecimal.valueOf(interestPaymentDuePeriods), mc.getRoundingMode());
      if (this.excludePeriodsForCalculation < this.periodsCompleted) {
        Money interestLeft = this.totalInterestDue.minus(this.totalInterestAccounted);
      ...
      if (!this.periodNumbersApplicableForInterestGrace.isEmpty()) {
        ...
        if (periodsElapsed > this.excludePeriodsForCalculation
          ...
          Integer interestDuePeriods =
              calculateNumberOfRemainingInterestPaymentPeriods(
                  this.actualNumberOfRepayments, periodsElapsed);
          interestForInstallment =
              interestLeft.dividedBy(Long.valueOf(interestDuePeriods), mc.getRoundingMode());
        }
      }
  ...
  private BigDecimal periodicInterestRate(
    ...
    return periodicInterestRate(
        calculator, mc, daysInMonthType, daysInYearType, periodStartDate, periodEndDate, false);
  }
  ...
  private Integer calculateNumberOfRemainingInterestPaymentPeriods(
      final Integer totalNumberOfRepaymentPeriods, int periodsElapsed) {
  ...
  private double paymentPerPeriod(
      final BigDecimal periodicInterestRate, final Money balance, final int periodsElapsed) {
    ...
    if (getFixedEmiAmount() == null) {
      ...
      if (this.installmentAmountInMultiplesOf != null) {
        installmentAmount =
            Money.roundToMultiplesOf(installmentAmount, this.installmentAmountInMultiplesOf);
      }
  ...
  private Money calculateDecliningInterestDueForInstallmentBeforeApplyingGrace(
    ...
    final BigDecimal periodicInterestRate =
        periodicInterestRate(
            ...
            periodEndDate);
    interestDue =
        outstandingBalance.multiplyRetainScale(periodicInterestRate, mc.getRoundingMode());
  ...
  public void updateFixedPrincipalAmount(
      final MathContext mc, final int periodNumber, final Money outstandingAmount) {
    final Integer numberOfPrincipalPaymentPeriods =
        calculateNumberOfRemainingPrincipalPaymentPeriods(
            this.actualNumberOfRepayments, periodNumber - 1);
    Money principal =
        outstandingAmount.dividedBy(numberOfPrincipalPaymentPeriods, mc.getRoundingMode());
    this.fixedPrincipalAmount = principal.getAmount();
  }

  private Integer calculateNumberOfRemainingPrincipalPaymentPeriods(
      final Integer totalNumberOfRepaymentPeriods, int periodsElapsed) {
    int principalFeePeriods = 0;
  ...
  private Money calculatePrincipalDueForInstallment(
      final int periodNumber, final Money totalDuePerInstallment, final Money periodInterest) {
  ...
  public LoanProductRelatedDetail toLoanProductRelatedDetail() {
    ...
    return LoanProductRelatedDetail.createFrom(
        currency,
  ...
  private boolean isFallingInRepaymentPeriod(LocalDate fromDate, LocalDate toDate) {
    ...
    if (this.interestCalculationPeriodMethod
        .getValue()
  ...
  public void updatePeriodNumberApplicableForPrincipalOrInterestGrace(
      final Integer periodsApplicationForGrace) {
  ...
  private void updateRecurringMoratoriumOnPrincipalPeriods(Integer periodNumber) {
    ...
    if (this.getRecurringMoratoriumOnPrincipalPeriods() > 0) {
      while (numberOfRepayments > 0) {
        isPrincipalGraceApplicableForThisPeriod =
            ((periodNumber > 0 && periodNumber <= getPrincipalGrace())
                || (periodNumber > 0
                    && (((periodNumber - getPrincipalGrace())
                            % (this.getRecurringMoratoriumOnPrincipalPeriods() + 1))
                        != 1)));
  ...
  public void updateAccountedTillPeriod(
      int periodNumber,
      Money totalPrincipalAccounted,
      Money totalInterestAccounted,
      int extendPeriods) {
