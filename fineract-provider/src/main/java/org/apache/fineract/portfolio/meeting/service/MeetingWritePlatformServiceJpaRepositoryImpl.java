...
public class MeetingWritePlatformServiceJpaRepositoryImpl implements MeetingWritePlatformService {
  ...
  public MeetingWritePlatformServiceJpaRepositoryImpl(
      final MeetingRepositoryWrapper meetingRepositoryWrapper,
      final MeetingRepository meetingRepository,
      final MeetingDataValidator meetingDataValidator,
  ...
  private CalendarInstance getCalendarInstance(final JsonCommand command) {
    ...
    CalendarEntityType entityType =
        CalendarEntityType
            .INVALID; // CalendarEntityType.valueOf(command.getSupportedEntityType().toUpperCase());
    ...
    if (calendarInstance == null) {
      ...
      final String defaultUserMessage =
          "No Calendar Instance details found for group with identifier "
              + entityId
              + " and calendar with identifier "
              + calendarId;
      throw new CalendarInstanceNotFoundException(
          postFix, defaultUserMessage, entityId, calendarId);
    }
    return calendarInstance;
  }

  private Collection<ClientAttendance> getClientsAttendance(
      final Meeting meeting, final JsonCommand command) {
    ...
    if (element.isJsonObject()) {
      if (topLevelJsonElement.has(clientsAttendanceParamName)
        ...
        for (int i = 0; i < array.size(); i++) {
          ...
          final Long clientId =
              this.fromApiJsonHelper.extractLongNamed(clientIdParamName, attendanceElement);
              ...
              if (!isChildClient) {
                final String defaultUserMessage =
                    "Client with identifier "
                        ...
                        + " is not in center "
                        + meeting.entityId();
                throw new ClientNotInGroupException(
                    "client.not.in.center", defaultUserMessage, clientId, meeting.entityId());
              }
            }
          }

          final ClientAttendance clientAttendance =
              ClientAttendance.createClientAttendance(client, meeting, attendanceTypeId);
  ...
  public CommandProcessingResult updateMeeting(final JsonCommand command) {
    ...
    final Map<String, Object> changes =
        meetingForUpdate.update(command, isSkipRepaymentOnFirstMonth, numberOfDays);
  ...
  public CommandProcessingResult deleteMeeting(final Long meetingId) {
    final Meeting meetingForDelete =
        this.meetingRepositoryWrapper.findOneWithNotFoundDetection(meetingId);
  ...
  public CommandProcessingResult saveOrUpdateAttendance(final JsonCommand command) {
    ...
    final Meeting meetingForUpdate =
        this.meetingRepositoryWrapper.findOneWithNotFoundDetection(command.entityId());
    final Collection<ClientAttendance> clientsAttendance =
        getClientsAttendance(meetingForUpdate, command);
  ...
  private void handleMeetingDataIntegrityIssues(
      final Date meetingDate, final DataIntegrityViolationException dve) {
    ...
    if (realCause.getMessage().contains("unique_calendar_instance_id_meeting_date")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.meeting.duplicate",
          "A meeting with date '" + meetingDateLocal + "' already exists",
          meetingDateParamName,
          meetingDateLocal);
    }

    throw new PlatformDataIntegrityException(
        "error.msg.meeting.unknown.data.integrity.issue",
  ...
  public void updateCollectionSheetAttendance(final JsonCommand command) {
    ...
    final Boolean isTransactionDateOnNonMeetingDate =
        command.booleanPrimitiveValueOfParameterNamed(isTransactionDateOnNonMeetingDateParamName);

    try {
      final CalendarInstance calendarInstance = getCalendarInstance(command);

      final Meeting meeting =
          this.meetingRepository.findByCalendarInstanceIdAndMeetingDate(
              calendarInstance.getId(), meetingDate);
      Boolean isSkipRepaymentOnFirstMonth = false;
      Integer numberOfDays = 0;
      boolean isSkipRepaymentOnFirstMonthEnabled =
          configurationDomainService.isSkippingMeetingOnFirstDayOfMonthEnabled();
