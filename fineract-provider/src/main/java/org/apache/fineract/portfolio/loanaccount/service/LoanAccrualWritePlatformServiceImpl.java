...
public class LoanAccrualWritePlatformServiceImpl implements LoanAccrualWritePlatformService {
  ...
  @Autowired
  public LoanAccrualWritePlatformServiceImpl(
  ...
  @Override
  @Transactional
  public void addAccrualAccounting(
      final Long loanId, final Collection<LoanScheduleAccrualData> loanScheduleAccrualDatas)
      throws Exception {
    ...
    for (final LoanScheduleAccrualData accrualData : loanScheduleAccrualDatas) {
      ...
      updateCharges(
          chargeData,
          accrualData,
          accrualData.getFromDateAsLocaldate(),
  ...
  @Override
  @Transactional
  public void addPeriodicAccruals(
    ...
    for (final LoanScheduleAccrualData accrualData : loanScheduleAccrualDatas) {
      ...
      if (accrualData.getDueDateAsLocaldate().isAfter(tilldate)) {
        ...
        if (accruredTill == null || accruredTill.isBefore(tilldate)) {
          ...
          updateInterestIncome(
              accrualData, loanWaiverTansactionData, loanWaiverScheduleData, tilldate);
  ...
  private void addAccrualTillSpecificDate(
      final LocalDate tilldate, final LoanScheduleAccrualData accrualData) throws Exception {
    ...
    int totalNumberOfDays =
        Days.daysBetween(interestStartDate, accrualData.getDueDateAsLocaldate()).getDays();
    ...
    interestportion =
        interestportion.setScale(
            accrualData.getCurrencyData().decimalPlaces(), MoneyHelper.getRoundingMode());
  ...
  private void addAccrualAccounting(
      LoanScheduleAccrualData scheduleAccrualData,
      BigDecimal amount,
      BigDecimal interestportion,
      BigDecimal totalAccInterest,
      BigDecimal feeportion,
      BigDecimal totalAccFee,
      BigDecimal penaltyportion,
      BigDecimal totalAccPenalty,
      final LocalDate accruedTill)
      throws DataAccessException {
    String transactionSql =
        "INSERT INTO m_loan_transaction "
            + " (loan_id,office_id,is_reversed,transaction_type_enum,transaction_date,amount,interest_portion_derived,fee_charges_portion_derived,penalty_charges_portion_derived,"
            + " submitted_on_date) VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?)";
    this.jdbcTemplate.update(
        transactionSql,
        scheduleAccrualData.getLoanId(),
        scheduleAccrualData.getOfficeId(),
    ...
    String repaymetUpdatesql =
        "UPDATE m_loan_repayment_schedule SET accrual_interest_derived=?,"
            + " accrual_fee_charges_derived=?, accrual_penalty_charges_derived=? WHERE  id=?";
    this.jdbcTemplate.update(
        repaymetUpdatesql,
        totalAccInterest,
        totalAccFee,
  ...
  public Map<String, Object> toMapData(
      ...
      final BigDecimal feeportion,
      final BigDecimal penaltyportion,
      final LoanScheduleAccrualData loanScheduleAccrualData,
  ...
  private void updateCharges(
      final Collection<LoanChargeData> chargesData,
    ...
    for (LoanChargeData loanCharge : chargesData) {
      ...
      if (loanCharge.getDueDate() == null) {
        if (loanCharge.isInstallmentFee() && accrualData.getDueDateAsLocaldate().isEqual(endDate)) {
          Collection<LoanInstallmentChargeData> installmentData =
              loanCharge.getInstallmentChargeData();
          for (LoanInstallmentChargeData installmentChargeData : installmentData) {

            if (installmentChargeData
                .getInstallmentNumber()
                .equals(accrualData.getInstallmentNumber())) {
              ...
              if (installmentChargeData.getAmountUnrecognized() != null) {
                accruableForInstallment =
                    accruableForInstallment.subtract(installmentChargeData.getAmountUnrecognized());
              }
              chargeAmount = accruableForInstallment;
              boolean canAddCharge = chargeAmount.compareTo(BigDecimal.ZERO) == 1;
              if (canAddCharge
                  && (installmentChargeData.getAmountAccrued() == null
                ...
                if (installmentChargeData.getAmountAccrued() != null) {
                  amountForAccrual =
                      chargeAmount.subtract(installmentChargeData.getAmountAccrued());
                }
        ...
        boolean canAddCharge = chargeAmount.compareTo(BigDecimal.ZERO) == 1;
        if (canAddCharge
            && (loanCharge.getAmountAccrued() == null
  ...
  private void updateInterestIncome(
    ...
    if (accrualData.getWaivedInterestIncome() != null) {
      ...
      for (LoanSchedulePeriodData loanSchedulePeriodData : loanSchedulePeriodDatas) {
        if (recognized.compareTo(BigDecimal.ZERO) != 1
            && unrecognized.compareTo(BigDecimal.ZERO) != 1
            && iterator.hasNext()) {
