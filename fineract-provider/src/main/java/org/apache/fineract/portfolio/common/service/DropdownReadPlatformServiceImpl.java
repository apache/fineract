...
public class DropdownReadPlatformServiceImpl implements DropdownReadPlatformService {
    ...
    public List<EnumOptionData> retrievePeriodFrequencyTypeOptions() {
        return Arrays.asList(termFrequencyType(PeriodFrequencyType.DAYS, "frequency"),
    ...
    public List<EnumOptionData> retrieveDaysInMonthTypeOptions() {
        ...
        return Arrays.asList(daysInMonthType(DaysInMonthType.ACTUAL), daysInMonthType(DaysInMonthType.DAYS_30));
    }

    @Override
    public List<EnumOptionData> retrieveDaysInYearTypeOptions() {

        return Arrays.asList(daysInYearType(DaysInYearType.ACTUAL), daysInYearType(DaysInYearType.DAYS_360),
                daysInYearType(DaysInYearType.DAYS_364), daysInYearType(DaysInYearType.DAYS_365));
    }
