...
public class LoanInstallmentCharge extends AbstractPersistableCustom
  ...
  public Money updatePaidAmountBy(final Money incrementBy, final Money feeAmount) {
    ...
    Money amountFromChargePayment =
        Money.of(incrementBy.getCurrency(), this.amountThroughChargePayment);
  ...
  private void updateAmountThroughChargePayment(final MonetaryCurrency currency) {
    ...
    if (amountThroughChargePayment.isGreaterThanZero()
        && amountThroughChargePayment.isGreaterThan(this.getAmount(currency))) {
      this.amountThroughChargePayment = this.getAmount();
    }
  }

  public Money updateWaivedAndAmountPaidThroughChargePaymentAmount(
