...
public class ProductMixCreateCommandHandler implements CommandHandler<ProductMixCreateRequest, ProductMixCreateResponse> {

    private final ProductMixWritePlatformService writePlatformService;

    @Retry(name = "commandProductMixCreate", fallbackMethod = "fallback")
    @Override
    @Transactional
    public ProductMixCreateResponse handle(Command<ProductMixCreateRequest> command) {
        return writePlatformService.createProductMix(command.getPayload());
    }

    @Override
    public ProductMixCreateResponse fallback(Command<ProductMixCreateRequest> command, Throwable t) {
