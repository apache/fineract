...
public class LoanTermVariationsData implements Comparable<LoanTermVariationsData> {
  ...
  public LoanTermVariationsData(
      final Long id,
      final EnumOptionData termType,
      final LocalDate termVariationApplicableFrom,
  ...
  private boolean occursOnDayFromAndUpTo(
      final LocalDate fromNotInclusive, final LocalDate upToInclusive, final LocalDate target) {
  ...
  public int compareTo(LoanTermVariationsData o) {
    ...
    if (comparsion == 0) {
      if (o.getTermVariationType().isDueDateVariation()
          || o.getTermVariationType().isInsertInstallment()) {
        comparsion = 1;
      }
    }
    return comparsion;
