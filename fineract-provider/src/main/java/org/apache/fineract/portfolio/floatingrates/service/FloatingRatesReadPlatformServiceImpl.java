...
public class FloatingRatesReadPlatformServiceImpl implements FloatingRatesReadPlatformService {
  ...
  private final class FloatingRatePeriodRowMapper implements RowMapper<FloatingRatePeriodData> {
    ...
    @Override
    public FloatingRatePeriodData mapRow(
  ...
  private final class FloatingRateLookupMapper implements RowMapper<FloatingRateData> {
    ...
    public FloatingRateData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
      ...
      final boolean isBaseLendingRate = rs.getBoolean("isBaseLendingRate");
      return new FloatingRateData(
          id, name, isBaseLendingRate, true, null, null, null, null, null, null);
    }
  ...
  private final class FloatingInterestRatePeriodRowMapper
      implements RowMapper<InterestRatePeriodData> {
    ...
    private final StringBuilder sqlQuery =
        new StringBuilder()
            ...
            .append(
                "left join m_floating_rates_periods as linkedrateperiods on (linkedrate.id ="
                    + " linkedrateperiods.floating_rates_id and linkedrateperiods.is_active = 1) ")
            .append("left join ( ")
            ...
            .append(
                "    left join m_floating_rates_periods as blrperiods on (blr.id ="
                    + " blrperiods.floating_rates_id and blrperiods.is_active = 1) ")
            .append("    where blr.is_base_lending_rate = 1 and blr.is_active = 1 ")
            .append(
                ") as baserate on (linkedrateperiods.is_differential_to_base_lending_rate = 1 and"
