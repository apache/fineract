...
@Path("/v2/groups")
@Component
@Tag(name = "GroupsV2", description = "V2 endpoint that always returns a paged response (totalFilteredRecords + pageItems) for groups, removing the ambiguous paged response typing of the v1 endpoint.")
@RequiredArgsConstructor
public class GroupsV2ApiResource {

    private final PlatformSecurityContext context;
    private final GroupsV2ApiDelegate delegate;
    private final SqlValidator sqlValidator;

    @GET
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "List Groups", operationId = "retrieveAllGroupsV2", description = "Get a paged list of groups that match the criteria supplied and sorted by hierarchy. Unlike the v1 endpoint, this endpoint always returns a paged response containing totalFilteredRecords and pageItems.\n\n"
            + "Example Requests:\n\n" + "groups\n\n" + "groups?fields=name,officeName,joinedDate\n\n" + "groups?offset=10&limit=50\n\n"
            + "groups?orderBy=name&sortOrder=DESC")
    public Page<GroupGeneralData> retrieveAllGroups(@QueryParam("officeId") @Parameter(description = "officeId") final Long officeId,
        ...
        return delegate.retrieveAllGroups(searchParameters, parameters);
    }
}
