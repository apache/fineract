...
public class CollectionSheetApiResourse {
  ...
  @Autowired
  public CollectionSheetApiResourse(
  ...
  @ApiOperation(
      value = "Generate Individual Collection Sheet | Save Collection Sheet",
      httpMethod = "POST",
      notes =
          "Generate Individual Collection Sheet:\n\n"
              + "This Api retrieves repayment details of all individual loans under a office as on"
              + " a specified meeting date.\n\n"
              + "Save Collection Sheet:\n\n"
              + "This Api allows the loan officer to perform bulk repayments of individual loans"
              + " and deposit of mandatory savings on a given meeting date.")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        required = true,
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = CollectionSheetApiResourceSwagger.PostCollectionSheetRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = CollectionSheetApiResourceSwagger.PostCollectionSheetResponse.class)
  })
  public String generateCollectionSheet(
    ...
    if (is(commandParam, "generateCollectionSheet")) {
      this.context
          .authenticatedUser()
          .validateHasReadPermission(CollectionSheetConstants.COLLECTIONSHEET_RESOURCE_NAME);
      ...
      final IndividualCollectionSheetData collectionSheet =
          this.collectionSheetReadPlatformService.generateIndividualCollectionSheet(query);
