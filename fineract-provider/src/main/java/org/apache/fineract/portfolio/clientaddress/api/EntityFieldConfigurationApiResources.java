...
public class EntityFieldConfigurationApiResources 
{
    ...
    @Autowired
    public EntityFieldConfigurationApiResources(final PlatformSecurityContext context,final AddressReadPlatformServiceImpl readPlatformService,
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String addEntityConfiguration(@PathParam("entity")final String entityname,final String apiRequestBodyAsJson) 
    {

        final CommandWrapper commandRequest = new CommandWrapperBuilder().addEntityFieldConfiguration(entityname).withJson(apiRequestBodyAsJson).build();
