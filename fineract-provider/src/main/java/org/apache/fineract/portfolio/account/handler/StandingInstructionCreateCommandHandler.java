...
public class StandingInstructionCreateCommandHandler
        implements CommandHandler<StandingInstructionCreationRequest, StandingInstructionCreateResponse> {

    private final StandingInstructionWriteService writeService;

    @Retry(name = "commandStandingInstructionCreate", fallbackMethod = "fallback")
    @Override
    @Transactional
    public StandingInstructionCreateResponse handle(Command<StandingInstructionCreationRequest> command) {
        return writeService.create(command.getPayload());
    }

    @Override
    public StandingInstructionCreateResponse fallback(Command<StandingInstructionCreationRequest> command, Throwable t) {
