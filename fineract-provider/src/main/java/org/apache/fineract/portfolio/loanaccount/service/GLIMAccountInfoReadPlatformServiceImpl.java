...
public class GLIMAccountInfoReadPlatformServiceImpl implements GLIMAccountInfoReadPlatformService {
  ...
  private static final class GLIMFieldsMapper
      implements RowMapper<GroupLoanIndividualMonitoringAccountData> {
    public String schema() {
      return "glim.id as glimId,ln.group_id as groupId,glim.account_number as accountNumber,"
          + " ln.account_no as childAccountNumber,ln.principal_amount as"
          + " childPrincipalAmount,glim.principal_amount as"
          + " parentPrincipalAmount,glim.child_accounts_count as"
          + " childAccountsCount,glim.loan_status_id as loanStatus from glim_accounts"
          + " glim,m_loan ln where glim.id=ln.glim_id";
    }

    @Override
    public GroupLoanIndividualMonitoringAccountData mapRow(
      ...
      return GroupLoanIndividualMonitoringAccountData.getInstance(
          glimId,
          groupId,
          accountNumber,
          childAccountNumber,
  ...
  @Override
  public Collection<GroupLoanIndividualMonitoringAccountData> findGlimAccountsByGLIMId(
      final Long glimId) {
  ...
  @Override
  public Collection<GroupLoanIndividualMonitoringAccountData> findGlimAccountsByGroupId(
      String groupId) {
    this.context.authenticatedUser();
  ...
  @Override
  public Collection<GroupLoanIndividualMonitoringAccountData> findGlimAccountByGroupId(
      String groupId) {
    this.context.authenticatedUser();
  ...
  @Override
  public Collection<GroupLoanIndividualMonitoringAccountData> findGlimAccountByParentAccountId(
  ...
  @Override
  public Collection<GroupLoanIndividualMonitoringAccountData> findGlimAccountByGroupIdandAccountNo(
  ...
  public Collection<GLIMContainer> findGlimAccount(Long groupId) {
    this.context.authenticatedUser();
    Collection<GroupLoanIndividualMonitoringAccountData> glimInfo =
        findGlimAccountByGroupId(groupId + "");
    ...
    for (GroupLoanIndividualMonitoringAccountData glimAccount : glimInfo) {
      ...
      List<LoanAccountSummaryData> childLoans =
          accountDetailsReadPlatforService.retrieveLoanAccountDetailsByGroupIdAndGlimAccountNumber(
              groupId, glimAccount.getAccountNumber());
      glimAccounts.add(
          new GLIMContainer(
              glimAccount.getGlimId(),
              glimAccount.getGroupId(),
              glimAccount.getAccountNumber(),
              childLoans,
              glimAccount.getParentPrincipalAmount(),
              glimAccount.getLoanStatus()));
    }
  ...
  @Override
  public Collection<GLIMContainer> findGlimAccountbyGroupAndAccount(
      Long groupId, String accountNo) {
    this.context.authenticatedUser();
    Collection<GroupLoanIndividualMonitoringAccountData> glimInfo =
        findGlimAccountByGroupIdandAccountNo(groupId + "", accountNo + "");
  ...
  private static final class GLIMMapper
      implements RowMapper<GroupLoanIndividualMonitoringAccountData> {
    public String schema() {
      return "glim.id as glimId,glim.group_id as groupId,glim.account_number as"
          + " accountNumber,glim.principal_amount as"
          + " principalAmount,glim.child_accounts_count as"
          + " childAccountsCount,glim.loan_status_id as loanStatus from glim_accounts glim";
    }
    ...
    public GroupLoanIndividualMonitoringAccountData mapRow(
      ...
      return GroupLoanIndividualMonitoringAccountData.getInstance1(
          glimId, groupId, accountNumber, principalAmount, loanStatus);
    }
  }

  private static final class GLIMRepaymentMapper implements RowMapper<GlimRepaymentTemplate> {
    public String schema() {
      return "glim.id as glimId,loan.group_id as groupId,client.id as clientId,glim.account_number"
          + " as parentLoanAccountNo,glim.principal_amount as parentPrincipalAmount,loan.id"
          + " as childLoanId,loan.account_no as childLoanAccountNo,loan.approved_principal"
          + " as childPrincipalAmount,client.display_name as clientName from glim_accounts"
          + " glim left join m_loan loan on loan.glim_id=glim.id left join m_client client"
          + " on client.id=loan.client_id";
    }

    @Override
    public GlimRepaymentTemplate mapRow(
        final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
      ...
      return GlimRepaymentTemplate.getInstance(
          glimId,
          groupId,
          clientId,
          clientName,
