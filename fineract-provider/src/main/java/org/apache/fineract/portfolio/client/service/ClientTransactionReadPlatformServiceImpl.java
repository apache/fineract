...
public class ClientTransactionReadPlatformServiceImpl
  ...
  private static final class ClientTransactionMapper implements RowMapper<ClientTransactionData> {
    ...
    public ClientTransactionMapper() {
      ...
      sqlBuilder.append(
          "pd.receipt_number as receiptNumber, pd.bank_number as bankNumber,pd.routing_code as"
              + " routingCode,  ");
      sqlBuilder.append(
          "tr.currency_code as currencyCode, curr.decimal_places as currencyDigits,"
              + " curr.currency_multiplesof as inMultiplesOf, ");
      sqlBuilder.append(
    ...
    @Override
    public ClientTransactionData mapRow(
      ...
      final EnumOptionData transactionType =
          ClientEnumerations.clientTransactionType(transactionTypeInt);
      ...
      if (ClientTransactionType.fromInt(transactionType.getId().intValue())
          .equals(ClientTransactionType.PAY_CHARGE)) {
      ...
      return ClientTransactionData.create(
          id,
          officeId,
          officeName,
          transactionType,
  ...
  public Page<ClientTransactionData> retrieveAllTransactions(
      Long clientId, SearchParameters searchParameters) {
  ...
  public ClientTransactionData retrieveTransaction(Long clientId, Long transactionId) {
    try {
      final String sql =
          "select " + this.clientTransactionMapper.schema() + " where c.id = ? and tr.id= ?";
      return this.jdbcTemplate.queryForObject(
          sql, this.clientTransactionMapper, new Object[] {clientId, transactionId});
