...
import org.apache.fineract.portfolio.savings.exception.PostInterestAsOnException;
import org.apache.fineract.portfolio.savings.exception.PostInterestAsOnException.PostInterestAsOnException_Enum;
...
public class SavingsAccountWritePlatformServiceJpaRepositoryImpl implements SavingsAccountWritePlatformService {
    ...
    public CommandProcessingResult postInterest(final JsonCommand command) {
      ...
      final LocalDate transactionDate = command.localDateValueOfParameterNamed("transactionDate");
        ...
        if (postInterestAs == true) {
            ...
            throw new PostInterestAsOnException(PostInterestAsOnException_Enum.VALID_DATE); }
            if (transactionDate.isBefore(account.accountSubmittedOrActivationDate())) { throw new PostInterestAsOnException(
                    PostInterestAsOnException_Enum.ACTIVATION_DATE); }
            ...
            for (SavingsAccountTransaction savingTransaction : savingTransactions) {
                if (transactionDate.toDate().before(savingTransaction.getDateOf())) { throw new PostInterestAsOnException(
                        PostInterestAsOnException_Enum.LAST_TRANSACTION_DATE); }
            }

            LocalDate today = DateUtils.getLocalDateOfTenant();
            if (transactionDate.isAfter(today)) { throw new PostInterestAsOnException(PostInterestAsOnException_Enum.FUTURE_DATE); }
    ...
    public CommandProcessingResult close(final Long savingsId, final JsonCommand command) {
        ...
        final boolean isPostInterest = command.booleanPrimitiveValueOfParameterNamed(SavingsApiConstants.closeSavingsAccountValidation);
