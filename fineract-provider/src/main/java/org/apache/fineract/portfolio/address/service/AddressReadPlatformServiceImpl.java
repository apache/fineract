...
public class AddressReadPlatformServiceImpl implements AddressReadPlatformService {
  ...
  @Autowired
  public AddressReadPlatformServiceImpl(
  ...
  private static final class AddFieldsMapper implements RowMapper<AddressData> {
    public String schema() {
      return "addr.id as id,client.id as client_id,addr.street as street,addr.address_line_1 as"
  ...
  private static final class AddMapper implements RowMapper<AddressData> {
    public String schema() {
      return "cv2.code_value as addressType,ca.client_id as client_id,addr.id as"
          + " id,ca.address_type_id as addresstyp,ca.is_active as is_active,addr.street as"
          + " street,addr.address_line_1 as address_line_1,addr.address_line_2 as"
          + " address_line_2,addr.address_line_3 as address_line_3,addr.town_village as"
          + " town_village, addr.city as city,addr.county_district as"
          + " county_district,addr.state_province_id as state_province_id,cv.code_value as"
          + " state_name, addr.country_id as country_id,c.code_value as"
          + " country_name,addr.postal_code as postal_code,addr.latitude as"
          + " latitude,addr.longitude as longitude,addr.created_by as"
          + " created_by,addr.created_on as created_on,addr.updated_by as"
          + " updated_by,addr.updated_on as updated_on from m_address addr left join"
          + " m_code_value cv on addr.state_province_id=cv.id left join  m_code_value c on"
          + " addr.country_id=c.id join m_client_address ca on addr.id= ca.address_id join"
          + " m_code_value cv2 on ca.address_type_id=cv2.id";
    }
  ...
  @Override
  public Collection<AddressData> retrieveAddressbyTypeAndStatus(
      final long clientid, final long typeid, final String status) {
    this.context.authenticatedUser();
    Boolean temp = false;
    ...
    final AddMapper rm = new AddMapper();
    final String sql =
        "select " + rm.schema() + " and ca.client_id=? and ca.address_type_id=? and ca.is_active=?";
  ...
  public AddressData retrieveTemplate() {
    final List<CodeValueData> countryoptions =
        new ArrayList<>(this.readService.retrieveCodeValuesByCode("COUNTRY"));

    final List<CodeValueData> StateOptions =
        new ArrayList<>(this.readService.retrieveCodeValuesByCode("STATE"));

    final List<CodeValueData> addressTypeOptions =
        new ArrayList<>(this.readService.retrieveCodeValuesByCode("ADDRESS_TYPE"));
