...
public class DepositProductReadPlatformServiceImpl implements DepositProductReadPlatformService {
  ...
  private final FixedDepositProductMapper fixedDepositProductRowMapper =
      new FixedDepositProductMapper();
  private final RecurringDepositProductMapper recurringDepositProductRowMapper =
      new RecurringDepositProductMapper();
  private final DepositProductLookupMapper depositProductLookupsRowMapper =
      new DepositProductLookupMapper();

  @Autowired
  public DepositProductReadPlatformServiceImpl(
  ...
  public Collection<DepositProductData> retrieveAll(final DepositAccountType depositAccountType) {
    ...
    final DepositProductMapper depositProductMapper =
        this.getDepositProductMapper(depositAccountType);
    ...
    return this.jdbcTemplate.query(
        sqlBuilder.toString(), depositProductMapper, new Object[] {depositAccountType.getValue()});
  }

  @Override
  public Collection<DepositProductData> retrieveAllForLookup(
    ...
    return this.jdbcTemplate.query(
        sqlBuilder.toString(),
        this.depositProductLookupsRowMapper,
        new Object[] {depositAccountType.getValue()});
  }

  @Override
  public DepositProductData retrieveOne(
      final DepositAccountType depositAccountType, final Long fixedDepositProductId) {
    try {
      this.context.authenticatedUser();
  ...
  private abstract static class DepositProductMapper implements RowMapper<DepositProductData> {
    ...
    public DepositProductMapper() {
      final StringBuilder sqlBuilder = new StringBuilder(400);
      sqlBuilder.append(
          "sp.id as id, sp.name as name, sp.short_name as shortName, sp.description as"
              + " description, ");
      sqlBuilder.append(
  ...
  private static class FixedDepositProductMapper extends DepositProductMapper {
    ...
    public FixedDepositProductMapper() {
      ...
      sqlBuilder.append(
          "dptp.min_deposit_amount as minDepositAmount, dptp.deposit_amount as depositAmount, ");
      sqlBuilder.append("dptp.max_deposit_amount as maxDepositAmount ");
      sqlBuilder.append("from m_savings_product sp ");
      sqlBuilder.append(
          "left join m_deposit_product_term_and_preclosure dptp on sp.id=dptp.savings_product_id ");
    ...
    @Override
    public FixedDepositProductData mapRow(
      ...
      return FixedDepositProductData.instance(
          depositProductData,
  ...
  private static class RecurringDepositProductMapper extends DepositProductMapper {
    ...
    public RecurringDepositProductMapper() {
      ...
      sqlBuilder.append(
          "left join m_deposit_product_term_and_preclosure dptp on sp.id=dptp.savings_product_id ");
      sqlBuilder.append(
          "left join m_deposit_product_recurring_detail dprd on sp.id=dprd.savings_product_id ");
      sqlBuilder.append("join m_currency curr on curr.code = sp.currency_code ");
      sqlBuilder.append(" left join m_tax_group tg on tg.id = sp.tax_group_id  ");
  ...
  private DepositProductMapper getDepositProductMapper(
      final DepositAccountType depositAccountType) {
    if (depositAccountType.isFixedDeposit()) {
