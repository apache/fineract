...
public class ExecuteStandingInstructionsWorkerConfig {
    ...
    public Step executeStandingInstructionsWorkerStep() {
        ...
        final ChunkOrientedStepBuilder<StandingInstructionData, DueStandingInstruction> stepBuilder = stepBuilderFactory
                .get("Execute Standing Instructions worker - Step") //
                .inputChannel(inboundRequests) //
                .<StandingInstructionData, DueStandingInstruction>chunk(chunkSize) //
                ...
                // pad the mandate's history. ConcurrencyFailureException needs no entry of its own - it is a
                // TransientDataAccessException.
                .retryPolicy(StepRetryPolicyFactory.immediateRetryPolicy(
                        propertyService.getRetryLimit(ExecuteStandingInstructionsConstant.JOB_NAME), TransientDataAccessException.class,
                        AbstractPlatformServiceUnavailableException.class)) //
                .skipPolicy(new StandingInstructionSkipPolicy()) //
                .listener(standingInstructionItemListener()) //
                .transactionManager(transactionManager);

        // No task executor is registered, deliberately. Batch 6's ChunkOrientedStep keeps the chunk transaction on the
        // step thread and, when a task executor is present, submits the ITEMS of a chunk to it. An instruction would
        // then be executed outside the chunk transaction, which is precisely what this job exists to prevent: the
        // chunk-then-single-item replay only guarantees a failed instruction cannot revert a sibling while both are
        // written under the chunk's own transaction. Concurrency comes from partitioning instead - lower
        // partition-size and add worker instances.
