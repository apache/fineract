...
public interface SavingsAccountRepository
  ...
  @Query(
      "select sa from SavingsAccount sa where sa.client.id = :clientId and sa.group.id = :groupId")
  List<SavingsAccount> findByClientIdAndGroupId(
      @Param("clientId") Long clientId, @Param("groupId") Long groupId);

  @Query(
      "select case when (count (saving) > 0) then 'true' else 'false' end from SavingsAccount"
          + " saving where saving.client.id = :clientId and saving.status in"
          + " (100,200,300,303,304)")
  ...
  @Query(
      "select sa from SavingsAccount sa where sa.id = :accountId and sa.depositType ="
          + " :depositAccountTypeId")
  SavingsAccount findByIdAndDepositAccountType(
      @Param("accountId") Long accountId,
      @Param("depositAccountTypeId") Integer depositAccountTypeId);

  @Query(
      "select sa from SavingsAccount sa where sa.accountNumber = :accountNumber and sa.status in"
          + " (100, 200, 300, 303, 304) ")
