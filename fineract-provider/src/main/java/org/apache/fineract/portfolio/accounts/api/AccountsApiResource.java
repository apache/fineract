...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Share Account",
          description =
              "Share accounts are instances of a praticular share product created for an"
                  + " individual. An application process around the creation of accounts is also"
                  + " supported.")
    })
public class AccountsApiResource {
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.GetAccountsTypeTemplateResponse.class)
  })
  public String template(
    ...
    try {
      ...
      AccountReadPlatformService service =
          (AccountReadPlatformService) this.applicationContext.getBean(serviceName);
      ...
      return this.toApiJsonSerializer.serialize(
          settings, accountData, service.getResponseDataParams());
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.GetAccountsTypeAccountIdResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "List share applications/accounts",
      httpMethod = "GET",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.GetAccountsTypeResponse.class)
  })
  ...
  @ApiOperation(
      ...
      notes =
          "Submits new share application\n\n"
              + "Mandatory Fields: clientId, productId, submittedDate, savingsAccountId,"
              + " requestedShares, applicationDate\n\n"
              + "Optional Fields: accountNo, externalId\n\n"
              + "Inherited from Product (if not provided): minimumActivePeriod,"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = AccountsApiResourceSwagger.PostAccountsTypeRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.PostAccountsTypeResponse.class)
  })
  ...
  @ApiOperation(
      value =
          "Approve share application | Undo approval share application | Reject share application"
              + " | Activate a share account | Close a share account | Apply additional shares on"
              + " a share account | Approve additional shares request on a share account | Reject"
              + " additional shares request on a share account | Redeem shares on a share account",
      httpMethod = "POST",
      notes =
          "Approve share application:\n\n"
              + "Approves share application so long as its in 'Submitted and pending approval'"
              + " state.\n\n"
              + "Undo approval share application:\n\n"
              + "Will move 'approved' share application back to 'Submitted and pending approval'"
              + " state.\n\n"
              + "Reject share application:\n\n"
              + "Rejects share application so long as its in 'Submitted and pending approval'"
              + " state.\n\n"
              + "Activate a share account:\n\n"
              + "Results in an approved share application being converted into an 'active' share"
              + " account.\n\n"
              + "Close a share account:\n\n"
              + "Results in an Activated share application being converted into an 'closed' share"
              + " account.\n"
              + "\n"
              ...
              + " https://demo.mifos.io/api-docs/apiLive.htm#shareaccounts")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        required = true,
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = AccountsApiResourceSwagger.PostAccountsTypeAccountIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.PostAccountsTypeAccountIdResponse.class)
  })
  public String handleCommands(
    ...
    commandWrapper =
        new CommandWrapperBuilder()
            .createAccountCommand(accountType, accountId, commandParam)
            .withJson(apiRequestBodyAsJson)
            .build();
    final CommandProcessingResult commandProcessingResult =
        this.commandsSourceWritePlatformService.logCommandSource(commandWrapper);
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Modify a share application",
      httpMethod = "PUT",
      notes =
          "Share application can only be modified when in 'Submitted and pending approval' state."
              + " Once the application is approved, the details cannot be changed using this"
              + " method. Specific api endpoints will be created to allow change of interest"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = AccountsApiResourceSwagger.PutAccountsTypeAccountIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = AccountsApiResourceSwagger.PutAccountsTypeAccountIdResponse.class)
  })
  ...
  public Response getSharedAccountsTemplate(
      @QueryParam("officeId") final Long officeId,
      @QueryParam("dateFormat") final String dateFormat) {
    return bulkImportWorkbookPopulatorService.getTemplate(
        GlobalEntityType.SHARE_ACCOUNTS.toString(), officeId, null, dateFormat);
  }
  ...
  public String postSharedAccountsTemplate(
    ...
    final Long importDocumentId =
        this.bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.SHARE_ACCOUNTS.toString(),
            uploadedInputStream,
            fileDetail,
            locale,
            dateFormat);
