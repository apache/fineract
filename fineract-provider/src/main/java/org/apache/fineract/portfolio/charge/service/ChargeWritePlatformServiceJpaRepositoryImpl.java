...
public class ChargeWritePlatformServiceJpaRepositoryImpl implements ChargeWritePlatformService {
  ...
  public CommandProcessingResult updateCharge(final Long chargeId, final JsonCommand command) {
    ...
    try {
      ...
      this.fromApiJsonDeserializer.validateChargeTimeNCalculationType(
          chargeForUpdate.getChargeTimeType(), chargeForUpdate.getChargeCalculation());
      ...
      if (changes.containsKey("active")) {
        ...
        if (!chargeForUpdate.isActive()) {
          ...
          final Boolean isChargeExistWithSavings =
              isAnySavingsProductsAssociateWithThisCharge(chargeId);
  ...
  public CommandProcessingResult deleteCharge(final Long chargeId) {
    ...
    final Collection<LoanProduct> loanProducts =
        this.loanProductRepository.retrieveLoanProductsByChargeId(chargeId);
    ...
    if (!loanProducts.isEmpty() || isChargeExistWithLoans || isChargeExistWithSavings) {
      throw new ChargeCannotBeDeletedException(
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("name")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.charge.duplicate.name",
          "Charge with name `" + name + "` already exists",
          "name",
          name);
    }
  ...
  private boolean isAnyLoansAssociateWithThisCharge(final Long chargeId) {
    ...
    final String sql =
        "select if((exists (select 1 from m_loan_charge lc where lc.charge_id = ? and lc.is_active"
            + " = 1)) = 1, 'true', 'false')";
  ...
  private boolean isAnySavingsAssociateWithThisCharge(final Long chargeId) {
    ...
    final String sql =
        "select if((exists (select 1 from m_savings_account_charge sc where sc.charge_id = ? and"
            + " sc.is_active = 1)) = 1, 'true', 'false')";
  ...
  private boolean isAnyLoanProductsAssociateWithThisCharge(final Long chargeId) {
    ...
    final String sql =
        "select if((exists (select 1 from m_product_loan_charge lc where lc.charge_id = ?)) = 1,"
            + " 'true', 'false')";
    final String isLoansUsingCharge =
        this.jdbcTemplate.queryForObject(sql, String.class, new Object[] {chargeId});
    return Boolean.valueOf(isLoansUsingCharge);
  ...
  private boolean isAnySavingsProductsAssociateWithThisCharge(final Long chargeId) {
    ...
    final String sql =
        "select if((exists (select 1 from m_savings_product_charge sc where sc.charge_id = ?)) ="
            + " 1, 'true', 'false')";
    final String isSavingsUsingCharge =
        this.jdbcTemplate.queryForObject(sql, String.class, new Object[] {chargeId});
    return Boolean.valueOf(isSavingsUsingCharge);
