...
public class InterestRateChartDataValidator {
  ...
  private static final Set<String> INTERESTRATE_CHART_CREATE_REQUEST_DATA_PARAMETERS =
      new HashSet<>(
          Arrays.asList(
  ...
  private static final Set<String> INTERESTRATE_CHART_UPDATE_REQUEST_DATA_PARAMETERS =
      new HashSet<>(
          Arrays.asList(
              InterestRateChartApiConstants.localeParamName,
              InterestRateChartApiConstants.dateFormatParamName,
              ...
              endDateParamName,
              chartSlabs,
              InterestRateChartApiConstants.deleteParamName,
              isPrimaryGroupingByAmountParamName));

  @Autowired
  public InterestRateChartDataValidator(
  ...
  public void validateForCreate(final String json, final DataValidatorBuilder baseDataValidator) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, INTERESTRATE_CHART_CREATE_REQUEST_DATA_PARAMETERS);
  ...
  public void validateForUpdate(final String json, final DataValidatorBuilder baseDataValidator) {
    final Type typeOfMap = new TypeToken<Map<String, Object>>() {}.getType();
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, INTERESTRATE_CHART_UPDATE_REQUEST_DATA_PARAMETERS);

    final JsonElement element = this.fromApiJsonHelper.parse(json);

    if (this.fromApiJsonHelper.parameterExists(productIdParamName, element)) {
      final Long savingsProductId =
          this.fromApiJsonHelper.extractLongNamed(productIdParamName, element);
      baseDataValidator
          .reset()
  ...
  private void validateChartSlabs(
      JsonElement element,
        ...
        for (int i = 0; i < array.size(); i++) {
          ...
          if (this.fromApiJsonHelper.parameterExists(idParamName, interstRateChartElement)) {
            ...
            baseDataValidator
                ...
                .notNull()
                .integerGreaterThanZero();
            this.chartSlabDataValidator.validateChartSlabsUpdate(
                interstRateChartElement, baseDataValidator, locale, isPrimaryGroupingByAmount);
          } else {
            this.chartSlabDataValidator.validateChartSlabsCreate(
                interstRateChartElement, baseDataValidator, locale, isPrimaryGroupingByAmount);
          }
        }
      }
