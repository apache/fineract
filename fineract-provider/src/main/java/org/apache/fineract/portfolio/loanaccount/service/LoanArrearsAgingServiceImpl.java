...
public class LoanArrearsAgingServiceImpl implements LoanArrearsAgingService, BusinessEventListner {
  ...
  private static final Logger logger = LoggerFactory.getLogger(LoanArrearsAgingServiceImpl.class);
  private final BusinessEventNotifierService businessEventNotifierService;
  private final DateTimeFormatter formatter = DateTimeFormat.forPattern("yyyy-MM-dd");
  private final JdbcTemplate jdbcTemplate;

  @Autowired
  public LoanArrearsAgingServiceImpl(
  ...
  public void registerForNotification() {
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_REFUND, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_ADJUST_TRANSACTION, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_MAKE_REPAYMENT, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_UNDO_WRITTEN_OFF, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_WAIVE_INTEREST, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_ADD_CHARGE, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_WAIVE_CHARGE, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_CHARGE_PAYMENT, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_APPLY_OVERDUE_CHARGE, this);
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_DISBURSAL, new DisbursementEventListner());
    this.businessEventNotifierService.addBusinessEventPostListners(
        BusinessEvents.LOAN_FORECLOSURE, this);
  }
  ...
  public void updateLoanArrearsAgeingDetails() {
    ...
    final String interestOverdueCalculationSql =
        "SUM(ifnull(mr.interest_amount, 0) - ifnull(mr.interest_writtenoff_derived, 0) -"
            + " ifnull(mr.interest_waived_derived, 0) - ifnull(mr.interest_completed_derived, 0))";
    ...
    final String penaltyChargesOverdueCalculationSql =
        "SUM(ifnull(mr.penalty_charges_amount, 0) - ifnull(mr.penalty_charges_writtenoff_derived,"
            + " 0) - ifnull(mr.penalty_charges_waived_derived, 0) -"
    ...
    updateSqlBuilder.append(
        penaltyChargesOverdueCalculationSql + " as penalty_charges_overdue_derived,");
    updateSqlBuilder.append(
        principalOverdueCalculationSql + "+" + interestOverdueCalculationSql + "+");
    updateSqlBuilder.append(
        feeChargesOverdueCalculationSql
            + "+"
            + penaltyChargesOverdueCalculationSql
    ...
    updateSqlBuilder.append(
        " and mr.duedate < SUBDATE(CURDATE(),INTERVAL  ifnull(ml.grace_on_arrears_ageing,0) day) ");
    updateSqlBuilder.append(
        " and (prd.arrears_based_on_original_schedule = 0 or"
            + " prd.arrears_based_on_original_schedule is null) ");
    ...
    logger.info(
        "{}: Results affected by update: {}", ThreadLocalContextUtil.getTenant().getName(), result);
  }
  ...
  public void updateLoanArrearsAgeingDetailsWithOriginalSchedule(final Loan loan) {
    ...
    OriginalScheduleExtractor originalScheduleExtractor =
        new OriginalScheduleExtractor(loan.getId().toString());
    Map<Long, List<LoanSchedulePeriodData>> scheduleDate =
        this.jdbcTemplate.query(originalScheduleExtractor.schema, originalScheduleExtractor);
    if (scheduleDate.size() > 0) {
      ...
      if (updateStatement.size() == 1) {
        ...
        String deletestatement =
            "DELETE FROM `m_loan_arrears_aging` WHERE  `loan_id`=" + loan.getId();
  ...
  private String constructUpdateStatement(final Loan loan, boolean isInsertStatement) {
    ...
    for (LoanRepaymentScheduleInstallment installment : installments) {
      if (installment.getDueDate().isBefore(LocalDate.now())) {
        principalOverdue =
            principalOverdue.add(
                installment.getPrincipalOutstanding(loan.getCurrency()).getAmount());
        interestOverdue =
            interestOverdue.add(installment.getInterestOutstanding(loan.getCurrency()).getAmount());
        feeOverdue =
            feeOverdue.add(installment.getFeeChargesOutstanding(loan.getCurrency()).getAmount());
        penaltyOverdue =
            penaltyOverdue.add(
  ...
  private List<String> updateLoanArrearsAgeingDetailsWithOriginalSchedule() {
    ...
    loanIdentifier.append(
        "inner join m_product_loan_recalculation_details prd on prd.product_id = ml.product_id and"
            + " prd.arrears_based_on_original_schedule = 1  ");
    loanIdentifier.append(
        "WHERE ml.loan_status_id = 300  and mr.completed_derived is false  and mr.duedate <"
            + " SUBDATE(CURDATE(),INTERVAL  ifnull(ml.grace_on_arrears_ageing,0) day) group by"
            + " ml.id");
  ...
  private List<Map<String, Object>> getLoanSummary(final String loanIdsAsString) {
    ...
    transactionsSql.append(
        "ml.principal_repaid_derived as principalAmtPaid, ml.principal_writtenoff_derived as "
            + " principalAmtWrittenoff, ");
    transactionsSql.append(
        " ml.interest_repaid_derived as interestAmtPaid, ml.interest_waived_derived as"
            + " interestAmtWaived, ");
    transactionsSql.append(
        "ml.fee_charges_repaid_derived as feeAmtPaid, ml.fee_charges_waived_derived as"
            + " feeAmtWaived, ");
    transactionsSql.append(
        "ml.penalty_charges_repaid_derived as penaltyAmtPaid, ml.penalty_charges_waived_derived as"
            + " penaltyAmtWaived ");
    ...
    List<Map<String, Object>> loanSummary =
        this.jdbcTemplate.queryForList(transactionsSql.toString());
    return loanSummary;
  }

  private List<Map<String, Object>> getLoanSummary(
      final Long loanId, final LoanSummary loanSummary) {
          ...
          principalOverdue =
              principalOverdue.add(
                  loanSchedulePeriodData
                      .principalDue()
                      .subtract(loanSchedulePeriodData.principalPaid()));
          interestOverdue =
              interestOverdue.add(
                  loanSchedulePeriodData
                      .interestDue()
                      .subtract(loanSchedulePeriodData.interestPaid()));
          feeOverdue =
              feeOverdue.add(
                  loanSchedulePeriodData
                      .feeChargesDue()
  ...
  private String constructUpdateStatement(
    ...
    insertStatementBuilder.append(
        "UPDATE m_loan_arrears_aging mla SET mla.principal_overdue_derived=");
  ...
  private void updateSchheduleWithPaidDetail(
      Map<Long, List<LoanSchedulePeriodData>> scheduleDate, List<Map<String, Object>> loanSummary) {
    for (Map<String, Object> transactionMap : loanSummary) {
      String longValue =
          transactionMap
              .get("loanId")
              .toString(); // From JDBC Template API, we are getting BigInteger but in other call,
      // we are getting Long
      ...
      if (loanSchedulePeriodDatas != null) {
        List<LoanSchedulePeriodData> updatedPeriodData =
            new ArrayList<>(loanSchedulePeriodDatas.size());
        for (LoanSchedulePeriodData loanSchedulePeriodData : loanSchedulePeriodDatas) {
          ...
          LoanSchedulePeriodData periodData =
              LoanSchedulePeriodData.WithPaidDetail(
                  loanSchedulePeriodData,
                  isComplete,
                  principalPaid,
                  interestPaid,
  ...
  private static final class OriginalScheduleExtractor
      implements ResultSetExtractor<Map<Long, List<LoanSchedulePeriodData>>> {
    ...
    public OriginalScheduleExtractor(final String loanIdsAsString) {
      ...
      scheduleDetail.append(
          "select ml.id as loanId, mr.duedate as dueDate, mr.principal_amount as"
              + " principalAmount, ");
      scheduleDetail.append(
          "mr.interest_amount as interestAmount, mr.fee_charges_amount as feeAmount,"
              + " mr.penalty_charges_amount as penaltyAmount  ");
      scheduleDetail.append(
          "from m_loan ml  INNER JOIN m_loan_repayment_schedule_history mr on mr.loan_id = ml.id ");
      scheduleDetail.append(
          "where mr.duedate  < SUBDATE(CURDATE(),INTERVAL  ifnull(ml.grace_on_arrears_ageing,0)"
              + " day) and ");
      scheduleDetail.append("ml.id IN(").append(loanIdsAsString).append(") and  mr.version = (");
      scheduleDetail.append(
          "select max(lrs.version) from m_loan_repayment_schedule_history lrs where mr.loan_id ="
              + " lrs.loan_id");
    ...
    private LoanSchedulePeriodData fetchLoanSchedulePeriodData(ResultSet rs) throws SQLException {
      ...
      final BigDecimal principalDue =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "principalAmount");
      final BigDecimal interestDueOnPrincipalOutstanding =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "interestAmount");
      ...
      final BigDecimal feeChargesDueForPeriod =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "feeAmount");
      final BigDecimal penaltyChargesDueForPeriod =
          JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "penaltyAmount");
  ...
  public void businessEventWasExecuted(Map<BusinessEntity, Object> businessEventEntity) {
    ...
    Object loanAdjustTransactionEntity =
        businessEventEntity.get(BusinessEntity.LOAN_ADJUSTED_TRANSACTION);
