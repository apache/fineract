...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Client Charges",
          description =
              "It is typical for MFI's to directly associate charges with an implicit Client"
                  + " account. These can be either fees or penalties\n"
                  + "\n"
                  + "Client Charges are client specific instances of Charges. Refer Charges for"
                  + " documentation of the various properties of a charge, Only additional"
                  + " properties ( specific to the context of a Charge being associated with a"
                  + " Client account) are described here")
    })
public class ClientChargesApiResource {
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientChargesApiResourceSwagger.GetClientsClientIdChargesResponse.class)
  })
  public String retrieveAllClientCharges(
    ...
    if (!(is(chargeStatus, ClientApiConstants.CLIENT_CHARGE_QUERY_PARAM_STATUS_VALUE_ALL)
      ...
      throw new UnrecognizedQueryParamException(
          ClientApiConstants.CLIENT_CHARGE_QUERY_PARAM_STATUS,
          chargeStatus,
          new Object[] {
    ...
    final Page<ClientChargeData> clientCharges =
        this.clientChargeReadPlatformService.retrieveClientCharges(
            clientId, chargeStatus, pendingPayment, searchParameters);
    return this.toApiJsonSerializer.serialize(
        settings, clientCharges, ClientApiConstants.CLIENT_CHARGES_RESPONSE_DATA_PARAMETERS);
  }
  ...
  public String retrieveTemplate(@Context final UriInfo uriInfo) {
    ...
    final Collection<ChargeData> chargeOptions =
        this.chargeReadPlatformService.retrieveAllChargesApplicableToClients();
    final ClientChargeData clientChargeData = ClientChargeData.template(chargeOptions);

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, clientChargeData, ClientApiConstants.CLIENT_CHARGES_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        message = "",
        response =
            ClientChargesApiResourceSwagger.GetClientsClientIdChargesResponse
                .GetClientsChargesPageItems.class)
  })
  public String retrieveClientCharge(
    ...
    this.context
        .authenticatedUser()
        .validateHasReadPermission(ClientApiConstants.CLIENT_CHARGES_RESOURCE_NAME);
    ClientChargeData clientCharge =
        this.clientChargeReadPlatformService.retrieveClientCharge(clientId, chargeId);
    ...
    if (!associationParameters.isEmpty()) {
      if (associationParameters.contains("all")) {
        associationParameters.addAll(
            Arrays.asList(ClientApiConstants.CLIENT_CHARGE_ASSOCIATIONS_TRANSACTIONS));
      }
      ApiParameterHelper.excludeAssociationsForResponseIfProvided(
          uriInfo.getQueryParameters(), associationParameters);
      if (associationParameters.contains(
          ClientApiConstants.CLIENT_CHARGE_ASSOCIATIONS_TRANSACTIONS)) {
        Collection<ClientTransactionData> clientTransactionDatas =
            this.clientTransactionReadPlatformService.retrieveAllTransactions(clientId, chargeId);
    ...
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.toApiJsonSerializer.serialize(
        settings, clientCharge, ClientApiConstants.CLIENT_CHARGES_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = ClientChargesApiResourceSwagger.PostClientsClientIdChargesRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = ClientChargesApiResourceSwagger.PostClientsClientIdChargesResponse.class)
  })
  public String applyClientCharge(
      @PathParam("clientId") @ApiParam(value = "clientId") final Long clientId,
      @ApiParam(hidden = true) final String apiRequestBodyAsJson) {

    final CommandWrapper commandRequest =
        new CommandWrapperBuilder()
            .createClientCharge(clientId)
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Pay a Client Charge | Waive a Client Charge",
      notes =
          "Pay a Client Charge:\n\n"
              + "Mandatory Fields:transactionDate and amount \"Pay either a part of or the entire"
              + " due amount for a charge.(command=paycharge)\n"
              + "\n"
              + "Waive a Client Charge:\n"
              + "\n"
              + "\n"
              + "This API provides the facility of waiving off the remaining amount on a client"
              + " charge (command=waive)\n\n"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        format = "body",
        dataTypeClass =
            ClientChargesApiResourceSwagger.PostClientsClientIdChargesChargeIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientChargesApiResourceSwagger.PostClientsClientIdChargesChargeIdResponse.class)
  })
  public String payOrWaiveClientCharge(
    ...
    if (is(commandParam, ClientApiConstants.CLIENT_CHARGE_COMMAND_WAIVE_CHARGE)) {
      ...
      final CommandWrapper commandRequest =
          new CommandWrapperBuilder().inactivateClientCharge(clientId, chargeId).build();
      ...
      throw new UnrecognizedQueryParamException(
          "command",
          commandParam,
          ClientApiConstants.CLIENT_CHARGE_COMMAND_WAIVE_CHARGE,
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        message = "OK",
        response =
            ClientChargesApiResourceSwagger.DeleteClientsClientIdChargesChargeIdResponse.class)
  })
  public String deleteClientCharge(
      @PathParam("clientId") @ApiParam(value = "clientId") final Long clientId,
      @PathParam("chargeId") @ApiParam(value = "chargeId") final Long chargeId) {

    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().deleteClientCharge(clientId, chargeId).build();
