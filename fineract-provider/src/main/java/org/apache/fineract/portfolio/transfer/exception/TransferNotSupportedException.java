...
public class TransferNotSupportedException extends AbstractPlatformDomainRuleException {
  ...
  public static enum TransferNotSupportedReason {
    ...
    public String errorMessage() {
      if (name().toString().equalsIgnoreCase("ACTIVE_SAVINGS_ACCOUNT")) {
        ...
        return "Destination Group for transfer of clients originally linked to a group not"
            + " selected ";
        ...
        return "Transfer of clients linked to multiple groups is not supported  ";
          ...
          .toString()
          .equalsIgnoreCase("DESTINATION_GROUP_MEETING_FREQUENCY_MISMATCH")) {
        return "Cannot transfer Clients with active accounts between groups with different meeting"
            + " frequency";
      } else if (name()
          .toString()
          .equalsIgnoreCase("SOURCE_AND_DESTINATION_GROUP_CANNOT_BE_SAME")) {
  ...
  public TransferNotSupportedException(
      final TransferNotSupportedReason reason, final Object... defaultUserMessageArgs) {
