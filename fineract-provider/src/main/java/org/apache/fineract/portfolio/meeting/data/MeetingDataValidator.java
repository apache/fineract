...
public class MeetingDataValidator {
  ...
  private static final Set<String> MEETING_REQUEST_DATA_PARAMETERS =
      new HashSet<>(
          Arrays.asList(
              MeetingApiConstants.meetingDateParamName,
              MeetingApiConstants.localeParamName,
              MeetingApiConstants.dateFormatParamName,
              MeetingApiConstants.calendarIdParamName,
  ...
  public void validateForCreate(final JsonCommand command) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, MEETING_REQUEST_DATA_PARAMETERS);

    final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
    final DataValidatorBuilder baseDataValidator =
        new DataValidatorBuilder(dataValidationErrors).resource(MEETING_RESOURCE_NAME);
    final JsonElement element = this.fromApiJsonHelper.parse(json);

    final String meetingDateStr =
        this.fromApiJsonHelper.extractStringNamed(meetingDateParamName, element);
    ...
    if (!StringUtils.isBlank(meetingDateStr)) {
      final LocalDate meetingDate =
          this.fromApiJsonHelper.extractLocalDateNamed(meetingDateParamName, element);
  ...
  private void validateAttendanceDetails(
    ...
    if (element.isJsonObject()) {
      if (topLevelJsonElement.has(clientsAttendanceParamName)
        ...
        for (int i = 0; i < array.size(); i++) {
          ...
          final Long attendanceType =
              this.fromApiJsonHelper.extractLongNamed(attendanceTypeParamName, attendanceElement);
          baseDataValidator
              .reset()
