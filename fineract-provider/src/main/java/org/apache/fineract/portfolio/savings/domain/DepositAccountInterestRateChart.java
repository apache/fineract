...
public class DepositAccountInterestRateChart extends AbstractPersistableCustom {
  ...
  @OneToMany(
      mappedBy = "depositAccountInterestRateChart",
  ...
  public static DepositAccountInterestRateChart from(InterestRateChart productChart) {
    ...
    final DepositAccountInterestRateChart depositChart =
        new DepositAccountInterestRateChart(productChart.chartFields(), null, depostiChartSlabs);
    // update deposit account interest rate chart ference to chart Slabs
    depositChart.updateChartSlabsReference();
    return depositChart;
  }

  private DepositAccountInterestRateChart(
  ...
  public BigDecimal getApplicableInterestRate(
    ...
    for (DepositAccountInterestRateChartSlabs slab : setOfChartSlabs()) {
      if (slab.slabFields().isBetweenPeriod(periodStartDate, periodEndDate)
          && slab.slabFields().isAmountBetween(depositAmount)) {
        ...
        for (DepositAccountInterestIncentives incentives : depositInterestIncentives) {
          AttributeIncentiveCalculation attributeIncentiveCalculation =
              AttributeIncentiveCalculationFactory.findAttributeIncentiveCalculation(
                  incentives.interestIncentivesFields().entiryType());
          IncentiveDTO incentiveDTO =
              new IncentiveDTO(
                  client, effectiveInterestRate, incentives.interestIncentivesFields());
        ...
        if (effectiveInterestRate == null
            || effectiveInterestRate.compareTo(BigDecimal.ZERO) == 0) {
          effectiveInterestRate = slab.slabFields().annualInterestRate();
        }
      }
    }
