...
public class ChargeReadPlatformServiceImpl implements ChargeReadPlatformService {
  ...
  public Collection<ChargeData> retrieveAllChargesForCurrency(String currencyCode) {
    ...
    String sql =
        "select " + rm.chargeSchema() + " where c.is_deleted=false and c.currency_code= ? ";
  ...
  public ChargeData retrieveNewChargeDetails() {
    ...
    final List<EnumOptionData> allowedChargeAppliesToOptions =
        this.chargeDropdownReadPlatformService.retrieveApplicableToTypes();
    final List<EnumOptionData> allowedChargeTimeOptions =
        this.chargeDropdownReadPlatformService.retrieveCollectionTimeTypes();
    final List<EnumOptionData> chargePaymentOptions =
        this.chargeDropdownReadPlatformService.retrivePaymentModes();
    ...
    final List<EnumOptionData> clientChargeCalculationTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveClientCalculationTypes();
    final List<EnumOptionData> clientChargeTimeTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveClientCollectionTimeTypes();
    ...
    final Map<String, List<GLAccountData>> incomeOrLiabilityAccountOptions =
        this.accountingDropdownReadPlatformService.retrieveAccountMappingOptionsForCharges();
    final List<EnumOptionData> shareChargeCalculationTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveSharesCalculationTypes();
    ...
    return ChargeData.template(
        ...
        allowedChargeAppliesToOptions,
        allowedChargeTimeOptions,
        chargePaymentOptions,
  ...
  @Override
  public Collection<ChargeData> retrieveLoanProductCharges(
      final Long loanProductId, final ChargeTimeType chargeTime) {
    ...
    String sql =
        "select "
            + rm.loanProductChargeSchema()
            + " where c.is_deleted=false and c.is_active=true and plc.product_loan_id=? and"
            + " c.charge_time_enum=? ";
    sql += addInClauseToSQL_toLimitChargesMappedToOffice_ifOfficeSpecificProductsEnabled();
  ...
  public Collection<ChargeData> retrieveLoanApplicableFees() {
    ...
    String sql =
        "select "
            + rm.chargeSchema()
            + " where c.is_deleted=false and c.is_active=true and c.is_penalty=false and"
  ...
  @Override
  public Collection<ChargeData> retrieveLoanAccountApplicableCharges(
    ...
    String sql =
        "select "
            + rm.chargeSchema()
            + " join m_loan la on la.currency_code = c.currency_code where la.id=:loanId and"
            + " c.is_deleted=false and c.is_active=true and"
            + " c.charge_applies_to_enum=:chargeAppliesTo"
            + excludeClause
            + " ";
    sql += addInClauseToSQL_toLimitChargesMappedToOffice_ifOfficeSpecificProductsEnabled();
    sql += " order by c.name ";
   ...
   * @return
   */
  private void processChargeExclusionsForLoans(
      ChargeTimeType[] excludeChargeTimes, StringBuilder excludeClause) {
    if (excludeChargeTimes != null && excludeChargeTimes.length > 0) {
      ...
      excludeClause =
          excludeClause.append(" and c.charge_time_enum not in(" + sb.toString() + ") ");
      excludeClause.append(" ");
    }
  }

  @Override
  public Collection<ChargeData> retrieveLoanProductApplicableCharges(
    ...
    String sql =
        "select "
            + rm.chargeSchema()
            + " join m_product_loan lp on lp.currency_code = c.currency_code where"
            + " lp.id=:productId and c.is_deleted=false and c.is_active=true and"
            + " c.charge_applies_to_enum=:chargeAppliesTo"
            + excludeClause
            + " ";
    sql += addInClauseToSQL_toLimitChargesMappedToOffice_ifOfficeSpecificProductsEnabled();
  ...
  public Collection<ChargeData> retrieveLoanApplicablePenalties() {
    ...
    String sql =
        "select "
            + rm.chargeSchema()
            + " where c.is_deleted=false and c.is_active=true and c.is_penalty=true and"
            + " c.charge_applies_to_enum=? ";
    sql += addInClauseToSQL_toLimitChargesMappedToOffice_ifOfficeSpecificProductsEnabled();
    sql += " order by c.name ";
  ...
  private static final class ChargeMapper implements RowMapper<ChargeData> {
    ...
    public String chargeSchema() {
      return "c.id as id, c.name as name, c.amount as amount, c.currency_code as currencyCode,"
          + " c.charge_applies_to_enum as chargeAppliesTo, c.charge_time_enum as"
          + " chargeTime, c.charge_payment_mode_enum as chargePaymentMode,"
          + " c.charge_calculation_enum as chargeCalculation, c.is_penalty as penalty,"
          + " c.is_active as active, oc.name as currencyName, oc.decimal_places as"
          ...
          + " currencyNameCode, c.fee_on_day as feeOnDay, c.fee_on_month as feeOnMonth,"
          + " c.fee_interval as feeInterval, c.fee_frequency as feeFrequency,c.min_cap as"
          + " minCap,c.max_cap as maxCap, c.income_or_liability_account_id as glAccountId ,"
          + " acc.name as glAccountName, acc.gl_code as glCode, tg.id as taxGroupId,"
          + " tg.name as taxGroupName from m_charge c join m_organisation_currency oc on"
          + " c.currency_code = oc.code  LEFT JOIN acc_gl_account acc on acc.id ="
          + " c.income_or_liability_account_id  LEFT JOIN m_tax_group tg on tg.id ="
          + " c.tax_group_id ";
    }
    ...
    @Override
    public ChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
      ...
      final EnumOptionData chargeAppliesToType =
          ChargeEnumerations.chargeAppliesTo(chargeAppliesTo);
  ...
  @Override
  public Collection<ChargeData> retrieveSavingsProductApplicableCharges(
    ...
    String sql =
        "select "
            + rm.chargeSchema()
            + " where c.is_deleted=false and c.is_active=true and c.charge_applies_to_enum=? ";
    if (feeChargesOnly) {
      sql =
          "select "
              + rm.chargeSchema()
  ...
  public Collection<ChargeData> retrieveSavingsAccountApplicableCharges(Long savingsAccountId) {
    ...
    return this.jdbcTemplate.query(
        sql, rm, new Object[] {ChargeAppliesTo.SAVINGS.getValue(), savingsAccountId});
  }
