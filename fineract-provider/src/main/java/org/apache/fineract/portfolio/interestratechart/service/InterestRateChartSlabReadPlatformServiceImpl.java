...
public class InterestRateChartSlabReadPlatformServiceImpl
  ...
  private final InterestRateChartSlabExtractor chartSlabExtractor =
      new InterestRateChartSlabExtractor();
  ...
  public Collection<InterestRateChartSlabData> retrieveAll(Long chartId) {
    ...
    final String sql =
        "select "
            + this.chartSlabExtractor.schema()
            + " where ircd.interest_rate_chart_id = ? order by ircd.id";
  ...
  public InterestRateChartSlabData retrieveOne(Long chartId, Long chartSlabId) {
    this.context.authenticatedUser();
    final String sql =
        "select " + this.chartSlabExtractor.schema() + " where irc.id = ? order by ircd.id asc";
    Collection<InterestRateChartSlabData> chartDatas =
        this.jdbcTemplate.query(sql, this.chartSlabExtractor, new Object[] {chartSlabId, chartId});
  ...
  public InterestRateChartSlabData retrieveWithTemplate(InterestRateChartSlabData chartSlab) {
    ...
    return InterestRateChartSlabData.withTemplate(
        chartSlab,
  ...
  public InterestRateChartSlabData retrieveTemplate() {
    ...
    final List<CodeValueData> clientClassificationOptions =
        new ArrayList<>(
            this.codeValueReadPlatformService.retrieveCodeValuesByCode(
                ClientApiConstants.CLIENT_CLASSIFICATION));
    return InterestRateChartSlabData.template(
        this.chartDropdownReadPlatformService.retrievePeriodTypeOptions(),
        this.interestIncentiveDropdownReadPlatformService.retrieveEntityTypeOptions(),
        this.interestIncentiveDropdownReadPlatformService.retrieveAttributeNameOptions(),
        this.interestIncentiveDropdownReadPlatformService.retrieveConditionTypeOptions(),
  ...
  private static final class InterestRateChartSlabExtractor
      implements ResultSetExtractor<Collection<InterestRateChartSlabData>> {
    ...
    @Override
    public Collection<InterestRateChartSlabData> extractData(ResultSet rs)
      ...
      while (rs.next()) {
        ...
        if (chartSlabData == null
            || (interestRateChartSlabId != null && !interestRateChartSlabId.equals(tempIrcdId))) {
