...
public class LoanProductReadPlatformServiceImpl implements LoanProductReadPlatformService {
  ...
  public LoanProductData retrieveLoanProduct(final Long loanProductId) {
    ...
    try {
      ...
      final Collection<LoanProductBorrowerCycleVariationData> borrowerCycleVariationDatas =
          retrieveLoanProductBorrowerCycleVariations(loanProductId);
      final LoanProductMapper rm =
          new LoanProductMapper(charges, borrowerCycleVariationDatas, rates);
  ...
  public Collection<LoanProductData> retrieveAllLoanProducts() {
    ...
    String inClause =
        fineractEntityAccessUtil.getSQLWhereClauseForProductIDsForUserOffice_ifGlobalConfigEnabled(
  ...
  private static final class LoanProductMapper implements RowMapper<LoanProductData> {
    ...
    public String loanProductSchema() {
      return "lp.id as id, lp.fund_id as fundId, f.name as fundName,"
          + " lp.loan_transaction_strategy_id as transactionStrategyId, ltps.name as"
          + " transactionStrategyName, lp.name as name, lp.short_name as shortName,"
          + " lp.description as description, lp.principal_amount as principal,"
          ...
          + " as allowPartialPeriodInterestCalcualtion, lp.repay_every as repaidEvery,"
          ...
          + " lp.recurring_moratorium_principal_periods as"
          ...
          + " minimumDaysBetweenDisbursalAndFirstRepayment, lp.amortization_method_enum as"
          + " amortizationMethod, lp.arrearstolerance_amount as tolerance,"
          + " lp.accounting_type as accountingType, lp.include_in_borrower_cycle as"
          + " includeInBorrowerCycle,lp.use_borrower_cycle as useBorrowerCycle,"
          + " lp.start_date as startDate, lp.close_date as closeDate, "
          + " lp.allow_multiple_disbursals as multiDisburseLoan, lp.max_disbursals as"
          + " maxTrancheCount, lp.max_outstanding_loan_balance as outstandingLoanBalance,"
          + " lp.days_in_month_enum as daysInMonth, lp.days_in_year_enum as daysInYear,"
          ...
          + " preCloseInterestCalculationStrategy, lpr.id as lprId, lpr.product_id as"
          + " productId, lpr.compound_type_enum as compoundType,"
          ...
          + " isCompoundingToBePostedAsTransaction, lpr.allow_compounding_on_eod as"
          + " allowCompoundingOnEod, lp.hold_guarantee_funds as holdGuaranteeFunds,"
          + " lp.principal_threshold_for_last_installment as"
          + " principalThresholdForLastInstallment, lp.sync_expected_with_disbursement_date"
          + " as syncExpectedWithDisbursementDate, lpg.id as lpgId, lpg.mandatory_guarantee"
          + " as mandatoryGuarantee, lpg.minimum_guarantee_from_own_funds as"
          + " minimumGuaranteeFromOwnFunds, lpg.minimum_guarantee_from_guarantor_funds as"
          + " minimumGuaranteeFromGuarantor,"
          + " lp.account_moves_out_of_npa_only_on_arrears_completion as"
          + " accountMovesOutOfNPAOnlyOnArrearsCompletion, curr.name as currencyName,"
          + " curr.internationalized_name_code as currencyNameCode, curr.display_symbol as"
          + " currencyDisplaySymbol, lp.external_id as externalId, lca.id as lcaId,"
          + " lca.amortization_method_enum as amortizationBoolean, lca.interest_method_enum"
          + " as interestMethodConfigBoolean, lca.loan_transaction_strategy_id as"
          + " transactionProcessingStrategyBoolean,lca.interest_calculated_in_period_enum"
          + " as interestCalcPeriodBoolean, lca.arrearstolerance_amount as"
          + " arrearsToleranceBoolean, lca.repay_every as repaymentFrequencyBoolean,"
          ...
          + " isFloatingInterestRateCalculationAllowed, lp.allow_variabe_installments as"
          + " isVariableIntallmentsAllowed, lvi.minimum_gap as minimumGap, lvi.maximum_gap"
          + " as maximumGap, lp.can_use_for_topup as canUseForTopup,"
          + " lp.is_equal_amortization as isEqualAmortization  from m_product_loan lp  left"
          + " join m_fund f on f.id = lp.fund_id  left join"
          + " m_product_loan_recalculation_details lpr on lpr.product_id=lp.id  left join"
          + " m_product_loan_guarantee_details lpg on lpg.loan_product_id=lp.id  left join"
          + " ref_loan_transaction_processing_strategy ltps on ltps.id ="
          + " lp.loan_transaction_strategy_id left join"
          + " m_product_loan_configurable_attributes lca on lca.loan_product_id = lp.id "
          + " left join m_product_loan_floating_rates as lfr on lfr.loan_product_id = lp.id"
          + "  left join m_floating_rates as fr on lfr.floating_rates_id = fr.id  left join"
          + " m_product_loan_variable_installment_config as lvi on lvi.loan_product_id ="
          + " lp.id  join m_currency curr on curr.code = lp.currency_code";
    }
    ...
    public LoanProductData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
      ...
      final Integer recurringMoratoriumOnPrincipalPeriods =
          JdbcSupport.getIntegerDefaultToNullIfZero(rs, "recurringMoratoriumOnPrincipalPeriods");
      ...
      if (isInterestRecalculationEnabled) {
        ...
        if (restFrequencyNthDayEnumValue != null) {
          restFrequencyNthDayEnum =
              LoanEnumerations.interestRecalculationCompoundingNthDayType(
                  restFrequencyNthDayEnumValue);
        }
        ...
        if (restFrequencyWeekDayEnumValue != null) {
          restFrequencyWeekDayEnum =
              LoanEnumerations.interestRecalculationCompoundingDayOfWeekType(
                  restFrequencyWeekDayEnumValue);
        }
        ...
        if (compoundingFrequencyNthDayEnumValue != null) {
          compoundingFrequencyNthDayEnum =
              LoanEnumerations.interestRecalculationCompoundingNthDayType(
                  compoundingFrequencyNthDayEnumValue);
        }
        ...
        final EnumOptionData preCloseInterestCalculationStrategy =
            LoanEnumerations.preCloseInterestCalculationStrategy(
                preCloseInterestCalculationStrategyEnumValue);
      ...
      final boolean transactionProcessingStrategy =
          rs.getBoolean("transactionProcessingStrategyBoolean");
      ...
      final boolean graceOnPrincipalAndInterest =
          rs.getBoolean("graceOnPrincipalAndInterestBoolean");
      ...
      allowAttributeOverrides =
          new LoanProductConfigurableAttributes(
              amortization,
              interestMethod,
  ...
  private static final class LoanProductLookupMapper implements RowMapper<LoanProductData> {
    ...
    public String schema() {
      return "lp.id as id, lp.name as name, lp.allow_multiple_disbursals as multiDisburseLoan from"
          + " m_product_loan lp";
    }
    ...
    public String productMixSchema() {
      return "lp.id as id, lp.name as name, lp.allow_multiple_disbursals as multiDisburseLoan FROM"
          + " m_product_loan lp left join m_product_mix pm on pm.product_id=lp.id where"
          + " lp.id not IN(select lp.id from m_product_loan lp inner join m_product_mix pm"
          + " on pm.product_id=lp.id)";
    }

    public String restrictedProductsSchema() {
      return "pm.restricted_product_id as id, rp.name as name, rp.allow_multiple_disbursals as"
          + " multiDisburseLoan from m_product_mix pm join m_product_loan rp on rp.id ="
          + " pm.restricted_product_id ";
    }

    public String derivedRestrictedProductsSchema() {
      return "pm.product_id as id, lp.name as name, lp.allow_multiple_disbursals as"
          + " multiDisburseLoan from m_product_mix pm join m_product_loan lp on"
          + " lp.id=pm.product_id";
    }
  ...
  private static final class LoanProductBorrowerCycleMapper
    ...
    public String schema() {
      return "bc.id as id,bc.borrower_cycle_number as cycleNumber,bc.value_condition as"
          + " conditionType,bc.param_type as paramType,bc.default_value as"
          + " defaultValue,bc.max_value as maxVal,bc.min_value as minVal from"
          + " m_product_loan_variations_borrower_cycle bc";
    }
  ...
  private static final class LoanProductFloatingRateMapper implements RowMapper<LoanProductData> {
    ...
    public String schema() {
      return "lp.id as id,  lp.name as name,lp.is_linked_to_floating_interest_rates as"
          + " isLinkedToFloatingInterestRates, lfr.floating_rates_id as floatingRateId,"
          + " fr.name as floatingRateName, lfr.interest_rate_differential as"
          + " interestRateDifferential, lfr.min_differential_lending_rate as"
          + " minDifferentialLendingRate, lfr.default_differential_lending_rate as"
          + " defaultDifferentialLendingRate, lfr.max_differential_lending_rate as"
          + " maxDifferentialLendingRate, lfr.is_floating_interest_rate_calculation_allowed"
          + " as isFloatingInterestRateCalculationAllowed  from m_product_loan lp  left"
          + " join m_product_loan_floating_rates as lfr on lfr.loan_product_id = lp.id "
    ...
    public LoanProductData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
      ...
      return LoanProductData.loanProductWithFloatingRates(
          id,
          name,
          isLinkedToFloatingInterestRates,
          floatingRateId,
