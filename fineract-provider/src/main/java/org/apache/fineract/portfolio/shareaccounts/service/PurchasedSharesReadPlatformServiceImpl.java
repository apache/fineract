...
public class PurchasedSharesReadPlatformServiceImpl implements PurchasedSharesReadPlatformService {
  ...
  public Collection<ShareAccountTransactionData> retrievePurchasedShares(Long accountId) {
    ...
    final String sql =
        "select " + mapper.schema() + " where saps.account_id=? and saps.is_active = 1";
  ...
  private static final class PurchasedSharesDataRowMapper
    ...
    public PurchasedSharesDataRowMapper() {
      StringBuilder buff =
          new StringBuilder()
              .append(
                  "saps.id, saps.account_id, saps.transaction_date, saps.total_shares,"
                      + " saps.unit_price, ")
              .append(
                  "saps.status_enum, saps.type_enum, saps.amount, saps.charge_amount as"
                      + " chargeamount, ")
