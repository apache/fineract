...
public class GSIMReadPlatformServiceImpl implements GSIMReadPlatformService {
  ...
  @Autowired
  public GSIMReadPlatformServiceImpl(
  ...
  private static final class GSIMFieldsMapper
      implements RowMapper<GroupSavingsIndividualMonitoringAccountData> {
    public String schema() {
      return "gsim.id as gsimId,sv.group_id as groupId,sv.client_id as"
          + " clientId,gsim.account_number as accountNumber, sv.id as"
          + " childAccountId,sv.account_no as childAccountNumber,sv.account_balance_derived"
          + " as childBalance,gsim.parent_deposit as"
          + " parentBalance,gsim.child_accounts_count as"
          + " childAccountsCount,gsim.savings_status_id as savingsStatus from gsim_accounts"
          + " gsim,m_savings_account sv where gsim.id=sv.gsim_id";
    }

    @Override
    public GroupSavingsIndividualMonitoringAccountData mapRow(
      ...
      return GroupSavingsIndividualMonitoringAccountData.getInstance2(
          gsimId,
          groupId,
          clientId,
          accountNumber,
  ...
  private static final class GSIMMapper
      implements RowMapper<GroupSavingsIndividualMonitoringAccountData> {
    public String schema() {
      return "gsim.id as gsimId,gsim.group_id as groupId,gsim.account_number as"
          + " accountNumber,gsim.parent_deposit as parentDeposit,gsim.child_accounts_count"
          + " as childAccountsCount,gsim.savings_status_id as savingsStatus from"
          + " gsim_accounts gsim";
    }
    ...
    public GroupSavingsIndividualMonitoringAccountData mapRow(
      ...
      return GroupSavingsIndividualMonitoringAccountData.getInstance1(
          glimId, groupId, accountNumber, parentDeposit, loanStatus);
    }
  }
  ...
  public Collection<GSIMContainer> findGSIMAccountContainerByGroupId(Long groupId) {
    ...
    // retrieveLoanAccountDetails(loanWhereClauseForGroupAndLoanType, new Object[] { groupId });
    ...
    for (GroupSavingsIndividualMonitoringAccountData gsimAccount : gsimInfo) {
      ...
      savingswhereClauseForGroup =
          " where sa.group_id = ? and sa.gsim_id = ? order by sa.status_enum ASC, sa.account_no"
              + " ASC";

      childSavings =
          retrieveAccountDetails(savingswhereClauseForGroup, new Object[] {groupId, gsimId});

      gsimAccounts.add(
          new GSIMContainer(
              gsimAccount.getGsimId(),
              gsimAccount.getGroupId(),
              gsimAccount.getAccountNumber(),
              childSavings,
              gsimAccount.getParentDeposit(),
              gsimAccount.getSavingsStatus()));
    }
  ...
  @Override
  public Collection<GSIMContainer> findGsimAccountContainerbyGsimAccountNumber(
      String accountNumber) {
    this.context.authenticatedUser();
    Collection<GroupSavingsIndividualMonitoringAccountData> gsimInfo =
        findGsimAccountByParentAccountNumber(accountNumber);

    // List<LoanAccountSummaryData> glimAccounts =
    // retrieveLoanAccountDetails(loanWhereClauseForGroupAndLoanType, new Object[] { groupId });
    final String savingswhereClauseForGroup =
        " where gsim.account_number = ? order by sa.status_enum ASC, sa.account_no ASC";

    List<GSIMContainer> gsimAccounts = new ArrayList<GSIMContainer>();
    for (GroupSavingsIndividualMonitoringAccountData gsimAccount : gsimInfo) {

      List<SavingsSummaryCustom> childSavings =
          retrieveAccountDetails(savingswhereClauseForGroup, new Object[] {accountNumber});
  ...
  public List<GSIMContainer> findGsimAccountContainerbyGsimAccountId(Long parentAccountId) {
    this.context.authenticatedUser();
    GroupSavingsIndividualMonitoringAccountData gsimAccount =
        findGSIMAccountByGSIMId(parentAccountId);

    // List<LoanAccountSummaryData> glimAccounts =
    // retrieveLoanAccountDetails(loanWhereClauseForGroupAndLoanType, new Object[] { groupId });
    final String savingswhereClauseForGroup =
        " where sa.gsim_id = ? order by sa.status_enum ASC, sa.account_no ASC";

    List<SavingsSummaryCustom> childSavings =
        retrieveAccountDetails(savingswhereClauseForGroup, new Object[] {parentAccountId});
  ...
  @Override
  public Collection<GroupSavingsIndividualMonitoringAccountData> findGSIMAccountsByGSIMId(
      final Long gsimId) {
    this.context.authenticatedUser();
  ...
  @Override
  public Collection<GroupSavingsIndividualMonitoringAccountData> findGSIMAccountsByGroupId(
      String groupId) {
    this.context.authenticatedUser();
  ...
  @Override
  public Collection<GroupSavingsIndividualMonitoringAccountData>
      findGsimAccountByParentAccountNumber(String parentAccountIds) {
    this.context.authenticatedUser();
  ...
  @Override
  public Collection<GroupSavingsIndividualMonitoringAccountData>
      findGsimAccountByGroupIdandAccountNo(String groupId, String accountNo) {
    this.context.authenticatedUser();
  ...
  private List<SavingsSummaryCustom> retrieveAccountDetails(
      final String savingswhereClause, final Object[] inputs) {
    final SavingsAccountSummaryDataMapper savingsAccountSummaryDataMapper =
        new SavingsAccountSummaryDataMapper();
    final String savingsSql =
        "select " + savingsAccountSummaryDataMapper.schema() + savingswhereClause;
    this.columnValidator.validateSqlInjection(
        savingsAccountSummaryDataMapper.schema(), savingswhereClause);
    return this.jdbcTemplate.query(savingsSql, savingsAccountSummaryDataMapper, inputs);
  }

  private static final class SavingsAccountSummaryDataMapper
      implements RowMapper<SavingsSummaryCustom> {
    ...
    public SavingsAccountSummaryDataMapper() {
      ...
      accountsSummary.append(
          "sa.id as id, CONCAT('(',clnt.id,') ',clnt.display_name) as displayName,sa.account_no as"
              + " accountNo, sa.external_id as externalId, sa.gsim_id as"
              + " gsimId,gsim.account_number as parentAccountNo,sa.status_enum as statusEnum, ");
    ...
    @Override
    public SavingsSummaryCustom mapRow(
        final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
      ...
      return new SavingsSummaryCustom(
          id,
          displayName,
          accountNo,
          externalId,
