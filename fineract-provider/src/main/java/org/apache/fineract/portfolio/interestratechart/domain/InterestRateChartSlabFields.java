...
public class InterestRateChartSlabFields {
  ...
  public static InterestRateChartSlabFields createNew(
    ...
    return new InterestRateChartSlabFields(
        description,
        periodFrequencyType,
        fromPeriod,
        toPeriod,
  ...
  private InterestRateChartSlabFields(
      final String description,
      final SavingsPeriodFrequencyType periodFrequencyType,
      final Integer fromPeriod,
      final Integer toPeriod,
    ...
    this.periodType =
        (periodFrequencyType == null || periodFrequencyType.isInvalid())
            ? null
  ...
  public void update(
    ...
    if (command.isChangeInBigDecimalParameterNamed(
        amountRangeFromParamName, this.amountRangeFrom, locale)) {
      final BigDecimal newValue =
          command.bigDecimalValueOfParameterNamed(amountRangeFromParamName, locale);
      actualChanges.put(amountRangeFromParamName, newValue);
      this.amountRangeFrom = newValue;
    }

    if (command.isChangeInBigDecimalParameterNamed(
        amountRangeToParamName, this.amountRangeTo, locale)) {
      final BigDecimal newValue =
          command.bigDecimalValueOfParameterNamed(amountRangeToParamName, locale);
      actualChanges.put(amountRangeToParamName, newValue);
      this.amountRangeTo = newValue;
    }

    if (command.isChangeInBigDecimalParameterNamed(
        annualInterestRateParamName, this.annualInterestRate, locale)) {
      final BigDecimal newValue =
          command.bigDecimalValueOfParameterNamed(annualInterestRateParamName, locale);
  ...
  public void validateChartSlabPlatformRules(
      final JsonCommand chartSlabsCommand,
      final DataValidatorBuilder baseDataValidator,
      Locale locale) {
    if (isFromPeriodGreaterThanToPeriod()) {
      ...
      baseDataValidator
          .parameter(fromPeriodParamName)
          .value(fromPeriod)
          .failWithCode("from.period.is.greater.than.to.period");
    }
    ...
    if (isAmountRangeFromGreaterThanTo()) {
      ...
      baseDataValidator
          .parameter(amountRangeFromParamName)
  ...
  public boolean isAmountRangeFromGreaterThanTo() {
    boolean isGreater = false;
    if (this.amountRangeFrom != null
        && this.amountRangeTo != null
        && this.amountRangeFrom.compareTo(this.amountRangeTo) > 1) {
      isGreater = true;
    }
    return isGreater;
  }
  ...
  public boolean isRateChartHasGap(
      final InterestRateChartSlabFields that, final boolean isPrimaryGroupingByAmount) {
  ...
  public static boolean isNotProperAmountStart(
      final InterestRateChartSlabFields interestRateChartSlabFields) {
  ...
  private boolean isNotProperAmountStart(final BigDecimal amount) {
    return this.amountRangeTo == null
        || (amount != null && amount.compareTo(this.amountRangeTo.add(BigDecimal.ONE)) != 0);
  }
  ...
  public static boolean isNotProperPeriodStart(
  ...
  public boolean isRateChartOverlapping(
      final InterestRateChartSlabFields that, final boolean isPrimaryGroupingByAmount) {
    ...
    if (isPrimaryGroupingByAmount) {
      isOverlapping =
          (isAmountOverLapping && !isAmountSame) || (isPeriodOverLapping && isAmountSame);
    } else {
      isOverlapping =
          (isPeriodOverLapping && !isPeriodSame) || (isAmountOverLapping && isPeriodSame);
    }
  ...
  private boolean isAmountOverlapping(final InterestRateChartSlabFields that) {
    if (isBigDecimalSame(that.amountRangeFrom, this.amountRangeFrom)) {
      ...
      return that.amountRangeFrom.compareTo(this.amountRangeTo) < 1;
    }
  ...
  public Integer depositPeriod(final LocalDate periodStartDate, final LocalDate periodEndDate) {
    ...
    final SavingsPeriodFrequencyType periodFrequencyType =
        SavingsPeriodFrequencyType.fromInt(periodType());
  ...
  public boolean isAmountBetween(final BigDecimal depositAmount) {
    ...
    if (amountRangeFrom != null && amountRangeTo != null) {
      returnValue =
          depositAmount.compareTo(amountRangeFrom) >= 0
