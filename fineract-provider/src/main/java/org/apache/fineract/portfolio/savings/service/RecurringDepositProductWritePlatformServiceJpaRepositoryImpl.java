...
public class RecurringDepositProductWritePlatformServiceJpaRepositoryImpl
  ...
  public CommandProcessingResult create(final JsonCommand command) {
    ...
    try {
      ...
      final RecurringDepositProduct product =
          this.depositProductAssembler.assembleRecurringDepositProduct(command);
      ...
      this.accountMappingWritePlatformService.createSavingProductToGLAccountMapping(
          product.getId(), command, DepositAccountType.RECURRING_DEPOSIT);
    ...
    try {
      ...
      if (changes.containsKey(chargesParamName)) {
        final Set<Charge> savingsProductCharges =
            this.depositProductAssembler.assembleListOfSavingsProductCharges(
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("sp_unq_name")) {
      ...
      throw new PlatformDataIntegrityException(
          ...
          "Recurring Deposit product with name `" + name + "` already exists",
          "name",
          name);
      ...
      throw new PlatformDataIntegrityException(
          ...
          "Recurring Deposit product with short name `" + shortName + "` already exists",
          "shortName",
          shortName);
    }
