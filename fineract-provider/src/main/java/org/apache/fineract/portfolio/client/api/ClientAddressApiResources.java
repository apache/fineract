...
@SwaggerDefinition(
    tags = {
      @Tag(
          ...
          description =
              "Address module is an optional module and can be configured into the system by using"
                  + " GlobalConfiguration setting: enable-address. In order to activate Address"
                  + " module, we need to enable the configuration, enable-address by setting its"
                  + " value to true.")
    })
public class ClientAddressApiResources {
  ...
  @Autowired
  public ClientAddressApiResources(
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Create an address for a Client",
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = ClientAddressApiResourcesSwagger.PostClientClientIdAddressesRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = ClientAddressApiResourcesSwagger.PostClientClientIdAddressesResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "List all addresses for a Client",
      notes =
          "Example Requests:\n"
              + "\n"
              + "client/1/addresses\n"
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        response = ClientAddressApiResourcesSwagger.GetClientClientIdAddressesResponse.class)
  })
  public String getAddresses(
      @QueryParam("status") @ApiParam(value = "status") final String status,
      @QueryParam("type") @ApiParam(value = "type") final long addressTypeId,
      @PathParam("clientid") @ApiParam(value = "clientId") final long clientid,
    ...
    if (addressTypeId == 0 && status == null) {
      ...
      address =
          this.readPlatformService.retrieveAddressbyTypeAndStatus(clientid, addressTypeId, status);
    } else {
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Update an address for a Client",
      notes =
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = ClientAddressApiResourcesSwagger.PutClientClientIdAddressesRequest.class)
  })
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "OK",
            response = ClientAddressApiResourcesSwagger.PutClientClientIdAddressesResponse.class)
      })
