...
public class ClientChargeReadPlatformServiceImpl implements ClientChargeReadPlatformService {
  ...
  public static final class ClientChargeMapper implements RowMapper<ClientChargeData> {
    ...
    @Override
    public ClientChargeData mapRow(ResultSet rs, @SuppressWarnings("unused") int rowNum)
        throws SQLException {
      ...
      return ClientChargeData.instance(
          id,
          clientId,
          chargeId,
          name,
    ...
    public String schema() {
      return " cc.id as id, c.id as chargeId, cc.client_id as clientId, c.name as name, cc.amount"
          + " as amountDue, cc.amount_paid_derived as amountPaid, cc.amount_waived_derived"
          + " as amountWaived, cc.amount_writtenoff_derived as amountWrittenOff,"
          + " cc.amount_outstanding_derived as amountOutstanding, cc.charge_time_enum as"
          + " chargeTime, cc.is_penalty as penalty, cc.charge_due_date as dueAsOfDate,"
          + " cc.charge_calculation_enum as chargeCalculation, cc.is_paid_derived as"
          + " isPaid, cc.waived as waived, cc.is_active as isActive, cc.inactivated_on_date"
          + " as inactivationDate, c.currency_code as currencyCode, oc.name as"
          + " currencyName, oc.decimal_places as currencyDecimalPlaces,"
          + " oc.currency_multiplesof as inMultiplesOf, oc.display_symbol as"
          + " currencyDisplaySymbol, oc.internationalized_name_code as currencyNameCode"
          + " from m_charge c join m_organisation_currency oc on c.currency_code = oc.code"
          + " join m_client_charge cc on cc.charge_id = c.id ";
    }
  }
  ...
  @Override
  public Page<ClientChargeData> retrieveClientCharges(
      Long clientId, String status, Boolean pendingPayment, SearchParameters searchParameters) {
    final ClientChargeMapper rm = new ClientChargeMapper();
    final StringBuilder sqlBuilder = new StringBuilder();
    sqlBuilder
        .append("select SQL_CALC_FOUND_ROWS ")
        .append(rm.schema())
        .append(" where cc.client_id=? ");
    ...
    sqlBuilder.append(
        " order by cc.charge_time_enum ASC, cc.charge_due_date DESC, cc.is_penalty ASC ");
