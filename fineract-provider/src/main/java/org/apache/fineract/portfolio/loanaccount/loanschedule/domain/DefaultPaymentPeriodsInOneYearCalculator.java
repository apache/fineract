...
public class DefaultPaymentPeriodsInOneYearCalculator implements PaymentPeriodsInOneYearCalculator {
  ...
  @Override
  public double calculatePortionOfRepaymentPeriodInterestChargingGrace(
      final LocalDate repaymentPeriodStartDate,
    ...
    final LocalDateInterval repaymentPeriod =
        new LocalDateInterval(repaymentPeriodStartDate, scheduledDueDate);
    ...
    } else if (interestChargedFromLocalDate != null
        && repaymentPeriod.contains(interestChargedFromLocalDate)) {

      final int numberOfDaysInterestCalculationGraceInPeriod =
          Days.daysBetween(repaymentPeriodStartDate, interestChargedFromLocalDate).getDays();
      periodFraction =
          calculateRepaymentPeriodFraction(
  ...
  private double calculateRepaymentPeriodFraction(
      final PeriodFrequencyType repaymentPeriodFrequencyType,
      final Integer every,
    ...
    switch (repaymentPeriodFrequencyType) {
      ...
      case WEEKS:
        fraction =
            numberOfDaysInterestCalculationGrace.doubleValue()
                / (Double.valueOf("7.0") * every.doubleValue());
        break;
      case MONTHS:
        fraction =
            numberOfDaysInterestCalculationGrace.doubleValue()
                / (Double.valueOf("30.0") * every.doubleValue());
        break;
      case YEARS:
        fraction =
            numberOfDaysInterestCalculationGrace.doubleValue()
                / (Double.valueOf("365.0") * every.doubleValue());
        break;
      case INVALID:
