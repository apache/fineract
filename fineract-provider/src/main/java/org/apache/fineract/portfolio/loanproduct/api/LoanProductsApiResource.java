...
public class LoanProductsApiResource {
  ...
  @ApiOperation(
      value = "Create a Loan Product",
      httpMethod = "POST",
      notes =
          "Depending of the Accounting Rule (accountingRule) selected, additional fields with"
              + " details of the appropriate Ledger Account identifiers would need to be passed"
              + " in.\n"
              + "\n"
              + "Refer MifosX Accounting Specs Draft for more details regarding the significance"
              + " of the selected accounting rule\n\n"
              + "Mandatory Fields: name, shortName, currencyCode, digitsAfterDecimal,"
              + " inMultiplesOf, principal, numberOfRepayments, repaymentEvery,"
              + " repaymentFrequencyType, interestRatePerPeriod, interestRateFrequencyType,"
              ...
              + " loanPortfolioAccountId, interestOnLoanAccountId, incomeFromFeeAccountId,"
              + " incomeFromPenaltyAccountId, writeOffAccountId, transfersInSuspenseAccountId,"
              ...
              + " recalculationRestFrequencyType is not same as repayment period:"
              + " recalculationRestFrequencyInterval, recalculationRestFrequencyDate\n\n"
              + "Additional Optional Fields if interest recalculation is enabled(true) and"
              + " interestRecalculationCompoundingMethod is enabled:"
              + " recalculationCompoundingFrequencyType\n\n"
              + "Additional Optional Fields if interest recalculation is enabled(true) and"
              + " interestRecalculationCompoundingMethod is enabled and"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = LoanProductsApiResourceSwagger.PostLoanProductsRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = LoanProductsApiResourceSwagger.PostLoanProductsResponse.class)
  })
  public String createLoanProduct(@ApiParam(hidden = true) final String apiRequestBodyAsJson) {

    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().createLoanProduct().withJson(apiRequestBodyAsJson).build();
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        response = LoanProductsApiResourceSwagger.GetLoanProductsResponse.class)
  })
  public String retrieveAllLoanProducts(@Context final UriInfo uriInfo) {
    ...
    if (!associationParameters.isEmpty()) {
      if (associationParameters.contains("productMixes")) {
        ...
        final Collection<ProductMixData> productMixes =
            this.productMixReadPlatformService.retrieveAllProductMixes();
        return this.productMixDataApiJsonSerializer.serialize(
            settings, productMixes, this.PRODUCT_MIX_DATA_PARAMETERS);
      }
    }
    ...
    return this.toApiJsonSerializer.serialize(
        settings, products, this.LOAN_PRODUCT_DATA_PARAMETERS);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve Loan Product Details Template",
      httpMethod = "GET",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = LoanProductsApiResourceSwagger.GetLoanProductsTemplateResponse.class)
  })
  public String retrieveTemplate(
      @Context final UriInfo uriInfo,
      @QueryParam("isProductMixTemplate") @ApiParam(value = "isProductMixTemplate")
          final boolean isProductMixTemplate) {
    ...
    LoanProductData loanProduct =
        this.loanProductReadPlatformService.retrieveNewLoanProductDetails();
    loanProduct = handleTemplate(loanProduct);

    return this.toApiJsonSerializer.serialize(
        settings, loanProduct, this.LOAN_PRODUCT_DATA_PARAMETERS);
  }
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = LoanProductsApiResourceSwagger.GetLoanProductsProductIdResponse.class)
  })
  public String retrieveLoanProductDetails(
    ...
    if (loanProduct.hasAccountingEnabled()) {
      accountingMappings =
          this.accountMappingReadPlatformService.fetchAccountMappingDetailsForLoanProduct(
              productId, loanProduct.accountingRuleType().getId().intValue());
      paymentChannelToFundSourceMappings =
          this.accountMappingReadPlatformService.fetchPaymentTypeToFundSourceMappingsForLoanProduct(
              productId);
      ...
      penaltyToGLAccountMappings =
          this.accountMappingReadPlatformService.fetchPenaltyToIncomeAccountMappingsForLoanProduct(
              productId);
      loanProduct =
          LoanProductData.withAccountingDetails(
              loanProduct,
              accountingMappings,
              paymentChannelToFundSourceMappings,
              feeToGLAccountMappings,
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Update a Loan Product",
      httpMethod = "PUT",
      notes = "Updates a Loan Product")
  @ApiImplicitParams({
    @ApiImplicitParam(
        value = "body",
        required = true,
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = LoanProductsApiResourceSwagger.PutLoanProductsProductIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = LoanProductsApiResourceSwagger.PutLoanProductsProductIdResponse.class)
  })
  ...
  private LoanProductData handleTemplate(final LoanProductData productData) {
    ...
    Collection<ChargeData> penaltyOptions =
        this.chargeReadPlatformService.retrieveLoanApplicablePenalties();
    ...
    final Map<String, List<GLAccountData>> accountOptions =
        this.accountingDropdownReadPlatformService.retrieveAccountMappingOptionsForLoanProducts();

    final List<EnumOptionData> accountingRuleTypeOptions =
        this.accountingDropdownReadPlatformService.retrieveAccountingRuleTypeOptions();

    final List<EnumOptionData> loanCycleValueConditionTypeOptions =
        this.dropdownReadPlatformService.retrieveLoanCycleValueConditionTypeOptions();

    final List<EnumOptionData> daysInMonthTypeOptions =
        commonDropdownReadPlatformService.retrieveDaysInMonthTypeOptions();
    final List<EnumOptionData> daysInYearTypeOptions =
        commonDropdownReadPlatformService.retrieveDaysInYearTypeOptions();
    ...
    final List<EnumOptionData> rescheduleStrategyTypeOptions =
        dropdownReadPlatformService.retrieveRescheduleStrategyTypeOptions();
    final List<EnumOptionData> interestRecalculationFrequencyTypeOptions =
        dropdownReadPlatformService.retrieveInterestRecalculationFrequencyTypeOptions();
    final List<EnumOptionData> interestRecalculationNthDayTypeOptions =
        dropdownReadPlatformService.retrieveInterestRecalculationNthDayTypeOptions();
    final List<EnumOptionData> interestRecalculationDayOfWeekTypeOptions =
        dropdownReadPlatformService.retrieveInterestRecalculationDayOfWeekTypeOptions();
    ...
    return new LoanProductData(
        ...
        loanCycleValueConditionTypeOptions,
