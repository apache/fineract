...
public class ShareProductWritePlatformServiceJpaRepositoryImpl
  ...
  public ShareProductWritePlatformServiceJpaRepositoryImpl(
      final ShareProductRepositoryWrapper repository,
  ...
  public CommandProcessingResult createShareProduct(JsonCommand jsonCommand) {
    try {
      ...
      this.accountMappingWritePlatformService.createShareProductToGLAccountMapping(
          product.getId(), jsonCommand);
  ...
  public CommandProcessingResult updateProduct(Long productId, JsonCommand jsonCommand) {
    try {
      ...
      final Map<String, Object> accountingMappingChanges =
          this.accountMappingWritePlatformService.updateShareProductToGLAccountMapping(
  ...
  @Override
  public CommandProcessingResult createShareProductDividend(
      final Long productId, final JsonCommand jsonCommand) {
    try {
      ...
      if (dividendPayOutDetails == null) {
        throw new DividentProcessingException(
            "eligible.shares.not.found", "No eligible shares for creating dividends");
      }
      ...
      this.businessEventNotifierService.notifyBusinessEventWasExecuted(
          BusinessEvents.SHARE_PRODUCT_DIVIDENDS_CREATE,
  ...
  public CommandProcessingResult approveShareProductDividend(final Long PayOutDetailId) {
    try {
      ShareProductDividendPayOutDetails dividendPayOutDetails =
          this.shareProductDividentPayOutDetailsRepository.findOneWithNotFoundDetection(
              PayOutDetailId);
      if (dividendPayOutDetails.getStatus().isApproved()) {
        throw new DividentProcessingException(
            "alreay.approved", "Can't approve already appoved  dividends ");
      }
  ...
  public CommandProcessingResult deleteShareProductDividend(final Long PayOutDetailId) {
    try {
      ...
      if (dividendPayOutDetails.getStatus().isApproved()) {
        throw new DividentProcessingException(
            "alreay.approved", "Can't delete already appoved  dividends ");
      }
  ...
  private void handleDataIntegrityIssues(final Exception e) {
    throw new PlatformDataIntegrityException(
        "error.msg.shareproduct.unknown.data.integrity.issue",
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("'name'")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.shareproduct.duplicate.name",
          "Share Product with name `" + name + "` already exists",
          "name",
          name);
    }
