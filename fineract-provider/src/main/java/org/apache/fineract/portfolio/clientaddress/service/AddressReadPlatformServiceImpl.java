...
+"addr.updated_on as updated_on from m_address as addr,m_client client";
    ...
    private static final class AddMapper implements RowMapper<AddressData>
    {
        ...
        public AddressData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException
        {
            ...
            return AddressData.instance(addressType,client_id,addressId,address_type_id,is_active,street,address_line_1,address_line_2,address_line_3,
                    town_village,city,county_district,state_province_id,
    ...
    public AddressData retrieveTemplate()
    {
        final List<CodeValueData> countryoptions=new ArrayList<>(
                this.readService.retrieveCodeValuesByCode("COUNTRY ID"));
                
        final List<CodeValueData> StateOptions=new ArrayList<>(
                this.readService.retrieveCodeValuesByCode("INDIAN STATES"));
        
        final List<CodeValueData> addressTypeOptions=new ArrayList<>(
                this.readService.retrieveCodeValuesByCode("ADDRESS TYPE ID"));
