...
public class MeetingReadPlatformServiceImpl implements MeetingReadPlatformService {
  ...
  private static final class MeetingDataMapper implements RowMapper<MeetingData> {
    ...
    @Override
    public MeetingData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  @Override
  public MeetingData retrieveMeeting(
      final Long meetingId, final Long entityId, final Integer entityTypeId) {

    try {
      final MeetingDataMapper rm = new MeetingDataMapper();

      final String sql =
          rm.schema() + " where m.id = ? and ci.entity_id = ? and ci.entity_type_enum = ? ";

      return this.jdbcTemplate.queryForObject(
          sql, rm, new Object[] {meetingId, entityId, entityTypeId});
  ...
  @Override
  public Collection<MeetingData> retrieveMeetingsByEntity(
      final Long entityId, final Integer entityTypeId, final Integer limit) {
    final MeetingDataMapper rm = new MeetingDataMapper();
  ...
  @Override
  public Collection<MeetingData> retrieveMeetingsByEntityByCalendarType(
    ...
    final String sql =
        rm.schema()
            + " inner join m_calendar c on ci.calendar_id=c.id  where ci.entity_id = ? and"
            + " ci.entity_type_enum = ? and c.calendar_type_enum in ("
  ...
  public MeetingData retrieveLastMeeting(Long calendarInstanceId) {
    try {
      ...
      final String sql =
          rm.schema() + " where ci.id = ? order by m.meeting_date desc, m.id desc limit 1";
