...
public class TransferWritePlatformServiceJpaRepositoryImpl implements TransferWritePlatformService {
  ...
  public TransferWritePlatformServiceJpaRepositoryImpl(
      ...
      final TransfersDataValidator transfersDataValidator,
      final NoteWritePlatformService noteWritePlatformService,
  ...
  @Override
  @Transactional
  public CommandProcessingResult transferClientsBetweenGroups(
    ...
    final Group destinationGroup =
        this.groupRepository.findOneWithNotFoundDetection(destinationGroupId);
    final Long staffId =
    ...
    if (staffId != null) {
      staff =
          this.staffRepositoryWrapper.findByOfficeHierarchyWithNotFoundDetection(
              staffId, sourceOffice.getHierarchy());
    }
    ...
    if (sourceGroupId.equals(destinationGroupId)) {
      throw new TransferNotSupportedException(
          TransferNotSupportedReason.SOURCE_AND_DESTINATION_GROUP_CANNOT_BE_SAME,
          sourceGroupId,
          destinationGroupId);
    }
    ...
    if (!(sourceOffice.getId().equals(destinationGroup.getOffice().getId()))) {
      throw new TransferNotSupportedException(
          TransferNotSupportedReason.BULK_CLIENT_TRANSFER_ACROSS_BRANCHES,
          sourceGroupId,
          destinationGroupId);
    }
    ...
    for (final Client client : clients) {
      transferClientBetweenGroups(
          sourceGroup, client, destinationGroup, inheritDestinationGroupLoanOfficer, staff);
    }
   ...
   * ***/
  @Transactional
  public void transferClientBetweenGroups(
     ...
     **/
    final CalendarInstance sourceGroupCalendarInstance =
    ...
    final List<CalendarInstance> activeLoanCalendarInstances =
        this.calendarInstanceRepository.findCalendarInstancesForActiveLoansByGroupIdAndClientId(
            sourceGroup.getId(), client.getId());
    ...
    if (sourceGroupCalendarInstance != null && !activeLoanCalendarInstances.isEmpty()) {
      ...
      final CalendarInstance destinationGroupCalendarInstance =
          this.calendarInstanceRepository.findByEntityIdAndEntityTypeIdAndCalendarTypeId(
       ...
       ***/
      if (!(CalendarUtils.isFrequencySame(
              sourceGroupCalendar.getRecurrence(), destinationGroupCalendar.getRecurrence())
          && CalendarUtils.isIntervalSame(
              sourceGroupCalendar.getRecurrence(), destinationGroupCalendar.getRecurrence()))) {
        throw new TransferNotSupportedException(
            TransferNotSupportedReason.DESTINATION_GROUP_MEETING_FREQUENCY_MISMATCH,
            sourceGroup.getId(),
            destinationGroup.getId());
      }
      ...
      this.loanWritePlatformService.applyMeetingDateChanges(
          destinationGroupCalendar, activeLoanCalendarInstances);
    }
    ...
    } /*** Else default to destination group Officer (If present) ***/ else if (destinationGroupLoanOfficer
        != null) {
    ...
    final List<Loan> allClientJLGLoans =
        this.loanRepositoryWrapper.findByClientIdAndGroupId(client.getId(), sourceGroup.getId());
    for (final Loan loan : allClientJLGLoans) {
      if (loan.status().isActiveOrAwaitingApprovalOrDisbursal()) {
        ...
        if (inheritDestinationGroupLoanOfficer != null
  ...
  @Transactional
  @Override
  public CommandProcessingResult proposeAndAcceptClientTransfer(
    ...
    handleClientTransferLifecycleEvent(
        client, client.getTransferToOffice(), TransferEventType.ACCEPTANCE, jsonCommand);
  ...
  @Transactional
  @Override
  public CommandProcessingResult proposeClientTransfer(
      final Long clientId, final JsonCommand jsonCommand) {
    // validation
  ...
  @Transactional
  @Override
  public CommandProcessingResult acceptClientTransfer(
      final Long clientId, final JsonCommand jsonCommand) {
    // validation
  ...
  @Override
  @Transactional
  public CommandProcessingResult withdrawClientTransfer(
      final Long clientId, final JsonCommand jsonCommand) {
    // validation
    ...
    handleClientTransferLifecycleEvent(
        client, client.getOffice(), TransferEventType.WITHDRAWAL, jsonCommand);
    this.clientRepositoryWrapper.save(client);
    return new CommandProcessingResultBuilder() //
        .withClientId(clientId) //
        .withEntityId(clientId) //
  ...
  @Override
  @Transactional
  public CommandProcessingResult rejectClientTransfer(
      final Long clientId, final JsonCommand jsonCommand) {
    // validation
    this.transfersDataValidator.validateForRejectClientTransfer(jsonCommand.json());
    final Client client = this.clientRepositoryWrapper.findOneWithNotFoundDetection(clientId);
    handleClientTransferLifecycleEvent(
        client, client.getOffice(), TransferEventType.REJECTION, jsonCommand);
    this.clientRepositoryWrapper.save(client);
  ...
  private void handleClientTransferLifecycleEvent(
      final Client client,
      final Office destinationOffice,
      final TransferEventType transferEventType,
    ...
    if (staffId != null) {
      staff =
          this.staffRepositoryWrapper.findByOfficeHierarchyWithNotFoundDetection(
              staffId, destinationOffice.getHierarchy());
    }
    if (transferEventType.isAcceptance() && destinationGroupId != null) {
      destinationGroup =
          this.groupRepository.findByOfficeWithNotFoundDetection(
              destinationGroupId, destinationOffice);
    }
      ...
      for (final Loan loan : this.loanRepositoryWrapper.findLoanByClientId(client.getId())) {
        ...
        if (loan.isDisbursed() && !loan.isClosed()) {
          switch (transferEventType) {
            case ACCEPTANCE:
              this.loanWritePlatformService.acceptLoanTransfer(
                  loan, loan.getLastUserTransactionDate(), destinationOffice, staff);
              break;
            case PROPOSAL:
            ...
            case REJECTION:
              ...
              break;
            case WITHDRAWAL:
              this.loanWritePlatformService.withdrawLoanTransfer(
                  loan, loan.getLastUserTransactionDate());
          }
        }
      }
    }
    ...
    if (this.savingsAccountRepositoryWrapper.doNonClosedSavingAccountsExistForClient(
      ...
      for (final SavingsAccount savingsAccount :
        ...
        if (savingsAccount.isActivated() && !savingsAccount.isClosed()) {
          switch (transferEventType) {
            case ACCEPTANCE:
              this.savingsAccountWritePlatformService.acceptSavingsTransfer(
                  savingsAccount,
              ...
              break;
            case PROPOSAL:
              this.savingsAccountWritePlatformService.initiateSavingsTransfer(
                  savingsAccount, transferDate);
              break;
            case REJECTION:
              this.savingsAccountWritePlatformService.rejectSavingsTransfer(savingsAccount);
              break;
            case WITHDRAWAL:
              this.savingsAccountWritePlatformService.withdrawSavingsTransfer(
      ...
      case ACCEPTANCE:
        ...
        if (client.getGroups().size() == 1) {
          if (destinationGroup == null) {
            throw new TransferNotSupportedException(
                TransferNotSupportedReason.CLIENT_DESTINATION_GROUP_NOT_SPECIFIED, client.getId());
          } else if (!destinationGroup.isActive()) {
            throw new GroupNotActiveException(destinationGroup.getId());
          }
          transferClientBetweenGroups(
              Iterables.get(client.getGroups(), 0), client, destinationGroup, true, staff);
    ...
    this.clientTransferDetailsRepositoryWrapper.save(
        ClientTransferDetails.instance(
            client.getId(),
