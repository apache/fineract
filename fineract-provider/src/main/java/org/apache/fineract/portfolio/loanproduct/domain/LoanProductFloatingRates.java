...
public class LoanProductFloatingRates extends AbstractPersistableCustom {
  ...
  public Map<? extends String, ? extends Object> update(
      JsonCommand command, FloatingRate floatingRate) {
    ...
    if (command.isChangeInBigDecimalParameterNamed(
        interestRateDifferential, this.interestRateDifferential)) {
    ...
    if (command.isChangeInBigDecimalParameterNamed(
        minDifferentialLendingRate, this.minDifferentialLendingRate)) {
    ...
    if (command.isChangeInBigDecimalParameterNamed(
        defaultDifferentialLendingRate, this.defaultDifferentialLendingRate)) {
      final BigDecimal newValue =
    ...
    if (command.isChangeInBigDecimalParameterNamed(
        maxDifferentialLendingRate, this.maxDifferentialLendingRate)) {
      final BigDecimal newValue =
    ...
    final String isFloatingInterestRateCalculationAllowed =
        "isFloatingInterestRateCalculationAllowed";
    if (command.isChangeInBooleanParameterNamed(
        isFloatingInterestRateCalculationAllowed, this.isFloatingInterestRateCalculationAllowed)) {
      final boolean newValue =
          command.booleanPrimitiveValueOfParameterNamed(isFloatingInterestRateCalculationAllowed);
