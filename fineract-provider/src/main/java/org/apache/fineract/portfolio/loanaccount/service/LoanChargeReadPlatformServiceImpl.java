...
public class LoanChargeReadPlatformServiceImpl implements LoanChargeReadPlatformService {
  ...
  private static final class LoanChargeMapper implements RowMapper<LoanChargeData> {
    ...
    public String schema() {
      return "lc.id as id, c.id as chargeId, c.name as name, lc.amount as amountDue,"
          + " lc.amount_paid_derived as amountPaid, lc.amount_waived_derived as"
          + " amountWaived, lc.amount_writtenoff_derived as amountWrittenOff,"
          + " lc.amount_outstanding_derived as amountOutstanding, lc.calculation_percentage"
          + " as percentageOf, lc.calculation_on_amount as amountPercentageAppliedTo,"
          + " lc.charge_time_enum as chargeTime, lc.is_penalty as penalty,"
          + " lc.due_for_collection_as_of_date as dueAsOfDate, lc.charge_calculation_enum"
          + " as chargeCalculation, lc.charge_payment_mode_enum as chargePaymentMode,"
          + " lc.is_paid_derived as paid, lc.waived as waied, lc.min_cap as minCap,"
          + " lc.max_cap as maxCap, lc.charge_amount_or_percentage as amountOrPercentage,"
          + " c.currency_code as currencyCode, oc.name as currencyName,"
          + " date(ifnull(dd.disbursedon_date,dd.expected_disburse_date)) as"
          + " disbursementDate, oc.decimal_places as currencyDecimalPlaces,"
          ...
          + " join m_loan_charge lc on lc.charge_id = c.id left join"
          + " m_loan_tranche_disbursement_charge dc on dc.loan_charge_id=lc.id left join"
          + " m_loan_disbursement_detail dd on dd.id=dc.disbursement_detail_id ";
    }

    @Override
    public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  public ChargeData retrieveLoanChargeTemplate() {
    ...
    final List<EnumOptionData> loansChargeCalculationTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveLoanCalculationTypes();
    final List<EnumOptionData> loansChargeTimeTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveLoanCollectionTimeTypes();
    final List<EnumOptionData> savingsChargeCalculationTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveSavingsCalculationTypes();
    final List<EnumOptionData> savingsChargeTimeTypeOptions =
        this.chargeDropdownReadPlatformService.retrieveSavingsCollectionTimeTypes();
    ...
    return ChargeData.template(
        null,
        allowedChargeCalculationTypeOptions,
        null,
  ...
  public Collection<LoanChargeData> retrieveLoanCharges(final Long loanId) {
    ...
    final String sql =
        "select "
            + rm.schema()
            + " where lc.loan_id=? AND lc.is_active = 1 order by"
  ...
  @Override
  public Collection<LoanChargeData> retrieveLoanChargesForFeePayment(
      final Integer paymentMode, final Integer loanStatus) {
    final LoanChargeMapperWithLoanId rm = new LoanChargeMapperWithLoanId();
    final String sql =
        "select "
            + rm.schema()
            + "where loan.loan_status_id= ? and lc.charge_payment_mode_enum=? and lc.waived =0 and"
            + " lc.is_paid_derived=0 and lc.is_active = 1";
  ...
  @Override
  public Collection<LoanInstallmentChargeData> retrieveInstallmentLoanCharges(
  ...
  private static final class LoanInstallmentChargeMapper
    ...
    @Override
    public LoanInstallmentChargeData mapRow(
      ...
      return new LoanInstallmentChargeData(
          installmentNumber, dueAsOfDate, amount, amountOutstanding, amountWaived, paid, waived);
    }
  }

  @Override
  public Collection<Integer> retrieveOverdueInstallmentChargeFrequencyNumber(
      final Long loanId, final Long chargeId, final Integer periodNumber) {
    String sql =
        "select oic.frequency_number from m_loan_overdue_installment_charge oic  inner join"
            + " m_loan_charge lc on lc.id=oic.loan_charge_id inner join m_loan_repayment_schedule"
            + " rs on rs.id = oic.loan_schedule_id inner join m_loan loan on loan.id=rs.loan_id"
  ...
  public Collection<LoanChargeData> retrieveLoanChargesForAccural(final Long loanId) {
    ...
    final String sql =
        "select "
            + rm.schema()
            + " where lc.loan_id=? AND lc.is_active = 1 group by  lc.id  order by"
            + " lc.charge_time_enum ASC, lc.due_for_collection_as_of_date ASC, lc.is_penalty ASC";

    Collection<LoanChargeData> charges =
        this.jdbcTemplate.query(
            sql, rm, new Object[] {LoanTransactionType.ACCRUAL.getValue(), loanId, loanId});
    ...
    for (LoanChargeData loanChargeData : removeCharges) {
      if (loanChargeData.isInstallmentFee()) {
        Collection<LoanInstallmentChargeData> installmentChargeDatas =
            retrieveInstallmentLoanChargesForAccrual(loanChargeData.getId());
        LoanChargeData modifiedChargeData =
            new LoanChargeData(loanChargeData, installmentChargeDatas);
  ...
  private static final class LoanChargeAccrualMapper implements RowMapper<LoanChargeData> {
    ...
    public LoanChargeAccrualMapper() {
      ...
      sb.append(
          "inner join m_loan_transaction lt on lt.id = lcp.loan_transaction_id and lt.is_reversed"
              + " = 0 and lt.transaction_type_enum = ? and lt.loan_id = ?");
    ...
    public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
      ...
      return new LoanChargeData(
          id, chargeId, dueAsOfDate, chargeTimeType, amount, amountAccrued, amountWaived, penalty);
    }
  }

  private Collection<LoanChargeData> updateLoanChargesWithUnrecognizedIncome(
      final Long loanId, Collection<LoanChargeData> loanChargeDatas) {

    final LoanChargeUnRecognizedIncomeMapper rm =
        new LoanChargeUnRecognizedIncomeMapper(loanChargeDatas);
    ...
    return this.jdbcTemplate.query(
        sql, rm, new Object[] {LoanTransactionType.WAIVE_CHARGES.getValue(), loanId, loanId});
  }

  private static final class LoanChargeUnRecognizedIncomeMapper
      implements RowMapper<LoanChargeData> {
    ...
    public LoanChargeUnRecognizedIncomeMapper(final Collection<LoanChargeData> datas) {
      ...
      sb.append(
          "inner join m_loan_transaction lt on lt.id = cpb.loan_transaction_id and lt.is_reversed"
              + " = 0 and lt.transaction_type_enum = ?  and lt.loan_id = ? ");
  ...
  private Collection<LoanInstallmentChargeData> retrieveInstallmentLoanChargesForAccrual(
      Long loanChargeId) {
    final LoanInstallmentChargeAccrualMapper rm = new LoanInstallmentChargeAccrualMapper();
    String sql =
        "select "
            + rm.schema()
            + " where lic.loan_charge_id= ?  group by lsi.installment, lsi.duedate,"
            + " lic.amount_outstanding_derived, lic.amount, lic.is_paid_derived,"
            + " lic.amount_waived_derived, lic.waived";
    Collection<LoanInstallmentChargeData> chargeDatas =
        this.jdbcTemplate.query(
            sql, rm, new Object[] {LoanTransactionType.ACCRUAL.getValue(), loanChargeId});
    ...
    for (LoanInstallmentChargeData installmentChargeData : chargeDatas) {
      installmentChargeDatas.put(
          installmentChargeData.getInstallmentNumber(), installmentChargeData);
    }
    chargeDatas =
        updateInstallmentLoanChargesWithUnrecognizedIncome(loanChargeId, installmentChargeDatas);
    for (LoanInstallmentChargeData installmentChargeData : chargeDatas) {
      installmentChargeDatas.put(
          installmentChargeData.getInstallmentNumber(), installmentChargeData);
    }
  ...
  private static final class LoanInstallmentChargeAccrualMapper
      implements RowMapper<LoanInstallmentChargeData> {
    ...
    public LoanInstallmentChargeAccrualMapper() {
      ...
      sb.append(
          "inner join m_loan_transaction lt on lt.id = lcp.loan_transaction_id and lt.is_reversed"
              + " = 0 and lt.transaction_type_enum = ?");
      sb.append(
          ") cp on  cp.loan_charge_id= lic.loan_charge_id and  cp.installment_number ="
              + " lsi.installment ");
      schemaSql = sb.toString();
    }
  ...
  private Collection<LoanInstallmentChargeData> updateInstallmentLoanChargesWithUnrecognizedIncome(
      final Long loanChargeId,
      final Map<Integer, LoanInstallmentChargeData> installmentChargeDatas) {
    final LoanInstallmentChargeUnRecognizedIncomeMapper rm =
        new LoanInstallmentChargeUnRecognizedIncomeMapper(installmentChargeDatas);
    String sql =
        "select " + rm.schema() + " where cpb.loan_charge_id = ? group by cpb.installment_number  ";
    return this.jdbcTemplate.query(
        sql, rm, new Object[] {LoanTransactionType.WAIVE_CHARGES.getValue(), loanChargeId});
  }
  ...
  private static final class LoanInstallmentChargeUnRecognizedIncomeMapper
    ...
    public LoanInstallmentChargeUnRecognizedIncomeMapper(
        final Map<Integer, LoanInstallmentChargeData> installmentChargeDatas) {
    ...
    public LoanInstallmentChargeData mapRow(
      ...
      final Integer installmentNumber = rs.getInt("installmentNumber");
      final BigDecimal amountUnrecognized = rs.getBigDecimal("amountUnrecognized");
      LoanInstallmentChargeData installmentChargeData =
          this.installmentChargeDatas.get(installmentNumber);
      return new LoanInstallmentChargeData(installmentChargeData, amountUnrecognized);
    }
  }

  @Override
  public Collection<LoanChargePaidByData> retriveLoanChargesPaidBy(
      Long chargeId, final LoanTransactionType transactionType, final Integer installmentNumber) {
  ...
  private static final class LoanChargesPaidByMapper implements RowMapper<LoanChargePaidByData> {
    ...
    public LoanChargesPaidByMapper() {
      ...
      sb.append(" from m_loan_charge_paid_by lcp ");
      sb.append(
          " join m_loan_transaction lt on lt.id = lcp.loan_transaction_id and lt.is_reversed=0");
    ...
    @Override
    public LoanChargePaidByData mapRow(
        final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
