...
@Scope("singleton")
@Api(tags = {"Pocket"})
@SwaggerDefinition(tags = {@Tag(name = "Pocket", description = "")})
public class PocketApiResource {
  ...
  @ApiOperation(
      value = "Link/delink accounts to/from pocket",
      httpMethod = "POST",
      notes =
          "Pockets behave as favourites. An user can link his/her Loan, Savings and Share accounts"
              + " to pocket for faster access. In a similar way linked accounts can be delinked"
              + " from the pocket.\n\n"
              + "Example Requests:\n"
              + "\n"
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = PocketApiResourceSwagger.PostLinkDelinkAccountsToFromPocketResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve accounts linked to pocket",
      httpMethod = "GET",
      notes = "All linked loan\n\n" + "Example Requests:\n" + "\n" + "\n" + "self/pockets")
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = PocketApiResourceSwagger.GetAccountsLinkedToPocketResponse.class)
  })
