...
public class ClientIdentifierReadPlatformServiceImpl
  ...
  @Autowired
  public ClientIdentifierReadPlatformServiceImpl(
    ...
    try {
      ...
      final ClientIdentifierData clientIdentifierData =
          this.jdbcTemplate.queryForObject(
              sql, rm, new Object[] {clientId, hierarchySearchString, clientIdentifierId});
  ...
  private static final class ClientIdentityMapper implements RowMapper<ClientIdentifierData> {
    ...
    public String schema() {
      return "ci.id as id, ci.client_id as clientId, ci.document_type_id as documentTypeId,"
          + " ci.status as status, ci.document_key as documentKey, ci.description as"
          + " description, cv.code_value as documentType  from m_client_identifier ci,"
          + " m_client c, m_office o, m_code_value cv where ci.client_id=c.id and"
          + " c.office_id=o.id and ci.document_type_id=cv.id and ci.client_id = ? and"
          + " o.hierarchy like ? ";
    }

    @Override
    public ClientIdentifierData mapRow(
      ...
      return ClientIdentifierData.singleItem(
          id, clientId, documentType, documentKey, status, description);
    }
  }
}
