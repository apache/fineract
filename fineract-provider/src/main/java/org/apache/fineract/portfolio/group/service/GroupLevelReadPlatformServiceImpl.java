...
public class GroupLevelReadPlatformServiceImpl implements GroupLevelReadPlatformService {
  ...
  @Autowired
  public GroupLevelReadPlatformServiceImpl(
  ...
  private static final class GroupLevelDataMapper implements RowMapper<GroupLevelData> {
    ...
    public String groupLevelSchema() {
      return "gl.id as id, gl.level_name as levelName , gl.parent_id as parentLevelId ,"
          + " pgl.level_name as parentName , cgl.id as childLevelId,cgl.level_name as"
          + " childLevelName,gl.super_parent as superParent , gl.recursable as recursable ,"
          + " gl.can_have_clients as canHaveClients from m_group_level gl  left join"
          + " m_group_level pgl on pgl.id = gl.parent_id left join m_group_level cgl on"
          + " gl.id = cgl.parent_id";
    }

    @Override
    public GroupLevelData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
