...
public class PostingPeriod {
  ...
  public static PostingPeriod createFrom(
      final LocalDateInterval periodInterval,
      final Money periodStartingBalance,
      final List<SavingsAccountTransaction> orderedListOfTransactions,
      ...
      Collection<Long> interestPostTransactions,
  ...
  public static PostingPeriod createFrom(
    ...
    if (accountEndOfDayBalances.isEmpty()) {
      ...
      final EndOfDayBalance endOfDayBalance =
          EndOfDayBalance.from(
              balanceStartDate, openingDayBalance, closeOfDayBalance, numberOfDaysOfBalance);
    ...
    final List<CompoundingPeriod> compoundingPeriods =
        compoundingPeriodsInPostingPeriod(
            periodInterval,
            interestCompoundingPeriodType,
  ...
  private PostingPeriod(
    ...
    else this.dateOfPostingTransaction = periodInterval.endDate().plusDays(1);
  ...
  public BigDecimal calculateInterest(final CompoundInterestValues compoundInterestValues) {
    ...
    for (final CompoundingPeriod compoundingPeriod : this.compoundingPeriods) {
      ...
      final BigDecimal interestUnrounded =
          compoundingPeriod.calculateInterest(
              this.interestCompoundingType,
              this.interestCalculationType,
              compoundInterestValues.getcompoundedInterest(),
              this.interestRateAsFraction,
              this.daysInYear,
              this.minBalanceForInterestCalculation.getAmount(),
              this.overdraftInterestRateAsFraction,
              this.minOverdraftForInterestCalculation.getAmount());
      BigDecimal unCompoundedInterest =
          compoundInterestValues.getuncompoundedInterest().add(interestUnrounded);
      ...
      if (!SavingsCompoundingInterestPeriodType.DAILY.equals(this.interestCompoundingType)) {
        compoundingPeriodEndDate =
            determineInterestPeriodEndDateFrom(
                compoundingPeriod.getPeriodInterval().startDate(),
                this.interestCompoundingType,
  ...
  private static List<CompoundingPeriod> compoundingPeriodsInPostingPeriod(
    ...
    switch (interestPeriodType) {
      ...
      case DAILY:
        compoundingPeriod =
            DailyCompoundingPeriod.create(
                postingPeriodInterval, allEndOfDayBalances, upToInterestCalculationDate);
      ...
      case MONTHLY:
        ...
        while (!periodStartDate.isAfter(postingPeriodEndDate)
            && !periodEndDate.isAfter(postingPeriodEndDate)) {
          ...
          final LocalDateInterval compoundingPeriodInterval =
              LocalDateInterval.create(periodStartDate, periodEndDate);
      ...
      case QUATERLY:
        ...
        while (!periodStartDate.isAfter(qPostingPeriodEndDate)
          ...
          if (postingPeriodInterval.contains(compoundingPeriodInterval)) {
            ...
            compoundingPeriod =
                QuarterlyCompoundingPeriod.create(
                    compoundingPeriodInterval, allEndOfDayBalances, upToInterestCalculationDate);
            compoundingPeriods.add(compoundingPeriod);
          }
      ...
      case BI_ANNUAL:
        ...
        while (!periodStartDate.isAfter(bPostingPeriodEndDate)
          ...
          if (postingPeriodInterval.contains(compoundingPeriodInterval)) {
            ...
            compoundingPeriod =
                BiAnnualCompoundingPeriod.create(
                    compoundingPeriodInterval, allEndOfDayBalances, upToInterestCalculationDate);
            compoundingPeriods.add(compoundingPeriod);
          }
  ...
  private static LocalDate determineInterestPeriodEndDateFrom(
    ...
    switch (interestPeriodType) {
      ...
      case QUATERLY:
        periodsInMonth = 4;
        periodEndDate =
            getPeriodEndDate(periodEndDate, previousMonth, periodsInMonth, periodStartDate);
        break;
      case BI_ANNUAL:
        periodsInMonth = 2;
        periodEndDate =
  ...
  private static LocalDate getPeriodEndDate(
      LocalDate periodEndDate, int previousMonth, int periodsInMonth, LocalDate periodStartDate) {
      ...
      periodEndDate =
          new DateTime()
              .withDate(
                  ...
                  monthSet.first(),
                  DateUtils.getLocalDateOfTenant()
                      .withMonthOfYear(monthSet.first())
                      .dayOfMonth()
                      .withMaximumValue()
                      .getDayOfMonth())
              .toLocalDate();
