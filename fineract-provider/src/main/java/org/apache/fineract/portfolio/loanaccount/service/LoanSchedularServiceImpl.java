...
public class LoanSchedularServiceImpl implements LoanSchedularService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(LoanSchedularServiceImpl.class);
  ...
  @Autowired
  public LoanSchedularServiceImpl(
  ...
  public void applyChargeForOverdueLoans() throws JobExecutionException {
    ...
    final Collection<OverdueLoanScheduleData> overdueLoanScheduledInstallments =
        this.loanReadPlatformService.retrieveAllLoansWithOverdueInstallments(
    ...
    if (!overdueLoanScheduledInstallments.isEmpty()) {
      ...
      for (final Long loanId : overdueScheduleData.keySet()) {
        try {
          this.loanWritePlatformService.applyOverdueChargesForLoan(
              loanId, overdueScheduleData.get(loanId));

        } catch (final PlatformApiDataValidationException e) {
          final List<ApiParameterError> errors = e.getErrors();
          for (final ApiParameterError error : errors) {
            logger.error(
                "Apply Charges due for overdue loans failed for account {} with message: {}",
                loanId,
  ...
  private void recalculateInterest(OfficeData office, int threadPoolSize, int batchSize) {
    ...
    List<Long> loanIds =
        Collections.synchronizedList(
            this.loanReadPlatformService.fetchLoansForInterestRecalculation(
                pageSize, maxLoanIdInList, officeHierarchy));
  ...
  private void recalculateInterest(
    ...
    for (long i = 0; i < loopCount; i++) {
      ...
      RecalculateInterestPoster poster =
          (RecalculateInterestPoster) this.applicationContext.getBean("recalculateInterestPoster");
