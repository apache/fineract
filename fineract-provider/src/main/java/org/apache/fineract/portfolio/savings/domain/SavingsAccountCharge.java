...
public class SavingsAccountCharge extends AbstractPersistableCustom {
  ...
  public static SavingsAccountCharge createNewFromJson(
    ...
    return new SavingsAccountCharge(
        savingsAccount,
        chargeDefinition,
        amount,
        chargeTime,
  ...
  public static SavingsAccountCharge createNewWithoutSavingsAccount(
      final Charge chargeDefinition,
      final BigDecimal amountPayable,
  ...
  private SavingsAccountCharge(
    ...
    this.chargeTime =
        (chargeTime == null) ? chargeDefinition.getChargeTimeType() : chargeTime.getValue();
  ...
  private void populateDerivedFields(
      final BigDecimal transactionAmount, final BigDecimal chargeAmount) {
  ...
  private BigDecimal calculateAmountOutstanding(final MonetaryCurrency currency) {
    return getAmount(currency)
        .minus(getAmountWaived(currency))
        .minus(getAmountPaid(currency))
        .getAmount();
  }
  ...
  @SuppressFBWarnings(
      value =
          "NP_NULL_PARAM_DEREF_NONVIRTUAL") // https://issues.apache.org/jira/browse/FINERACT-987
  ...
  private LocalDate setDayOfMonth(LocalDate nextDueLocalDate) {
    ...
    int newDayOfMonth =
        (this.feeOnDay.intValue() < maxDayOfMonth) ? this.feeOnDay.intValue() : maxDayOfMonth;
  ...
  public boolean isChargeIsOverPaid(final LocalDate nextDueDate) {
    ...
    return this.getDueLocalDate().isAfter(nextDueDate)
        && amountPaid.compareTo(BigDecimal.ZERO) == 1;
  }
