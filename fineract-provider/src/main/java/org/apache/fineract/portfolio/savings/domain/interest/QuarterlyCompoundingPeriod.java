...
public class QuarterlyCompoundingPeriod implements CompoundingPeriod {
  ...
  public static QuarterlyCompoundingPeriod create(
  ...
  public BigDecimal calculateInterest(
    ...
    switch (interestCalculationType) {
      case DAILY_BALANCE:
        interestEarned =
            calculateUsingDailyBalanceMethod(
      ...
      case AVERAGE_DAILY_BALANCE:
        interestEarned =
            calculateUsingAverageDailyBalanceMethod(
                interestToCompound,
                interestRateAsFraction,
                daysInYear,
                minBalanceForInterestCalculation,
  ...
  private BigDecimal calculateUsingAverageDailyBalanceMethod(
      final BigDecimal interestToCompound,
      final BigDecimal interestRateAsFraction,
      final long daysInYear,
      final BigDecimal minBalanceForInterestCalculation,
    ...
    if (cumulativeBalance.compareTo(BigDecimal.ZERO) != 0 && numberOfDays > 0) {
      final BigDecimal averageDailyBalance =
          cumulativeBalance
              .divide(BigDecimal.valueOf(numberOfDays), MathContext.DECIMAL64)
      ...
      if (averageDailyBalance.compareTo(BigDecimal.ZERO) >= 0) {
        if (averageDailyBalance.compareTo(minBalanceForInterestCalculation) >= 0) {
          ...
          final BigDecimal periodicInterestRate =
              dailyInterestRate.multiply(BigDecimal.valueOf(numberOfDays), MathContext.DECIMAL64);
          interestEarned =
              averageDailyBalance
  ...
  private BigDecimal calculateUsingDailyBalanceMethod(
      final SavingsCompoundingInterestPeriodType compoundingInterestPeriodType,
      final BigDecimal interestToCompound,
      final BigDecimal interestRateAsFraction,
      final long daysInYear,
    ...
    for (final EndOfDayBalance balance : this.endOfDayBalances) {
      ...
      switch (compoundingInterestPeriodType) {
        ...
        case MONTHLY:
          interestOnBalanceUnrounded =
              balance.calculateInterestOnBalance(
                  interestToCompound,
                  interestRateAsFraction,
                  daysInYear,
                  minBalanceForInterestCalculation,
  ...
  private static List<EndOfDayBalance> endOfDayBalancesWithinPeriodInterval(
    ...
    for (final EndOfDayBalance endOfDayBalance : allEndOfDayBalances) {
      ...
      if (compoundingPeriodInterval.contains(endOfDayBalance.date())) {
        final EndOfDayBalance cappedToPeriodEndDate =
  ...
  private QuarterlyCompoundingPeriod(
