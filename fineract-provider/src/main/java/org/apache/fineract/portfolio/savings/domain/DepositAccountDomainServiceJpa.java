...
public class DepositAccountDomainServiceJpa implements DepositAccountDomainService {
  ...
  @Autowired
  public DepositAccountDomainServiceJpa(
  ...
  public SavingsAccountTransaction handleWithdrawal(
    ...
    SavingsTransactionBooleanValues transactionBooleanValues =
        new SavingsTransactionBooleanValues(
            ...
            isInterestTransfer,
            isWithdrawBalance);
    return this.savingsAccountDomainService.handleWithdrawal(
        account, fmt, transactionDate, transactionAmount, paymentDetail, transactionBooleanValues);
  }

  @Transactional
  @Override
  public SavingsAccountTransaction handleFDDeposit(
    ...
    return this.savingsAccountDomainService.handleDeposit(
        account,
        fmt,
        transactionDate,
        transactionAmount,
  ...
  public SavingsAccountTransaction handleRDDeposit(
    ...
    if (isAnyActivationChargesDue) {
      ...
      account.processAccountUponActivation(
          isSavingsInterestPostingAtCurrentPeriodEnd, financialYearBeginningMonth, user);
  ...
  @Transactional
  @Override
  public SavingsAccountTransaction handleSavingDeposit(
      final SavingsAccount account,
      final DateTimeFormatter fmt,
      final LocalDate transactionDate,
      final BigDecimal transactionAmount,
  ...
  private CalendarInstance getCalendarInstance(
      final RecurringDepositAccount account, RecurringDepositAccount reinvestedDeposit) {
    ...
    if (account.isCalendarInherited()) {
      ...
      final Calendar calendar =
          Calendar.createRepeatingCalendar(
              title, calendarStartDate, CalendarType.COLLECTION.getValue(), recurrence);
      calendarInstance =
          CalendarInstance.from(
              calendar, reinvestedDeposit.getId(), CalendarEntityType.SAVINGS.getValue());
    }
  ...
  private void updateAlreadyPostedTransactions(
      final Set<Long> existingTransactionIds, final SavingsAccount savingsAccount) {
