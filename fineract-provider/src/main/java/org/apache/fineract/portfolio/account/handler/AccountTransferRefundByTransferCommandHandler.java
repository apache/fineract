...
public class AccountTransferRefundByTransferCommandHandler
        implements CommandHandler<AccountTransferCreateRequest, AccountTransferCreateResponse> {
    ...
    public boolean matches(Command<AccountTransferCreateRequest> command) {
        return command instanceof AccountTransferRefundByTransferCommand;
    }

    @Retry(name = "commandAccountTransferRefundByTransfer", fallbackMethod = "fallback")
    @Override
    @Transactional
    public AccountTransferCreateResponse handle(Command<AccountTransferCreateRequest> command) {
        return writePlatformService.refundByTransfer(command.getPayload());
    }
