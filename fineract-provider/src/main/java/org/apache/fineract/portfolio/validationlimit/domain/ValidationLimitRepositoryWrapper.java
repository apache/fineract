 ...
 */
@Service
public class ValidationLimitRepositoryWrapper {

    private final ValidationLimitRepository repository;

    @Autowired
    public ValidationLimitRepositoryWrapper(final ValidationLimitRepository repository) {
        this.repository = repository;
    }

    public ValidationLimit findOneWithNotFoundDetection(final Long id) {

        final ValidationLimit validationLimit = this.repository.findById(id).orElse(null);
        if (validationLimit == null) {
            throw new ValidationLimitNotFoundException(id);
        }
