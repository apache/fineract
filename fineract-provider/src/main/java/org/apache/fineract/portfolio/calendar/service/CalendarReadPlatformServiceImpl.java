...
public class CalendarReadPlatformServiceImpl implements CalendarReadPlatformService {
    ...
    public Boolean isEntitySyncWithMeeting(final CalendarData calendarData) {
        ...
        isSkipMeetingOnFirstDay = this.findCalendarInstaneByEntityIdAndCalendarId(entityId, calendarId);

        return isSkipMeetingOnFirstDay;
    }

    @Override
    public Boolean findCalendarInstaneByEntityIdAndCalendarId(final Long entityId, final Long calendarId) {
        String query = "Select COUNT(*) from m_calendar_instance ci where ci.entity_id = ? and ci.calendar_id = ?";
        try {
            return this.jdbcTemplate.queryForObject(query, new Object[]{entityId, calendarId}, Boolean.class);
        } catch (final EmptyResultDataAccessException e) {
            return false;
        }
    }
