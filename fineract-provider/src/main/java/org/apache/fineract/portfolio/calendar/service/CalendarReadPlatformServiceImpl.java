...
public class CalendarReadPlatformServiceImpl implements CalendarReadPlatformService {
  ...
  private static final class CalendarDataMapper implements RowMapper<CalendarData> {
    ...
    public String schema() {
      return " select c.id as id, ci.id as calendarInstanceId, ci.entity_id as entityId,"
          + " ci.entity_type_enum as entityTypeId, c.title as title,  c.description as"
          + " description, c.location as location, c.start_date as startDate, c.end_date as"
          + " endDate,  c.duration as duration, c.calendar_type_enum as typeId, c.repeating"
          + " as repeating,  c.recurrence as recurrence, c.remind_by_enum as remindById,"
          + " c.first_reminder as firstReminder, c.second_reminder as secondReminder, "
          ...
          + "  updatingUser.id as updatingUserId, updatingUser.username as"
          + " updatingUserName,c.meeting_time as meetingTime  from m_calendar c join"
          + " m_calendar_instance ci on ci.calendar_id=c.id, m_appuser as creatingUser,"
          + " m_appuser as updatingUser where c.createdby_id=creatingUser.id and"
          + " c.lastmodifiedby_id=updatingUser.id ";
    }

    @Override
    public CalendarData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
      ...
      final EnumOptionData repeatsOnNthDayOfMonth =
          CalendarEnumerations.calendarFrequencyNthDayType(
              CalendarUtils.getRepeatsOnNthDayOfMonth(recurrence));
  ...
  @Override
  public CalendarData retrieveCalendar(
    ...
    try {
      ...
      final String sql =
          rm.schema() + " and c.id = ? and ci.entity_id = ? and ci.entity_type_enum = ? ";

      return this.jdbcTemplate.queryForObject(
          sql, rm, new Object[] {calendarId, entityId, entityTypeId});
  ...
  @Override
  public Collection<CalendarData> retrieveCalendarsByEntity(
      final Long entityId, final Integer entityTypeId, final List<Integer> calendarTypeOptions) {
    final CalendarDataMapper rm = new CalendarDataMapper();
    ...
    if (calendarTypeOptions == null || calendarTypeOptions.isEmpty()) {
      sql =
          rm.schema() + " and ci.entity_id = ? and ci.entity_type_enum = ? order by c.start_date ";
      result = this.jdbcTemplate.query(sql, rm, new Object[] {entityId, entityTypeId});
    } else if (!calendarTypeOptions.isEmpty()) {
      final String sqlCalendarTypeOptions =
          CalendarUtils.getSqlCalendarTypeOptionsInString(calendarTypeOptions);
      sql =
          rm.schema()
              + " and ci.entity_id = ? and ci.entity_type_enum = ? and c.calendar_type_enum in ( "
  ...
  @Override
  public CalendarData retrieveCollctionCalendarByEntity(
      final Long entityId, final Integer entityTypeId) {
    final CalendarDataMapper rm = new CalendarDataMapper();

    final String sql =
        rm.schema()
            + " and ci.entity_id = ? and ci.entity_type_enum = ? and calendar_type_enum = ? order"
            + " by c.start_date ";
    final List<CalendarData> result =
        this.jdbcTemplate.query(
            sql, rm, new Object[] {entityId, entityTypeId, CalendarType.COLLECTION.getValue()});
  ...
  @Override
  public Collection<CalendarData> retrieveParentCalendarsByEntity(
      final Long entityId, final Integer entityTypeId, final List<Integer> calendarTypeOptions) {
    ...
    if (calendarTypeOptions == null || calendarTypeOptions.isEmpty()) {
      ...
      result =
          this.jdbcTemplate.query(
              sql, rm, new Object[] {entityId, CalendarEntityType.CENTERS.getValue()});
    } else {
      final String sqlCalendarTypeOptions =
          CalendarUtils.getSqlCalendarTypeOptionsInString(calendarTypeOptions);
      sql =
          rm.schema()
              + " "
              + parentHeirarchyCondition
              + " and ci.entity_type_enum = ? and c.calendar_type_enum in ("
              + sqlCalendarTypeOptions
              + ") order by c.start_date ";
      result =
          this.jdbcTemplate.query(
  ...
  @Override
  public Collection<LocalDate> generateRecurringDates(
    ...
    Collection<LocalDate> recurringDates =
        generateRecurringDate(calendarData, fromDate, tillDate, -1);

    if (withHistory) {
      final Collection<CalendarData> calendarHistorys =
          this.retrieveCalendarsFromHistory(calendarData.getId());
  ...
  private Collection<LocalDate> generateRecurringDate(
    ...
    final LocalDate periodStartDate =
        this.getPeriodStartDate(seedDate, calendarData.getStartDate(), fromDate);
    /**
  ...
  @Override
  public Boolean isCalendarAssociatedWithEntity(
      final Long entityId, final Long calendarId, final Long entityTypeId) {
    String query =
        "Select COUNT(*) from m_calendar_instance ci where ci.entity_id = ? and ci.calendar_id = ?"
            + " and  ci.entity_type_enum = ?";
    try {
      int calendarInstaneId =
          this.jdbcTemplate.queryForObject(
              query, new Object[] {entityId, calendarId, entityTypeId}, Integer.class);
  ...
  private LocalDate getPeriodStartDate(
      final LocalDate seedDate, final LocalDate recurrenceStartDate, final LocalDate fromDate) {
  ...
  @Override
  public LocalDate generateNextEligibleMeetingDateForCollection(
      final CalendarData calendarData, final MeetingData lastMeetingData) {

    final LocalDate lastMeetingDate =
        (lastMeetingData == null) ? null : lastMeetingData.getMeetingDate();
    ...
    if (lastMeetingDate != null
      ...
      applicableCalendarData =
          this.retrieveApplicableCalendarFromHistory(calendarData.getId(), lastMeetingDate);
      nextEligibleMeetingDate =
          CalendarUtils.getRecentEligibleMeetingDate(
              applicableCalendarData.getRecurrence(),
              lastMeetingDate,
              isSkipRepaymentOnFirstMonthEnabled,
              numberOfDays);
    }
    ...
    if (nextEligibleMeetingDate == null) {
      final LocalDate seedDate =
          (lastMeetingDate != null) ? lastMeetingDate : calendarData.getStartDate();
      nextEligibleMeetingDate =
          CalendarUtils.getRecentEligibleMeetingDate(
              applicableCalendarData.getRecurrence(),
              seedDate,
  ...
  @Override
  public Collection<CalendarData> updateWithRecurringDates(
    ...
    for (final CalendarData calendarData : calendarsData) {
      final Collection<LocalDate> recurringDates =
          this.generateRecurringDates(calendarData, withHistory, tillDate);
      final Collection<LocalDate> nextTenRecurringDates =
          this.generateNextTenRecurringDates(calendarData);
      final LocalDate recentEligibleMeetingDate = null;
      final CalendarData updatedCalendarData =
  ...
  public CalendarData retrieveLoanCalendar(final Long loanId) {
    ...
    final Collection<CalendarData> calendars =
        this.jdbcTemplate.query(
            sql, rm, new Object[] {loanId, CalendarEntityType.LOANS.getValue()});
  ...
  public static String getParentHierarchyCondition(final CalendarEntityType calendarEntityType) {
    ...
    switch (calendarEntityType) {
      case CLIENTS:
        ...
        // hierarchy?
        conditionSql =
            " and ci.entity_id in (select gc.group_id from m_client c join m_group_client gc "
  ...
  private CalendarData retrieveApplicableCalendarFromHistory(
      Long calendarId, LocalDate compareDate) {
    try {
      ...
      return this.jdbcTemplate.queryForObject(
          sql, rm, new Object[] {calendarId, compareDate.toDate()});
    } catch (final EmptyResultDataAccessException e) {
      return null;
    }
  }
  ...
  private Collection<CalendarData> retrieveCalendarsFromHistory(Long calendarId) {
    try {
      ...
      final Collection<CalendarData> calendars =
          this.jdbcTemplate.query(sql, rm, new Object[] {calendarId});
      return calendars;
    } catch (final EmptyResultDataAccessException e) {
      return null;
    }
  ...
  private static final class CalendarDataFromHistoryMapper implements RowMapper<CalendarData> {
    ...
    public String schema() {
      return " select c.calendar_id as id, c.title as title, c.description as description,"
          + " c.location as location, c.start_date as startDate,  c.end_date as endDate,"
          + " c.duration as duration, c.calendar_type_enum as typeId, c.repeating as"
          + " repeating,  c.recurrence as recurrence, c.remind_by_enum as remindById,"
          + " c.first_reminder as firstReminder, c.second_reminder as secondReminder  from"
          + " m_calendar_history c ";
    }
