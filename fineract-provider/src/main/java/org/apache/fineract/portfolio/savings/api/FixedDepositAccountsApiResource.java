...
@SwaggerDefinition(
    tags = {
      @Tag(
          ...
          description =
              "Fixed Deposit accounts are instances of a praticular fixed deposit product created."
...
public class FixedDepositAccountsApiResource {
  ...
  @ApiOperation(
      value = "Retrieve Fixed Deposit Account Template",
      httpMethod = "GET",
      notes =
  ...
  public String template(
    ...
    return this.toApiJsonSerializer.serialize(
        settings, account, DepositsApiConstants.FIXED_DEPOSIT_ACCOUNT_RESPONSE_DATA_PARAMETERS);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "List Fixed deposit applications/accounts",
      httpMethod = "GET",
      notes =
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = FixedDepositAccountsApiResourceSwagger.GetFixedDepositAccountsResponse.class,
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Submit new fixed deposit application",
      httpMethod = "POST",
      notes =
          "Submits a new fixed deposit applicationMandatory Fields: clientId or groupId,"
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "OK",
        response = FixedDepositAccountsApiResourceSwagger.PostFixedDepositAccountsResponse.class)
  })
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Retrieve a fixed deposit application/account",
      httpMethod = "GET",
      notes =
  ...
  public String retrieveOne(
    ...
    final FixedDepositAccountData account =
    ...
    final FixedDepositAccountData accountTemplate =
  ...
  private FixedDepositAccountData populateTemplateAndAssociations(
    ...
    if (!associationParameters.isEmpty()) {
      ...
      if (associationParameters.contains("all")) {
        associationParameters.addAll(
            Arrays.asList(
                SavingsApiConstants.transactions,
      ...
      if (associationParameters.contains(SavingsApiConstants.linkedAccount)) {
        ...
        linkedAccount =
            this.accountAssociationsReadPlatformService.retriveSavingsLinkedAssociation(accountId);
      }
    }
    ...
    return FixedDepositAccountData.associationsAndTemplate(
        savingsAccount, templateData, transactions, charges, linkedAccount);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Modify a fixed deposit application",
      httpMethod = "PUT",
      notes =
          "Fixed deposit application can only be modified when in 'Submitted and pending approval'"
  ...
  public String update(
    ...
    final CommandWrapper commandRequest =
        new CommandWrapperBuilder()
            .updateFixedDepositAccount(accountId)
            .withJson(apiRequestBodyAsJson)
            .build();
  ...
  @ApiOperation(
      value =
          "Approve fixed deposit application | Undo approval fixed deposit application | Reject"
              + " fixed deposit application | Withdraw fixed deposit application | Activate a"
              + " fixed deposit account | Close a fixed deposit account | Premature Close a fixed"
              + " deposit account | Calculate Premature amount on Fixed deposit account |"
              + " Calculate Interest on Fixed Deposit Account | Post Interest on Fixed Deposit"
              + " Account",
      httpMethod = "POST",
      notes =
          "Approve fixed deposit application:\n\n"
              + "Approves fixed deposit application so long as its in 'Submitted and pending"
              + " approval' state.\n\n"
              + "Undo approval fixed deposit application:\n\n"
              + "Will move 'approved' fixed deposit application back to 'Submitted and pending"
              + " approval' state.\n\n"
              + "Reject fixed deposit application:\n\n"
              + "Rejects fixed deposit application so long as its in 'Submitted and pending"
              + " approval' state.\n\n"
              + "Withdraw fixed deposit application:\n\n"
              + "Used when an applicant withdraws from the fixed deposit application. It must be"
              + " in 'Submitted and pending approval' state.\n\n"
              + "Close a fixed deposit account:\n\n"
              + "Results in a Matured fixed deposit account being converted into a 'closed' fixed"
              + " deposit account.\n\n"
              + "Premature Close a fixed deposit account:\n\n"
              + "Results in an Active fixed deposit account being converted into a 'Premature"
              + " Closed' fixed deposit account with options to withdraw prematured amount."
              ...
              + "Calculate premature amount on fixed deposit account till premature close date."
              + " Premature amount is calculated based on interest chart and penal interest"
              + " applicable.\n\n"
              + "Calculate Interest on Fixed Deposit Account:\n\n"
              + "Calculates interest earned on a fixed deposit account based on todays date. It"
              + " does not attempt to post or credit the interest on the account. That is"
              + " responsibility of the Post Interest API that will likely be called by overnight"
              + " process.\n\n"
              + "Post Interest on Fixed Deposit Account:\n\n"
              + "Calculates and Posts interest earned on a fixed deposit account based on today's"
              + " date and whether an interest posting or crediting event is due.\n\n"
  ...
  public String handleCommands(
    ...
    if (is(commandParam, "reject")) {
      final CommandWrapper commandRequest =
          builder.rejectFixedDepositAccountApplication(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "withdrawnByApplicant")) {
      final CommandWrapper commandRequest =
          builder.withdrawFixedDepositAccountApplication(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "approve")) {
      final CommandWrapper commandRequest =
          builder.approveFixedDepositAccountApplication(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "undoapproval")) {
      final CommandWrapper commandRequest =
          builder.undoFixedDepositAccountApplication(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "activate")) {
      final CommandWrapper commandRequest =
          builder.fixedDepositAccountActivation(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "calculateInterest")) {
      final CommandWrapper commandRequest =
          builder.withNoJsonBody().fixedDepositAccountInterestCalculation(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else if (is(commandParam, "postInterest")) {
      final CommandWrapper commandRequest =
          builder.fixedDepositAccountInterestPosting(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
      ...
      final CommandWrapper commandRequest =
          builder.prematureCloseFixedDepositAccount(accountId).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Delete a fixed deposit application",
      httpMethod = "DELETE",
      notes =
          "At present we support hard delete of fixed deposit application so long as its in"
  ...
  @ApiResponses({
    @ApiResponse(
        ...
        message = "OK",
        response =
            FixedDepositAccountsApiResourceSwagger.DeleteFixedDepositAccountsAccountIdResponse
                .class)
  })
  public String delete(
    ...
    final CommandWrapper commandRequest =
        new CommandWrapperBuilder().deleteFixedDepositAccount(accountId).build();
  ...
  public Response getFixedDepositTemplate(
  ...
  public String postFixedDepositTemplate(
    ...
    Long importDocumentId =
        bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.FIXED_DEPOSIT_ACCOUNTS.toString(),
            uploadedInputStream,
            fileDetail,
            locale,
            dateFormat);
  ...
  @Produces("application/vnd.ms-excel")
  public Response getFixedDepositTransactionTemplate(
      @QueryParam("officeId") final Long officeId,
      @QueryParam("dateFormat") final String dateFormat) {
    return bulkImportWorkbookPopulatorService.getTemplate(
        GlobalEntityType.FIXED_DEPOSIT_TRANSACTIONS.toString(), officeId, null, dateFormat);
  }

  @POST
  @Path("transaction/uploadtemplate")
  @Consumes(MediaType.MULTIPART_FORM_DATA)
  public String postFixedDepositTransactionTemplate(
      @FormDataParam("file") InputStream uploadedInputStream,
      @FormDataParam("file") FormDataContentDisposition fileDetail,
      @FormDataParam("locale") final String locale,
      @FormDataParam("dateFormat") final String dateFormat) {
    final Long importDocumentId =
        this.bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.FIXED_DEPOSIT_TRANSACTIONS.toString(),
            uploadedInputStream,
            fileDetail,
            locale,
            dateFormat);
