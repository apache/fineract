...
public class TaxAssembler {
  ...
  public TaxComponent assembleTaxComponentFrom(final JsonCommand command) {
    ...
    final DataValidatorBuilder baseDataValidator =
        new DataValidatorBuilder(dataValidationErrors).resource("tax.component");

    final String name =
        this.fromApiJsonHelper.extractStringNamed(TaxApiConstants.nameParamName, element);
    final BigDecimal percentage =
        this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(
            TaxApiConstants.percentageParamName, element);
    final Integer debitAccountType =
        this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
            TaxApiConstants.debitAccountTypeParamName, element);
    final Long debitAccountId =
        this.fromApiJsonHelper.extractLongNamed(TaxApiConstants.debitAcountIdParamName, element);
    ...
    if (debitAccountId != null) {
      ...
      if (!debitGlAccount.getType().equals(debitAccountType) || debitGlAccount.isHeaderAccount()) {
        baseDataValidator
            .parameter(TaxApiConstants.debitAcountIdParamName)
    ...
    final Integer creditAccountType =
        this.fromApiJsonHelper.extractIntegerSansLocaleNamed(
            TaxApiConstants.creditAccountTypeParamName, element);
    ...
    final Long creditAccountId =
        this.fromApiJsonHelper.extractLongNamed(TaxApiConstants.creditAcountIdParamName, element);
    ...
    if (creditAccountId != null) {
      ...
      if (!creditGlAccount.getType().equals(creditAccountType)
        ...
        baseDataValidator
            .parameter(TaxApiConstants.creditAcountIdParamName)
    ...
    LocalDate startDate =
        this.fromApiJsonHelper.extractLocalDateNamed(TaxApiConstants.startDateParamName, element);
    if (startDate == null) {
      startDate = DateUtils.getLocalDateOfTenant();
    }

    return TaxComponent.createTaxComponent(
        name,
        percentage,
        debitGlAccountType,
        debitGlAccount,
  ...
  public Set<TaxGroupMappings> assembleTaxGroupMappingsFrom(
      final JsonCommand command, boolean isUpdate) {
    ...
    if (topLevelJsonElement.get(TaxApiConstants.taxComponentsParamName).isJsonArray()) {
      ...
      for (int i = 0; i < array.size(); i++) {
        ...
        final Long mappingId =
            this.fromApiJsonHelper.extractLongNamed(TaxApiConstants.idParamName, taxComponent);
        final Long taxComponentId =
            this.fromApiJsonHelper.extractLongNamed(
                TaxApiConstants.taxComponentIdParamName, taxComponent);
