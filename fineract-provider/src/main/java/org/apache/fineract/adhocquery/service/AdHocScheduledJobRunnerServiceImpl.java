...
public class AdHocScheduledJobRunnerServiceImpl implements AdHocScheduledJobRunnerService {
  ...
  @Autowired
  public AdHocScheduledJobRunnerServiceImpl(
      final RoutingDataSource dataSource, final AdHocReadPlatformService adHocReadPlatformService) {
  ...
  public void generateClientSchedule() {
    final Collection<AdHocData> adhocs =
        this.adHocReadPlatformService.retrieveAllActiveAdHocQuery();
    if (adhocs.size() > 0) {
      adhocs.forEach(
          adhoc -> {
            boolean run = true;
            ...
            if (run) {
              ...
              if (insertSqlBuilder.length() > 0) {
                ...
                logger.info(
                    "{}: Results affected by inserted: {}",
                    ThreadLocalContextUtil.getTenant().getName(),
                    result);

                this.jdbcTemplate.update(
                    "UPDATE m_adhoc SET last_run=? WHERE id=?", new Date(), adhoc.getId());
              }
            } else {
              logger.info(
                  "{}: Skipping execution of {}, scheduled for execution on {}",
                  new Object[] {
