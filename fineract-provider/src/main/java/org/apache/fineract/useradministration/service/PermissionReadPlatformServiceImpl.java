...
public class PermissionReadPlatformServiceImpl implements PermissionReadPlatformService {
  ...
  private static final class PermissionUsageDataMapper implements RowMapper<PermissionData> {
    ...
    @Override
    public PermissionData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
    ...
    public String permissionSchema() {
      ...
      return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName,"
          + " true as selected from m_permission p  where code not like '%\\_CHECKER' order"
          + " by p.grouping, ifnull(entity_name, ''), p.code";
    }
    ...
    public String makerCheckerablePermissionSchema() {
      ...
      return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName,"
          + " p.can_maker_checker as selected from m_permission p  where grouping !="
          + " 'special' and code not like 'READ_%' and code not like '%\\_CHECKER' order by"
          + " p.grouping, ifnull(entity_name, ''), p.code";
    }

    public String rolePermissionSchema() {
      return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName,"
          + " if(isnull(rp.role_id), false, true) as selected  from m_permission p  left"
