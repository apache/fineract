...
public final class UserDataValidator {
  ...
  private final Set<String> supportedParameters =
      new HashSet<>(
          Arrays.asList(
              ...
              AppUserConstants.IS_SELF_SERVICE_USER,
              AppUserConstants.CLIENTS));
  ...
  public void validateForCreate(final String json) {
    ...
    final Boolean sendPasswordToEmail =
        this.fromApiJsonHelper.extractBooleanNamed("sendPasswordToEmail", element);
    if (sendPasswordToEmail != null) {
      if (sendPasswordToEmail.booleanValue()) {
        ...
        final String repeatPassword =
            this.fromApiJsonHelper.extractStringNamed("repeatPassword", element);
    ...
    if (this.fromApiJsonHelper.parameterExists(AppUserConstants.PASSWORD_NEVER_EXPIRES, element)) {
      final boolean passwordNeverExpire =
          this.fromApiJsonHelper.extractBooleanNamed(
              AppUserConstants.PASSWORD_NEVER_EXPIRES, element);
      baseDataValidator
          .reset()
          .parameter("passwordNeverExpire")
          .value(passwordNeverExpire)
    ...
    if (this.fromApiJsonHelper.parameterExists(AppUserConstants.IS_SELF_SERVICE_USER, element)) {
      isSelfServiceUser =
          this.fromApiJsonHelper.extractBooleanNamed(
              AppUserConstants.IS_SELF_SERVICE_USER, element);
      if (isSelfServiceUser == null) {
        baseDataValidator
            .reset()
            .parameter(AppUserConstants.IS_SELF_SERVICE_USER)
            .trueOrFalseRequired(false);
      }
    }
    ...
    if (this.fromApiJsonHelper.parameterExists(AppUserConstants.CLIENTS, element)) {
      if (isSelfServiceUser == null || !isSelfServiceUser) {
        baseDataValidator
            .reset()
            .parameter(AppUserConstants.CLIENTS)
            .failWithCode(
                "not.supported.when.isSelfServiceUser.is.false",
  ...
  public void validateForUpdate(final String json) {
    ...
    if (this.fromApiJsonHelper.parameterExists("passwordNeverExpire", element)) {
      final boolean passwordNeverExpire =
          this.fromApiJsonHelper.extractBooleanNamed("passwordNeverExpire", element);
      baseDataValidator
          .reset()
          .parameter("passwordNeverExpire")
          .value(passwordNeverExpire)
