...
public class RoleWritePlatformServiceJpaRepositoryImpl implements RoleWritePlatformService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(RoleWritePlatformServiceJpaRepositoryImpl.class);
  ...
  private void handleDataIntegrityIssues(
    ...
    throw new PlatformDataIntegrityException(
        "error.msg.role.unknown.data.integrity.issue",
  ...
  public CommandProcessingResult updateRole(final Long roleId, final JsonCommand command) {
    try {
      ...
      final Role role =
          this.roleRepository.findById(roleId).orElseThrow(() -> new RoleNotFoundException(roleId));
  ...
  @Transactional
  @Override
  public CommandProcessingResult updateRolePermissions(
      final Long roleId, final JsonCommand command) {
    this.context.authenticatedUser();
  ...
  private Permission findPermissionByCode(
      final Collection<Permission> allPermissions, final String permissionCode) {
