...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Permissions",
          description =
              "An API capability to support management of application permissions for user"
                  + " administration.\n"
                  + "\n"
                  + "There is no Apache Fineract functionality for creating or deleting"
                  + " permissions. Permissions come pre-installed.\n"
                  + "\n"
                  + "Permissions are not updated, except in the case of enabling or disabling"
                  + " non-read transactions for Maker Checker functionality")
    })
public class PermissionsApiResource {
  ...
  @ApiOperation(
      value = "List Application Permissions",
      notes =
          "ARGUMENTS\n"
              + "makerCheckerableoptional, Values are true, false. Default is false.\n"
              + "If makerCheckerable=false or not supplied then a list of application permissions"
              + " is returned. The \"selected\" attribute is always true in this case.\n"
              + "\n"
              + "If makerCheckerable=true then the \"selected\" attribute shows whether the"
              + " permission is enabled for Maker Check functionality.\n"
              + "\n"
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = PermissionsApiResourceSwagger.GetPermissionsResponse.class,
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = PermissionsApiResourceSwagger.PutPermissionsRequest.class)
  })
  @ApiResponses({@ApiResponse(code = 200, message = "", response = CommandProcessingResult.class)})
  public String updatePermissionsDetails(
