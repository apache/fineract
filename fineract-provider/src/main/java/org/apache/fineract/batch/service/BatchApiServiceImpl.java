...
public class BatchApiServiceImpl implements BatchApiService {
  ...
  public BatchApiServiceImpl(
      final CommandStrategyProvider strategyProvider,
    ...
    for (BatchRequestNode rootNode : batchRequestNodes) {
      ...
      final CommandStrategy commandStrategy =
          this.strategyProvider.getCommandStrategy(
              CommandContext.resource(rootRequest.getRelativeUrl())
                  .method(rootRequest.getMethod())
                  .build());
  ...
  private List<BatchResponse> processChildRequests(
      final BatchRequestNode rootRequest, BatchResponse rootResponse, UriInfo uriInfo) {
      ...
      for (BatchRequestNode childNode : rootRequest.getChildRequests()) {
        ...
        try {
          ...
          if (rootResponse.getStatusCode().equals(200)) {
            ...
            final CommandStrategy commandStrategy =
                this.strategyProvider.getCommandStrategy(
                    CommandContext.resource(childRequest.getRelativeUrl())
                        .method(childRequest.getMethod())
                        .build());
  ...
  public List<BatchResponse> handleBatchRequestsWithEnclosingTransaction(
    ...
    try {
      return this.transactionTemplate.execute(
          new TransactionCallback<List<BatchResponse>>() {
