...
/**
 * Implements {@link CommandStrategy} and retrieves loan interest pauses by loan id. It passes the contents of the body
 * from the BatchRequest to {@link LoanInterestPauseApiResource} and gets back the response. This class will also catch
 * any errors raised by {@link LoanInterestPauseApiResource} and map those errors to appropriate status codes in
 * BatchResponse.
 */
@Component
...
public class GetLoanInterestPausesByLoanIdCommandStrategy implements CommandStrategy {
    ...
    public BatchResponse execute(final BatchRequest request, @SuppressWarnings("unused") final UriInfo uriInfo) {
        ...
        // Expected pattern - loans\/\d+\/interest-pauses
        ...
        final List<InterestPauseResponseDto> responseDtos = loanInterestPauseApiResource.retrieveInterestPauses(loanId);
