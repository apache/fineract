...
public class ProductToGLAccountMappingHelper {
  ...
  @Autowired
  public ProductToGLAccountMappingHelper(
  ...
  public void saveProductToAccountMapping(
    ...
    final ProductToGLAccountMapping accountMapping =
        new ProductToGLAccountMapping(
            glAccount, productId, portfolioProductType.getValue(), placeHolderTypeId);
    ...
    if (accountId != null) {
      final ProductToGLAccountMapping accountMapping =
          this.accountMappingRepository.findCoreProductToFinAccountMapping(
              productId, portfolioProductType.getValue(), accountTypeId);
      if (accountMapping == null) {
        throw new ProductToGLAccountMappingNotFoundException(
            portfolioProductType, productId, accountTypeName);
      }
      if (!accountMapping.getGlAccount().getId().equals(accountId)) {
        final GLAccount glAccount =
            getAccountByIdAndType(paramName, expectedAccountType, accountId);
  ...
  public void createOrmergeProductToAccountMappingChanges(
      final JsonElement element,
      final String paramName,
      final Long productId,
      final int accountTypeId,
    ...
    if (accountId != null) {
      ...
      if (accountMapping == null) {
        final GLAccount glAccount =
            getAccountByIdAndType(paramName, expectedAccountType, accountId);
        changes.put(paramName, accountId);
        ProductToGLAccountMapping newAccountMapping =
            new ProductToGLAccountMapping(
                glAccount, productId, portfolioProductType.getValue(), accountTypeId);
    ...
    if (paymentChannelMappingArray != null) {
      ...
      for (int i = 0; i < paymentChannelMappingArray.size(); i++) {
        ...
        final Long paymentTypeId =
            jsonObject.get(LoanProductAccountingParams.PAYMENT_TYPE.getValue()).getAsLong();
        final Long paymentSpecificFundAccountId =
            jsonObject.get(LoanProductAccountingParams.FUND_SOURCE.getValue()).getAsLong();
        savePaymentChannelToFundSourceMapping(
            productId, paymentTypeId, paymentSpecificFundAccountId, portfolioProductType);
      }
    }
  }
   ...
   * @param changes
   */
  public void saveChargesToIncomeOrLiabilityAccountMappings(
      final JsonCommand command,
      final JsonElement element,
      final Long productId,
      final Map<String, Object> changes,
    ...
    if (chargeToIncomeAccountMappingArray != null) {
      ...
      for (int i = 0; i < chargeToIncomeAccountMappingArray.size(); i++) {
        ...
        final Long chargeId =
            jsonObject.get(LoanProductAccountingParams.CHARGE_ID.getValue()).getAsLong();
   ...
   * @param changes
   */
  public void updateChargeToIncomeAccountMappings(
      final JsonCommand command,
      final JsonElement element,
      final Long productId,
      final Map<String, Object> changes,
    ...
    if (isPenalty) {
      existingChargeToIncomeAccountMappings =
          this.accountMappingRepository.findAllPenaltyToIncomeAccountMappings(
              productId, portfolioProductType.getValue());
      arrayFragmentName = LoanProductAccountingParams.PENALTY_INCOME_ACCOUNT_MAPPING.getValue();
    } else {
      existingChargeToIncomeAccountMappings =
          this.accountMappingRepository.findAllFeeToIncomeAccountMappings(
              productId, portfolioProductType.getValue());
      arrayFragmentName = LoanProductAccountingParams.FEE_INCOME_ACCOUNT_MAPPING.getValue();
    }

    final JsonArray chargeToIncomeAccountMappingArray =
        this.fromApiJsonHelper.extractJsonArrayNamed(arrayFragmentName, element);
    /**
    ...
    if (chargeToIncomeAccountMappingArray != null) {
       ...
       **/
        ...
        for (final ProductToGLAccountMapping chargeToIncomeAccountMapping :
            existingChargeToIncomeAccountMappings) {
        ...
        for (final Long newCharge : incomingCharges) {
          ...
          saveChargeToFundSourceMapping(
              productId, newCharge, newGLAccountId, portfolioProductType, isPenalty);
        }
      }
    }
  }
  ...
  public void updatePaymentChannelToFundSourceMappings(
      ...
      final PortfolioProductType portfolioProductType) {
    // find all existing payment Channel to Fund source Mappings
    final List<ProductToGLAccountMapping> existingPaymentChannelToFundSourceMappings =
        this.accountMappingRepository.findAllPaymentTypeToFundSourceMappings(
            productId, portfolioProductType.getValue());
    final JsonArray paymentChannelMappingArray =
        this.fromApiJsonHelper.extractJsonArrayNamed(
            LoanProductAccountingParams.PAYMENT_CHANNEL_FUND_SOURCE_MAPPING.getValue(), element);
    ...
    if (paymentChannelMappingArray != null) {
       ...
       **/
        ...
        for (final ProductToGLAccountMapping existingPaymentChannelToFundSourceMapping :
            existingPaymentChannelToFundSourceMappings) {
          final Long currentPaymentChannelId =
              existingPaymentChannelToFundSourceMapping.getPaymentType().getId();
          ...
          if (inputPaymentChannelFundSourceMap.containsKey(currentPaymentChannelId)) {
            ...
            if (!newGLAccountId.equals(
                existingPaymentChannelToFundSourceMapping.getGlAccount().getId())) {
              final GLAccount glAccount =
                  getAccountByIdAndType(
        ...
        for (final Long newPaymentType : incomingPaymentTypes) {
          ...
          savePaymentChannelToFundSourceMapping(
              productId, newPaymentType, newGLAccountId, portfolioProductType);
        }
      }
    }
  }
   ...
   * @param jsonObject
   */
  private void savePaymentChannelToFundSourceMapping(
      final Long productId,
      final Long paymentTypeId,
   ...
   * @param jsonObject
   */
  private void saveChargeToFundSourceMapping(
      final Long productId,
      final Long chargeId,
      final Long incomeAccountId,
      final PortfolioProductType portfolioProductType,
    ...
    final ProductToGLAccountMapping accountMapping =
        new ProductToGLAccountMapping(
            ...
            productId,
            portfolioProductType.getValue(),
            placeHolderAccountType.getValue(),
            charge);
    this.accountMappingRepository.save(accountMapping);
  }
   ...
   * @return
   */
  public GLAccount getAccountByIdAndType(
      final String paramName, final GLAccountType expectedAccountType, final Long accountId) {
    final GLAccount glAccount =
    ...
    if (glAccount.getType().intValue() != expectedAccountType.getValue()) {
      throw new ProductToGLAccountMappingInvalidException(
          paramName,
          glAccount.getName(),
          accountId,
          GLAccountType.fromInt(glAccount.getType()).toString(),
  ...
  public GLAccount getAccountByIdAndType(
    ...
    if (!glAccountTypeValues.contains(glAccount.getType())) {
      throw new ProductToGLAccountMappingInvalidException(
          ...
          accountId,
          GLAccountType.fromInt(glAccount.getType()).toString(),
          glAccountTypeValues.toString());
    }
    return glAccount;
  }

  public void deleteProductToGLAccountMapping(
      final Long loanProductId, final PortfolioProductType portfolioProductType) {
