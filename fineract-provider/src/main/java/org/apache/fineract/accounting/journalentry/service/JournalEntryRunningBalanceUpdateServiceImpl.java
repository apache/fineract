...
public class JournalEntryRunningBalanceUpdateServiceImpl
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(JournalEntryRunningBalanceUpdateServiceImpl.class);
  ...
  // if a limit is not added to the running balance select statements below and the resultset is
  // more than 400,000,
  // the script will eat up all of the server memory
  private final String selectRunningBalanceSqlLimit = "limit 0, 10000";

  private final String officeRunningBalanceSql =
      "select je.office_running_balance as runningBalance,je.account_id as accountId from"
          + " acc_gl_journal_entry je inner join (select max(id) as id from acc_gl_journal_entry"
          + " where office_id=?  and entry_date < ? group by account_id,entry_date) je2 inner join"
          + " (select max(entry_date) as date from acc_gl_journal_entry where office_id=? and"
          + " entry_date < ? group by account_id) je3 where je2.id = je.id and je.entry_date ="
          + " je3.date group by je.id order by je.entry_date DESC "
          + selectRunningBalanceSqlLimit;

  private final String organizationRunningBalanceSql =
      "select je.organization_running_balance as runningBalance,je.account_id as accountId from"
          + " acc_gl_journal_entry je inner join (select max(id) as id from acc_gl_journal_entry"
          + " where entry_date < ? group by account_id,entry_date) je2 inner join (select"
          + " max(entry_date) as date from acc_gl_journal_entry where entry_date < ? group by"
          + " account_id) je3 where je2.id = je.id and je.entry_date = je3.date group by je.id"
          + " order by je.entry_date DESC "
          + selectRunningBalanceSqlLimit;

  private final String officesRunningBalanceSql =
      "select je.office_running_balance as runningBalance,je.account_id as accountId,je.office_id"
          + " as officeId from acc_gl_journal_entry je inner join (select max(id) as id from"
          + " acc_gl_journal_entry where entry_date < ? group by office_id,account_id,entry_date)"
          + " je2 inner join (select max(entry_date) as date from acc_gl_journal_entry where"
          + " entry_date < ? group by office_id,account_id) je3 where je2.id = je.id and"
          + " je.entry_date = je3.date group by je.id order by je.entry_date DESC "
          + selectRunningBalanceSqlLimit;
  ...
  public JournalEntryRunningBalanceUpdateServiceImpl(
      ...
      final JournalEntryDataValidator dataValidator,
  ...
  public void updateRunningBalance() {
    String dateFinder =
        "select MIN(je.entry_date) as entityDate from acc_gl_journal_entry  je "
  ...
  private void updateOrganizationRunningBalance(Date entityDate) {
    ...
    for (Map<String, Object> entries : list) {
      Long accountId =
          Long.parseLong(
              entries
                  .get("accountId")
                  .toString()); // Drizzle is returning Big Integer where as MySQL returns Long.
    ...
    if (entryDatas.size() > 0) {
      ...
      for (int index = 0; index < entryDatas.size(); index++) {
        ...
        String sql =
            "UPDATE acc_gl_journal_entry je SET je.is_running_balance_calculated=1,"
  ...
  private static final class GLJournalEntryMapper implements RowMapper<JournalEntryData> {
    ...
    public String officeRunningBalanceSchema() {
      return "select je.id as id,je.account_id as glAccountId,je.type_enum as entryType,je.amount"
          + " as amount, glAccount.classification_enum as classification,je.office_id as"
          + " officeId from acc_gl_journal_entry je , acc_gl_account glAccount where"
          + " je.account_id = glAccount.id and je.office_id=? and je.entry_date >= ? order"
          + " by je.entry_date,je.id";
    }

    public String organizationRunningBalanceSchema() {
      return "select je.id as id,je.account_id as glAccountId,"
          + "je.type_enum as entryType,je.amount as amount, "
          + "glAccount.classification_enum as classification,je.office_id as officeId  "
          + "from acc_gl_journal_entry je , acc_gl_account glAccount "
          + "where je.account_id = glAccount.id "
