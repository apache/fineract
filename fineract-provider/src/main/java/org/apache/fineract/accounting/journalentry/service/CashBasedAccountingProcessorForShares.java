...
public class CashBasedAccountingProcessorForShares implements AccountingProcessorForShares {
  ...
  public void createJournalEntriesForShares(SharesDTO sharesDTO) {
    ...
    for (SharesTransactionDTO transactionDTO : sharesDTO.getNewTransactions()) {
      ...
      if (transactionDTO.getTransactionType().isPurchased()) {
        createJournalEntriesForPurchase(
            shareAccountId,
            shareProductId,
            currencyCode,
            transactionDTO,
        ...
        this.helper.createCashBasedJournalEntriesForSharesCharges(
            office,
            currencyCode,
            CashAccountsForShares.SHARES_REFERENCE,
            CashAccountsForShares.INCOME_FROM_FEES,
            shareProductId,
            paymentTypeId,
            shareAccountId,
            transactionId,
  ...
  public void createJournalEntriesForRedeem(
    ...
    if (chargeAmount == null || chargeAmount.compareTo(BigDecimal.ZERO) != 1) {
      this.helper.createJournalEntriesForShares(
          office,
          currencyCode,
      ...
      this.helper.createDebitJournalEntryForShares(
          office,
          currencyCode,
          CashAccountsForShares.SHARES_EQUITY.getValue(),
          shareProductId,
          ...
          transactionId,
          transactionDate,
          amount.add(chargeAmount));
      this.helper.createCreditJournalEntryForShares(
          office,
          currencyCode,
          CashAccountsForShares.SHARES_REFERENCE.getValue(),
          shareProductId,
          ...
          transactionDate,
          amount);
      this.helper.createCashBasedJournalEntryForSharesCharges(
          office,
          currencyCode,
          CashAccountsForShares.INCOME_FROM_FEES,
          shareProductId,
  ...
  public void createJournalEntriesForPurchase(
      final Long shareAccountId,
      final Long shareProductId,
      final String currencyCode,
      SharesTransactionDTO transactionDTO,
      final Date transactionDate,
      final String transactionId,
      final Office office,
      final Long paymentTypeId,
    ...
    if (transactionDTO.getTransactionStatus().isApplied()) {
      ...
      if (chargeAmount != null && chargeAmount.compareTo(BigDecimal.ZERO) == 1) {
        this.helper.revertCashBasedJournalEntryForSharesCharges(
            office,
            currencyCode,
            CashAccountsForShares.INCOME_FROM_FEES,
            shareProductId,
