...
public interface ProvisioningEntryRepository
    extends JpaRepository<ProvisioningEntry, Long>, JpaSpecificationExecutor<ProvisioningEntry> {
  ...
  @Query(
      "select entry1 from ProvisioningEntry entry1 where entry1.createdDate = (select"
          + " max(entry2.createdDate) from ProvisioningEntry entry2 where"
          + " entry2.isJournalEntryCreated=true)")
