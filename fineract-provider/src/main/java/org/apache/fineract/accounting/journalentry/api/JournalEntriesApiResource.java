...
@SwaggerDefinition(
    tags = {
      @Tag(
          ...
          description =
              "A journal entry refers to the logging of transactions against general ledger"
                  + " accounts. A journal entry may consist of several line items, each of which"
                  + " is either a \"debit\" or a \"credit\". The total amount of the debits must"
                  + " equal the total amount of the credits or the journal entry is said to be"
                  + " \"unbalanced\" \n"
                  + "\n"
...
public class JournalEntriesApiResource {
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = JournalEntryData.class,
  ...
  public String retrieveAll(
      ...
      @QueryParam("manualEntriesOnly") @ApiParam(value = "manualEntriesOnly")
      ...
      @QueryParam("runningBalance") @ApiParam(value = "runningBalance")
          final boolean runningBalance,
      @QueryParam("transactionDetails") @ApiParam(value = "transactionDetails")
          final boolean transactionDetails) {
    ...
    final SearchParameters searchParameters =
        SearchParameters.forJournalEntries(
            officeId, offset, limit, orderBy, sortOrder, loanId, savingsId);
    JournalEntryAssociationParametersData associationParametersData =
        new JournalEntryAssociationParametersData(transactionDetails, runningBalance);

    final Page<JournalEntryData> glJournalEntries =
        this.journalEntryReadPlatformService.retrieveAll(
            searchParameters,
            glAccountId,
            onlyManualEntries,
            fromDate,
    ...
    return this.apiJsonSerializerService.serialize(
        settings, glJournalEntries, RESPONSE_DATA_PARAMETERS);
  }
  ...
  public String retreiveJournalEntryById(
      @PathParam("journalEntryId") @ApiParam(value = "journalEntryId") final Long journalEntryId,
      @Context final UriInfo uriInfo,
      @QueryParam("runningBalance") @ApiParam(value = "runningBalance")
          final boolean runningBalance,
      @QueryParam("transactionDetails") @ApiParam(value = "transactionDetails")
    ...
    final JournalEntryData glJournalEntryData =
        this.journalEntryReadPlatformService.retrieveGLJournalEntryById(
            journalEntryId, associationParametersData);

    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    return this.apiJsonSerializerService.serialize(
        settings, glJournalEntryData, RESPONSE_DATA_PARAMETERS);
  }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Create \"Balanced\" Journal Entries",
      notes =
          "Note: A Balanced (simple) Journal entry would have atleast one \"Debit\" and one"
              + " \"Credit\" entry whose amounts are equal \n"
              + "Compound Journal entries may have \"n\" debits and \"m\" credits where both \"m\""
              + " and \"n\" are greater than 0 and the net sum or all debits and credits are equal"
              + " \n\n"
              + "\n"
              + "Mandatory Fields\n"
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        value = "body",
        dataType = "body",
        dataTypeClass = JournalEntryCommand.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = JournalEntriesApiResourceSwagger.PostJournalEntriesResponse.class)
  })
    ...
    if (is(commandParam, "updateRunningBalance")) {
      final CommandWrapper commandRequest =
          new CommandWrapperBuilder()
              .updateRunningBalanceForJournalEntry()
      ...
      final CommandWrapper commandRequest =
          new CommandWrapperBuilder()
              .defineOpeningBalanceForJournalEntry()
              .withJson(jsonRequestBody)
              .build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else {
      final CommandWrapper commandRequest =
          new CommandWrapperBuilder().createJournalEntry().withJson(jsonRequestBody).build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    }
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @ApiOperation(
      value = "Update Running balances for Journal Entries",
      notes =
          "This API calculates the running balances for office. If office ID not provided this API"
              + " calculates running balances for all offices. \n"
              + "Mandatory Fields\n"
              + "officeId")
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        dataTypeClass =
            JournalEntriesApiResourceSwagger.PostJournalEntriesTransactionIdRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = JournalEntriesApiResourceSwagger.PostJournalEntriesTransactionIdResponse.class)
  })
  public String createReversalJournalEntry(
      @ApiParam(hidden = true) final String jsonRequestBody,
    ...
    if (is(commandParam, "reverse")) {
      final CommandWrapper commandRequest =
          new CommandWrapperBuilder()
              .reverseJournalEntry(transactionId)
              .withJson(jsonRequestBody)
              .build();
      result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    } else {
  ...
  @Consumes({MediaType.APPLICATION_JSON})
  @Produces({MediaType.APPLICATION_JSON})
  public String retrieveOpeningBalance(
      @Context final UriInfo uriInfo,
    ...
    final OfficeOpeningBalancesData officeOpeningBalancesData =
        this.journalEntryReadPlatformService.retrieveOfficeOpeningBalances(officeId, currencyCode);
    final ApiRequestJsonSerializationSettings settings =
        this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
  ...
  public Response getJournalEntriesTemplate(
      @QueryParam("officeId") final Long officeId,
      @QueryParam("dateFormat") final String dateFormat) {
    return bulkImportWorkbookPopulatorService.getTemplate(
        GlobalEntityType.GL_JOURNAL_ENTRIES.toString(), officeId, null, dateFormat);
  }

  @POST
  @Path("uploadtemplate")
  @Consumes(MediaType.MULTIPART_FORM_DATA)
  public String postJournalEntriesTemplate(
    ...
    final Long importDocumentId =
        this.bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.GL_JOURNAL_ENTRIES.toString(),
            uploadedInputStream,
            fileDetail,
            locale,
            dateFormat);
