...
@Service
public class ProvisioningEntriesReadPlatformServiceImpl
    implements ProvisioningEntriesReadPlatformService {

  private final JdbcTemplate jdbcTemplate;

  private final PaginationHelper<LoanProductProvisioningEntryData>
      loanProductProvisioningEntryDataPaginationHelper = new PaginationHelper<>();
  ...
  public Collection<LoanProductProvisioningEntryData> retrieveLoanProductsProvisioningData(
    ...
    LoanProductProvisioningEntryMapper mapper =
        new LoanProductProvisioningEntryMapper(formattedDate);
    final String sql = mapper.schema();
    return this.jdbcTemplate.query(sql, mapper, new Object[] {});
  }

  private static final class LoanProductProvisioningEntryMapper
    ...
    protected LoanProductProvisioningEntryMapper(String formattedDate) {
      sqlQuery =
          new StringBuilder()
              .append(
                  "select if(loan.loan_type_enum=1, mclient.office_id, mgroup.office_id) as"
                      + " office_id, loan.loan_type_enum, pcd.criteria_id as criteriaid,"
                      + " loan.product_id,loan.currency_code,")
              .append("GREATEST(datediff(")
              .append(formattedDate)
              .append(
                  ",sch.duedate),0) as numberofdaysoverdue,sch.duedate, pcd.category_id,"
                      + " pcd.provision_percentage,")
              .append(
                  "loan.total_outstanding_derived as outstandingbalance, pcd.liability_account,"
                      + " pcd.expense_account from m_loan_repayment_schedule sch")
              .append(" LEFT JOIN m_loan loan on sch.loan_id = loan.id")
              .append(
                  " JOIN m_loanproduct_provisioning_mapping lpm on lpm.product_id ="
                      + " loan.product_id")
              .append(
                  " JOIN m_provisioning_criteria_definition pcd on pcd.criteria_id ="
                      + " lpm.criteria_id and ")
              .append("(pcd.min_age <= GREATEST(datediff(")
              .append(formattedDate)
              .append(",sch.duedate),0) and ")
              .append("GREATEST(datediff(")
              .append(formattedDate)
              ...
              .append(
                  "(select MIN(sch1.duedate) from m_loan_repayment_schedule sch1 where"
                      + " sch1.loan_id=loan.id and sch1.completed_derived=false)");
    }
    ...
    public LoanProductProvisioningEntryData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      return new LoanProductProvisioningEntryData(
          historyId,
          officeId,
          currentcyCode,
          productId,
  ...
  public ProvisioningEntryData retrieveProvisioningEntryData(Long entryId) {
    ProvisioningEntryDataMapperWithSumReserved mapper1 =
        new ProvisioningEntryDataMapperWithSumReserved();
    final String sql1 = "select" + mapper1.getSchema() + " where entry.id = ?";
    ProvisioningEntryData data =
        this.jdbcTemplate.queryForObject(sql1, mapper1, new Object[] {entryId});
    return data;
  }

  private static final class ProvisioningEntryDataMapper
      implements RowMapper<ProvisioningEntryData> {

    private final StringBuilder sqlQuery =
        new StringBuilder()
            .append(
                " entry.id, entry.journal_entry_created, entry.createdby_id, entry.created_date,"
    ...
    public ProvisioningEntryData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      return new ProvisioningEntryData(
          id,
          journalEntry,
          createdById,
          createdUser,
  ...
  private static final class LoanProductProvisioningEntryRowMapper
    ...
    private final StringBuilder sqlQuery =
        new StringBuilder()
            .append(
                " entry.id, entry.history_id as historyId, office_id, entry.criteria_id as"
                    + " criteriaid, office.name as officename, product.name as productname,"
                    + " entry.product_id, ")
            .append(
                "category_id, category.category_name, liability.id as liabilityid,"
                    + " liability.gl_code as liabilitycode, liability.name as liabilityname, ")
            .append(
                "expense.id as expenseid, expense.gl_code as expensecode, expense.name as"
                    + " expensename, entry.currency_code, entry.overdue_in_days,"
    ...
    public LoanProductProvisioningEntryData mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      return new LoanProductProvisioningEntryData(
          ...
          amountreserved,
          liabilityAccountCode,
          liabilityAccountglCode,
          liabilityAccountName,
          expenseAccountCode,
          expenseAccountglCode,
          expenseAccountName,
          criteriaId);
    }
  ...
  private static final class ProvisioningEntryDataMapperWithSumReserved
    ...
    private final StringBuilder sqlQuery =
        new StringBuilder()
            .append(
                " entry.id, journal_entry_created, createdby_id, created_date, created.username as"
                    + " createduser,")
            .append(
                "lastmodifiedby_id, modified.username as modifieduser, lastmodified_date,"
                    + " SUM(reserved.reseve_amount) as totalreserved ")
            .append("from m_provisioning_history entry ")
            .append(
  ...
  public Page<ProvisioningEntryData> retrieveAllProvisioningEntries(Integer offset, Integer limit) {
    ...
    return this.provisioningEntryDataPaginationHelper.fetchPage(
  ...
  public ProvisioningEntryData retrieveProvisioningEntryDataByCriteriaId(Long criteriaId) {
    ...
    Collection<LoanProductProvisioningEntryData> entries =
        this.jdbcTemplate.query(sql, mapper, new Object[] {criteriaId});
  ...
  private static final class ProvisioningEntryIdDateRowMapper
    ...
    StringBuilder buff =
        new StringBuilder()
            ...
            .append(
                "where history1.created_date = (select max(history2.created_date) from"
                    + " m_provisioning_history history2 ")
  ...
  public Page<LoanProductProvisioningEntryData> retrieveProvisioningEntries(
    ...
    final String sqlCountRows = "SELECT FOUND_ROWS()";
    return this.loanProductProvisioningEntryDataPaginationHelper.fetchPage(
        this.jdbcTemplate, sqlCountRows, sqlBuilder.toString(), whereClauseItemsitems, mapper);
  }
}
