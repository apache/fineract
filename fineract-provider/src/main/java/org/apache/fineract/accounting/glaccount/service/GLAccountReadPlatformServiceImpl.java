...
public class GLAccountReadPlatformServiceImpl implements GLAccountReadPlatformService {
  ...
  private static final String nameDecoratedBaseOnHierarchy =
      "concat(substring('........................................', 1, ((LENGTH(hierarchy) -"
          + " LENGTH(REPLACE(hierarchy, '.', '')) - 1) * 4)), name)";
  ...
  private static final class GLAccountMapper implements RowMapper<GLAccountData> {
    ...
    public String schema() {
      ...
      sb.append(
              " gl.id as id, name as name, parent_id as parentId, gl_code as glCode, disabled as"
                  + " disabled, manual_journal_entries_allowed as manualEntriesAllowed, ")
          .append(
              "classification_enum as classification, account_usage as accountUsage,"
    ...
    @Override
    public GLAccountData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
        throws SQLException {
  ...
  public List<GLAccountData> retrieveAllGLAccounts(
      final Integer accountClassification,
      final String searchParam,
      final Integer usage,
      final Boolean manualTransactionsAllowed,
      ...
      if (associationParametersData.isRunningBalanceRequired()) {
        sql =
            sql
                ...
                + " (select id, max(entry_date) as entry_date, account_id from"
                + " acc_gl_journal_entry where is_running_balance_calculated = 1 group by"
                + " account_id desc, id) t3 inner join acc_gl_journal_entry t2 on t2.account_id ="
                + " t3.account_id and t2.entry_date = t3.entry_date group by t2.account_id desc)"
                + " t1)";
      }
    }
  ...
  @Override
  public GLAccountData retrieveGLAccountById(
      final long glAccountId, JournalEntryAssociationParametersData associationParametersData) {
    try {
      ...
      final GLAccountData glAccountData =
          this.jdbcTemplate.queryForObject(sql.toString(), rm, new Object[] {glAccountId});
  ...
  public List<GLAccountData> retrieveAllEnabledDetailGLAccounts(final GLAccountType accountType) {
    return retrieveAllGLAccounts(
        accountType.getValue(),
        null,
        GLAccountUsage.DETAIL.getValue(),
        null,
        false,
  ...
  public List<GLAccountData> retrieveAllEnabledHeaderGLAccounts(final GLAccountType accountType) {
    return retrieveAllGLAccounts(
        accountType.getValue(),
        null,
        GLAccountUsage.HEADER.getValue(),
        null,
        false,
        new JournalEntryAssociationParametersData());
  }
  ...
  private static final class GLAccountDataLookUpMapper
      implements RowMapper<GLAccountDataForLookup> {

    public String schema() {
      return " gl.id as id, gl.name as name, gl.gl_code as glCode from acc_accounting_rule rule"
          + " join acc_rule_tags tags on tags.acc_rule_id = rule.id join acc_gl_account gl"
          + " on gl.tag_id=tags.tag_id";
    }

    @Override
    public GLAccountDataForLookup mapRow(
