...
@Service
public class AccountingRuleWritePlatformServiceJpaRepositoryImpl
    implements AccountingRuleWritePlatformService {

  private static final Logger logger =
      LoggerFactory.getLogger(AccountingRuleWritePlatformServiceJpaRepositoryImpl.class);
  ...
  public AccountingRuleWritePlatformServiceJpaRepositoryImpl(
      ...
      final AccountingRuleRepository ruleRepository,
      final AccountingRuleCommandFromApiJsonDeserializer fromApiJsonDeserializer,
   ...
   * @param dve
   */
  private void handleAccountingRuleIntegrityIssues(
    ...
    throw new PlatformDataIntegrityException(
        "error.msg.accounting.rule.unknown.data.integrity.issue",
  ...
  private AccountingRule assembleAccountingRuleAndTags(
      final Office office, final JsonCommand command) {
    // get the GL Accounts or tags to Debit and Credit
    final String[] debitTags =
        command.arrayValueOfParameterNamed(
    ...
    final Set<String> incomingDebitTags =
        debitTags == null ? new HashSet<String>() : new HashSet<>(Arrays.asList(debitTags));
    final Set<String> incomingCreditTags =
        creditTags == null ? new HashSet<String>() : new HashSet<>(Arrays.asList(creditTags));
    final Long accountToDebitId =
        command.longValueOfParameterNamed(
            AccountingRuleJsonInputParams.ACCOUNT_TO_DEBIT.getValue());
    final Long accountToCreditId =
        command.longValueOfParameterNamed(
    ...
    if ((accountToDebitId != null && debitTags != null)
        || (accountToDebitId == null && debitTags == null)) {
      throw new AccountingRuleDataException(
          AccountingRuleJsonInputParams.ACCOUNT_TO_DEBIT.getValue(),
          AccountingRuleJsonInputParams.DEBIT_ACCOUNT_TAGS.getValue());
      ...
      accountingTagRules =
          saveDebitOrCreditTags(incomingDebitTags, JournalEntryType.DEBIT, accountingTagRules);
      allowMultipleDebitEntries =
          command.booleanPrimitiveValueOfParameterNamed(
              AccountingRuleJsonInputParams.ALLOW_MULTIPLE_DEBIT_ENTRIES.getValue());
    }

    if ((accountToCreditId != null && creditTags != null)
        || (accountToCreditId == null && creditTags == null)) {
      throw new AccountingRuleDataException(
          AccountingRuleJsonInputParams.ACCOUNT_TO_CREDIT.getValue(),
          AccountingRuleJsonInputParams.CREDIT_ACCOUNT_TAGS.getValue());
      ...
      accountingTagRules =
          saveDebitOrCreditTags(incomingCreditTags, JournalEntryType.CREDIT, accountingTagRules);
      allowMultipleCreditEntries =
          command.booleanPrimitiveValueOfParameterNamed(
              AccountingRuleJsonInputParams.ALLOW_MULTIPLE_CREDIT_ENTRIES.getValue());
    }

    final AccountingRule accountingRule =
        AccountingRule.fromJson(
            office,
            debitAccount,
            creditAccount,
            command,
  ...
  public CommandProcessingResult updateAccountingRule(
      final Long accountingRuleId, final JsonCommand command) {
    ...
    try {
      ...
      if (command.parameterExists(AccountingRuleJsonInputParams.OFFICE_ID.getValue())) {
        officeId =
            command.longValueOfParameterNamed(AccountingRuleJsonInputParams.OFFICE_ID.getValue());
      }
      ...
      final AccountingRule accountingRule =
          this.accountingRuleRepositoryWrapper.findOneWithNotFoundDetection(accountingRuleId);
      ...
      if (creditTags != null
          && creditTags.length > 0
          && command.parameterExists(
              AccountingRuleJsonInputParams.CREDIT_ACCOUNT_TAGS.getValue())) {

        final Set<String> creditTagsToAdd =
            determineCreditTagToAddAndRemoveOldTags(
                creditTags, JournalEntryType.CREDIT, accountingRule);

        if (!creditTagsToAdd.isEmpty()) {
          List<AccountingTagRule> accountingTagRules = new ArrayList<>();
          accountingTagRules =
              saveDebitOrCreditTags(creditTagsToAdd, JournalEntryType.CREDIT, accountingTagRules);
          accountingRule.updateAccountingRuleForTags(accountingTagRules);
          ...
          changesOnly.put(
              AccountingRuleJsonInputParams.CREDIT_ACCOUNT_TAGS.getValue(), creditTagsToAdd);
        }
      }
      ...
      if (debitTags != null
        ...
        final Set<String> debitTagsToAdd =
            determineCreditTagToAddAndRemoveOldTags(
                debitTags, JournalEntryType.DEBIT, accountingRule);
        if (!debitTagsToAdd.isEmpty()) {
          List<AccountingTagRule> accountingTagRules = new ArrayList<>();
          accountingTagRules =
              saveDebitOrCreditTags(debitTagsToAdd, JournalEntryType.DEBIT, accountingTagRules);
          accountingRule.updateAccountingRuleForTags(accountingTagRules);
          ...
          changesOnly.put(
              AccountingRuleJsonInputParams.DEBIT_ACCOUNT_TAGS.getValue(), debitTagsToAdd);
        }
      }
  ...
  private Set<String> determineCreditTagToAddAndRemoveOldTags(
      final String[] creditOrDebitTags,
  ...
  private List<AccountingTagRule> saveDebitOrCreditTags(
      final Set<String> creditOrDebitTagArray,
    ...
    for (final String creditOrDebitTag : creditOrDebitTagArray) {
      if (creditOrDebitTag != null && StringUtils.isNotBlank(creditOrDebitTag)) {
        ...
        final CodeValue creditOrDebitAccount =
            this.codeValueRepository
                .findById(creditOrDebitTagIdLongValue)
                .orElseThrow(() -> new CodeValueNotFoundException(creditOrDebitTagIdLongValue));
        final AccountingTagRule accountingTagRule =
            AccountingTagRule.create(creditOrDebitAccount, transactionType.getValue());
