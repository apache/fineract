...
public class GLAccountData implements Serializable {
  ...
  public static GLAccountData importInstance(
      String name,
      Long parentId,
      String glCode,
      Boolean manualEntriesAllowed,
      EnumOptionData type,
      EnumOptionData usage,
      String description,
      CodeValueData tagId,
      Integer rowIndex) {
    return new GLAccountData(
        name, parentId, glCode, manualEntriesAllowed, type, usage, description, tagId, rowIndex);
  }
  ...
  public GLAccountData(
      ...
      final EnumOptionData usage,
      final String description,
      final String nameDecorated,
      final CodeValueData tagId,
      final Long organizationRunningBalance) {
    this.id = id;
    this.name = name;
    this.parentId = parentId;
    this.glCode = glCode;
  ...
  public static GLAccountData sensibleDefaultsForNewGLAccountCreation(final Integer glAccType) {
    ...
    final EnumOptionData type;
    if (glAccType != null
        && glAccType >= GLAccountType.getMinValue()
        && glAccType <= GLAccountType.getMaxValue()) {
