...
public class XBRLBuilder {
  ...
  @Autowired private NamespaceReadPlatformService readNamespaceService;

  public String build(final XBRLData xbrlData) {
    return this.build(
        xbrlData.getResultMap(),
        xbrlData.getStartDate(),
        xbrlData.getEndDate(),
        xbrlData.getCurrency());
  }

  public String build(
      final Map<MixTaxonomyData, BigDecimal> map,
  ...
  Element addTaxonomy(
      final Element rootElement, final MixTaxonomyData taxonomy, final BigDecimal value) {
    ...
    if (!this.contextMap.containsKey(context)) {
      ...
      final String contextRefID =
          (context.getPeriodType() == 0)
              ? ("As_Of_" + endDateStr + (this.instantScenarioCounter++))
              : ("Duration_"
                  + startDateStr
                  + "_To_"
                  + endDateStr
                  + (this.durationScenarioCounter++));
