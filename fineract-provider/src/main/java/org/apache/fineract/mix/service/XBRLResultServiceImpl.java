...
public class XBRLResultServiceImpl implements XBRLResultService {
  ...
  private HashMap<MixTaxonomyData, BigDecimal> retrieveTaxonomyConfig(
    ...
    if (config != null) {
      ...
      for (final Entry<String, String> entry : configMap.entrySet()) {
        ...
        if (value != null) {
          final MixTaxonomyData taxonomy =
              this.readTaxonomyService.retrieveOne(Long.parseLong(entry.getKey()));
  ...
  private String getAccountSql(final Date startDate, final Date endDate) {
    final String sql =
        "select debits.glcode as 'glcode', debits.name as 'name',"
            ...
            + " (select acc_gl_account.gl_code as 'glcode',name,sum(amount) as 'debitamount' from"
            ...
            + " acc_gl_journal_entry.entry_date <= "
            + endDate
    ...
    for (final String glcode : glCodes) {
      ...
      mappingString =
          mappingString.replaceAll(
              "\\{" + glcode + "\\}", balance != null ? balance.toString() : "0");
    }
