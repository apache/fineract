...
public class ProvisioningCategoryReadPlatformServiceImpl
    implements ProvisioningCategoryReadPlatformService {
  ...
  public Collection<ProvisioningCategoryData> retrieveAllProvisionCategories() {
    ...
    final String sql =
        "select "
            + this.provisionCategoryRowMapper.schema()
            + " from m_provision_category pc order by pc.id";
    return this.jdbcTemplate.query(sql, this.provisionCategoryRowMapper, new Object[] {});
  }

  private static final class ProvisioningCategoryRowMapper
      implements RowMapper<ProvisioningCategoryData> {
    @Override
    public ProvisioningCategoryData mapRow(
