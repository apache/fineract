...
public class TellerManagementReadPlatformServiceImpl
  ...
  private static final class TellerMapper implements RowMapper<TellerData> {
    ...
    public String schema() {
      ...
      sqlBuilder.append(
          "t.id as id,t.office_id as office_id, t.name as teller_name, t.description as"
              + " description, ");
      sqlBuilder.append(
          "t.valid_from as start_date, t.valid_to as end_date, t.state as status, o.name as"
              + " office_name, ");
      sqlBuilder.append(
          "t.debit_account_id as debit_account_id, t.credit_account_id as credit_account_id ");
  ...
  private static final class TellerInOfficeHierarchyMapper implements RowMapper<TellerData> {
    ...
    public String schema() {
      ...
      sqlBuilder.append(
          "join m_office ohierarchy on o.hierarchy like concat(ohierarchy.hierarchy, '%') ");
  ...
  public Collection<TellerData> retrieveAllTellersForDropdown(final Long officeId) {
    ...
    final String sql =
        "select " + this.lookupMapper.schema() + " where s.office_id = ? and s.is_active=1 ";
  ...
  @Override
  public Collection<TellerData> retrieveAllTellers(
      final String sqlSearch, final Long officeId, final String status) {
    final String extraCriteria = getTellerCriteria(sqlSearch, officeId, status);
    return retrieveAllTeller(extraCriteria, officeId);
  }

  private Collection<TellerData> retrieveAllTeller(
      final String extraCriteria, final Long officeId) {
  ...
  private String getTellerCriteria(
      final String sqlSearch, final Long officeId, final String status) {
    ...
    if (status.equalsIgnoreCase("active")) {
      ...
      throw new UnrecognizedQueryParamException(
          "status", status, new Object[] {"all", "active", "inactive"});
    }
  ...
  @Override
  public Collection<CashierData> retrieveCashiersForTellers(
      final String sqlSearch, final Long tellerId) {
  ...
  public Collection<CashierData> getCashierData(
      Long officeId, Long tellerId, Long staffId, Date date) {
  ...
  @Override
  public Collection<TellerTransactionData> fetchTellerTransactionsByTellerId(
      Long tellerId, Date fromDate, Date toDate) {
    // TODO Auto-generated method stub
    return null;
  }

  @Override
  public Collection<TellerJournalData> getJournals(
      Long officeId, Long tellerId, Long cashierId, Date dateFrom, Date dateTo) {
    // TODO Auto-generated method stub
    return null;
  }

  @Override
  public Collection<TellerJournalData> fetchTellerJournals(
      Long tellerId, Long cashierId, Date fromDate, Date toDate) {
    // TODO Auto-generated method stub
    return null;
  }
  ...
  @Cacheable(
      value = "tellers",
      key =
          "T(org.apache.fineract.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'of')")
  ...
  @Override
  public CashierData retrieveCashierTemplate(
      Long officeId, Long tellerId, boolean staffInSelectedOfficeOnly) {
    final Long defaultOfficeId = defaultToUsersOfficeIfNull(officeId);
  ...
  public CashierTransactionData retrieveCashierTxnTemplate(Long cashierId) {
    ...
    return CashierTransactionData.template(
        cashierId,
        tellerId,
        tellerName,
        officeId,
  ...
  public CashierTransactionsWithSummaryData retrieveCashierTransactionsWithSummary(
      ...
      final boolean includeAllTellers,
      final Date fromDate,
      final Date toDate,
    ...
    final Page<CashierTransactionData> cashierTransactions =
        retrieveCashierTransactions(
            cashierId, includeAllTellers, fromDate, toDate, currencyCode, searchParameters);
    ...
    CashierTransactionsWithSummaryData txnsWithSummary =
        CashierTransactionsWithSummaryData.instance(
            ...
            cashierTxnTemplate.getTellerId(),
            cashierTxnTemplate.getTellerName(),
            cashierTxnTemplate.getCashierId(),
            cashierTxnTemplate.getCashierName());
    return txnsWithSummary;
  }

  @Override
  public Page<CashierTransactionData> retrieveCashierTransactions(
      final Long cashierId,
      final boolean includeAllTellers,
      final Date fromDate,
      final Date toDate,
    ...
    String sql =
        "select * from (select "
            + ctm.cashierTxnSchema()
            + " where txn.cashier_id = ? and txn.currency_code = ? and o.hierarchy like ? AND"
            + " ((case when c.full_day then Date(txn.created_date) between c.start_date AND"
            + " c.end_date else ( Date(txn.created_date) between c.start_date AND c.end_date ) and"
            + " ( TIME(txn.created_date) between TIME(c.start_time) AND TIME(c.end_time)) end) or"
            + " txn.txn_type = 101))  cashier_txns  union (select "
            + ctm.savingsTxnSchema()
            + " where sav_txn.is_reversed = 0 and c.id = ? and sav.currency_code = ? and"
            + " o.hierarchy like ? and  sav_txn.transaction_date between c.start_date and"
            + " date_add(c.end_date, interval 1 day)  and renum.enum_value in"
            + " ('deposit','withdrawal fee', 'Pay Charge', 'withdrawal', 'Annual Fee', 'Waive"
            + " Charge', 'Interest Posting', 'Overdraft Interest')  and (sav_txn.payment_detail_id"
            + " IS NULL OR payType.is_cash_payment = 1)  AND acnttrans.id IS NULL )  union (select"
            + " "
            + ctm.loansTxnSchema()
            + " where loan_txn.is_reversed = 0 and c.id = ? and loan.currency_code = ? and"
            + " o.hierarchy like ? and  loan_txn.transaction_date between c.start_date and"
            + " date_add(c.end_date, interval 1 day)  and renum.enum_value in"
            ...
            + " (loan_txn.payment_detail_id IS NULL OR payType.is_cash_payment = 1)  AND"
            + " acnttrans.id IS NULL )  union (select "
            + ctm.clientTxnSchema()
            + " where cli_txn.is_reversed = 0 and c.id = ? and cli_txn.currency_code = ? and"
            + " o.hierarchy like ? and cli_txn.transaction_date  between c.start_date and"
            + " date_add(c.end_date, interval 1 day)  and renum.enum_value in ('PAY_CHARGE',"
            + " 'WAIVE_CHARGE')  and (cli_txn.payment_detail_id IS NULL OR payType.is_cash_payment"
            + " = 1) )  order by created_date ";
    ...
    //        return this.jdbcTemplate.query(sql, ctm, new Object[] { cashierId, currencyCode,
    // hierarchySearchString, cashierId, currencyCode,
    //                hierarchySearchString, cashierId, currencyCode, hierarchySearchString,
    // cashierId, currencyCode, hierarchySearchString });
    Object[] params =
        new Object[] {
          cashierId,
          currencyCode,
  ...
  private static final class CashierMapper implements RowMapper<CashierData> {
    ...
    public String schema() {
      ...
      sqlBuilder.append(
          "c.id as id,c.teller_id as teller_id, t.name as teller_name, c.description as"
              + " description, ");
      ...
      sqlBuilder.append(
          "c.full_day as full_day, c.start_time as start_time, c.end_time as end_time ");
  ...
  private static final class CashierTransactionMapper implements RowMapper<CashierTransactionData> {
    ...
    public String cashierTxnSchema() {
      ...
      sqlBuilder.append(
          " txn.entity_type as entity_type, txn.entity_id as entity_id, txn.created_date as"
              + " created_date, ");
      sqlBuilder.append(
          " o.id as office_id, o.name as office_name, t.id as teller_id, t.name as teller_name,"
              + " s.display_name as cashier_name ");
    ...
    public String savingsTxnSchema() {
      ...
      sqlBuilder.append(
          "     when renum.enum_value in ('deposit','withdrawal fee', 'Pay Charge', 'Annual"
              + " Fee') ");
      ...
      sqlBuilder.append(
          " concat (renum.enum_value, ', Sav:', sav.id, '-', sav.account_no, ',Client:', cl.id,"
              + " '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
          " 'savings' as entity_type, sav.id as entity_id, sav_txn.created_date as created_date, ");
      sqlBuilder.append(
          " o.id as office_id, o.name as office_name, null as teller_id, null as teller_name,"
              + " staff.display_name as cashier_name ");
      sqlBuilder.append(" from m_savings_account_transaction sav_txn ");
      sqlBuilder.append(
          " left join r_enum_value renum on sav_txn.transaction_type_enum = renum.enum_id and"
              + " renum.enum_name = 'savings_transaction_type_enum' ");
      ...
      sqlBuilder.append(
          " left join m_payment_detail payDetails on payDetails.id = sav_txn.payment_detail_id ");
      sqlBuilder.append(
    ...
    public String loansTxnSchema() {
      ...
      sqlBuilder.append(" case ");
      sqlBuilder.append(
          "     when renum.enum_value in ('REPAYMENT_AT_DISBURSEMENT','REPAYMENT',"
              + " 'RECOVERY_REPAYMENT', 'CHARGE_PAYMENT') ");
      sqlBuilder.append("         then 103 ");
      sqlBuilder.append(
          "     when renum.enum_value in ('DISBURSEMENT', 'WAIVE_INTEREST', 'WRITEOFF',"
              + " 'WAIVE_CHARGES') ");
      sqlBuilder.append("         then 104 ");
      sqlBuilder.append("     else ");
      sqlBuilder.append("         105 ");
      ...
      sqlBuilder.append(
          " concat (renum.enum_value, ', Loan:', loan.id, '-', loan.account_no, ',Client:', cl.id,"
              + " '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
      ...
      sqlBuilder.append(
          " left join r_enum_value renum on loan_txn.transaction_type_enum = renum.enum_id and"
              + " renum.enum_name = 'loan_transaction_type_enum' ");
    ...
    public String clientTxnSchema() {
      ...
      sqlBuilder.append(
          " concat (renum.enum_value, ', Client:', cl.id, '-', cl.account_no, ',Client:', cl.id,"
              + " '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
      ...
      sqlBuilder.append(
          " left join r_enum_value renum on cli_txn.transaction_type_enum = renum.enum_id AND"
              + " renum.enum_name = 'client_transaction_type_enum' ");
      ...
      sqlBuilder.append(" left join m_cashiers c on c.staff_id = staff.id ");
      sqlBuilder.append(
          " left join m_payment_detail payDetails on payDetails.id = cli_txn.payment_detail_id ");
      sqlBuilder.append(
          " left join m_payment_type payType on payType.id = payDetails.payment_type_id ");
    ...
    public CashierTransactionData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
      ...
      return CashierTransactionData.instance(
          id,
          cashierId,
          txnType,
          txnAmount,
  ...
  private static final class CashierTransactionSummaryMapper
    ...
    public String cashierTxnSummarySchema() {
      ...
      sqlBuilder.append(
          " AND (( case when c.full_day then Date(txn.created_date) between c.start_date AND"
              + " c.end_date ");
      sqlBuilder.append(
      ...
      sqlBuilder.append(
          "    concat (renum.enum_value, ', Sav:', sav.id, '-', sav.account_no, ',Client:', cl.id,"
              + " '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
          "    'savings' as entity_type, sav.id as entity_id, sav_txn.created_date as"
              + " created_date, ");
      sqlBuilder.append(
          "    o.id as office_id, o.name as office_name, null as teller_id, null as teller_name,"
              + " staff.display_name as cashier_name ");
      ...
      sqlBuilder.append(
          "    left join r_enum_value renum on sav_txn.transaction_type_enum = renum.enum_id and"
              + " renum.enum_name = 'savings_transaction_type_enum' ");
      sqlBuilder.append(
          "    left join m_savings_account sav on sav_txn.savings_account_id = sav.id ");
      sqlBuilder.append("    left join m_client cl on sav.client_id = cl.id ");
      sqlBuilder.append("    left join m_office o on cl.office_id = o.id ");
      sqlBuilder.append("    left join m_appuser user on sav_txn.appuser_id = user.id ");
      sqlBuilder.append("    left join m_staff staff on user.staff_id = staff.id ");
      ...
      sqlBuilder.append(
          "    and sav_txn.transaction_date between c.start_date and date_add(c.end_date, interval"
              + " 1 day) ");
      sqlBuilder.append(
      ...
      sqlBuilder.append(
          "    concat (renum.enum_value, ', Loan:', loan.id, '-', loan.account_no, ',Client:',"
              + " cl.id, '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
          "    'loans' as entity_type, loan.id as entity_id, loan_txn.created_date as"
              + " created_date, ");
      sqlBuilder.append(
          "    o.id as office_id, o.name as office_name, null as teller_id, null as teller_name,"
              + " staff.display_name as cashier_name ");
      ...
      sqlBuilder.append("    cli_txn.amount as txn_amount, cli_txn.transaction_date as txn_date, ");
      sqlBuilder.append(
          "    concat (renum.enum_value, ', Client:', cl.id, '-', cl.account_no, ',Client:',"
              + " cl.id, '-',cl.display_name) as txn_note, ");
      sqlBuilder.append(
          "    'client' as entity_type, cl.id as entity_id, cli_txn.created_date as"
              + " created_date, ");
      sqlBuilder.append(
          "    o.id as office_id, o.name as office_name, null as teller_id, null as teller_name,"
              + " staff.display_name as cashier_name ");
      ...
      sqlBuilder.append("    and o.hierarchy LIKE ? ");
      sqlBuilder.append(
          "    and cli_txn.transaction_date between c.start_date and date_add(c.end_date, interval"
              + " 1 day) ");
      sqlBuilder.append(
