...
public class OfficeReadPlatformServiceImpl implements OfficeReadPlatformService {
  ...
  private static final String nameDecoratedBaseOnHierarchy =
      "concat(substring('........................................', 1, ((LENGTH(o.hierarchy) -"
          + " LENGTH(REPLACE(o.hierarchy, '.', '')) - 1) * 4)), o.name)";
  ...
  private static final class OfficeMapper implements RowMapper<OfficeData> {
    ...
    public String officeSchema() {
      return " o.id as id, o.name as name, "
          ...
          + " o.hierarchy as hierarchy, parent.id as parentId, parent.name as parentName from"
          + " m_office o LEFT JOIN m_office AS parent ON parent.id = o.parent_id ";
    }
    ...
    public OfficeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
      ...
      return new OfficeData(
          id, name, nameDecorated, externalId, openingDate, hierarchy, parentId, parentName, null);
    }
  }
  ...
  private static final class OfficeTransactionMapper implements RowMapper<OfficeTransactionData> {
    ...
    public String schema() {
      return " ot.id as id, ot.transaction_date as transactionDate, ot.from_office_id as"
          + " fromOfficeId, fromoff.name as fromOfficeName,  ot.to_office_id as toOfficeId,"
          + " tooff.name as toOfficeName, ot.transaction_amount as transactionAmount,"
          + " ot.description as description,  ot.currency_code as currencyCode,"
          + " rc.decimal_places as currencyDigits, rc.currency_multiplesof as"
          + " inMultiplesOf,  rc.name as currencyName, rc.internationalized_name_code as"
          + " currencyNameCode, rc.display_symbol as currencyDisplaySymbol  from"
          + " m_office_transaction ot  left join m_office fromoff on fromoff.id ="
          + " ot.from_office_id  left join m_office tooff on tooff.id = ot.to_office_id "
          + " join m_currency rc on rc.`code` = ot.currency_code";
    }

    @Override
    public OfficeTransactionData mapRow(
      ...
      return OfficeTransactionData.instance(
          id,
          transactionDate,
          fromOfficeId,
          fromOfficeName,
  ...
  public Collection<OfficeData> retrieveAllOffices(
      final boolean includeAllOffices, final SearchParameters searchParameters) {
  ...
  public OfficeData retrieveOffice(final Long officeId) {
    ...
    try {
      ...
      final OfficeData selectedOffice =
          this.jdbcTemplate.queryForObject(sql, rm, new Object[] {officeId});
  ...
  public Collection<OfficeTransactionData> retrieveAllOfficeTransactions() {
    ...
    final String sql =
        "select "
            + rm.schema()
            + " where (fromoff.hierarchy like ? or tooff.hierarchy like ?) order by"
            + " ot.transaction_date, ot.id";

    return this.jdbcTemplate.query(
        sql, rm, new Object[] {hierarchySearchString, hierarchySearchString});
  }
