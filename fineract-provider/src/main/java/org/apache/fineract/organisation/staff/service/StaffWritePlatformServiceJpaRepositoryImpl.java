...
public class StaffWritePlatformServiceJpaRepositoryImpl implements StaffWritePlatformService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(StaffWritePlatformServiceJpaRepositoryImpl.class);
  ...
  public StaffWritePlatformServiceJpaRepositoryImpl(
      final StaffCommandFromApiJsonDeserializer fromApiJsonDeserializer,
  ...
  public CommandProcessingResult updateStaff(final Long staffId, final JsonCommand command) {
    ...
    try {
      ...
      return new CommandProcessingResultBuilder()
          .withCommandId(command.commandId())
          .withEntityId(staffId)
          .withOfficeId(staffForUpdate.officeId())
          .with(changesOnly)
          .build();
    } catch (final DataIntegrityViolationException dve) {
      handleStaffDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
   ...
   * is.
   */
  private void handleStaffDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("external_id")) {
      ...
      throw new PlatformDataIntegrityException(
          ...
          "Staff with externalId `" + externalId + "` already exists",
          "externalId",
          externalId);
