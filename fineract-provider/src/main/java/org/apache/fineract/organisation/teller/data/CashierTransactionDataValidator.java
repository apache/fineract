...
public class CashierTransactionDataValidator {
  ...
  public void validateSettleCashAndCashOutTransactions(
      final Long cashierId, String currencyCode, final BigDecimal transactionAmount) {
    ...
    final CashierTransactionsWithSummaryData cashierTxnWithSummary =
        this.tellerManagementReadPlatformService.retrieveCashierTransactionsWithSummary(
            cashierId, false, fromDate, toDate, currencyCode, searchParameters);
    if (cashierTxnWithSummary.getNetCash().subtract(transactionAmount).compareTo(BigDecimal.ZERO)
        < 0) {
  ...
  public void validateCashierAllowedDateAndTime(final Cashier cashier, final Teller teller) {
    ...
    if (fromDate.isBefore(tellerFromDate)
        ...
        || (tellerEndDate != null
            && (fromDate.isAfter(tellerEndDate) || endDate.isAfter(tellerEndDate)))) {
     ...
     */
    String sql =
        "select count(*) from m_cashiers c where c.staff_id = "
            + staffId
            + " AND "
            + "(('"
            + fromDate
            ...
            + " OR ( c.start_date BETWEEN '"
            + fromDate
            + "' AND '"
            + endDate
            + "' OR c.end_date BETWEEN '"
            + fromDate
            + "' AND '"
            + endDate
            + "'))";
    if (!cashier.isFullDay()) {
      ...
      sql =
          sql
              + " AND ( Time(c.start_time) BETWEEN TIME('"
              + startTime
              + "') and TIME('"
              + endTime
              + "') or Time(c.end_time) BETWEEN TIME('"
              + startTime
              + "') and TIME('"
              + endTime
              + "')) ";
  ...
  public void validateOnLoanDisbursal(
      AppUser user, String currencyCode, BigDecimal transactionAmount) {
    LocalDateTime localDateTime = DateUtils.getLocalDateTimeOfTenant();
    if (user.getStaff() != null) {
      String sql =
          "select c.id from m_cashiers c where c.staff_id = "
              + user.getStaff().getId()
              + " AND "
