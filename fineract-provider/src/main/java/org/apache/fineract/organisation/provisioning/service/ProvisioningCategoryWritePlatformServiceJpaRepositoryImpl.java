...
public class ProvisioningCategoryWritePlatformServiceJpaRepositoryImpl
  ...
  @Autowired
  public ProvisioningCategoryWritePlatformServiceJpaRepositoryImpl(
      final ProvisioningCategoryRepository provisioningCategoryRepository,
      final ProvisioningCategoryDefinitionJsonDeserializer fromApiJsonDeserializer,
  ...
  @Override
  public CommandProcessingResult updateProvisioningCategory(
      final Long categoryId, JsonCommand command) {
    try {
      this.fromApiJsonDeserializer.validateForUpdate(command.json());
      final ProvisioningCategory provisioningCategoryForUpdate =
          this.provisioningCategoryRepository
              .findById(categoryId)
  ...
  private boolean isAnyLoanProductsAssociateWithThisProvisioningCategory(final Long categoryID) {
    final String sql =
        "select if((exists (select 1 from m_loanproduct_provisioning_details lpd where"
            + " lpd.category_id = ?)) = 1, 'true', 'false')";
  ...
  private void handleDataIntegrityIssues(
    ...
    if (realCause.getMessage().contains("category_name")) {
      ...
      throw new PlatformDataIntegrityException(
          "error.msg.provisioning.duplicate.categoryname",
          "Provisioning Cateory with name `" + name + "` already exists",
          "category name",
          name);
    }
