...
public class ApplicationCurrency extends AbstractPersistableCustom {
    ...
    public static ApplicationCurrency from(final ApplicationCurrency currency, final int decimalPlaces, final Integer inMultiplesOf) {
        return new ApplicationCurrency().setCode(currency.code).setName(currency.name).setDecimalPlaces(decimalPlaces)
                .setInMultiplesOf(inMultiplesOf).setDisplaySymbol(currency.displaySymbol).setNameCode(currency.nameCode);
    }

    public CurrencyData toData() {
        return new CurrencyData().setCode(this.code).setName(this.name).setDecimalPlaces(this.decimalPlaces)
                .setInMultiplesOf(this.inMultiplesOf).setDisplaySymbol(this.displaySymbol).setNameCode(this.nameCode);
    }
