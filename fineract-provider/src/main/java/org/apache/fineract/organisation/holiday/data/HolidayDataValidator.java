...
public class HolidayDataValidator {
  ...
  private static final Set<String> HOLIDAY_CREATE_OR_UPDATE_REQUEST_DATA_PARAMETERS =
      new HashSet<>(
          Arrays.asList(
              HolidayApiConstants.localeParamName,
              HolidayApiConstants.dateFormatParamName,
              HolidayApiConstants.nameParamName,
              HolidayApiConstants.fromDateParamName,
              HolidayApiConstants.toDateParamName,
              HolidayApiConstants.descriptionParamName,
              HolidayApiConstants.officesParamName,
  ...
  public void validateForCreate(final String json) {
    ...
    this.fromApiJsonHelper.checkForUnsupportedParameters(
        typeOfMap, json, HOLIDAY_CREATE_OR_UPDATE_REQUEST_DATA_PARAMETERS);
    ...
    final String name =
        this.fromApiJsonHelper.extractStringNamed(HolidayApiConstants.nameParamName, element);
    baseDataValidator
        .reset()
        .parameter(HolidayApiConstants.nameParamName)
        .value(name)
        .notNull()
    ...
    baseDataValidator
        .reset()
        .parameter(HolidayApiConstants.fromDateParamName)
        .value(fromDate)
        .notNull();

    final LocalDate toDate =
        this.fromApiJsonHelper.extractLocalDateNamed(HolidayApiConstants.toDateParamName, element);
    baseDataValidator
        .reset()
        .parameter(HolidayApiConstants.toDateParamName)
        .value(toDate)
        .notNull();
    ...
    if (this.fromApiJsonHelper.parameterExists(HolidayApiConstants.reschedulingType, element)) {
      reschedulingType =
          this.fromApiJsonHelper.extractIntegerNamed(
              HolidayApiConstants.reschedulingType, element, Locale.ENGLISH);
    }
    ...
    if (reschedulingType == null
      ...
      baseDataValidator
          .reset()
          .parameter(HolidayApiConstants.repaymentsRescheduledToParamName)
    ...
    if (topLevelJsonElement.has(HolidayApiConstants.officesParamName)
      ...
      final JsonArray array =
          topLevelJsonElement.get(HolidayApiConstants.officesParamName).getAsJsonArray();
      if (array.size() > 0) {
        ...
        for (int i = 0; i < array.size(); i++) {
          ...
          final Long officeId =
              this.fromApiJsonHelper.extractLongNamed(
                  HolidayApiConstants.officeIdParamName, officeElement);
          baseDataValidator
              .reset()
              .parameter(HolidayApiConstants.officesParamName)
              .value(officeId)
              .notNull();
  ...
  public void validateForUpdate(final String json) {
    ...
    if (this.fromApiJsonHelper.parameterExists(HolidayApiConstants.toDateParamName, element)) {
      final LocalDate toDate =
          this.fromApiJsonHelper.extractLocalDateNamed(
              HolidayApiConstants.toDateParamName, element);
      baseDataValidator
          .reset()
          .parameter(HolidayApiConstants.toDateParamName)
          .value(toDate)
