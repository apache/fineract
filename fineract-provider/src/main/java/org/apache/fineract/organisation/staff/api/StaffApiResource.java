...
@SwaggerDefinition(
    tags = {
      @Tag(
          name = "Staff",
          description =
              "Allows you to model staff members. At present the key role of significance is"
                  + " whether this staff member is a loan officer or not.")
    })
public class StaffApiResource {
  ...
  public StaffApiResource(
      ...
      final StaffReadPlatformService readPlatformService,
      final OfficeReadPlatformService officeReadPlatformService,
      final DefaultToApiJsonSerializer<StaffData> toApiJsonSerializer,
  ...
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = StaffApiResourceSwagger.GetStaffResponse.class,
        responseContainer = "List"),
    @ApiResponse(
        code = 200,
        message = "GET https://DomainName/api/v1/staff?status={ACTIVE|INACTIVE|ALL}",
        response = StaffApiResourceSwagger.GetStaffResponse.class)
  })
  public String retrieveStaff(
      ...
      @DefaultValue("false")
          @QueryParam("staffInOfficeHierarchy")
          @ApiParam(value = "staffInOfficeHierarchy")
          final boolean staffInOfficeHierarchy,
      @DefaultValue("false") @QueryParam("loanOfficersOnly") @ApiParam(value = "loanOfficersOnly")
          final boolean loanOfficersOnly,
      @DefaultValue("active") @QueryParam("status") @ApiParam(value = "status")
          final String status) {
    ...
    if (staffInOfficeHierarchy) {
      staff =
          this.readPlatformService.retrieveAllStaffInOfficeAndItsParentOfficeHierarchy(
              officeId, loanOfficersOnly);
    } else {
      staff =
          this.readPlatformService.retrieveAllStaff(sqlSearch, officeId, loanOfficersOnly, status);
    }
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = StaffApiResourceSwagger.PostStaffRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = StaffApiResourceSwagger.PostStaffResponse.class)
  })
  ...
  public String retreiveStaff(
      @PathParam("staffId") @ApiParam(value = "staffId") final Long staffId,
      @Context final UriInfo uriInfo) {
  ...
  @ApiImplicitParams({
    @ApiImplicitParam(
        ...
        dataType = "body",
        format = "body",
        dataTypeClass = StaffApiResourceSwagger.PutStaffRequest.class)
  })
  @ApiResponses({
    @ApiResponse(
        code = 200,
        message = "",
        response = StaffApiResourceSwagger.PutStaffResponse.class)
  })
  ...
  public Response getStaffTemplate(
      @QueryParam("officeId") final Long officeId,
      @QueryParam("dateFormat") final String dateFormat) {
    return bulkImportWorkbookPopulatorService.getTemplate(
        GlobalEntityType.STAFF.toString(), officeId, null, dateFormat);
  }

  @POST
  @Path("uploadtemplate")
  @Consumes(MediaType.MULTIPART_FORM_DATA)
  public String postStaffTemplate(
    ...
    final Long importDocumentId =
        this.bulkImportWorkbookService.importWorkbook(
            GlobalEntityType.STAFF.toString(), uploadedInputStream, fileDetail, locale, dateFormat);
