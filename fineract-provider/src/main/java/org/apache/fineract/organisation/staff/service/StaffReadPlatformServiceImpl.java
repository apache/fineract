...
public class StaffReadPlatformServiceImpl implements StaffReadPlatformService {
  ...
  private final StaffInOfficeHierarchyMapper staffInOfficeHierarchyMapper =
      new StaffInOfficeHierarchyMapper();
  ...
  private static final class StaffMapper implements RowMapper<StaffData> {
    ...
    public String schema() {
      return " s.id as id,s.office_id as officeId, o.name as officeName, s.firstname as firstname,"
          + " s.lastname as lastname, s.display_name as displayName, s.is_loan_officer as"
          + " isLoanOfficer, s.external_id as externalId, s.mobile_no as mobileNo,"
          + " s.is_active as isActive, s.joining_date as joiningDate from m_staff s  join"
  ...
  private static final class StaffInOfficeHierarchyMapper implements RowMapper<StaffData> {
    ...
    public String schema(final boolean loanOfficersOnly) {
      ...
      sqlBuilder.append(
          "s.display_name as displayName, s.is_loan_officer as isLoanOfficer, s.external_id as"
              + " externalId, ");
      sqlBuilder.append(
  ...
  public Collection<StaffData> retrieveAllLoanOfficersInOfficeById(final Long officeId) {
    return retrieveAllStaff(
        " office_id = ? and is_loan_officer=1 and o.hierarchy like ?", officeId);
  }

  @Override
  public Collection<StaffData> retrieveAllStaffForDropdown(final Long officeId) {

    // adding the Authorization criteria so that a user cannot see an employee who does not belong
    // to his office or     a sub office for his office.
    ...
    final String sql =
        "select "
            + this.lookupMapper.schema()
            + " where s.office_id = ? and s.is_active=1 and o.hierarchy like ? ";

    return this.jdbcTemplate.query(
        sql, this.lookupMapper, new Object[] {defaultOfficeId, hierarchy});
  }
  ...
  private String getStaffCriteria(
      final String sqlSearch,
      final Long officeId,
      final boolean loanOfficersOnly,
      final String status) {
  ...
  public Object[] hasAssociatedItems(final Long staffId) {
    ...
    String sql =
        "select c.display_name as client, g.display_name as grp,l.loan_officer_id as loan,"
            + " s.field_officer_id as sav from m_staff staff  left outer join m_client c on"
            + " staff.id = c.staff_id  AND c.status_enum < ?  left outer join m_group g on"
            + " staff.id = g.staff_id  left outer join m_loan l on staff.id = l.loan_officer_id"
            + " and l.loan_status_id < ?  left outer join m_savings_account s on c.staff_id ="
            + " s.field_officer_id and s.status_enum < ?  where  staff.id  = ?  group by staff.id,"
            + " client, grp, loan, sav";
    ...
    List<Map<String, Object>> result =
        this.jdbcTemplate.queryForList(
            ...
            new Object[] {
              ...
              SavingsAccountStatusType.WITHDRAWN_BY_APPLICANT.getValue(),
              staffId
            });
    if (result != null) {
