...
public class HolidayWritePlatformServiceJpaRepositoryImpl implements HolidayWritePlatformService {
  ...
  public HolidayWritePlatformServiceJpaRepositoryImpl(
      final HolidayDataValidator fromApiJsonDeserializer,
  ...
  private Set<Office> getSelectedOffices(final JsonCommand command) {
    ...
    final JsonObject topLevelJsonElement =
        this.fromApiJsonHelper.parse(command.json()).getAsJsonObject();
  ...
  private void validateInputDates(
    ...
    if (repaymentsRescheduledTo != null) {
      ...
      if (repaymentsRescheduledTo.isBefore(fromDate.minusDays(7))
        ...
        defaultUserMessage =
            "Repayments Rescheduled to date must be within 7 days before or after from and to"
                + " dates";
        throw new HolidayDateException(
