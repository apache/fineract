...
public class SurveyMapper {
  ...
  public static SurveyData map(final Survey survey) {
    final SurveyData surveyData =
        new SurveyData(
            survey.getId(),
            SurveyMapper.mapComponents(survey.getComponents()),
            SurveyMapper.mapQuestions(survey.getQuestions()),
            survey.getKey(),
            survey.getName(),
            survey.getDescription(),
            survey.getCountryCode(),
            survey.getValidFrom(),
            survey.getValidTo());
    return surveyData;
  }
  ...
  private static List<Component> mapComponentDatas(
      final List<ComponentData> componentDatas, final Survey survey) {
    ...
    if (questions != null) {
      for (final Question question : questions) {
        questionDatas.add(
            new QuestionData(
                question.getId(),
                SurveyMapper.mapResponses(question.getResponses()),
                question.getComponentKey(),
                question.getKey(),
                question.getText(),
  ...
  private static List<Question> mapQuestionDatas(
      final List<QuestionData> questionDatas, final Survey survey) {
    ...
    if (questionDatas != null) {
      for (final QuestionData questionData : questionDatas) {
        ...
        question.setResponses(
            SurveyMapper.mapResponseDatas(questionData.getResponseDatas(), question));
  ...
  private static List<Response> mapResponseDatas(
      final List<ResponseData> responseDatas, final Question question) {
