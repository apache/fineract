...
-- client
SET
@client_name = 'InteropMerchant';
SET
@saving_account_no = 'a6b6c10b2aaa4778ac2f';
SET
@saving_account_ext_id = 'a6b6c10b2aaa4778ac2fc9';
SET
@IBAN = 'IC11in02tn04' + @saving_account_ext_id;
SET
@MSISDN = '27710204999';

INSERT INTO `m_client` (`account_no`, `external_id`, `status_enum`, `sub_status`, `activation_date`,
                        `office_joining_date`,
                        `office_id`, `transfer_to_office_id`, `staff_id`, `firstname`, `middlename`, `lastname`,
                        `fullname`,
                        `display_name`, `mobile_no`, `gender_cv_id`, `date_of_birth`, `image_id`,
                        `closure_reason_cv_id`,
                        `closedon_date`, `updated_by`, `updated_on`, `submittedon_date`, `submittedon_userid`,
                        `activatedon_userid`,
                        `closedon_userid`, `default_savings_product`, `default_savings_account`, `client_type_cv_id`,
                        `client_classification_cv_id`,
                        `reject_reason_cv_id`, `rejectedon_date`, `rejectedon_userid`, `withdraw_reason_cv_id`,
                        `withdrawn_on_date`,
                        `withdraw_on_userid`, `reactivated_on_date`, `reactivated_on_userid`, `legal_form_enum`,
                        `reopened_on_date`,
                        `reopened_by_userid`)
...
SELECT id
FROM m_client
WHERE fullname = @client_name INTO @client_id;
...
VALUES (@saving_account_no, @saving_account_ext_id, @client_id, NULL, @saving_prod_id, NULL, 300, 0, 1, 100,
        ADDDATE(curdate(), -100),
        NULL, ADDDATE(curdate(), -100), NULL, ADDDATE(curdate(), -100), NULL, 'TZS', 2, NULL, 1.000000, 1, 4,
        1, -- 29. - 4
...
WHERE account_no = @saving_account_no INTO @saving_acc_id;

INSERT INTO interop_identifier (id, account_id, type, a_value, sub_value_or_type, created_by, created_on, modified_by,
                                modified_on)
...
INSERT INTO `m_savings_account_charge` (`savings_account_id`, `charge_id`, `is_penalty`, `charge_time_enum`,
                                        `charge_calculation_enum`,
                                        `amount`, `amount_outstanding_derived`, `is_paid_derived`, `waived`,
                                        `is_active`)
