...
class GroupDeleteCommandHandlerTest {

    @Mock
    private GroupingTypesWritePlatformService groupingTypesWritePlatformService;

    @InjectMocks
    private GroupDeleteCommandHandler underTest;

    @Test
    void handle_delegatesToServiceAndReturnsResponse() {
        GroupDeleteRequest request = GroupDeleteRequest.builder().groupId(42L).build();

        GroupDeleteResponse expectedResponse = GroupDeleteResponse.builder() //
                .resourceId(42L) //
                .groupId(42L) //
                .build();

        when(groupingTypesWritePlatformService.deleteGroup(request)).thenReturn(expectedResponse);

        GroupDeleteCommand command = new GroupDeleteCommand();
        command.setPayload(request);

        GroupDeleteResponse response = underTest.handle(command);

        verify(groupingTypesWritePlatformService).deleteGroup(request);
