...
class CalendarDeleteCommandHandlerTest {

    @Mock
    private CalendarWritePlatformService writeService;

    @InjectMocks
    private CalendarDeleteCommandHandler handler;

    @Test
    void dispatchesToService() {
        CalendarDeleteCommand cmd = new CalendarDeleteCommand();
        cmd.setPayload(42L);
        CalendarDeleteResponse expected = CalendarDeleteResponse.builder().resourceId(42L).build();
        when(writeService.deleteCalendar(42L)).thenReturn(expected);

        CalendarDeleteResponse result = handler.handle(cmd);

        assertEquals(42L, result.getResourceId());
        verify(writeService).deleteCalendar(42L);
    }
}
