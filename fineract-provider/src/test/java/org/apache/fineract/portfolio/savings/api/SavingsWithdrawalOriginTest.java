...
class SavingsWithdrawalOriginTest {
    ...
    void apiAssignsStaffOriginDespiteSpoofedQueryAndHeaders(String route) {
        ...
        assertThat(wrapper.getValue().getSavingsTransactionOrigin()).isEqualTo(SavingsTransactionOrigin.STAFF_API);
    ...
    void clientEnvelopeInjectionIsRejectedByPersistenceBoundary(String route) {
        ...
        assertThat(failure.getGlobalisationMessageCode()).isEqualTo("error.msg.savings.withdrawal.reserved.metadata");
    }

    @Test
    void spreadsheetBothKindsCarryTrustedProvenance() throws Exception {
        try (var workbook = new HSSFWorkbook()) {
            ...
            for (int i = 1; i <= 2; i++) {
                ...
                row.createCell(TransactionConstants.TRANSACTION_DATE_COL).setCellValue(LocalDate.of(2026, 9, 24));
            }
            ...
            assertThat(captured.getAllValues().get(0).getSavingsTransactionOrigin()).isEqualTo(SavingsTransactionOrigin.SPREADSHEET_IMPORT);
            assertThat(captured.getAllValues().get(1).getSavingsTransactionOrigin()).isEqualTo(SavingsTransactionOrigin.SPREADSHEET_IMPORT);
    ...
    private void invoke(String route, String body) {
        switch (route) {
            case "externalId" -> {
                ...
                resource.transaction("external-reference", "withdrawal", body);
            }
            case "batch" ->
                new SavingsAccountTransactionCommandStrategy(resource).execute(new BatchRequest().setRequestId(1L).setRelativeUrl(
                        "savingsaccounts/42/transactions?command=withdrawal&origin=SPREADSHEET_IMPORT&_serverCommand=SPREADSHEET_IMPORT")
                        .setBody(body).setHeaders(Set.of(new Header().setName("origin").setValue("SPREADSHEET_IMPORT"))), null);
            default -> resource.transaction(42L, "withdrawal", body);
        }
    }
}
