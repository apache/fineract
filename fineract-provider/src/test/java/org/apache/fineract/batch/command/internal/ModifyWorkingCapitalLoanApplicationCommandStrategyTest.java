...
/**
 * Test class for {@link ModifyWorkingCapitalLoanApplicationCommandStrategy}.
 */
public class ModifyWorkingCapitalLoanApplicationCommandStrategyTest {

    /**
     * Test {@link ModifyWorkingCapitalLoanApplicationCommandStrategy#execute} happy path scenario without command.
     */
    @Test
    public void testExecuteSuccessScenarioWithoutCommand() {
        ...
        given(testContext.workingCapitalLoanApiResource.modifyLoanApplicationById(eq(loanId), eq(null), eq(request.getBody())))
        ...
        verify(testContext.workingCapitalLoanApiResource).modifyLoanApplicationById(eq(loanId), eq(null), eq(request.getBody()));
        verify(testContext.toApiJsonSerializer).serialize(processingResult);
    }

    /**
     * Test {@link ModifyWorkingCapitalLoanApplicationCommandStrategy#execute} happy path scenario with command.
     */
    @Test
    public void testExecuteSuccessScenarioWithCommand() {
        ...
        given(testContext.workingCapitalLoanApiResource.modifyLoanApplicationById(eq(loanId), eq(command), eq(request.getBody())))
                .willReturn(processingResult);
        given(testContext.toApiJsonSerializer.serialize(processingResult)).willReturn(serializedResponse);
        ...
        verify(testContext.workingCapitalLoanApiResource).modifyLoanApplicationById(eq(loanId), eq(command), eq(request.getBody()));
    }

    /**
     * Creates and returns a batch request for {@code PUT working-capital-loans/{loanId}}.
     *
     * @param loanId
     *            the loan id
     * @param command
    ...
    private static class TestContext {
        ...
        private final ModifyWorkingCapitalLoanApplicationCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new ModifyWorkingCapitalLoanApplicationCommandStrategy(workingCapitalLoanApiResource, toApiJsonSerializer);
        }
    }
}
