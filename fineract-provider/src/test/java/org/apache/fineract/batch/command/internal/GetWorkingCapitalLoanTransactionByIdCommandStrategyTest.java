...
import org.apache.fineract.portfolio.workingcapitalloan.transaction.api.WorkingCapitalLoanTransactionsApiResource;
...
/**
 * Test class for {@link GetWorkingCapitalLoanTransactionByIdCommandStrategy}.
 */
public class GetWorkingCapitalLoanTransactionByIdCommandStrategyTest {

    /**
     * Test {@link GetWorkingCapitalLoanTransactionByIdCommandStrategy#execute} happy path scenario without query
     * params.
     */
    @Test
    public void testExecuteSuccessScenarioWithoutQueryParams() {
        ...
        given(testContext.workingCapitalLoanTransactionsApiResource.retrieveTransactionByLoanIdAndTransactionId(eq(loanId),
        ...
        verify(testContext.workingCapitalLoanTransactionsApiResource).retrieveTransactionByLoanIdAndTransactionId(eq(loanId),
                eq(transactionId));
        verify(testContext.toApiJsonSerializer).serialize(transactionData);
    }

    /**
     * Test {@link GetWorkingCapitalLoanTransactionByIdCommandStrategy#execute} happy path scenario with query params.
     */
    @Test
    public void testExecuteSuccessScenarioWithQueryParams() {
        ...
        final Long loanId = Long.valueOf(RandomStringUtils.randomNumeric(4));
        final Long transactionId = Long.valueOf(RandomStringUtils.randomNumeric(4));
        final BatchRequest request = getBatchRequest(loanId, transactionId, "fields=id");
        final WorkingCapitalLoanTransactionData transactionData = WorkingCapitalLoanTransactionData.builder().id(transactionId).build();
        final String serializedResponse = "{\"id\":" + transactionId + "}";
    ...
    /**
     * Creates and returns a batch request for {@code GET working-capital-loans/{loanId}/transactions/{transactionId}}.
     *
     * @param loanId
     *            the loan id
     * @param transactionId
     ...
     * @return BatchRequest
     */
    private BatchRequest getBatchRequest(final Long loanId, final Long transactionId, final String queryParams) {
        final BatchRequest br = new BatchRequest();
        String relativeUrl = "v1/working-capital-loans/" + loanId + "/transactions/" + transactionId;
        if (queryParams != null) {
    ...
    private static class TestContext {
        ...
        private final GetWorkingCapitalLoanTransactionByIdCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new GetWorkingCapitalLoanTransactionByIdCommandStrategy(workingCapitalLoanTransactionsApiResource,
                    toApiJsonSerializer);
        }
    }
}
