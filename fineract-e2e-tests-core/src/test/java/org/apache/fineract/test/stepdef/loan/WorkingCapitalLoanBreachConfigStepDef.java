...
public class WorkingCapitalLoanBreachConfigStepDef extends AbstractStepDef {
    ...
    public void adminCallsBreachTemplate() {
        final WorkingCapitalLoanBreachTemplateResponse template = ok(
                () -> fineractFeignClient.workingCapitalBreaches().retrieveWorkingCapitalLoanBreachTemplate());
        ...
        log.info("Template WorkingCapitalLoanBreach: {}", template);
    }

    @When("Admin creates WC Breach With Values")
    public void adminCreatesWCBreachWithValues() {
        final WorkingCapitalLoanBreachRequest request = workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest();
        final CommandProcessingResult response = ok(
                () -> fineractFeignClient.workingCapitalBreaches().createWorkingCapitalLoanBreach(request));
    ...
    public void adminCreatesWCBreachWithValuesForUpdate() {
        final WorkingCapitalLoanBreachRequest request = workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest()
                ...
                .breachFrequency(5) //
                .breachFrequencyType(WorkingCapitalLoanBreachFrequencyType.YEARS.getCode()) //
                .breachAmountCalculationType(WorkingCapitalLoanBreachCalculationType.FLAT.getCode()) //
    ...
    public void adminCreateWCBreachWithDuplicateNameFailure() {
        ...
        WorkingCapitalLoanBreachRequest breachRequestForUpdate = TestContext.INSTANCE
                .get(TestContextKey.WORKING_CAPITAL_BREACH_CREATE_REQUEST_FOR_UPDATE);
        String name = breachRequestForUpdate.getName();
        WorkingCapitalLoanBreachRequest breachRequest = workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest().name(name); //
    ...
    public void checkCreatedBreachHasTheFollowingValues() {
        ...
        final WorkingCapitalLoanBreachData data = ok(
                () -> fineractFeignClient.workingCapitalBreaches().retrieveWorkingCapitalLoanBreach(id));
        final WorkingCapitalLoanBreachRequest request = TestContext.INSTANCE.get(TestContextKey.WORKING_CAPITAL_BREACH_CREATE_REQUEST);
    ...
    public void adminModifiesWCBreachWithValues() {
        ...
        final WorkingCapitalLoanBreachRequest request = workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest() //
                .breachFrequency(4) //
                .breachFrequencyType(WorkingCapitalLoanBreachFrequencyType.DAYS.getCode()) //
                .breachAmountCalculationType(WorkingCapitalLoanBreachCalculationType.FLAT.getCode()) //
                .breachAmount(new BigDecimal("7.89"));
        ok(() -> fineractFeignClient.workingCapitalBreaches().updateWorkingCapitalLoanBreach(id, request));
    ...
    public void adminUpdateWCBreachWithDuplicateNameFailure() {
        ...
        WorkingCapitalLoanBreachRequest breachRequestForUpdate = TestContext.INSTANCE
                .get(TestContextKey.WORKING_CAPITAL_BREACH_CREATE_REQUEST);
        String name = breachRequestForUpdate.getName();
        WorkingCapitalLoanBreachRequest breachRequest = workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest() //
                .name(name); //
        String errorMessage = ErrorMessageHelper.workingCapitalBreachDuplicateNameFailure(existingBreachId);
    ...
    public void checkUpdatedBreachHasTheFollowingValues() {
        ...
        final WorkingCapitalLoanBreachData data = ok(
                () -> fineractFeignClient.workingCapitalBreaches().retrieveWorkingCapitalLoanBreach(id));
        final WorkingCapitalLoanBreachRequest request = TestContext.INSTANCE.get(TestContextKey.WORKING_CAPITAL_BREACH_UPDATE_REQUEST);
        checkBreachData(request, data);
    }
    ...
    public void adminDeletesWCBreachWithValues() {
        final Long id = TestContext.INSTANCE.get(TestContextKey.WORKING_CAPITAL_BREACH_ID);
        ok(() -> fineractFeignClient.workingCapitalBreaches().deleteWorkingCapitalLoanBreach(id));
    }
    ...
    public void createWCBreachWithInvalidDataFailed(final String fieldName, final String value, final String errorMessage) {
        final WorkingCapitalLoanBreachRequest request = setWCBreachFieldValue(
                workingCapitalRequestFactory.defaultWorkingCapitalLoanBreachRequest(), fieldName, value);
    ...
    private void checkCreateWCBreachWithInvalidDataFailure(final WorkingCapitalLoanBreachRequest request, final String errorMessage,
            final int errorCode) {
    ...
    private void checkUpdateWCBreachWithInvalidDataFailure(final Long id, final WorkingCapitalLoanBreachRequest request,
            final String errorMessage, final int errorCode) {
        final CallFailedRuntimeException exception = fail(
                () -> fineractFeignClient.workingCapitalBreaches().updateWorkingCapitalLoanBreach(id, request));
        assertThat(exception.getStatus()).as(ErrorMessageHelper.incorrectExpectedValueInResponse()).isEqualTo(errorCode);
        assertThat(exception.getDeveloperMessage()).contains(errorMessage);
    }
    ...
    private void checkDeleteWCBreachNotFoundFailure(final Long id) {
        final CallFailedRuntimeException exception = fail(
                () -> fineractFeignClient.workingCapitalBreaches().deleteWorkingCapitalLoanBreach(id));
    ...
    private WorkingCapitalLoanBreachRequest setWCBreachFieldValue(final WorkingCapitalLoanBreachRequest request, final String fieldName,
            String fieldValue) {
    ...
    private void checkBreachData(final WorkingCapitalLoanBreachRequest request, final WorkingCapitalLoanBreachData response) {
