...
/**
 * Test class for {@link CreateWorkingCapitalLoanBreachActionCommandStrategy}.
 */
public class CreateWorkingCapitalLoanBreachActionCommandStrategyTest {
    ...
    public void testExecuteSuccessScenario() {
        ...
        given(testContext.workingCapitalLoanBreachActionApiResource.createBreachAction(eq(loanId), eq(request.getBody())))
        ...
        verify(testContext.workingCapitalLoanBreachActionApiResource).createBreachAction(eq(loanId), eq(request.getBody()));
        verify(testContext.toApiJsonSerializer).serialize(processingResult);
    }

    /** Creates a batch sub-request for {@code POST working-capital-loans/{loanId}/breach-actions}. */
    private BatchRequest getBatchRequest(final Long loanId) {
        final BatchRequest br = new BatchRequest();
        br.setRequestId(Long.valueOf(RandomStringUtils.randomNumeric(5)));
        br.setRelativeUrl("v1/working-capital-loans/" + loanId + "/breach-actions");
    ...
    private static class TestContext {
        ...
        private final CreateWorkingCapitalLoanBreachActionCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new CreateWorkingCapitalLoanBreachActionCommandStrategy(workingCapitalLoanBreachActionApiResource,
                    toApiJsonSerializer);
        }
    }
}
