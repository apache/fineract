...
/**
 * Test class for {@link CreateWorkingCapitalLoanNearBreachActionCommandStrategy}.
 */
public class CreateWorkingCapitalLoanNearBreachActionCommandStrategyTest {
    ...
    public void testExecuteSuccessScenario() {
        ...
        given(testContext.workingCapitalLoanNearBreachActionApiResource.createNearBreachActionById(eq(loanId), eq(request.getBody())))
        ...
        verify(testContext.workingCapitalLoanNearBreachActionApiResource).createNearBreachActionById(eq(loanId), eq(request.getBody()));
        verify(testContext.toApiJsonSerializer).serialize(processingResult);
    }

    /** Creates a batch sub-request for {@code POST working-capital-loans/{loanId}/near-breach-actions}. */
    private BatchRequest getBatchRequest(final Long loanId) {
        final BatchRequest br = new BatchRequest();
        br.setRequestId(Long.valueOf(RandomStringUtils.randomNumeric(5)));
        br.setRelativeUrl("v1/working-capital-loans/" + loanId + "/near-breach-actions");
    ...
    private static class TestContext {
        ...
        private final CreateWorkingCapitalLoanNearBreachActionCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new CreateWorkingCapitalLoanNearBreachActionCommandStrategy(workingCapitalLoanNearBreachActionApiResource,
                    toApiJsonSerializer);
        }
    }
}
