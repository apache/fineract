...
/**
 * Test class for {@link StateTransitionWorkingCapitalLoanCommandStrategy}.
 */
public class StateTransitionWorkingCapitalLoanCommandStrategyTest {

    /**
     * Test {@link StateTransitionWorkingCapitalLoanCommandStrategy#execute} happy path scenario.
     */
    @Test
    public void testExecuteSuccessScenario() {
        ...
        given(testContext.workingCapitalLoanApiResource.stateTransitionById(eq(loanId), eq(command), eq(request.getBody())))
        ...
        verify(testContext.workingCapitalLoanApiResource).stateTransitionById(eq(loanId), eq(command), eq(request.getBody()));
        verify(testContext.toApiJsonSerializer).serialize(processingResult);
    }

    /**
     * Creates and returns a batch request for {@code POST working-capital-loans/{loanId}?command={command}}.
     *
     * @param loanId
     *            the loan id
     * @param command
    ...
    private BatchRequest getBatchRequest(final Long loanId, final String command) {
        ...
        br.setRelativeUrl("v1/working-capital-loans/" + loanId + "?command=" + command);
    ...
    private static class TestContext {
        ...
        private final StateTransitionWorkingCapitalLoanCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new StateTransitionWorkingCapitalLoanCommandStrategy(workingCapitalLoanApiResource, toApiJsonSerializer);
        }
    }
}
