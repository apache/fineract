...
import org.apache.fineract.portfolio.workingcapitalloan.product.domain.WorkingCapitalLoanAccountingRuleType;
...
import org.apache.fineract.portfolio.workingcapitalloan.product.domain.WorkingCapitalLoanProductConfigurableAttributes;
import org.apache.fineract.portfolio.workingcapitalloan.product.domain.WorkingCapitalLoanProductMinMaxConstraints;
import org.apache.fineract.portfolio.workingcapitalloan.product.domain.WorkingCapitalLoanProductPaymentAllocationRule;
import org.apache.fineract.portfolio.workingcapitalloan.product.domain.WorkingCapitalLoanProductRelatedDetail;
import org.apache.fineract.portfolio.workingcapitalloan.product.exception.WorkingCapitalLoanProductCannotBeDeletedException;
import org.apache.fineract.portfolio.workingcapitalloan.product.exception.WorkingCapitalLoanProductDuplicateExternalIdException;
import org.apache.fineract.portfolio.workingcapitalloan.product.exception.WorkingCapitalLoanProductDuplicateNameException;
import org.apache.fineract.portfolio.workingcapitalloan.product.exception.WorkingCapitalLoanProductDuplicateShortNameException;
import org.apache.fineract.portfolio.workingcapitalloan.product.exception.WorkingCapitalLoanProductNotFoundException;
...
public class WorkingCapitalLoanProductWritePlatformServiceImpl implements WorkingCapitalLoanProductWritePlatformService {
    ...
    private final WorkingCapitalLoanAdvancedPaymentAllocationsJsonParser advancedPaymentAllocationsJsonParser;
    private final WorkingCapitalLoanBreachRepository breachRepository;
    private final WorkingCapitalLoanProductAccountingMappingService wcAccountingMappingService;
    private final WorkingCapitalLoanNearBreachRepository nearBreachRepository;
    ...
    public CommandProcessingResult createWorkingCapitalLoanProduct(final JsonCommand command) {
        ...
        final DelinquencyBucket delinquencyBucket = findDelinquencyBucketByIdIfProvided(
                command.parameterExists(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName)
                        ...
                        : null);
        final WorkingCapitalLoanBreach breach = findBreachByIdIfProvided(
                command.parameterExists(WorkingCapitalLoanProductConstants.breachIdParamName)
                        ...
                        : null);
        final WorkingCapitalLoanNearBreach nearBreach = (breach == null) ? null
    ...
    public CommandProcessingResult updateWorkingCapitalLoanProduct(final Long productId, final JsonCommand command) {
        ...
        if (command.parameterExists(WorkingCapitalLoanProductConstants.accountingRuleParamName)) {
            ...
            final WorkingCapitalLoanAccountingRuleType newAccountingRule = WorkingCapitalLoanAccountingRuleType
                    .valueOf(newAccountingRuleValue);
    ...
    private Map<String, Object> updateProductFields(final WorkingCapitalLoanProduct product, final JsonCommand command) {
        ...
        if (command.isChangeInLongParameterNamed(WorkingCapitalLoanProductConstants.breachIdParamName, existingBreachId)) {
            ...
            final WorkingCapitalLoanBreach breach = findBreachByIdIfProvided(breachId);
        ...
        if (command.isChangeInLongParameterNamed(WorkingCapitalLoanProductConstants.nearBreachIdParamName, existingNearBreachId)) {
            ...
            final WorkingCapitalLoanNearBreach nearBreach = findNearBreachByIdIfProvided(nearBreachId);
    ...
    private WorkingCapitalLoanProduct createProductFromCommand(final Fund fund, final DelinquencyBucket delinquencyBucket,
            final WorkingCapitalLoanBreach breach, final WorkingCapitalLoanNearBreach nearBreach, final JsonCommand command,
        ...
        final WorkingCapitalLoanAmortizationType amortizationType = WorkingCapitalLoanAmortizationType.fromString(amortizationTypeValue);
        ...
        final String accountingRuleValue = command.parameterExists(WorkingCapitalLoanProductConstants.accountingRuleParamName)
                ...
                : WorkingCapitalLoanAccountingRuleType.NONE.name();
        final WorkingCapitalLoanAccountingRuleType accountingRule = WorkingCapitalLoanAccountingRuleType.valueOf(accountingRuleValue);
    ...
    private WorkingCapitalLoanBreach findBreachByIdIfProvided(final Long breachId) {
        if (breachId == null) {
            return null;
        }
    ...
    private WorkingCapitalLoanNearBreach findNearBreachByIdIfProvided(final Long nearBreachId) {
