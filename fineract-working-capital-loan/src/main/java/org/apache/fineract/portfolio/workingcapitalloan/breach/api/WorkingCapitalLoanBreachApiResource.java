 ...
 * under the License.
 */
package org.apache.fineract.portfolio.workingcapitalloan.breach.api;
...
public class WorkingCapitalLoanBreachApiResource {

    private final PlatformSecurityContext context;
    private final WorkingCapitalLoanBreachReadPlatformService breachReadPlatformService;
    ...
    @Operation(operationId = "retrieveWorkingCapitalLoanBreachTemplate", summary = "Retrieve Working Capital Breach template", description = "Returns breach options for frequency type and amount calculation type.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = WorkingCapitalLoanBreachTemplateResponse.class))) })
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(operationId = "retrieveAllWorkingCapitalLoanBreaches", summary = "List Working Capital Breaches")
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(operationId = "retrieveWorkingCapitalLoanBreach", summary = "Retrieve Working Capital Breach")
    public WorkingCapitalLoanBreachData retrieveOne(@PathParam("breachId") @Parameter(description = "breachId") final Long breachId) {
        this.context.authenticatedUser().validateHasReadPermission(WorkingCapitalLoanProductConstants.WCLP_RESOURCE_NAME);
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(operationId = "createWorkingCapitalLoanBreach", summary = "Create Working Capital Breach")
    public CommandProcessingResult create(final WorkingCapitalLoanBreachRequest request) {
        this.context.authenticatedUser().validateHasCreatePermission(WorkingCapitalLoanProductConstants.WCLP_RESOURCE_NAME);
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createWorkingCapitalLoanBreach()
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(operationId = "updateWorkingCapitalLoanBreach", summary = "Update Working Capital Breach")
    public CommandProcessingResult update(@PathParam("breachId") @Parameter(description = "breachId") final Long breachId,
            final WorkingCapitalLoanBreachRequest request) {
        this.context.authenticatedUser().validateHasUpdatePermission(WorkingCapitalLoanProductConstants.WCLP_RESOURCE_NAME);
        final CommandWrapper commandRequest = new CommandWrapperBuilder().updateWorkingCapitalLoanBreach(breachId)
                .withJson(jsonSerializer.serialize(request)).build();
        return this.commandSourceWritePlatformService.logCommandSource(commandRequest);
    }
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(operationId = "deleteWorkingCapitalLoanBreach", summary = "Delete Working Capital Breach")
    public CommandProcessingResult delete(@PathParam("breachId") @Parameter(description = "breachId") final Long breachId) {
        ...
        final CommandWrapper commandRequest = new CommandWrapperBuilder().deleteWorkingCapitalLoanBreach(breachId).build();
        return this.commandSourceWritePlatformService.logCommandSource(commandRequest);
    }
}
