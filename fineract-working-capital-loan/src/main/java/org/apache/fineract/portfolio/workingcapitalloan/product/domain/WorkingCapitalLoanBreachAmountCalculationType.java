...
public enum WorkingCapitalLoanBreachAmountCalculationType implements ApiFacingEnum<WorkingCapitalLoanBreachAmountCalculationType> {
    ...
    public static List<StringEnumOptionData> toStringEnumOptions() {
        return Arrays.stream(values()).map(WorkingCapitalLoanBreachAmountCalculationType::toData).toList();
    }
