...
@CommandType(entity = ENTITY_WORKINGCAPITALLOAN, action = ACTION_ADJUST)
public class AdjustWorkingCapitalLoanTransactionCommandHandler implements NewCommandSourceHandler {

    private final WorkingCapitalLoanWritePlatformService writePlatformService;

    @Override
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return writePlatformService.adjustTransaction(command.getLoanId(), command.entityId(), command);
    }
}
