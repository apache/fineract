...
@Service
public class WorkingCapitalLoanAdvancedPaymentAllocationsValidator {
    ...
    public void validatePairOfOrderAndPaymentAllocationType(final List<Pair<Integer, WorkingCapitalLoanPaymentAllocationType>> rules) {
        final int expectedCount = WorkingCapitalLoanPaymentAllocationType.values().length;
        ...
        final List<WorkingCapitalLoanPaymentAllocationType> deduced = rules.stream().map(Pair::getRight).distinct().toList();
