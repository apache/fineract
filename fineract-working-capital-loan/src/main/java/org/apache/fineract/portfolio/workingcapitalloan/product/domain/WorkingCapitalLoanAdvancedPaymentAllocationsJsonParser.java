...
@Service
@AllArgsConstructor
public class WorkingCapitalLoanAdvancedPaymentAllocationsJsonParser {

    public final WorkingCapitalLoanAdvancedPaymentAllocationsValidator validator;
    ...
    @NonNull
    private List<WorkingCapitalLoanPaymentAllocationType> getPaymentAllocationTypes(final JsonArray paymentAllocationOrder) {
        if (paymentAllocationOrder != null) {
            ...
            final List<Pair<Integer, WorkingCapitalLoanPaymentAllocationType>> parsedListWithOrder = paymentAllocationOrder.asList()
                    .stream().map(json -> {
                        final Map<String, JsonElement> map = json.getAsJsonObject().asMap();
                        WorkingCapitalLoanPaymentAllocationType paymentAllocationType = null;
                        final String paymentAllocationRule = asStringOrNull(map.get("paymentAllocationRule"));
                        if (paymentAllocationRule != null) {
                            paymentAllocationType = Enums.getIfPresent(WorkingCapitalLoanPaymentAllocationType.class, paymentAllocationRule)
                                    .orNull();
                        }
            ...
            final List<Pair<Integer, WorkingCapitalLoanPaymentAllocationType>> sorted = parsedListWithOrder.stream()
