...
public class WorkingCapitalLoanDiscountFeeAdjustmentCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return Optional.ofNullable(command.getLoanId())
                .map(loanId -> writePlatformService.makeDiscountFeeAdjustment(loanId, command.entityId(), command))
                .orElseGet(() -> writePlatformService.makeDiscountFeeAdjustment(command.getResourceId(), command));
    }
}
