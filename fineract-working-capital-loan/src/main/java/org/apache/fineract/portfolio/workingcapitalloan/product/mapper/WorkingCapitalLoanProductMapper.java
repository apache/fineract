 ...
 * under the License.
 */
package org.apache.fineract.portfolio.workingcapitalloan.product.mapper;
...
import org.apache.fineract.portfolio.workingcapitalloan.breach.mapper.WorkingCapitalLoanBreachMapper;
...
import org.apache.fineract.portfolio.workingcapitalloan.product.data.WorkingCapitalLoanProductConfigurableAttributesData;
import org.apache.fineract.portfolio.workingcapitalloan.product.data.WorkingCapitalLoanProductData;
...
@Mapper(config = MapstructMapperConfig.class, uses = { DelinquencyBucketMapper.class, WorkingCapitalLoanBreachMapper.class,
        WorkingCapitalLoanNearBreachMapper.class })
public interface WorkingCapitalLoanProductMapper {
    ...
    default StringEnumOptionData amortizationToStringEnumOptionData(final WorkingCapitalLoanAmortizationType amortizationType) {
    ...
    default List<WorkingCapitalLoanPaymentAllocationData> paymentAllocationRulesToData(
        ...
        return rules.stream().map(rule -> {
            final List<WorkingCapitalLoanPaymentAllocationData.PaymentAllocationOrder> paymentAllocationOrder = new ArrayList<>();
            int counter = 1;
            for (final WorkingCapitalLoanPaymentAllocationType allocationType : rule.getAllocationTypes()) {
                paymentAllocationOrder
                        .add(new WorkingCapitalLoanPaymentAllocationData.PaymentAllocationOrder(allocationType.name(), counter));
                counter++;
            }
            return new WorkingCapitalLoanPaymentAllocationData(rule.getTransactionType() != null ? rule.getTransactionType() : null,
                    paymentAllocationOrder);
        }).toList();
    }

    @Named("accountingRuleToStringEnumOptionData")
    default StringEnumOptionData accountingRuleToStringEnumOptionData(final WorkingCapitalLoanAccountingRuleType accountingRule) {
