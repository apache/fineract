...
public class WorkingCapitalLoanProductDataValidator {
    ...
    public void validateForUpdate(final String json, final WorkingCapitalLoanProduct product) {
        ...
        validatePaymentAmountCalculationStrategy(element, baseDataValidator, false, existingDetail);
    ...
    private void validatePaymentAmountCalculationStrategy(final JsonElement element, final DataValidatorBuilder baseDataValidator,
        ...
        validatePaymentAmountCalculationStrategyFields(element, baseDataValidator, strategy, existingDetail);
    }
    ...
    private void validatePaymentAmountCalculationStrategyFields(final JsonElement element, final DataValidatorBuilder baseDataValidator,
        ...
        // Request-only values: used for "not allowed for strategy" conflict checks (must not look at persisted
        // opposite-strategy leftovers that UpdateUtil clears after validation).
        final BigDecimal annualEirInRequest = extractBigDecimalIfPresent(element, WorkingCapitalLoanProductConstants.annualEirParamName);
        final BigDecimal periodPaymentRateInRequest = extractBigDecimalIfPresent(element,
                WorkingCapitalLoanProductConstants.periodPaymentRateParamName);
        final BigDecimal minPeriodPaymentRateInRequest = extractBigDecimalIfPresent(element,
                WorkingCapitalLoanProductConstants.minPeriodPaymentRateParamName);
        final BigDecimal maxPeriodPaymentRateInRequest = extractBigDecimalIfPresent(element,
                WorkingCapitalLoanProductConstants.maxPeriodPaymentRateParamName);
        final BigDecimal minAnnualEirInRequest = extractBigDecimalIfPresent(element,
                WorkingCapitalLoanProductConstants.minAnnualEirParamName);
        final BigDecimal maxAnnualEirInRequest = extractBigDecimalIfPresent(element,
                WorkingCapitalLoanProductConstants.maxAnnualEirParamName);

        // Effective post-update values: request when present (including explicit null), else persisted.
        ...
        if (strategy.isTpv()) {
            if (annualEirInRequest != null) {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.annualEirParamName)
                        .failWithCode("not.allowed.for.tpv.strategy");
            }
            if (minAnnualEirInRequest != null) {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.minAnnualEirParamName)
                        .failWithCode("not.allowed.for.tpv.strategy");
            }
            if (maxAnnualEirInRequest != null) {
            ...
            if (periodPaymentRateInRequest != null) {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.periodPaymentRateParamName)
                        .failWithCode("not.allowed.for.annual.eir.strategy");
            }
            if (minPeriodPaymentRateInRequest != null) {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.minPeriodPaymentRateParamName)
                        .failWithCode("not.allowed.for.annual.eir.strategy");
            }
            if (maxPeriodPaymentRateInRequest != null) {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.maxPeriodPaymentRateParamName)
                        .failWithCode("not.allowed.for.annual.eir.strategy");
            }
