...
public class WorkingCapitalLoanTransactionTemplateData implements Serializable {
    ...
    /**
     * The single amount field every command fills, with the meaning the requested command gives it - outstanding
     * principal for a repayment, the overpayment for a credit balance refund, what is still recoverable for a recovery
     * payment, the full payoff for a prepayment, the outstanding balance for a charge-off. The caller asked for one
     * specific command, so it already knows which of those it is holding, and there is no reason for a command to carry
     * an amount field of its own.
     */
