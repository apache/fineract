...
public interface WorkingCapitalLoanDiscountFeeAmortizationService {
    ...
    /**
     * Reconciles posted discount-fee amortization to the net discount as of a terminal event (charge-off or write-off).
     * Derives the signed delta from active amortization transactions versus the net discount pool -- not from the
     * clipped unrealized balance -- so an over-recognition after a same-day discount-fee adjustment still posts a
     * negative correction. Links the resulting transaction to {@code relatedTransaction} for undo. No-op when already
     * aligned.
     */
