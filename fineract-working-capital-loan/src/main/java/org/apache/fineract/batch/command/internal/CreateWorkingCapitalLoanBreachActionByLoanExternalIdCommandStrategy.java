 ...
 *
 * <p>
 * The action itself (pause, reschedule, resume, reset, undo_reset, disable, enable) travels in the request body, not as
 * a query parameter, so the sub-request carries nothing this strategy has to read beyond the loan the URL names.
 */
@Component
@RequiredArgsConstructor
public class CreateWorkingCapitalLoanBreachActionByLoanExternalIdCommandStrategy implements CommandStrategy {
    ...
    public BatchResponse execute(final BatchRequest request, @SuppressWarnings("unused") final UriInfo uriInfo) {
        ...
        final CommandProcessingResult commandProcessingResult = workingCapitalLoanBreachActionApiResource.createBreachAction(loanExternalId,
                request.getBody());
