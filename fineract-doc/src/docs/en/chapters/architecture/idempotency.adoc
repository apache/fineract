...
IMPORTANT: The idempotency mechanics of the new command processing infrastructure still need to be documented. The implementation is done as a command hook. Detailed documentation will be provided soon. The only thing that new and legacy idempotency capabilities have in common is the name and configuration of the HTTP header name that contains  the idempotency key.
