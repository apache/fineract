...
** On `/transactions?command=discountFee`, `relatedResourceId` is mandatory and must be the disbursement transaction ID.
** `relatedResourceId` carries a transaction ID only. A transaction external id, or any other value that is not a positive whole number, is rejected with 400 (`validation.msg.WORKINGCAPITALLOAN.relatedResourceId.not.a.number`).
** On `/transactions/{transactionId}` and `/transactions/external-id/{transactionExternalId}` the path names the disbursement, and `relatedResourceId` is not accepted in the body.
** An unknown `transactionExternalId`, or one that belongs to another loan, returns 404; a `transactionId` that belongs to another loan returns 400.
* Only `/transactions?command=discountFee` is routed by the Batch API. The forms that name the transaction in the path are available over the regular API only.
...
** On `/transactions?command=discountFeeAdjustment`, `relatedResourceId` is mandatory and must be the Discount Fee transaction ID.
** `relatedResourceId` carries a transaction ID only. A transaction external id, or any other value that is not a positive whole number, is rejected with 400 (`validation.msg.WORKINGCAPITALLOAN.relatedResourceId.not.a.number`).
** On `/transactions/{transactionId}` and `/transactions/external-id/{transactionExternalId}` the path names the Discount Fee transaction, and `relatedResourceId` is not accepted in the body.
** An unknown `transactionExternalId`, or one that belongs to another loan, returns 404; a `transactionId` that belongs to another loan returns 400.
* Only `/transactions?command=discountFeeAdjustment` is routed by the Batch API. The forms that name the transaction in the path are available over the regular API only.
