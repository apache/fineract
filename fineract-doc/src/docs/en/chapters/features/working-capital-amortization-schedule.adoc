...
* *Calculated Annual EIR* — effective interest rate compounded over `npvDayCount` periods and rounded to six decimals, stored on the amortization schedule. The base schedule's daily rate is back-calculated from it; a payment rate change does not restate it, and each rate segment solves its own rate
